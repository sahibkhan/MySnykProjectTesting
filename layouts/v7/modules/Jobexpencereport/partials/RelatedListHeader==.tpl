	...
	<div class="relatedHeader">
		<div class="btn-toolbar row">
			<div class="col-lg-6 col-md-6 col-sm-6 btn-toolbar">
								...
								{if $IS_SELECT_BUTTON || $IS_CREATE_PERMITTED}
									<button type="button" module="{$RELATED_MODULE_NAME}" class="btn btn-default
										...
										{if ($RELATED_LINK->isPageLoadLink())}
											...
											data-url="yes am here{$RELATED_LINK->getUrl()}{if $RELATED_MODULE_NAME eq 'Jobexpencereport'}&jrertype=expence&cf_1457=Expence{/if}{if $SELECTED_MENU_CATEGORY}&app={$SELECTED_MENU_CATEGORY}{/if}"
										{/if}
										>{if $IS_SELECT_BUTTON eq false}<i class="fa fa-plus"></i>&nbsp;{/if}&nbsp;{$RELATED_LINK->getLabel()}</button>
								{/if}
							{/if}
						</div>
						{if $RELATED_MODULE_NAME eq 'Jobexpencereport'}
						<div class="btn-group">
							{if $IS_SELECT_BUTTON || $IS_CREATE_PERMITTED}	
								{if $JOB_OWNER}
								<button type="button" module="{$RELATED_MODULE_NAME}" class="btn btn-default
										...
										{if ($RELATED_LINK->isPageLoadLink())}
											{if $RELATION_FIELD} data-name="{$RELATION_FIELD->getName()}" {/if}
											data-url="{$RELATED_LINK->getUrl()}&jrertype=selling&cf_1457=Selling{if $SELECTED_MENU_CATEGORY}&app={$SELECTED_MENU_CATEGORY}{/if}"
										{/if}
										>{if $IS_SELECT_BUTTON eq false}<i class="fa fa-plus"></i>&nbsp;{/if}&nbsp;Add Selling</button>
								{/if}   
							{/if}
						</div>
						{/if}
				...
				<font style="color:#F00;font-size:16px; font-weight:bold;">Please create Job Costing before proceeding to Actual Expense and Selling
                </font>

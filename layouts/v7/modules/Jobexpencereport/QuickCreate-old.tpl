                    ...
                    <div class="quickCreateContent">
                        ...
                        {foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name=blockIterator}
                            {if $BLOCK_FIELDS|@count lte 0}{continue}{/if}
                             ...
                             {* Mehtab Code: Hide and Show selling and expence block on the basis of JRER type *}
                            {if $BLOCK_LABEL eq 'Selling' AND $JRER_TYPE eq 'selling'}{continue}{/if}
                            {if $BLOCK_LABEL eq 'Expence' AND $JRER_TYPE eq 'expence'}{continue}{/if}

                            <tr {if $BLOCK_LABEL eq 'JRER Type'}style="display:none"{/if}>
                                {assign var=COUNTER value=0}
                                {foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}

                                    {if $FIELD_MODEL->getFieldName() == 'name'} {continue} {/if}

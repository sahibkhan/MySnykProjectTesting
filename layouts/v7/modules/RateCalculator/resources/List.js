...
//			http://stackoverflow.com/questions/26482645/number-isintegerx-which-is-created-can-not-work-in-ie
	...
	registerListViewSearch: function () {
		...
		var thisInstance = this;
		//add by azhar
		$("#RateCalculator_listView_basicAction_LBL_ADD_RECORD").hide();
		listViewPageDiv.on('click', '[data-trigger="routSearch"]', function (e) {
			e.preventDefault();
            ...
            var dest= $("#dest :selected").val();
            var ctype= $("#conttype").val();
            var sdate= $("#shipdate").val();

            if(origin=='' || dest=='' || ctype=='' || sdate=='')
            {
                alert("To filter please select all required fields");
				app.helper.hideProgress();
                return;
            }

            var data = {origin,dest,ctype,sdate};
            console.log(data);
			$("#routResult").html("<center><img src='include/RateCalculator/icons/loader.gif' alt='Wait . . .' width='300px' /></center>");
			$.ajax({
                url:'include/RateCalculator/getroutdata.php',
                type:'post',
                data:data,
                success:function(resp){
                    console.log(resp)
                    $("#routResult").html(resp);
			...
			//$("#routResult").html("<b>This is change</b>")
			app.helper.hideProgress();
			
		});
		// search filter
		listViewPageDiv.on('click', '[data-trigger="routSearchClear"]', function (e) {
			e.preventDefault();
			// getting filter parameters
            $("#origin").val("").change();
            $("#dest").val("").change();
		...
		// change percentage
		listViewPageDiv.on('change', '[data-trigger="routRateChange"]', function (e) {
			e.preventDefault();
			// getting filter parameters
            console.log("tets");
			console.log(e);
			...
			var idno1 = $(this).attr("id");
			var idno = idno1.replace("percent", "percentnew");
			var cval = $("#"+idno1.replace("percent", "totalusd")).val();
			//Percent increase = [(new value - original value)/original value] * 100
			var newval = cval;
			if(parseInt($(this).val())>0)
			{
				newval = parseInt(cval) + (cval * ($(this).val()/100)+1);	
			}
		...
		// create quotation
		listViewPageDiv.on('click', '[data-trigger="routAddQuote"]', function (e) {
			e.preventDefault();
			// getting filter parameters
			var rslt = $("#routResult").html();
			if(rslt == "")
			{
				app.helper.showErrorNotification({message:"No search result!"});
				return false;
			...
			for (let i = 0; i < 10; i++) {
				...
				{
					console.log($('#txtdetails-'+i));
					var txt = $('#txtdetails-'+i).val();
					var prcnt = $('#percent-'+i).val();
					var prcntval = $('#percentnew-'+i).val();
					
					txt = txt.replace('ratepercenttemp-'+i, prcnt);
					txt = txt.replace('ratenewtemp-'+i, prcntval);
					txt = txt.replace('percentrate-'+i, prcntval);
			...
			if(rout=="")
			{
				app.helper.showErrorNotification({message:"Please select a result!"});
				return false;
			}
			var rslt ="";
			...
			$.ajax({
				url: 'include/RateCalculator/createratedata.php',
				type: 'POST',
				data:{data:rout,fromport:fromport,fromcountry:fromcountry,toport:toport,tocountry:tocountry},
				async: false,
				cache: false,
				timeout: 30000,
				error: function(){
			...
			*/
			...
			if(qtid=="New" || qtid=="")
			{
				var url = "http://tiger.globalink.net/live-gems/index.php?module=Quotes&view=Edit&app=SALES&rout="+rslt;
			}
			else
			{
				var url = "http://tiger.globalink.net/live-gems/index.php?module=Quotes&view=Edit&record="+qtid+"&app=MARKETING&rout="+rslt;
			}
			
			console.log(url);
			
			//localStorage.setItem("rout",rout);

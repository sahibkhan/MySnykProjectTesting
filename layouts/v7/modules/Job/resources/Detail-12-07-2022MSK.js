	...
	*/
	...
	registerRelatedRowClickEvent: function() {
		...
		detailContentsHolder.on('click','.listViewEntries',function(e){
			const allowedModulesForDetailView = ['MasterProfiles'];
			if (allowedModulesForDetailView.includes(app.getModuleName())){
				e.preventDefault();
				const currentElem = jQuery(e.currentTarget);
				window.location.href = currentElem.data('recordurl');
			}			
			var selection = window.getSelection().toString();
			if(selection.length == 0) {
...
//show and hide block created by sahib 

...
{strip}
	...
	<div class="relatedContainer">
		...
		{*{include file="partials/RelatedListHeader.tpl"|vtemplate_path:$RELATED_MODULE_NAME}*}
			...
			<div class="relatedContents col-lg-12 col-md-12 col-sm-12 table-container" style="margin-top: 10px;">
                ...
                <tr class="listViewEntries">    
					...
					<a href="index.php?module=Jobexpencereport&view=Print&record={$ASSIGNED_USER['job_id']}&userid={$ASSIGNED_USER['user_id']}&expense=SUBJCR&type=JCR"
                     target="_blank" title=""><i class="fa fa-print alignMiddle" title="Job Completion Report from {$ASSIGNED_USER['cf_1559']} {$ASSIGNED_USER['cf_1542']}"></i></a>
                     &nbsp;&nbsp;
					<a href="index.php?module=Jobexpencereport&view=Print&record={$ASSIGNED_USER['job_id']}&userid={$ASSIGNED_USER['user_id']}&expense=SUBJCR&type=LJCR"
                    	target="_blank" title=""><i class="fa fa-print alignMiddle" title="Local JC Report from {$ASSIGNED_USER['cf_1559']} {$ASSIGNED_USER['cf_1542']}"></i></a>
                    </td>
                </tr>    
                 {/foreach}   
                </tbody>
		...
		<div class="relatedContents col-lg-12 col-md-12 col-sm-12 table-container" style="margin-top: 10px;">
		   	<div class="bottomscroll-div">
			   	 <button id="HeadButton" type="button" class="btn btn-success"><strong>Send to Payables</strong></button>
			</div>
		</div>		
		  ...
		  <div class="relatedContents col-lg-12 col-md-12 col-sm-12 table-container" style="margin-top: 10px;">
			...
			<div class="relatedContents col-lg-6 col-md-6 col-sm-6 table-container" style="margin-top: 10px;">
				<input type="hidden" value="[]" name="picklistDependency">
                          ...
                          <input type="hidden" value="{$JOB_ID}" name="record">
...
$grid.jqGrid({
    datatype: "json",	
	url:$(location).attr('href')+'&grid=4',
	//datatype: "local",
    colNames: [{/literal}{$EXPENCE_HEADER_HEAD}{literal}],
    colModel: {/literal}{$EXPENCE_FIELD_HEAD}{literal},
...
$grid.jqGrid('navGrid', '#pager', {refreshstate: 'current', add: false, edit: false, del: false, search:false});
	...
	$("#HeadButton").click( function() {
		...
		for(i=0;i<data.length;i++){
			...
			var head_status = data[i].send_to_payables_and_generate_payment_voucher;
			
			if(head_status=='Accept' || head_status=='Reject' || head_status=='Select')
			{
				var selectedText = head_status;
			}
			else{
				var dropdown = $('#' + k + '_send_to_payables_and_generate_payment_voucher')[0];
	   ...
	   if(rowIdAccept)
	   {
		   ...
		   $.ajax({
					url: 'include/Jobexpencereport/send_to_payables_by_head_of_department.php?job_id='+{/literal}{$JOB_ID}{literal}+'&expenseids='+rowIdAccept+'&status=accept',
	   ...
	   {
		    $.ajax({
					url: 'include/Jobexpencereport/send_to_payables_by_head_of_department.php?job_id='+{/literal}{$JOB_ID}{literal}+'&expenseids='+rowIdReject+'&status=reject',
					success: function(data){
					   //var result=JSON.parse(data);
					$grid.trigger('reloadGrid', [{ current: true}]);
					$grid.jqGrid('bindKeys');   

...
{if $TYPE neq 'False'}
   <div class="fieldBlockContainer duplicationMessageContainer" style="background-color:#fff98b !important;">
      <div class="duplicationMessageHeader"><b>Note : You can only upload files with extention ( jpg, jpeg, png, gif, pdf, xlsx, docx, zip, 7z, msg ) !
         </b>
      </div>
   </div>
{/if}
...
{if $TYPE eq 'False'}
   <div class="fieldBlockContainer duplicationMessageContainer">
      <div class="duplicationMessageHeader"><b>You can only upload files with extention ( jpg, jpeg, png, gif, pdf, xlsx, docx, zip, 7z, msg ) !</br>
...
{if $DOCS_REQUIRED eq 'False'}
   <div class="fieldBlockContainer duplicationMessageContainer">
      <div class="duplicationMessageHeader">
         <b>
         Please Upload the Following Mandatory Documents First !
         </b>
         <p>
            {$DOCS_REQUIRED_NAMES}
         </p>
      ...
      <div class="relatedContents col-lg-12 col-md-12 col-sm-12 table-container">
         <div class="bottomscroll-div">
            <table id="listview-table" class="table listview-table listViewEntriesTable">
               ...
               {if $DOC_COUNT > 0 }
                  ...
                  <tr class="listViewEntries" data-id="4802585">
                     <td class="related-list-actions">
                        {if $JOBSTATUS neq 'Archive Received' && $JOBSTATUS neq 'Passed to Archive'}
                        {if $USERID neq '405'}
                        <span class="actionImages">&nbsp;&nbsp;&nbsp;<a name="relationEdit" data-url="index.php?module=EDocs&amp;view=Edit&amp;record={$doc.edocsid}">
                        <i class="fa fa-pencil" title="Edit"></i></a> &nbsp;&nbsp;
                        </span>
                        {/if}
                        {/if}
                        &nbsp;&nbsp;&nbsp;
                        {if $doc['required'] eq 1}Mandatory 
                     ...
                     <td class="" title="{$doc.document_name}" data-field-type="image" nowrap="">
                        ...
                        <span class="value textOverflowEllipsis">        
                        <a name="downloadfile" href="{$link.doc_path}{$link.upload_document_clone}" download="{$link.upload_document_clone}" target="_blank" title="{$link.file_name}">
                        {substr($link.file_name, 0, 25)}&nbsp;
                        ...
                        {if $USERID neq '405'}
                        <span class="value textOverflowEllipsis">        
                        <a name="deletefile" onclick="return confirm('Are you sure you want to delete this Document?');" href="index.php?module=EDocs&view=Edit&jobid={$link.job_id}&displayMode=overlay&returnmodule=Job&returnrelatedModuleName=EDocs&returnview=Detail&deleterecord={$link.edocsrecordsid}&returnpage=1&returnmode=showRelatedList&returnrelationId=325&returntab_label=EDocs">
               ...
               {if $DOC_COUNT eq 0 && $JOBSTATUS neq 'Archive Received' && $JOBSTATUS neq 'Passed to Archive'}
                  {**{if {$TYPES_COUNT} > 0}   **}
                  ...
                  <tr class="listViewEntries" id="2" name="everyrecord[]">
                     <td class="related-list-actions">&nbsp;&nbsp; {if $TYPE_MODEL['required_doc'] eq 1}
                        Mandatory <span class="redColor">*</span>{else}Optional{/if}
                     </td>
                     ...
                     <td id="upload_edocument" class="relatedListEntryValues" colspan="1">
                        <input id="EDocs_editView_fieldName_upload_edocument" type="file" data-fieldname="upload_edocument" data-fieldtype="string" multiple="" class="inputElement multi max-6  MultiFile-applied" name="upload_edocument[{$TYPE_MODEL['echecklistid']}][]" value="">
                     </td>
                  </tr>
                  {/foreach}
                  {/if}
                  {if $DOC_COUNT > 0 && $JOBSTATUS neq 'Archive Received' && $JOBSTATUS neq 'Passed to Archive'}
                  {foreach key=TYPE item=TYPE_MODEL from=$TYPES}
                  {foreach key=REMAINING item=REMAINING_MODEL from=$REMAINING_TYPES}
                  {if $TYPE_MODEL['echecklistid'] eq $REMAINING_MODEL['echecklist'] }
                  <tr class="listViewEntries" id="2" name="everyrecord[]">
                     <td class="related-list-actions">&nbsp;&nbsp; {if $TYPE_MODEL['required_doc'] eq '1'}
                        Mandatory <span class="redColor">*</span>{else}Optional{/if}
                     </td>
               ...
               <tfoot>
                  <tr class="listViewEntries">
                     <td  nowrap=""  class="relatedListEntryValues" align="right" colspan="3"> 
                        ...
                        {if $JOBSTATUS neq 'Archive Received'}
         ...
         {if $ASSIGNEE_COUNT > 0}
               <div class="table-container container">
                  <table id="listview-table" class="table listview-table listViewEntriesTable">
                     <thead>
                        <tr class="listViewHeaders">
                           <th style="min-width:100px">Job Assignee</th>
                           <th style="min-width:100px">Archive SubStatus</th>
                           <th class="nowrap">Document Name</th>
                           <th class="nowrap">Uploaded Documents</th>  
                        </tr>
                     </thead>
                  </table>
               </div>
...
{if $USERTYPE eq 'Job Owner'}
   


   {if $ASSIGNEE_COUNT > 0}

      {foreach key=row_no item=assignee from=$ASSIGNEES}

         <div class="relatedContents col-lg-12 col-md-12 col-sm-12 table-container" style="margin-top:20px;margin-bottom:20px;">

            <p><strong>Job Assignee # {$assignee.first_name} Checklist</strong></p>

            <p><strong>Archive SubStatus : {$assignee.archive_substatus}</strong></p>

            {*<p><strong>Coordinator name : {$COORDINATOR}. </strong></p>*} 
               ...
               <div class="bottomscroll-div">
                  ...
                  <table id="listview-table" class="table listview-table listViewEntriesTable">
                     ...
                     {foreach key=row_no item=fetch from=$ASSIGNEE_EDOCS_FETCH}

                           {if $fetch.user_id eq $assignee.user_id}

                              <tr></tr>
                              <tr class="listViewEntries" data-id="4802585">
                                <td>
                                    &nbsp;&nbsp;&nbsp;
                                    {if $fetch['required'] eq 1}Mandatory 
                                    <span class="redColor">*</span>
                                    {else}
                                    Optional
                                    {/if}
                                ...
                                <td>
                                 ...
                                 {foreach key=row_no item=edoc from=$ASSIGNEE_EDOCS}

                                    {if $edoc.document_name eq $fetch.document_name && $edoc.user_id eq $assignee.user_id}
                                    
                                       <span class="value textOverflowEllipsis">        
                                          <a name="downloadfile" href="{$edoc.doc_path}{$edoc.upload_document_clone}" download="{$edoc.upload_document_clone}" target="_blank" title="{$edoc.file_name}">
                                          {substr($edoc.file_name, 0, 25)}&nbsp;
                                          <i title="Download file" class="fa fa-download  fa-lg"></i>
                                          </a>
                                       </span>

		    ...
		    <div class="relatedContents col-lg-12 col-md-12 col-sm-12 table-container" style="margin-top: 10px;">
             ...
             <div class="relatedContents col-lg-12 col-md-12 col-sm-12 table-container" style="margin-top: 10px;">
		   	 	<div class="bottomscroll-div">
						 ...
						 <button id="InvoiceButtonApproved" type="button" class="btn btn-success"><strong>Submit to 1C</strong></button>
	...
	*/
	...
	$("#InvoiceButton").click( function() {
		...
		for(i=0;i<data.length;i++){
			...
			var local_invoice_no = data[i].local_invoice_no;			
			...
			if(invoice_status=='Accept' || invoice_status=='Reject' || invoice_status=='Select')
			{
				...
				var str = local_invoice_no; 
				var res = str.match(/<textarea/);
				if(res=='<textarea')
				{
					local_invoice_no = 'blank';
				}
			
				if(local_invoice_no=='blank')
				{
					var local_invoice_no = $('#' + k + '_local_invoice_no').val();
				}
           ...
           // str += data[i].id+',';
				...
				var sell_id = $('#list_selling tr:eq('+j+')').attr('id');
				
				local_invoice +=local_invoice_no+','; 
			}
	   ...
	   {
		   $.ajax({
					url: 'include/Jobexpencereport/invoicing_accept_new_local.php?job_id='+{/literal}{$JOB_ID}{literal}+'&local_invoice_no='+local_invoice+'&sellingids='+rowIdAccept+'&status=accept',
	...
	$('#InvoiceButtonApproved').click( function() {
		var j=0;
		var mn = 0;
		var rowId = '';
		var rowIdAccept = '';
	   ...
	   if(rowIdAccept)
	   {
		   ...
		   $.ajax({
					url: 'include/Jobexpencereport/sent_to_1c.php?job_id='+{/literal}{$JOB_ID}{literal}+'&sellingids='+rowIdAccept+'&status=accept',
					success: function(data){
					   //var result=JSON.parse(data);
					$grid.trigger('reloadGrid', [{ current: true}]);
					$grid.jqGrid('bindKeys');   

...
<div class="table-actions">
    ...
    <span class="input" >
        {if $LISTVIEW_ENTRY->rawData['diff'] > 1}
        {assign var=entryid value='nonUploaded'}
        {else}
        {assign var=entryid value='uploaded'}
        {/if}
        <input type="checkbox" value="{$LISTVIEW_ENTRY->getId()}" class="listViewEntriesCheckBox" id="{$entryid}_{$LISTVIEW_ENTRY->getId()}" />
    </span>
    {/if}

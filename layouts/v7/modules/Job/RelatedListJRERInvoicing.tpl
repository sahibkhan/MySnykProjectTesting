         ...
         <div class="relatedContents col-lg-12 col-md-12 col-sm-12 table-container" style="margin-top: 10px;">
          ...
          <div class="relatedContents col-lg-12 col-md-12 col-sm-12 table-container" style="margin-top: 10px;">
		   	 	<div class="bottomscroll-div">
						...
						&nbsp;&nbsp;&nbsp;
						<button id="InvoiceButtonSalesTax" type="button" class="btn btn-success"><strong>Generate Tax Invoice</strong></button>
						&nbsp;&nbsp;&nbsp;
						<button id="localSalesTax" type="button" class="btn btn-success"><strong>Manual Tax Invoice</strong></button>
...
//$grid.jqGrid('setGridWidth', width, true);
	...
	/*------*/
	$("#InvoiceButtonSalesTax").click( function() {
		var j=0;
		...
		//var data = $("#list_selling").getRowData();		
		 ...
		 //console.log(data);
		$.each($("#list_selling input[name='check_invoice_tax']:checked"), function(event){
		//console.log('in func');
...
/*
		for(var i=0;i<data.length;i++){			
        	if(data[i].check_invoice_tax=='Yes'){
				var j=i+1;
...
*/
	   ...
	   if(rowId)
	   {
		  			...
		  			$.ajax({
						url: 'include/Jobexpencereport/job_invoicing_salestax.php?job_id='+{/literal}{$JOB_ID}{literal}+'&sellingids='+rowId,
						success: function(data){
						if (data > 0) {
							alert('Sales tax generated successfully.'); 
						}
						$grid.trigger('reloadGrid', [{ current: true}]);
						$grid.jqGrid('bindKeys');
	...
	$("#localSalesTax").click( function() {
		...
		//local_tax[0] = 'hi';
		//info[1] = 'hello';
		$.each($("#list_selling textarea[name='local_tax_invoice_no']"), function(event){
		
                rowId = $( this ).closest( "tr" ).attr('id');
                invoice_tax = $( this ).val();
                if (invoice_tax.trim()!='') {
                	local_tax.push({[rowId]:invoice_tax});
            	}
                //local_tax[rowId] = invoice_tax;
        });
        //console.log(local_tax);
/*
		for(var i=0;i<data.length;i++){			
        	if(data[i].check_invoice_tax=='Yes'){
				var j=i+1;
...
*/
	   if (local_tax.length < 1) {
	   		alert('Please enter Tax Invoice Number');
	   } else if(rowId) {
		 			//var local_tax_ser = $.serialize(local_tax);
		  			$.ajax({
		  				type: "POST",
						url: 'include/Jobexpencereport/job_local_salestax.php?job_id='+{/literal}{$JOB_ID}{literal},
						//data: local_tax_ser,
						data: {local_tax:local_tax},
						success: function(data){
							if (data == 'already') {
								//alert('Your manual tax invoice number ('+invoice_tax+') already exsit'); 
								alert('Please add correct Tax Invoice Number');
							}
							/*
						if (data > 0) {
							alert('Local sales tax invoice generated successfully.'); 
						}
						*/
						$grid.trigger('reloadGrid', [{ current: true}]);
						$grid.jqGrid('bindKeys');
...
/*
	$(".check_invoice_tax").click( function() {	
	console.log('ere');
		if ($(this).is(':checked')) {
			var selected_data_att = $(this).data('check_tax');
			alert(selected_data_att);
			//$('[data-attribute="value"]');
		}
	});
*/
	$('body').on('click', '.check_invoice_tax', function() {
    	
		if ($(this).is(':checked')) {
			var selected_data_att = $(this).data('check_tax');
			//alert(selected_data_att);
			$('[data-check_tax="'+selected_data_att+'"]').prop('checked', true);
		}
	});

	...
	<div class="relatedContainer">
		...
		<input type='hidden' value="{$IS_RELATION_FIELD_ACTIVE}" id='isRelationFieldActive'>
         <input type="hidden" id="recordsCount" value=""/>
          ...
          <optgroup label="  &nbsp; "><option data-editurl=""
            data-deleteurl=""
            data-approveurl=""
            data-denyurl=""
            data-editable="" data-deletable="" data-pending="" data-public="" id="filterOptionId_{$JOB_ID}"
            value="{$JOB_ID}" data-id="{$JOB_ID}" selected="selected" class="filterOptionId_{$JOB_ID}">All PackagingMaterial</option>
          </optgroup>
        </select>
						...
						<tr class="listViewEntries" data-id='{$RELATED_RECORD->getId()}' 
							...
							<td class="related-list-actions">
								<span class="actionImages">&nbsp;&nbsp;&nbsp;
                                    ...
                                    <a id="relationEdit" data-url="index.php?module=PackagingMaterial&view=MassActionAjax&mode=showMassEditFormRelated" data-id="{$RELATED_RECORD->getId()}">
                                    <i title="{vtranslate('LBL_EDIT', $MODULE)}" class="fa fa-pencil"></i></a>&nbsp;&nbsp;
									{/if}
							...
							{foreach item=HEADER_FIELD from=$RELATED_HEADERS}
								...
								{assign var=RELATED_LIST_VALUE value=$RELATED_RECORD->get($RELATED_HEADERNAME)}
                                 {if $HEADER_FIELD@first}
                                    ...
                                    <input type="checkbox" name="packaging_checkbox[]" value="{$RELATED_RECORD->getId()}" id="packaging_{$RELATED_RECORD->getId()}" {if $RELATED_RECORD->getDisplayValue('cf_5754') neq ''}disabled="disabled"{/if} >
                                    {if !in_array($RELATED_RECORD->getDisplayValue('cf_5754'),$PACKAGING_REF)}
                                    &nbsp;&nbsp;<a href="index.php?module=PackagingMaterial&view=Print&record={$RELATED_RECORD->getId()}&parentid={$JOB_ID}" target="_blank"><i title="Print Packaging Material" class="fa fa-print alignMiddle"></i></a>&nbsp;
                                    {append var='PACKAGING_REF' value=$RELATED_RECORD->getDisplayValue('cf_5754')}
                                    {/if}
                                    </td>
                                {/if}
        ...
        <hr />
        <tr>
        <td> <button id="PackagingButton" type="button" class="btn btn-success"><strong>Generate Packaging #</strong></button></td>
        <td colspan="5">Note: Select Multiple Item to Generate Group Packaging #</td>
        </tr>
        </table>
        </div>
...
<script language="javascript">
	$("#PackagingButton").click( function() {
        ...
        var current_user_id = $('#current_user_id').val();
		   ...
		   {
			    $.ajax({
				 url:'index.php?module=PackagingMaterial&view=MassActionAjax&mode=generatePackagingNo',
				 method:'POST',
				 data:{packaging_no:id, current_user_id:current_user_id},
				 success:function(data)
				 {
					 var final_data = JSON.parse(data);
					 //final_data.packaging
					 for(var i=0; i<id.length; i++)
      				 {
					 	$('td#cf_5754_'+id[i]+'').html(final_data.packaging_ref_no);
						$('#packaging_'+id[i]+'').attr("disabled", true);
					 }
					 //alert('Packaging # Generated');
					 location.reload();
				 }

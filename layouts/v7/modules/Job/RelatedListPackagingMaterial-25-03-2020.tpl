							...
							{foreach item=HEADER_FIELD from=$RELATED_HEADERS}
								...
								{assign var=RELATED_LIST_VALUE value=$RELATED_RECORD->get($RELATED_HEADERNAME)}
                                 {if $HEADER_FIELD@first}
                                    ...
                                    &nbsp;&nbsp;<a href="index.php?module=PackagingMaterial&view=Print&record={$RELATED_RECORD->getId()}&parentid={$JOB_ID}" target="_blank"><i title="Print Packaging Material" class="icon-print alignMiddle"></i></a>&nbsp;

								...
								<td class="relatedListEntryValues" title="{strip_tags($RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME))}" data-field-type="{$HEADER_FIELD->getFieldDataType()}" nowrap>
									<span class="value textOverflowEllipsis">
										{if $RELATED_MODULE_NAME eq 'Documents' && $RELATED_HEADERNAME eq 'document_source'}
                                            ...
                                            <a target="_blank" 
                                            href="index.php?module=Job&relatedModule=Jobexpencereport&view=Detail&record={$RELATED_RECORD->get($RELATED_HEADERNAME)}&mode=showRelatedList&tab_label=Job%20Revenue%20and%20Expence">
                    ...
                    <table class="table listview-table" id="listview-table" >
                      ...
                      <tbody>
                       {foreach item=PROFIT from=$PROFIT_SHARE}                     
                        ...
                        <td><a href="SubJCR_export_pdf.php?record={$PROFIT['job_id']}&userid={$PROFIT['user_id']}" target="_blank" title=""><i class="fa fa-print alignMiddle" title=""></i></a></td>
                        </tr>
                         {/foreach} 
                      </tbody>
                       <thead>

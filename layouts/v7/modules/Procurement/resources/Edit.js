...
function getlocalprice(id){
	if($(id+" #childvalues").val()=='Select Option')
	{
		alert('Expense Type not selected. Please select expense item from available list');
		$(id+" #childvalues").focus();
		return false;
	}
  var qty = $(id+" #qty1").val();
  var psc = $(id+" #psc1").val();
...
$(document).ready(function(){
  ...
  $('[name="proc_current_approval"]').not('.searchRow [name="proc_current_approval"]').parent().hide();
  $('[name="proc_current_approval"]').not('.searchRow [name="proc_current_approval"]').parent().prev().hide();
  $('[name="proc_current_approval"]').not('.searchRow [name="proc_current_approval"]').attr('readonly',true);
  $('#Procurement_detailView_fieldValue_proc_current_approval').hide();
  $('#Procurement_detailView_fieldLabel_proc_current_approval').hide();
  ...
  $("#proc_supplier_display").closest( "td" ).append('<br><i class="fa fa-refresh" title="Find Agreement" aria-hidden="true" id="searchagreement" style="cursor:pointer;"></i>');
	$("#Procurement_editView_fieldName_proc_agreement_no").closest( "td" ).html('<select name="proc_agreement_no" class="select2 agreementdropdown" style="width:75%" required><option value="">Select Option</option></select><span class="agreement_error" style="display:block;">To get Agreement, click <i class="fa fa-refresh" title="Find Agreement"></i> icon under supplier field</span>');
												 ...
												 data: {
													...
													success: function(res){
														...
														else 
														{
															...
															$('#searchagreement').removeClass('fa-spin');
															$(".agreementdropdown").closest( "td" ).html('<select name="proc_agreement_no" class="select2 agreementdropdown" style="width:75%" required><option value="">Select Option</option></select>');
															
															$(".agreementdropdown").select2();
															$(".agreement_error").html('');
															$(".agreementdropdown").closest( "td" ).append('<span class="agreement_error" style="color:#f30;display:block;">No Adminstrative Agreement Found for Selected Supplier</span>');

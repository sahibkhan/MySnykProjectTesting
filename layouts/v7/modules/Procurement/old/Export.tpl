				...
				<div class="datacontent row">
					...
					<div class="col-lg-6">
						<div class="well exportContents">
							...
							<div><b>Select Filter Options</b></div>
							<br>
								<div>
									<select name="request_type">
										<option value="">Select Request Type</option>							
										{foreach item=procurementType from=$procurementTypes key=procTypeID}
										<option value="{$procTypeID}">{$procurementType}</option>
										{/foreach}
									</select> &nbsp;&nbsp;
									<select name="location_id">
										<option value="">Select Location</option>
										{foreach item=locationList from=$locationsList key=locationID}
										<option value="{$locationID}">{$locationList}</option>
										{/foreach}
									</select>
									&nbsp;
									<input type="text" class="dateField form-control " data-fieldtype="date" name="date_range1" data-date-format="yyyy-mm-dd" value="" data-rule-date="true" placeholder="Creation Date From" style="width:138px;height:24px;display:inline-block;">
									&nbsp;
									<input type="text" class="dateField form-control " data-fieldtype="date" name="date_range2" data-date-format="yyyy-mm-dd" value="" data-rule-date="true" placeholder="Creation Date To" style="width:120px;height:24px;display:inline-block;">
									
								</div>
							<br>
							{** Custom Filter Ends **}
							<div><b>{vtranslate('LBL_EXPORT_DATA',$MODULE)}</b></div><br>
							<div style="margin-left: 50px;">
								<div>

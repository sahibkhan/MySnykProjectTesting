...
<input type="hidden" name="proc_location" id="proc_location" value="{$locations}"/>
				...
				<div class="bottomscroll-div">
					<table id="tablemainid" class="table table-striped table-bordered nowrap table-responsive" style="max-width:100%;border: 1px solid Red">
						...
						<tr class="listViewHeaders" style="max-width:100%">
							...
							<th nowrap="" style="border:1px solid #D4D4D4 !important;">Last Purchase Price</th>
	...
	<input type="hidden" value="{$expensetotalrecords}" name="expensetotalrecords">
							...
							<input type="hidden" value="{$FIELD_MODEL['procurementitemid']}" name="procurementitemid[]">

							<tr class="listViewEntries" style="max-width:100%" id="apend-{$FIELD_MODEL['procurementitemid']}">
								...
								{if $CHILD_TABLE_RECORD}
													<td nowrap="" style="border:1px solid #D4D4D4 !important;">
														<select {if $proc_proctype eq '4786741'} onchange="get_stock(this);" {/if} class="attribute select2" id="toselect" name="childvales[]">
															...
															<option value="{$CHILD_MODEL['procurementsubitemsid']}" {if $FIELD_MODEL['expence_type'] eq $CHILD_MODEL['procurementsubitemsid']} selected {/if}>{$CHILD_MODEL['childname']}</option>
															{/foreach}
														</select>
													</td>
							<td nowrap="" style="border:1px solid #D4D4D4 !important;">
							...
							<td nowrap="" style="border:1px solid #D4D4D4 !important;"><input class="attribute" type="text" name="qty1[]" value="{$FIELD_MODEL['quantity']}" id="qty1"/></td>
								<td nowrap="" style="border:1px solid #D4D4D4 !important;"><input class="attribute" type="text" name="psc1[]" value="{$FIELD_MODEL['ppu']}" id="psc1" onblur="getlocalprice1('{$FIELD_MODEL['procurementitemid']}')"/></td>
									<td nowrap="" style="border:1px solid #D4D4D4 !important;"><input class="attribute" type="text" name="localprice[]" readonly value="{$FIELD_MODEL['local_price']}" id="localprice-apend-{$FIELD_MODEL['procurementitemid']}"/></td>
									<td nowrap="" style="border:1px solid #D4D4D4 !important;">
										<input type="hidden" value="{$FIELD_MODEL['vat_rate']}" id="vat_original_value">
										<input class="attribute" type="text" name="VAT[]" value="{$FIELD_MODEL['vat_rate']}" id="VAT" onblur="get_currency_data('{$FIELD_MODEL['procurementitemid']}')"/></td>
										...
										<td nowrap="" style="border:1px solid #D4D4D4 !important;"><input class="attribute" type="text" name="gross[]" readonly value="{$FIELD_MODEL['gross']}" id="gross-apend-{$FIELD_MODEL['procurementitemid']}"/></td>

									{if $CURRENCYDATA}
														<td nowrap="" style="border:1px solid #D4D4D4 !important;">
	<input type="hidden" id="hidden_change_currency-apend-{$FIELD_MODEL['procurementitemid']}" value="{$FIELD_MODEL['local_currency_code']}">

															<select class="attribute select2" id="currency-apend-{$FIELD_MODEL['procurementitemid']}" name="currency[]" onchange="get_currency_data1('apend-{$FIELD_MODEL['procurementitemid']}')">
												...
												<td nowrap="" style="border:1px solid #D4D4D4 !important;"><input class="attribute" type="text" name="gross_local[]" readonly value="{$FIELD_MODEL['gross_local']}" id="gross_local-apend-{$FIELD_MODEL['procurementitemid']}"/></td>
												<td nowrap="" style="border:1px solid #D4D4D4 !important;">
													<input class="attribute" type="hidden" name="Total_local_currency_hidden" readonly id="Total_local_currency_hidden" value="0"/>
													<input class="attribute" type="text" name="Total_local_currency[]" readonly value="{$FIELD_MODEL['total_local_amount']}" id="Total_local_currency-apend-{$FIELD_MODEL['procurementitemid']}"/></td>
													<td nowrap="" style="border:1px solid #D4D4D4 !important;"><input class="attribute" type="text" name="Total_USD[]" readonly value="{$FIELD_MODEL['total_in_usd']}" id="Total_USD-apend-{$FIELD_MODEL['procurementitemid']}"/></td>

													<td nowrap="" style="border:1px solid #D4D4D4 !important;"><input class="attribute" type="text" name="current_qty[]" readonly value="{$FIELD_MODEL['current_qty']}" id="current_qty_{$FIELD_MODEL['procurementitemid']}"/></td>

													<td nowrap="" style="border:1px solid #D4D4D4 !important;"><input class="attribute" type="text" name="avg_consumption[]" readonly value="{$FIELD_MODEL['avg_consumption']}" id="avg_consumption_{$FIELD_MODEL['procurementitemid']}"/></td>
													<td nowrap="" style="border:1px solid #D4D4D4 !important;"><input class="attribute" type="text" name="last_purchase_price[]" readonly value="{$FIELD_MODEL['last_purchase_price']}" id="last_purchase_price_{$FIELD_MODEL['procurementitemid']}"/></td>
													<td nowrap="" style="border:1px solid #D4D4D4 !important;"><input class="attribute" type="text" name="last_qty[]" readonly value="{$FIELD_MODEL['last_qty']}" id="last_qty_{$FIELD_MODEL['procurementitemid']}"/></td>

							{/if}
							{if $sending_approvals[0]['who_approve_id'] eq 0}
							<td nowrap="">
								<button type="button" class="btn" id="deleteaddrecord" onClick="deleterecord({$FIELD_MODEL['procurementitemid']})"><span class="add-on"><i class="fa fa-trash"></i></span></button>
							</td>
							{/if}
								...
								{/foreach}
								{else}
								<input type="hidden" value="0" name="procurementitemid[]">
																	{/if}
						</tbody>
...
$("[name=assigned_user_id] option").filter(function() {
    return ($(this).text() == user_name); //To select Blue
}).prop('selected', true);

$("[name=proc_department] option").filter(function() {
    return ($(this).text() == department); //To select Blue
}).prop('selected', true);

$("[name=proc_location] option").filter(function() {
    return ($(this).text() == locationsname); //To select Blue
}).prop('selected', true);
</script>

...
<input type="hidden" name="proc_location" id="proc_location" value="{$locations}"/>
	...
	<div name='editContent'>
		...
		{/if}
		{if $items_missing_flag}
			<div class="fieldBlockContainer duplicationMessageContainer">
				<div class="duplicationMessageHeader"><b>{vtranslate('-	Procurement items missing. Please add items to proceed -', $MODULE)}</b></div>
				
			</div>
		{/if}
		{if $irrelevant_agreement_no_flag}
			<div class="fieldBlockContainer duplicationMessageContainer">
				<div class="duplicationMessageHeader"><b>{vtranslate('-	Agreement number is missing OR irrelevant for the selected supplier. After selecting the supplier always click the refresh icon <i class="fa fa-refresh" title="Find Agreement" aria-hidden="true"></i> and then choose the Agreement No  -', $MODULE)}</b></div>
			</div>
		{/if}

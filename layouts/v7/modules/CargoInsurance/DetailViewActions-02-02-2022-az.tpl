...
{strip}
    ...
    <div class="col-lg-6 detailViewButtoncontainer">
        <div class="pull-right btn-toolbar">
            <div class="btn-group">
				{$CANCEL_SENT} 
				referal {$REFERALL} cancel sent {$CANCEL_SENT_S}
				...
				<button class="btn btn-default">
                    <a onclick="javascript:window.location='index.php?module=Job&relatedModule=CargoInsurance&view=Detail&record={$HISTORY_JOB_ID}&mode=showRelatedList&tab_label=Insurance';" type="reset" ><strong> Back To Job </strong></a>
				</button>
				{if $INSURANCE_TYPE == 'WIS Insurance'}
					{if  $CANCEL_SENT_S != 'yes' }
						<button class="btn btn-default">
						<a href="javascript:void(0);" id="postwis">
						 {vtranslate('LBL_POST_TO_WIS',$MODULE)}
						 </a>
						</button>
					{/if}
					...
					{if $REFERALL == 'yes' || $CANCEL_SENT_S == 'yes' }
						<button class="btn btn-default">
							<a {*target="_blank"*} id="check_referal" style="display: block;" href="#{*include/CargoInsurance/webhookstatus.php?record={$RECORD->getId()}*}" c><strong> Check Referal </strong></a>
						</button>
					{/if}
					...
					{if $WIS_REF_NUMBER != null && $CANCEL_SENT_S != 'yes'}
						<td width="120"> 
					...
					<ul class="dropdown-menu">
						...
						{if $INSURANCE_TYPE == 'WIS Insurance'}
							{if $WIS_REF_NUMBER != null}
								<button class="btn btn-default">
									<a target="_blank" id="hidebutton" style="display: block;"						href="include/CargoInsurance/printcertificate.php?record={$RECORD->getId()}&declaration_id={$DECLARATION_ID}" ><strong> Print WIS Certificate </strong></a>
								</button>						
							{/if}
						{/if}
					</ul>

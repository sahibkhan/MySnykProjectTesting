		...
		<div class="relatedContents col-lg-12 col-md-12 col-sm-12 table-container">
			<div class="bottomscroll-div">
				<table id="listview-table" class="table listview-table">
								...
								<td class="relatedListEntryValues" title="{strip_tags($RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME))}" data-field-type="{$HEADER_FIELD->getFieldDataType()}" nowrap>
									<span class="value ">
										{if $RELATED_MODULE_NAME eq 'Documents' && $RELATED_HEADERNAME eq 'document_source'}
											...
											{elseif $RELATED_HEADERNAME eq 'cf_4317' || $RELATED_HEADERNAME eq 'cf_4319' || $RELATED_HEADERNAME eq 'cf_4321'}
                                            <input id="PostTrip_editView_fieldName_{$RELATED_HEADERNAME}_{$RELATED_RECORD->getId()}" data-id="{$RELATED_RECORD->getId()}" type="text" class="listSearchContributor inputElement" 
                                            name="{$RELATED_HEADERNAME}[{$RELATED_RECORD->getId()}]" value="{$RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME)}" 
                                            data-fieldinfo="">
                                                {if $HEADER_FIELD->get('uitype') eq '55'}
                                                &nbsp;{$RELATED_RECORD->getDisplayValue('cf_4329')}
                                                {/if}
                                             {elseif $RELATED_HEADERNAME eq 'cf_4557'}                            
                                            <input id="PostTrip_editView_fieldName_{$RELATED_HEADERNAME}_{$RELATED_RECORD->getId()}" type="checkbox" 
                                            name="{$RELATED_HEADERNAME}[{$RELATED_RECORD->getId()}]"
                                            {if $RELATED_RECORD->getDisplayValue('cf_4557') eq 'Yes'} checked {/if}
                                            />      
                                            {else}
                    ...
                    <td class="relatedListEntryValues">Total</td>
                    <td class="relatedListEntryValues"></td>
                    <td class="relatedListEntryValues"></td>
                    <td class="relatedListEntryValues"></td>
                    <td class="relatedListEntryValues"></td>
                    <td id="post_total_tenge_id" class="relatedListEntryValues">{$POST_TOTAL_TENGE}</td>
                    <td class="relatedListEntryValues"></td>
                    <td class="relatedListEntryValues"></td>
                    <td class="relatedListEntryValues">{$PRE_TOTAL_TENGE}</td>
                    <td class="relatedListEntryValues"></td>
                    </tr>

                    <tfoot>
                <tr class="listViewEntries">
                <td colspan="4" nowrap=""  class="medium" align="right">&nbsp;</td>
                    ...
                    <td colspan="4" nowrap=""  class="medium" align="right">&nbsp; 
                    </td>
                </tr>
                </tfoot>
				</table>

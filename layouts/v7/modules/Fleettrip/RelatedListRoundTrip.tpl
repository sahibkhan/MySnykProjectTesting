        ...
        <table id="listview-table" class="table listview-table listViewEntriesTable">
          ...
          <tbody>
            ...
            <td class="related-list-actions">
              ...
              {/if}
              {if ({$RELATED_RECORD->getDisplayValue('cf_3173')} eq '0' || {$RELATED_RECORD->getDisplayValue('cf_3173')} eq '0.00')}
            ...
            <td class="{if $RELATED_HEADERNAME eq 'cf_3175'}related-list-actions{else}relatedListEntryValues{/if}" title="{strip_tags($RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME))}" data-field-type="{$HEADER_FIELD->getFieldDataType()}" nowrap>
              <span class="value textOverflowEllipsis">
                ...
                {assign var=PICKLIST_FIELD_ID value={$HEADER_FIELD->getId()}}
                {/if}
                <span {if !empty($RELATED_LIST_VALUE)} class="picklist-color picklist-{$PICKLIST_FIELD_ID}-{Vtiger_Util_Helper::convertSpaceToHyphen($RELATED_LIST_VALUE)}" {/if}> {$RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME)} 
              </span>
              {elseif $RELATED_HEADERNAME eq 'cf_3173'}
              <input id="Roundtrip_editView_fieldName_cf_3173_{$RELATED_RECORD->getId()}" type="text" class="listSearchContributor inputElement" 
                name="internal_selling[{$RELATED_RECORD->getId()}-{$RELATED_RECORD->get('cf_3175')}]" value="{$RELATED_RECORD->getDisplayValue($RELATED_HEADERNAME)}" 
                data-fieldinfo=""> USD
              {else}
              {if $RELATED_HEADERNAME eq 'cf_3175'} 
              <a target="_blank" href="index.php?module=Job&relatedModule=Jobexpencereport&view=Detail&record={$RELATED_RECORD->get($RELATED_HEADERNAME)}&mode=showRelatedList&relationId=167&tab_label=Job%20Revenue%20and%20Expence&app=JOB">
          ...
          <tfoot>
            <tr class="listViewEntries">
              <td nowrap="" class="relatedListEntryValues"></td>
              <td nowrap="" class="relatedListEntryValues"></td>
              <td nowrap="" class="relatedListEntryValues"></td>
              <td nowrap="" class="relatedListEntryValues"></td>
              <td nowrap="" class="relatedListEntryValues" align="right">Total Trip Revenew: </td>
              <td nowrap="" class="relatedListEntryValues">{$TOTAL_REVENEW} USD</td>
              <td nowrap="" class="relatedListEntryValues"></td>
            </tr>
            <tr class="listViewEntries">
              <td nowrap="" class="relatedListEntryValues"></td>
              ...
              <td nowrap="" class="relatedListEntryValues"></td>
              <td nowrap="" class="relatedListEntryValues"></td>
              <td nowrap="" class="relatedListEntryValues" align="right">Internal Selling Final: </td>
              <td nowrap="" class="relatedListEntryValues">             
            ...
            <tr class="listViewEntries">
              ...
              <td nowrap="" class="relatedListEntryValues"></td>
              <td nowrap="" class="relatedListEntryValues"></td>
              <td nowrap="" class="relatedListEntryValues" align="right">Final Distribution:</td>
              <td nowrap="" class="relatedListEntryValues">
            ...
            <tr class="listViewEntries">
              <td colspan="4" nowrap=""  class="relatedListEntryValues" align="right">                
                {if ($ROUND_TRIP_STATUS neq 'Completed' || $USER_MODEL->get('id')==$PARENT_RECORD->get('cf_4947'))}                
                <button class="btn btn-success" type="submit" name="saveButton"><strong>{vtranslate('LBL_SAVE', $MODULE)}</strong></button>
                {else}
                <font style="color:#F00;font-size:14px; font-weight:bold;">
                For revision, please contact Maral Malikova (Accountant).
                </font>
                {/if}
              ...
              <td colspan="3" nowrap=""  class="relatedListEntryValues" align="right">
  ...
  <div class="relatedContents col-lg-12 col-md-12 col-sm-12 table-container" style="margin-top: 10px;">
    <div class="bottomscroll-div">
      <table class="table listview-table" id="listview-table" >
        <thead>
          <tr class="listViewHeaders">
            <th colspan="5">Costing Breakdown</th>
          </tr>
        </thead>
        <thead>
          <tr class="listViewHeaders">
            <th>&nbsp;</th>
            <th>{vtranslate('Job File', $RELATED_MODULE->get('name'))}</th>
            <th>{vtranslate('Internal Selling', $RELATED_MODULE->get('name'))}</th>
            <th>{vtranslate('Costing breakdown', $RELATED_MODULE->get('name'))}</th>
            <th>{vtranslate('Profit breakdown', $RELATED_MODULE->get('name'))}</th>
            <th>{vtranslate('JCR', $RELATED_MODULE->get('name'))}</th>
          </tr>
        </thead>
        <tbody>
          ...
          <tr class="listViewEntries">
            <td>&nbsp;</td>
            <td class="relatedListEntryValues">{$PROFIT['job_ref_no']}</td>
            <td class="relatedListEntryValues">{$PROFIT['internal_selling']}</td>
            <td class="relatedListEntryValues">{$PROFIT['cost']}</td>
            <td class="relatedListEntryValues">{$PROFIT['job_profit']}</td>
            <td class="related-list-actions">
              <a href="index.php?module=Jobexpencereport&view=Print&record={$PROFIT['job_id']}&userid={$PROFIT['user_id']}&expense=SUBJCR&type=JCR"
                target="_new" title="">
              <i class="fa fa-print alignMiddle" title="Job File Sub JCR"></i></a>
            </td>
          </tr>
          {/foreach} 
        </tbody>

			...
			<div class="row">
						...
						<div class="logo-container col-lg-9 col-md-9 col-sm-9 col-xs-9">
							<div class="row">
								<a href="index.php" class="company-logo">
									<img src="{$COMPANY_LOGO->get('imagepath')}" alt="{$COMPANY_LOGO->get('alt')}" title="GEMS :: Globalink Enterprise Management System" />
								</a>
							</div>
						</div>  
					</div>
				</div>
				<div class="search-links-container col-md-3 col-lg-3 col-sm-3" style="display:flex">
				<select name="module_names" class="select2" id="module_names" style="zoom:1; margin-top:5.5px;margin-right:5px;display:none;">
						{foreach key=MODULE_NAME item=fieldObject from=$SEARCHABLE_MODULES}
	                    <option {if $MODULE_NAME == $MODULE} selected{/if}  value="{$MODULE_NAME}" class="globalSearch_module_{$MODULE_NAME}">{vtranslate($MODULE_NAME,$MODULE_NAME)}</option>
	          			{/foreach}
					</select>
				...
				<div id="navbar" class="col-sm-6 col-md-3 col-lg-3 collapse navbar-collapse navbar-right global-actions">
					<ul class="nav navbar-nav">
						...
						<li><div><a href="/index.php?module=Project&view=List&app=SUPPORT" class="fa fa-briefcase" title="Task Manager" aria-hidden="true"></a></div></li>
						<li><div><a href="manual/index.php" target="_blank" class="fa fa-book" title="GEMS User Manual" aria-hidden="true"></a></div></li>
						<li><div><a href="index.php?module=Faq&view=List&app=SUPPORT" target="_blank" class="fa fa-question-circle" title="FAQ" aria-hidden="true"></a></div></li>
						<li><div><a href="manual/abbr.php" target="_blank" class="fa fa-info-circle" title="System Abbreviations" aria-hidden="true"></a></div></li>
						...
						{if $USER_PRIVILEGES_MODEL->hasModulePermission($REPORTS_MODULE_MODEL->getId())}
							<li><div><a href="index.php?module=Reports&view=List" class="fa fa-chart-bar" title="{vtranslate('Reports','Reports')}" aria-hidden="true"></a></div></li>
						{/if}
						{if $USER_PRIVILEGES_MODEL->hasModulePermission($CALENDAR_MODULE_MODEL->getId())}

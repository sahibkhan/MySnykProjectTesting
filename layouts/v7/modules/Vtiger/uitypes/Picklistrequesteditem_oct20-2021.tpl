...
{if $ITEMDATAID}
	<input type="hidden" id="requesteditemid" value="edit">
	{else}
	<input type="hidden" id="requesteditemid" value="add">
{/if}
...
<option value="{$PICKLIST_VALUE['requesteditemid']}" {if $PICKLIST_COLORS[$PICKLIST_NAME]}class="{$CLASS_NAME}"{/if} {if $PICKLIST_VALUE['requesteditemid'] eq $ITEMDATAID} selected {/if}>{$PICKLIST_VALUE['name']}</option>
	{/foreach}
</select>
{if $PICKLIST_COLORS}
	<style type="text/css">

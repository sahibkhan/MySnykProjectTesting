...
{assign var=WH_LOC value=$FIELD_MODEL->getWHLocationList($WHID, $PARENT_ID, $DOC_TYPE, $CUSTOMER_ID, $IN_HOUSE, $GLK_COMPANY_ID)} 
<select class="inputElement select2" 
	...
	<option value="{$WHL_KEY}" title="{vtranslate({$WHL_DSC.type}, $MODULE)}" data-picklistvalue= '{$WHL_KEY}' 
	   {if $FIELD_MODEL->get('fieldvalue') eq $WHL_KEY} selected {/if} > 
		{vtranslate({$WHL_DSC.location}, $MODULE)}</option>
     {/foreach}
</select>

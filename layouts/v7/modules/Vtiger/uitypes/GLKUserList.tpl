...
data-fieldinfo='{$FIELD_INFO}' {if $MODULE eq 'Potentials'} disabled="" {/if} >
	<option value="">--Select User--</option>
{foreach item=GLKUSER key=GLK_USER_ID from=$GLK_USER_LIST}
	<option value='{$GLK_USER_ID}' data-picklistvalue='{$GLK_USER_ID}' {if $FIELD_MODEL->get('fieldvalue') eq $GLK_USER_ID} selected {/if} > {vtranslate({$GLKUSER}, $MODULE)}</option>
{/foreach}
</select>

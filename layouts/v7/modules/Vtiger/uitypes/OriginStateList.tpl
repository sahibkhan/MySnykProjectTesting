...
{strip}
	...
	{assign var=STATES_LIST value=$FIELD_MODEL->getOriginStateList($GEN_COUNTRY_CODE,$GEN_STATE_CODE)}
    
    {if $MODULE eq 'Quotes' OR  $MODULE eq 'BO' OR  $MODULE eq 'VPO' OR  $MODULE eq 'CargoInsurance' OR  $MODULE eq 'RRSInsurance'}
    <input type="hidden" name="{$FIELD_MODEL->getFieldName()}" value=""  data-fieldtype="multipicklist"/>{/if}
	...
	{if $FIELD_MODEL->get('fieldvalue') neq '' || $GEN_COUNTRY_CODE eq 'US' || $GEN_COUNTRY_CODE eq 'CA'}

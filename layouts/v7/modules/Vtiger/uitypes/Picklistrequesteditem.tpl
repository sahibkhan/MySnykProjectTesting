...
{strip}
{assign var="FIELD_INFO" value=$ITEMDATA}
{*$ITEMDATA|@print_r*}
{assign var="SPECIAL_VALIDATOR" value=$FIELD_MODEL->getValidator()}
{assign var=PICKLIST_VALUES value=$FIELD_INFO}
{assign var=PICKLIST_COLORS value=$FIELD_INFO['picklistColors']}
{if $ITEMDATAID}
	<input type="hidden" id="procurementtypesid" value="edit">
	{else}
	<input type="hidden" id="procurementtypesid" value="add">
{/if}
<select data-fieldname="{$FIELD_MODEL->getFieldName()}" data-fieldtype="picklist" class="inputElement select2 {if $OCCUPY_COMPLETE_WIDTH} row {/if}" type="picklist" name="{$FIELD_MODEL->getFieldName()}" {if !empty($SPECIAL_VALIDATOR)}data-validator='{Zend_Json::encode($SPECIAL_VALIDATOR)}'{/if} data-selected-value='{$FIELD_MODEL->get('fieldvalue')}'
	...
	{foreach item=PICKLIST_VALUE key=PICKLIST_NAME from=$FIELD_INFO}
		{assign var=CLASS_NAME value="picklistColor_{$FIELD_MODEL->getFieldName()}_{$PICKLIST_NAME|replace:' ':'_'}"}
<option value="{$PICKLIST_VALUE['procurementtypesid']}" {if $PICKLIST_COLORS[$PICKLIST_NAME]}class="{$CLASS_NAME}"{/if} {if $PICKLIST_VALUE['procurementtypesid'] eq $ITEMDATAID} selected {/if}>{$PICKLIST_VALUE['name']}</option>
	{/foreach}
</select>
{if $PICKLIST_COLORS}
	<style type="text/css">

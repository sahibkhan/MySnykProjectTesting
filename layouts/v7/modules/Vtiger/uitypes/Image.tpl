	...
	{else}
		...
		<div class="fileUploadContainer text-left">
			<div class="fileUploadBtn btn btn-primary">
				...
				<input type="file" {if $MODULE eq 'ProjectCargoCheckList'}accept=".jpg, .jpeg, .png, .gif, .pdf, .xlsx, .docx"{/if} class="inputElement {if $MODULE eq 'Products' || $MODULE eq 'ContractAgreementTemplate'}multi max-6{/if} {if $FIELD_MODEL->get('fieldvalue') and $FIELD_INFO["mandatory"] eq true} ignore-validation {/if}" name="{$FIELD_MODEL->getFieldName()}[]" value="{$FIELD_MODEL->get('fieldvalue')}"
		...
		{if $FIELD_MODEL->getFieldDataType() eq 'image' || $FIELD_MODEL->getFieldDataType() eq 'file'}
			{if $MODULE neq 'Products'}
				...
				{vtranslate('LBL_NOTE_EXISTING_ATTACHMENTS_WILL_BE_REPLACED', $MODULE)}
                        ...
                        {/if}
		{/if}
		{if $MODULE eq 'Products' || $MODULE eq 'ContractAgreementTemplate'}<div id="MultiFile1_wrap_list" class="MultiFile-list"></div>{/if}

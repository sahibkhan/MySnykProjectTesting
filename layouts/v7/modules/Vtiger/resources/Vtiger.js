...
Vtiger.Class('Vtiger_Index_Js', {
	...
	handleQuickCreateDataRoundTrip: function(data, params) {
        ...
        var thisInstance = this;
		//alert(data);
        $('.listViewEntriesTable >tbody>tr:last').after(data);
		//$('select2').chosen();
		//app.event.trigger("select2");
		//$('select').trigger('liszt:updated');
	
		//var select2Elements = jQuery(data).find('.select2');
		//select2Elements.trigger('liszt:updated');
		...
		handleQuickCreateDataEDocuments: function(data, params) {
	        ...
	        var thisInstance = this;
			//alert(data);
			//$("div .fieldBlockContainer[data-block='Files Cloning']").hide();
	        $('.listViewEntriesTable >tbody>tr:last').after(data);
			//$('select2').chosen();
			//app.event.trigger("select2");
			//$('select').trigger('liszt:updated');
    ...
    handleQuickCreateDataPMItems: function(data, params) {
	...
	handleQuickCreateDataNCRRaised: function(data, params) {
	...
	handleQuickCreateDataRailwayFleet: function(data, params) {
	...
	registerQuickCreateEvent : function (){
		...
		jQuery("#quickCreateModules").on("click",".quickCreateModule",function(e,params){
			...
			thisInstance.getQuickCreateForm(quickCreateUrl,quickCreateModuleName,params).then(function(data){
				app.helper.hideProgress();
				//console.log(quickCreateModuleName);

				if(quickCreateModuleName=='Roundtrip'){
					
					thisInstance.handleQuickCreateDataRoundTrip(data, params);
				}

				else if(quickCreateModuleName=='EDocuments'){
					
					thisInstance.handleQuickCreateDataEDocuments(data, params);
				}

				else if(quickCreateModuleName=='RailwayFleet'){
					thisInstance.handleQuickCreateDataRailwayFleet(data, params);
				}
				else if(quickCreateModuleName=='PMItems'){
					
					thisInstance.handleQuickCreateDataPMItems(data, params);
				}
				else if(quickCreateModuleName=='NCRS'){
					//console.log(data);
					thisInstance.handleQuickCreateDataNCRRaised(data, params);
				}
                else{
				var callbackparams = {
		...
		jQuery("#table_KZ").on("click",".quickCreateModule2",function(e,params){
			
			var quickCreateElem = jQuery(e.currentTarget);
			var quickCreateUrl = quickCreateElem.data('url');			
			//var quickCreateModuleName = quickCreateElem.data('name');
			var quickCreateModuleName = "TruckTracking";
			if (typeof params === 'undefined') {
				params = {};
			}
		...
		jQuery("#table_KZWAGON").on("click",".quickCreateModule3",function(e,params){
			
			var quickCreateElem = jQuery(e.currentTarget);
			var quickCreateUrl = quickCreateElem.data('url');			
			//var quickCreateModuleName = quickCreateElem.data('name');
			var quickCreateModuleName = "WaggonTracker";
			if (typeof params === 'undefined') {
				params = {};
			}
			...
			submitHandler: function(form) {
				...
				app.request.post({data:formData}).then(function(err,data){
					...
					if(err === null) {
						...
						if(module=='TruckTracking'){
							//alert(JSON.stringify(module));	
							var location_name = data['name']['value'];							
							var truck_status = data['status'];
							var truck_id =  data['truck_id'];
							var currentDate =data['cf_2187']['value'];
							var date = currentDate.split("-");
							var day_date = date[0];
							var td_id_status = truck_id+'_'+day_date+'_status';
							var td_id_location = truck_id+'_'+day_date+'_location';
							jQuery("[id="+td_id_location+"]").html(location_name);
							jQuery("[id="+td_id_status+"]").html(truck_status);
						}else if(module=='WaggonTracker')
						{
							var location_name = data['name']['value'];							
							var wagon_status = data['status'];
							var wagon_id =  data['wagon_id'];
							var currentDate =data['cf_5780']['value'];
							var date = currentDate.split("-");
							var day_date = date[0];
							var td_id_status = wagon_id+'_'+day_date+'_status';
							var td_id_location = wagon_id+'_'+day_date+'_location';
							jQuery("[id="+td_id_location+"]").html(location_name);
							jQuery("[id="+td_id_status+"]").html(wagon_status);
						}
	...
	registerAutoCompleteFields : function(container) {
		var thisInstance = this;
		container.find('input.autoComplete, input#Accounts_editView_fieldName_accountname').autocomplete({
			'minLength' : '3',
			'source' : function(request, response){
				...
				if (jQuery('#QuickCreate').length > 0) {
					params.module = jQuery('.editViewContents').find('[name="module"]').val();
				}
				/*Add new custom param in request*/
				params.field_id = inputElement[0]['attributes'][0]['nodeValue'];
				/*Custom param ends*/
				...
				if(params.search_module && params.search_module!= 'undefined') {
					thisInstance.searchModuleNames(params).then(function(data){
						...
						if(serverDataFormat.length <= 0) {
							if ( ! jQuery('input#Accounts_editView_fieldName_accountname').length) {
					...
					if ( ! jQuery(inputElement, '.editContent').length ) {
						jQuery(inputElement).val('');
					}
					serverDataFormat = new Array({
						'label' : 'No Results Found',
					...
					jQuery('.custom_preloader').remove();
				}
			},
			'select' : function(event, ui ){
				...
				if (jQuery('#QuickCreate').length > 0 && jQuery('input#Accounts_editView_fieldName_accountname').length > 0) {
					if (jQuery('input[name="related_to_display"]').length > 0) {
						element = jQuery('input[name="related_to_display"]');
					} else if (jQuery('input[name="account_id_display"]').length > 0) {
						element = jQuery('input[name="account_id_display"]');
					} else if (jQuery('input[name="accountid_display"]').length > 0) {
						element = jQuery('input[name="accountid_display"]');
					} else if (jQuery('input[name="parent_id_display"]').closest('td').find('input[name="popupReferenceModule"]').val() == 'Accounts' && jQuery('input[name="parent_id_display"]').length > 0) {
						element = jQuery('input[name="parent_id_display"]');
					} else if (jQuery('input[name="cf_4047_display"]').length > 0) {
						element = jQuery('input[name="cf_4047_display"]');
					} else if (jQuery('input[name="cf_1441_display"]').length > 0) {
						element = jQuery('input[name="cf_1441_display"]');
					} else if (jQuery('input[name="client_name_crg_display"]').length > 0) {
						element = jQuery('input[name="client_name_crg_display"]');
					}
				}
				...
				if (jQuery('#QuickCreate').length > 0 && jQuery('input#Accounts_editView_fieldName_accountname').length > 0) {
					if (jQuery('input[name="related_to_display"]').length > 0) {
						...
						fieldName = 'accountid';
					} else if (jQuery('input[name="popupReferenceModule"]').val() == 'Accounts' && jQuery('input[name="parent_id_display"]').length > 0) {
						fieldName = 'parent_id';
					} else if (jQuery('input[name="cf_4047_display"]').length > 0) {
						fieldName = 'cf_4047';
					} else if (jQuery('input[name="cf_1441_display"]').length > 0) {
						...
						fieldName = 'client_name_crg';
					}
				}
				parent.find('input[name="'+fieldName+'"]').val(selectedItemData.id);
				if (jQuery('#QuickCreate').length > 0 && jQuery('input#Accounts_editView_fieldName_accountname').length > 0) {
					if (element.attr('name') == 'related_to_display' || element.attr('name') == 'account_id_display' || element.attr('name') == 'accountid_display' || element.attr('name') == 'parent_id_display' || element.attr('name') == 'cf_4047_display' || element.attr('name') == 'cf_1441_display' || element.attr('name') == 'client_name_crg_display') {
						element.attr("value",selectedItemData.selectedName);
						app.helper.hideModal();
					}
				} else if (!jQuery('#QuickCreate').length && jQuery('input[name="module"]', '.editViewContents').val() == "Accounts") {
					element.attr("value",selectedItemData.selectedName);
				} else {
					element.attr("value",selectedItemData.id);
				}

				if (!jQuery('#QuickCreate').length && jQuery('input[name="module"]', '.editViewContents').val() == "Accounts" && element.attr('data-fieldtype') !== 'reference') {
					var domainName = location.hostname;
					var directory = '';
					if (domainName == 'tiger.globalink.net') {
						directory = '/live-gems';
					}
					window.open(directory+'/index.php?module=Accounts&view=Detail&record='+selectedItemData.id+'&app=SALES', '_blank');
				} else {
					...
					if ( jQuery('[name="returnrelationId"]').val() == 206 ) {
						jQuery('[name="cf_2435"]', container).val(selectedItemData.clientStatus).trigger("change");
						jQuery('[name="cf_2437"]', container).val(selectedItemData.accountType).trigger("change");
						jQuery('[name="cf_2425"]', container).val(selectedItemData.salesPerson).trigger("change");
					}
				}
...
//					
					if (jQuery('[name="returnrelationId"]').val() == 206) {
						var data = {
							'name' : value.name,
							'id' : key,
							'account_type' : value.info.account_type,
							'cf_725' : value.info.cf_725,
							'assigned_user_id' : value.info.assigned_user_id,
						}
					} else {
						var data = {
							'name' : value.name,
			...
			if ( jQuery('[name="returnrelationId"]').val() == 206 ) {
				jQuery('[name="cf_2437"]').val(dataList[0]['account_type']).trigger("change");
				jQuery('[name="cf_2435"]').val(dataList[0]['cf_725']).trigger("change");
				jQuery('[name="cf_2425"]').val(dataList[0]['assigned_user_id']).trigger("change");
			}
			if(isMultiple) {
	...
	*/
	getReferenceSearchParams : function(element){
		...
		var params = {};
		var quickCreateAccount = jQuery('input#Accounts_editView_fieldName_accountname', '#QuickCreate').length;
		var fullFormAccount = jQuery('input#Accounts_editView_fieldName_accountname', '#EditView').length;
		var referenceModuleElement = jQuery('input[name="popupReferenceModule"]',tdElement).length ? jQuery('input[name="popupReferenceModule"]',tdElement) : jQuery('input.popupReferenceModule',tdElement);
		if (quickCreateAccount) {
			referenceModuleElement = jQuery('input[name="module"]','#QuickCreate');
		} else if (fullFormAccount && element.attr('id') == 'Accounts_editView_fieldName_accountname') {
			referenceModuleElement = jQuery('input[name="module"]','#EditView');
		}

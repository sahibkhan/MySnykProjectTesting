			...
			if(terms_condition!='')
{
				...
				let tms = $('#select2-chosen-25').html();
				tms = tms.trim();
				...
				xhr.open('GET', 'include/Quote/pullout_terms_and_cond.php?tms='+tms);
				xhr.onload = function(){
					if (xhr.status == 200){
						let resTM = xhr.responseText;
						// console.log(resTM); 
						CKEDITOR.instances['Quotes_editView_fieldName_terms_conditions'].setData(resTM);
	...
	getRecipientList : function(container){
	 ...
	 registerEventForCkEditor : function(){
		...
		console.log(`loaded ckedititor ...`, noteContentElement)

...
//				}
	...
	public function getleavesdata($uid,$month,$year){
		...
		$breaktime = 0;
		$db = PearDatabase::getInstance();	
			$query = "SELECT * FROM vtiger_leaverequestcf inner join vtiger_leaverequest on vtiger_leaverequest.leaverequestid = vtiger_leaverequestcf.leaverequestid inner join vtiger_crmentity on vtiger_leaverequestcf.leaverequestid = vtiger_crmentity.crmid where cf_3423 = ".$uid." and (MONTH(cf_3391) = ".$month." or MONTH(cf_3393) = ".$month.") and YEAR(cf_3391) = ".$year." and vtiger_crmentity.deleted = 0 and cf_3411 != '' and cf_3415 != ''";
        	$result = $db->pquery($query, array());
				for($i=0;$i<$db->num_rows($result);$i++){
					...
					if($diff > 0)
					{
						...
						elseif($month2 == $month)
						{
							$fromdate = strtotime(date("Y-m-01", strtotime($db->query_result($result,$i,'cf_3393'))));
						}
					}
					$leavedays = ($tilldate - $fromdate)/(60 * 60 * 24);
					if($leavedays == 0)
					{
						if($db->query_result($result,$i,'cf_5355') != "")
						{
							$time = explode(" - ",$db->query_result($result,$i,'cf_5355')); 
						}
						else
						{
							$time = explode(" - ","09:00 - 18:00");
						}
						if(str_replace(":","",$time[0]) <= 1300 && str_replace(":","",$time[1]) >= 1400)
						{
							$breaktime = 0.125;
						}
						else
						...
						$fromtime = strtotime($db->query_result($result,$i,'cf_3391')." ".$time[0]);
						$tilltime = strtotime($db->query_result($result,$i,'cf_3391')." ".$time[1]);
						$leavedays = (0.125*($tilltime - $fromtime)/(60*60)) - $breaktime;
					}
					//echo $leavedays;
					if($leavedays <= 0)
					{
	...
	private function get_login_logout()
	{
		$arr = array();
		$db = PearDatabase::getInstance();	
		$query = "SELECT * FROM vtiger_userattendancecf inner join vtiger_userattendance on vtiger_userattendance.userattendanceid = vtiger_userattendancecf.userattendanceid inner join vtiger_crmentity on vtiger_userattendancecf.userattendanceid = vtiger_crmentity.crmid where cf_5914 = 0 and vtiger_crmentity.deleted = 0 order by cf_5904 DESC, cf_5916 ASC, cf_5894 ASC, cf_5896 ASC";
        $result = $db->pquery($query, array());
		for($i=0;$i<$db->num_rows($result);$i++)
		{
			//echo "<pre>";
			$arr[$db->query_result($result,$i,'cf_5904')][$db->query_result($result,$i,'cf_5894')][$db->query_result($result,$i,'cf_5912')][] = $db->query_result($result,$i,'cf_5896');
			//echo "<br>";
		}
		$userid = '';
		$userdate = '';
		foreach($arr as $id => $darr)
		{
			$userid = $id;
			foreach($darr as $date => $items)
			{
				$userdate = $date;
				if($items['Login'][0] > $items['Logout'][0] && !empty($items['Login'][0]) && !empty($items['Logout'][0]))
				{
					if(!empty($items['Logout'][1]))
						//echo $userid." logout first on ".$userdate."<br>";
						$this->logout_first($userid,$userdate);
					else
						$this->logout_first($userid,$userdate);
						$this->logout_pending($userid,$userdate);
				}
				elseif(empty($items['Logout'][0]) && !empty($items['Login'][0]))
				{
					//echo $userid." did not logout yet on ".$userdate."<br>";
					$this->logout_pending($userid,$userdate);
				}
				elseif(empty($items['Login'][0]) && !empty($items['Logout'][0]))
				{
					//echo $userid." did not login but logout on ".$userdate."<br>";
					$this->login_pending($userid,$userdate);
				}
			}
		}
		//exit;
	...
	//manual entry of logout on previous date having status 2 as login
	private function logout_first($userid,$userdate)
	{
		$db = PearDatabase::getInstance();	
		$query = "SELECT * FROM vtiger_userattendencecf inner join vtiger_userattendence on vtiger_userattendence2.userattendenceid = vtiger_userattendencecf.userattendence2id inner join vtiger_crmentity on vtiger_userattendencecf.userattendenceid = vtiger_crmentity.crmid where cf_5914 = 2 and cf_5894 <= '".$userdate."' and cf_5904 = ".$userid." and vtiger_crmentity.deleted = 0 order by cf_5904 DESC, cf_5916 ASC, cf_5894 ASC, cf_5896 ASC";
        $result = $db->pquery($query, array());
		if($db->num_rows($result) > 0)
		{
		//echo $db->query_result($result,0,'userattendenceid');
		$entry = Vtiger_Record_Model::getInstanceById($db->query_result($result,0,'userattendenceid'), 'UserAttendance');
		$entry->set('record_id','');
		//$entry->set('assigned_user_id',$current_user->id);
		$entry->set("cf_5896",'23:59:59');
		$entry->set("cf_5902","Has left firm");
		$entry->set("cf_5912","Logout");
		$entry->set("cf_6062","Manual entry of logout");
		$entry->set("cf_5914",0);
		$entry->save();
		...
		$request->set("module","UserAttendance");
		$request->set("action","Save");
		$request->set("record",$db->query_result($result,0,'userattendenceid'));
		$request->set("cf_5896",$db->query_result($result,0,'cf_5896'));
		$request->set("cf_5914",0);
		//print_r($request); exit;
		$recordModel2 = $saveattendencereport->saveRecord($request);
		}
	...
	// update status = 3 of all login on same date
	private function logout_pending($userid,$userdate)
	{
		$db = PearDatabase::getInstance();	
		$query = "SELECT * FROM vtiger_userattendencecf inner join vtiger_userattendence on vtiger_userattendence.userattendenceid = vtiger_userattendencecf.userattendenceid inner join vtiger_crmentity on vtiger_userattendencecf.userattendenceid = vtiger_crmentity.crmid where cf_5914 = 0 and cf_5894 = '".$userdate."' and cf_5904 = ".$userid." and vtiger_crmentity.deleted = 0 order by cf_5904 DESC, cf_5916 ASC, cf_5894 ASC, cf_5896 ASC";
        $result = $db->pquery($query, array());
		for($i=0;$i<$db->num_rows($result);$i++)
		{
			$saveattendencereport = new Vtiger_Save_Action();
			$request = new Vtiger_Request();
			$request->set("module","UserAttendence");
			$request->set("action","Save");
			$request->set("record",$db->query_result($result,$i,'userattendenceid'));
			$request->set("cf_5896",$db->query_result($result,$i,'cf_5896'));
			$request->set("cf_5914",3);
			//print_r($request); exit;
			$recordModel2 = $saveattendencereport->saveRecord($request);
		}
	}
	...
	//
	private function login_pending($userid,$userdate)
	{
		$db = PearDatabase::getInstance();	
		$query = "SELECT * FROM vtiger_userattendencecf inner join vtiger_userattendence on vtiger_userattendence.userattendenceid = vtiger_userattendencecf.userattendenceid inner join vtiger_crmentity on vtiger_userattendencecf.userattendenceid = vtiger_crmentity.crmid where cf_5914 = 3 and cf_5894 <= '".$userdate."' and cf_5904 = ".$userid." and vtiger_crmentity.deleted = 0 order by cf_5904 DESC, cf_5916 ASC, cf_5894 ASC, cf_5896 ASC";
        $result = $db->pquery($query, array());

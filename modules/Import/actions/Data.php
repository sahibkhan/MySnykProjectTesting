	...
	public function createRecords() {
		...
		for ($i = 0; $i < $numberOfRecords; ++$i) {
			...
			if ($createRecordExists) {
				...
				if($moduleName == 'UserAttendance')
				{
					unset($fieldData["cf_5906"]);	
					unset($fieldData["cf_6112"]);	
					unset($fieldData["cf_6300"]);
						...
						$entityInfo = null;
							...
							try {
								...
								$fieldData['source'] = $this->recordSource;
									...
									//echo $userattendid; exit;
								$db = PearDatabase::getInstance();	
								//$date=date('Y-m-d',strtotime($fieldData['cf_5894']));	
								//$fieldData['cf_5954'] = date('Y',strtotime($fieldData['cf_5894']));	
								$dt = new DateTime($fieldData['cf_5894']); 	
								$fieldData['cf_5894'] = $dt->format("Y-m-d"); 	
								$fieldData['cf_5954'] = $dt->format("Y");	
									
								//$date=date_create($fieldData['cf_5894']);	
								//$dateObj   = DateTime::createFromFormat('!m', date_format($date,"m"));	
								//$fieldData['cf_5956'] = $dateObj->format('F');	
								$dt1 = new DateTime($fieldData['cf_5896']); 	
								$fieldData['cf_5896'] = $dt1->format("H:i:s");	
									
								$fieldData['cf_5956'] = $dt->format('F');	
								
								if($fieldData['cf_5902'] == 'Has come to firm'){	
									$fieldData['cf_5912'] = 'Login';	
								} 	
								if($fieldData['cf_5902'] == 'Has left firm'){	
									$fieldData['cf_5912'] = 'Logout';		
								}	
								
								$fieldData['cf_5916'] = 'Picar';
								$fieldData['assigned_user_id'] = $this->user;	
								//print_r($fieldData); exit;							
								$sql = "SELECT vtiger_userattendancecf.*, vtiger_crmentity.smownerid, vtiger_crmentity.createdtime FROM vtiger_userattendancecf inner join vtiger_crmentity on vtiger_userattendancecf.userattendanceid = vtiger_crmentity.crmid WHERE vtiger_crmentity.deleted=0  AND vtiger_userattendancecf.cf_5894 = '".$fieldData['cf_5894']."' AND vtiger_userattendancecf.cf_5896 = '".$fieldData['cf_5896']."' AND vtiger_userattendancecf.cf_5904 = '".$fieldData['cf_5904']."'";	//echo $sql; exit;
								...
								if($db->num_rows($listResult) == 0){
									...
									if ($entityInfo) {
										$entityIdComponents = vtws_getIdComponents($entityInfo['id']);
										$createdRecords[] = $entityIdComponents[1];
										$userattendid = $entityIdComponents[1];	
                                
										$uiduserlist = $this->getuseridbypicar($fieldData['cf_5904'],"Picar");
										//print_r($uiduserlist); exit;	
										$saveattendencereport = new Vtiger_Save_Action();

										// $request1 = new Vtiger_Request();//echo "in"; exit;	
										// $request1->set("module","UserAttendance");	
										// $request1->set("action","Save");	
										// $request1->set("record",$userattendid);	
										// $request1->set("cf_5896",$fieldData['cf_5896']);	
										$update_sql = "update vtiger_userattendancecf set cf_5906 = ?, cf_6112 = ?, cf_6300 = ? where userattendanceid = ?";
										if($uiduserlist != 'no Record')	
										{	
											$splituserid = explode(',',$uiduserlist);
											$results = $db->pquery($update_sql, array($splituserid[2],$splituserid[1],$splituserid[0],$userattendid));
											
											// $request1->set("cf_5906",$splituserid[2]);	
											// $request1->set("cf_6112",$splituserid[1]); 	
											// $request1->set("cf_6300",$splituserid[0]);	
										}else{
											$results = $db->pquery($update_sql, array("","",0,$userattendid));	
											// $request1->set("cf_5906","");	
											// $request1->set("cf_6112","");	
											// $request1->set("cf_6300",0);	
										}	
										//print_r($request1); exit;
										//$recordModel2 = $saveattendencereport->saveRecord($request1);
									}
									//print_r($userattendid); exit;
									//$userattendid = explode('x',$entityInfo['id']);	
									//updateing records for departmanet and location/office	
				...
				//change by Azhar 11 Jan 2022
				else if ($moduleName=="WISPremiumCommodity")
				{
					$db = PearDatabase::getInstance();	
					
					//update commoditytypeid
					
					$comdid = $fieldData["name"];
					$comdname = $fieldData["cf_8310"];
					
					$foundrec = false;
					$fcomid="";
					$fcomname="";
					$sql = "select * from vtiger_commoditytype where wis_key='$comdid'";
					//echo "<br> SQL ".$sql; echo "comd name :".$comdname; exit;
					$rs = $db->pquery($sql, array());
					if($db->num_rows($rs) > 0){
						$r_comdty = $adb->fetch_array($rs);
						$fcomtypeid= $r_comdty["commoditytypeid"];
						$fieldData["name"]=$fcomtypeid;
					}
					//echo "<br>Test<pre>"; print_r($fieldData);exit;
					//update packiging id 
					
					$pkgid = $fieldData["wis_premium_calculator"];
					//echo "<br>bf pkg id ".$pkgid;
					if($pkgid=="")  //if no pkgid , its mean its containerized
					{
						$fieldData["wis_premium_calculator"]="457405"; //this may change on production
					}
					else
					{
						$sql = "select * from vtiger_specialrange where wis_key='$pkgid'";
						//echo "<br> SQL ".$sql;
						$rs = $db->pquery($sql, array());
						if($db->num_rows($rs) > 0){
							$r_rec = $adb->fetch_array($rs);
							$fpkgid= $r_rec["specialrangeid"];
							$fieldData["wis_premium_calculator"]=$fpkgid;
						}
						
					}
					//echo "<br>aftr pkg id ".$fieldData["wis_premium_calculator"];
					//echo "<br>policy cover ".$fieldData["wis_policy_cover"];
					//exit;
			...
			if ($createRecord || $mergeType == Import_Utils_Helper::$AUTO_MERGE_MERGEFIELDS || $mergeType == Import_Utils_Helper::$AUTO_MERGE_OVERWRITE) {
				...
				if (!empty($recordId)) {
					...
					// echo "<pre>";
					// print_r($fieldData);
					// echo $recordId."<br>".$moduleName;
					if($moduleName=='WaggonTracker')
					{
						//Wagon NO = cf_5844
						//echo "<pre>";
						//print_r($fieldData);
						$wagon_name = $fieldData['cf_5844'];
						//exit;
						$result_wagon = $adb->pquery('SELECT wagonid FROM vtiger_wagon WHERE name = ? ',array($wagon_name));
						//echo $adb->num_rows($result_wagon);
						if($adb->num_rows($result_wagon)) {
							$wagon_id = $adb->query_result($result_wagon, 0, 'wagonid');
							//For CRM Entity Rel
							//echo "INSERT INTO vtiger_crmentityrel SET crmid = '".$wagon_id."', module = 'Wagon', relcrmid = '".$recordId."', relmodule = 'WaggonTracker'";
							//echo "<br>";
							$adb->pquery("INSERT INTO vtiger_crmentityrel SET crmid = '".$wagon_id."', module = 'Wagon', relcrmid = '".$recordId."', relmodule = 'WaggonTracker'");
						}
...
//exit;
	...
	public function getuseridbypicar($picarid,$databy) {	
		...
		$db = PearDatabase::getInstance();		
			$query = "SELECT * FROM vtiger_userlistcf inner join vtiger_userlist on vtiger_userlist.userlistid = vtiger_userlistcf.userlistid inner join vtiger_crmentity on vtiger_userlistcf.userlistid = vtiger_crmentity.crmid where ".$fieldname." = ".$picarid." and vtiger_crmentity.deleted = 0 and cf_6206 = 'Active'";	
        	$result = $db->pquery($query, array());	
				for($i=0;$i<$db->num_rows($result);$i++){	
					...
					return $db->query_result($result,$i,'userlistid').",".$db->query_result($result,$i,'cf_3421').",".$db->query_result($result,$i,'cf_3349');	
				}	
				return "no Record";
	}

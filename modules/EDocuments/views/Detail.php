...
class EDocuments_Detail_View extends Vtiger_Detail_View
{
    ...
    public function checkRecordDuplicateByUser(Vtiger_Request $request)
    {
        ...
        if ($requestedUserId > 0 && $record > 0)
        {
            ...
            $queryRecord = $adb->pquery("SELECT $tableName.name 
																	 ...
																	 WHERE vtiger_crmentity.deleted = 0 AND $tableName.name = ? AND $tableName.$fieldId = ?", array(
                $requestedUserId,
                $record
            ));
        ...
        else if ($requestedUserId > 0 && $record == 0)
        {
            ...
            $queryRecord = $adb->pquery("SELECT $tableName.name
																		FROM $tableName
																		INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = $tableName.$fieldId
																		WHERE vtiger_crmentity.deleted = 0 AND $tableName.name = ?", array(
                $requestedUserId
            ));
            $countOfRecords = $adb->num_rows($queryRecord);
    ...
    */
    ...
    function preProcess(Vtiger_Request $request, $display = true)
    {
        ...
        $detailViewLinkParams = array(
            'MODULE' => $moduleName,
            'RECORD' => $recordId
        );

        $detailViewLinks = $this
            ->record
            ->getDetailViewLinks($detailViewLinkParams);
        ...
        $viewer->assign('MODULE_MODEL', $this
            ->record
            ->getModule());
        $viewer->assign('DETAILVIEW_LINKS', $detailViewLinks);

        $viewer->assign('IS_EDITABLE', $this
            ->record
            ->getRecord()
            ->isEditable($moduleName));
        $viewer->assign('IS_DELETABLE', $this
            ->record
            ->getRecord()
            ->isDeletable($moduleName));

        $linkParams = array(
            'MODULE' => $moduleName,
            'ACTION' => $request->get('view')
        );
        $linkModels = $this
            ->record

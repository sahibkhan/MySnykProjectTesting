...
class EDocuments_List_View extends Vtiger_List_View {
	...
	function process (Vtiger_Request $request) {
		...
		/*global $adb;
		$file="SELECT vtiger_edocuments.name, 
				vtiger_edocumentscf.document_name, 
				vtiger_edocumentscf.file_name, 
				vtiger_edocumentscf.path, 
				vtiger_edocuments.edocumentsid 
				FROM vtiger_edocuments 
				INNER JOIN vtiger_crmentity ON vtiger_edocuments.edocumentsid = vtiger_crmentity.crmid 
				INNER JOIN vtiger_edocumentscf ON vtiger_edocuments.edocumentsid = vtiger_edocumentscf.edocumentsid LEFT JOIN vtiger_users ON vtiger_crmentity.smownerid = vtiger_users.id 
				LEFT JOIN vtiger_groups ON vtiger_crmentity.smownerid = vtiger_groups.groupid 
				WHERE vtiger_crmentity.deleted=0 AND vtiger_edocuments.edocumentsid > 0 
				ORDER BY vtiger_crmentity.modifiedtime DESC LIMIT 0, 51";
		$file = $adb->pquery($file,array());
		
		$files = array();
		for($j=0; $j<$adb->num_rows($file); $j++) {
			$files[$j]['file_name'] = $adb->query_result($file, $j, 'file_name');
			$files[$j]['path'] = $adb->query_result($file, $j, 'path');

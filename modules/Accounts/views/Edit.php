...
Class Accounts_Edit_View extends Vtiger_Edit_View {
	...
	public function process(Vtiger_Request $request) {
		...
		// echo "<pre>";print_r($recordModel->getEventInvitee(5)); exit;
	 ...
	 foreach ($accessibleUsers as $key => $accessibleUser){
		...
		{php} echo crm_own_invitedformat({$USER_ID}); {/php}
		</option>
		{/foreach}
		*/

/*  		$INVITE_USERS='';
		$userList = $recordModel->getEventInvitees();
		foreach($userList as $USER){
			$userId = $USER['id'];
			$nameWithLocationAndDepartment = $USER['nameWithLocationAndDepartment'];

			$INVITE_USERS .='<option value="'.$userId.'"';
			if( in_array($userId, $INVITIES_SELECTED)){
				$INVITE_USERS .='selected';
			}

			$INVITE_USERS .='>';
			$INVITE_USERS .= $nameWithLocationAndDepartment;
			$INVITE_USERS .='</option>';
		...
		// echo "<pre>";print_r($recordModel->getEventInvitees()); exit;

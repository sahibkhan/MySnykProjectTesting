...
class Accounts extends CRMEntity
{
	...
	function insertIntoInviteeTableAccount($module, $invitees_array)
	{
		...
		if ($this->mode == 'edit') {
			...
			for ($i = 0; $i < $numRows; $i++) {
				...
				if (!in_array($cuser_id, $invitees_array) && $_REQUEST['action'] != 'MassSave' ) {
		...
		foreach ($invitees_array as $inviteeid) {
			...
			if ($inviteeid != '') {
				...
				if ($numRows == 0) {
					$query = "insert into vtiger_invitees(activityid, inviteeid, status) values(?,?,?)";
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId)
	{
		...
		$rel_table_arr = array(
			"Contacts" => "vtiger_contactdetails", "Potentials" => "vtiger_potential", "Quotes" => "vtiger_quotes",
		...
		$tbl_field_arr = array(
			"vtiger_contactdetails" => "contactid", "vtiger_potential" => "potentialid", "vtiger_quotes" => "quoteid",
		...
		$entity_tbl_field_arr = array(
			"vtiger_contactdetails" => "accountid", "vtiger_potential" => "related_to", "vtiger_quotes" => "accountid",
	...
	function syncRegionalAccountsWithMasterProfile()
	{
		...
		$accountResult = $adb->pquery("SELECT cf_3207 FROM `vtiger_accountscf` WHERE `accountid` = ?", array($this->id));
		...
		if ($masterProfileId) {
			...
			for ($i = 0; $i < $noOfAccounts; $i++) {
				...
				// $this->addAccountCreatorToInviteeList($smownerid, $masterProfileId);

			}

			$queryUpdateMasterProfile = "UPDATE `vtiger_masterprofilescf` 
																		 SET cf_3205 = ?, cf_3903 = ?, cf_3187 = ?, cf_3189 = ? 
																		 WHERE `masterprofilesid` = ?";


			$adb->pquery($queryUpdateMasterProfile, array($accountsDescriptions, $vendorRegistrationDetails, $masterProfileCountry, $masterProfileCity, $masterProfileId));
		} // end of master profile existing

	}


	/* 	public function addAccountCreatorToInviteeList($smownerid, $masterProfileId){
		global $adb;
		...
		if ($n_inviteeMPAssignedTo == 0){
			$adb->pquery("INSERT INTO `vtiger_invitees` (`activityid`, `inviteeid`) VALUES (?, ?);", array($masterProfileId, $smownerid));					 
		}
	...
	public function addInviteeUserToMasterProfile($accountId, $masterProfileId){
		...
		// $adb->pquery("UPDATE `a_test` SET `status` = '$noOfinvitee' WHERE `userlistid` = 993828");
	...
	public function handleEmailNotification($record)
	{
		...
		if ($masterProfileId > 0) {
			require_once('modules/MasterProfiles/models/Record.php');
			$MPRecordInstance = new MasterProfiles_Record_Model();
			$recipientList = $MPRecordInstance->getMPRecipientUsers($masterProfileId);
		...
		if ($event_status == 1) {
			...
			if ($isconverted == "1") {
				...
				if ($acc_type == 'Customer') $cc .= 'a.pominova@globalinklogistics.com,a.alekpayeva@globalinklogistics.com,';
				else 
				if ($acc_type == 'Agent') $cc .= 'z.kaynarbaeva@globalinklogistics.com,';
				else
				if ($acc_type == 'Vendor') $cc .= 'r.zhumadilova@globalinklogistics.com,';
	...
	public function send_customer_notification($creator_email, $creator_name, $who_updated_email, $who_updated_name, $module, $record, $subject, $message_status, $sum, $cc)
	{
		...
		$listOfemails = explode(',', $cc);
		foreach ($listOfemails as $email) {
			$body .= "<tr><td colspan=2>" . $email . " </td></tr>";
		}
	...
	error_reporting(E_ALL); */
		...
		if ($n > 0) {
			if ($event_status == 1) $lbl = 'Customer';
			else $lbl = 'Updated';
			...
			for ($i = 1; $i <= $n; $i++) {
				...
				// if ($label == 'Modifiedby') {
				// 	if ($event_status == 1) $label = "Created by";
				// 	else $label = "Updated by";
				...
				if ($label == 'Modifiedby') {
					...
					$user_model = Users_Record_Model::getInstanceById($value, 'Users');
					$value = $user_model->get('first_name') . ' ' . $user_model->get('last_name');
				}
				...
				if ($label == 'Cf_2709') {
					$label = 'Branch';
					$value = Vtiger_Record_Model::getInstanceById($value)->get('cf_1559');
				}
				if ($label == 'Creator') {
					continue;
				}

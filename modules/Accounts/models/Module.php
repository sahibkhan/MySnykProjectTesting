...
class Accounts_Module_Model extends Vtiger_Module_Model {
	...
	public function getQueryByModuleField($sourceModule, $field, $record, $listQuery) {
		...
		if (($sourceModule == 'Accounts' && $field == 'account_id' && $record)
			...
			if (($field == 'proc_supplier') ) {
				...
				AND (vtiger_accountscf.cf_2403 IN('Approved', 'Confirmed') OR payment_without_agreement=1) 
				
				"; //AND '".date('Y-m-d')."' between vtiger_serviceagreementcf.cf_6018 AND vtiger_serviceagreementcf.cf_6020 AND  vtiger_serviceagreementcf.cf_6068 = 'Administrative'
			}
			$condition = $db->convert2Sql($condition, $params);
			...
			if($position) {
				$split = preg_split('/where/i', $listQuery);
				$overRideQuery = $split[0] . '  WHERE ' . $split[1] . ' AND ' . $condition; //INNER JOIN vtiger_serviceagreementcf ON vtiger_serviceagreementcf.cf_6094 = vtiger_account.accountid
			} else {
				$overRideQuery = $listQuery. ' WHERE ' . $condition;
			}
		...
		if($module=="Accounts")
		{
			...
			*/
			...
			$query = 'SELECT CONCAT_WS(vtiger_crmentity.label,vtiger_accountscf.cf_2395) as label , vtiger_accountscf.cf_2403 as status, 
						...
						AND vtiger_crmentity.deleted = 0 '.$new_condition.'  '.$query_restricted.' '; //INNER JOIN vtiger_serviceagreementcf ON vtiger_serviceagreementcf.cf_6094 = vtiger_account.accountid
	 ...
	 * @return <Array> - List of Vtiger_Record_Model or Module Specific Record Model instances
	 */
	public static function getSearchResultForJObAndOthers($searchKey, $module=false) {
		//$_REQUEST['field_id']=='cf_1441_display' )
		$db = PearDatabase::getInstance();
		...
		if($module=="Accounts")
		{
			...
			*/
			...
			//mehtab code
			/* $new_condition = "vtiger_crmentity.setype='Accounts' AND (vtiger_account.account_type='Customer' || vtiger_account.account_type='Agent') 
			
			AND vtiger_accountscf.cf_2403 IN('Approved', 'Confirmed')"; */
			$field = $_REQUEST['field_id'];
			//echo "<pre>"; print_r($_REQUEST);exit;
			if ( ($field == 'cf_1441_display' || $field == 'cf_1443_display' || $field=='cf_1443_display[]' || $field=='related_to_display' || $field=='account_id_display' || $field=='Accounts_editView_fieldName_accountname') ) {
				
				$new_condition = " AND vtiger_crmentity.setype='Accounts' AND (vtiger_account.account_type='Customer' || vtiger_account.account_type='Agent') "; //AND vtiger_accountscf.cf_2403 IN('Approved', 'Confirmed') 
			}
			elseif(($field == 'cf_1082_display' || $field=='cf_1176_display' || $field=='cf_1445_display' || $field=='cf_1176_display[]' || $field=='cf_1367_display' || $field=='cf_1377_display' || $field=='campaignid_display' || $field=='cf_1827_display')) {
				//For claims customer
				if($field=='cf_1367_display') {
					$new_condition = " AND vtiger_crmentity.setype='Accounts' AND (vtiger_account.account_type='Vendor' || vtiger_account.account_type='Agent' || vtiger_accountscf.cf_7814='Yes') AND vtiger_accountscf.cf_2403 IN('Approved', 'Confirmed') ";
				}
				elseif($field=='cf_1445_display')
				{
					$new_condition = " AND vtiger_crmentity.setype='Accounts' AND (vtiger_account.account_type='Customer' || vtiger_account.account_type='Agent' || vtiger_accountscf.cf_7814='Yes') AND vtiger_accountscf.cf_2403 IN('Approved', 'Confirmed') ";
				}
				else{
					$new_condition = " AND vtiger_crmentity.setype='Accounts' AND (vtiger_account.account_type='Vendor' || vtiger_account.account_type='Agent') AND vtiger_accountscf.cf_2403 IN('Approved', 'Confirmed') ";	
				}
			...
			// $query = 'SELECT vtiger_crmentity.label as label , vtiger_accountscf.cf_2403 as status, crmid, setype, createdtime FROM vtiger_crmentity
	  		// 		  INNER JOIN vtiger_account ON vtiger_account.accountid = vtiger_crmentity.crmid
			// 		  LEFT JOIN  vtiger_accountscf ON vtiger_accountscf.accountid = vtiger_crmentity.crmid 
			// 			'.$tablelist_restricted.'
      		// 		  WHERE (label LIKE "'.addslashes($searchKey).'%" || vtiger_accountscf.cf_2395 like "'.addslashes($searchKey).'%") 
			...
			$query = 'SELECT vtiger_crmentity.label as label , vtiger_accountscf.cf_2403 as status, crmid, setype, createdtime FROM vtiger_crmentity
					  ...
					  LEFT JOIN  vtiger_accountscf ON vtiger_accountscf.accountid = vtiger_crmentity.crmid 
						'.$tablelist_restricted.'
      				  WHERE vtiger_crmentity.deleted = 0 '.$new_condition.'  '.$query_restricted.' ';
		...
		if ($module == "Accounts") {
			...
			// $query = R::getAll("SELECT * FROM vtiger_crmentity WHERE deleted = 0 AND setype = 'Accounts' ORDER BY 'createdtime' DESC");

			//$query = "SELECT * FROM vtiger_crmentity WHERE setype = 'Accounts' AND deleted = 0 ORDER BY 'createdtime' DESC";
			...
			// 	$accounts[$row['crmid']]['label'] = $row['label'];
			// }

			foreach ($result as $key => $account) {
				$accounts[$account['crmid']] = $account;
				$accounts[$account['crmid']]['label'] = $account['label'];
			}

			$keyToBeSearched = 'label';
			
			$fuzzy = new FuzzySearch($accounts, $keyToBeSearched);

			$term = $searchKey;
			$maxDistance = 4;
			$accountsList = $fuzzy->search($term, $maxDistance);

			// foreach ($accounts as $id => $account) {
			// 	// if ( levenshtein(metaphone($searchKey), metaphone($account['label']) < mb_strlen( metaphone($searchKey) ) / 2 ) ) {
			// 	// 	$accountsList[$id] = $account; 
			// 	// }

			// 	if ( levenshtein($searchKey, $account['label'] ) <= 3 ) {
			// 		$accountsList[$id] = $account;
			// 	}
			// }
			...
			foreach ($accountsList as $id => $row) {
				if(Users_Privileges_Model::isPermitted($row['setype'], 'DetailView', $row['crmid'])) {
					$row['id'] = $row['crmid'];
					$moduleName = $row['setype'];
					if(!array_key_exists($moduleName, $moduleModels)) {
	...
	public function getSearchRecordsQuery($searchValue,$searchFields, $parentId=false, $parentModule=false, $searchModule='') {
		...
		if(!empty($searchModule))
		{
			...
			$query = $db->convert2Sql("SELECT ".implode(',',$searchFields)." FROM vtiger_crmentity WHERE label LIKE ? AND vtiger_crmentity.setype=? AND vtiger_crmentity.deleted = 0", array("%$searchValue%", "$searchModule"));
		}else{
	...
	public function searchRecord($searchValue, $parentId=false, $parentModule=false, $relatedModule=false, $searchModule='') {
		...
		elseif(in_array($relatedModule, array('Job', 'JER', 'Jobexpencereport', 'Potentials', 'Quotes', 'BO','VPO')))
		{
			$matchingRecords = $this->getSearchResultForJObAndOthers($searchValue, $this->getName());
		}

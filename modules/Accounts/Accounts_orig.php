...
class Accounts extends CRMEntity {
	...
	*/
	function save_module($module) {
		...
		*/
		...
		//$this->getMasterProfileRelatedDetails();	  
	...
	*/
	...
	/* Function to get related accounts descriptions of Master profile */
	function getMasterProfileRelatedDetails() {
		global $adb;
		$accountid = $this->id;
		
		$res = $adb->pquery("SELECT * FROM `vtiger_accountscf` WHERE `accountid` = ?", array($accountid));
		$masterprofile_id = $adb->query_result($res, 0, 'cf_3207');	
		if ($masterprofile_id){

		$q = "SELECT vtiger_crmentity.description,vtiger_account.accountname,vtiger_accountscf.cf_729,
			...
			$r_consol = $adb->pquery($q, array($masterprofile_id));
		...
		for($i=0; $i < $numRows;$i++){
			$description = $adb->query_result($r_consol,$i,"description");
		$account_name = $adb->query_result($r_consol,$i,"accountname");	
		$bill_country = $adb->query_result($r_consol,$i,"bill_country");	
		$bill_city = $adb->query_result($r_consol,$i,"bill_city");	
		$account_id = $adb->query_result($r_consol,$i,"accountid");	
		$smownerid = $adb->query_result($r_consol,$i,"smownerid");

		$c_b = $adb->query_result($r_consol,$i,"cf_6572");
		$head_office = $adb->query_result($r_consol,$i,"cf_6574");
		$c_l = $adb->query_result($r_consol,$i,"cf_6576");
		$tender_platform = $adb->query_result($r_consol,$i,"cf_6578");
		$preffered_l = $adb->query_result($r_consol,$i,"cf_6580");
		$existing_cooperation = $adb->query_result($r_consol,$i,"cf_6582");
		$future_business = $adb->query_result($r_consol,$i,"cf_6584");
		$last_updated = $adb->query_result($r_consol,$i,"cf_6586");
		$comments = $adb->query_result($r_consol,$i,"cf_6588");


		$c_desc .= '<br/><b>'.$account_name.'</b> / '.$bill_city.' <br/>';
		$c_desc .= $description.'<br/><br/>';


		$c_desc .= '<br/><b>'.$account_name.'</b> / '.$bill_city.' / Customer Profile Questionnaire <br/><br/>';

		$c_desc .= '<b>Customer Business:</b>  '.$c_b.'  <br/>';
		$c_desc .= '<b>Head Office / Decision Maker for CIS/CA  :</b>  '.$head_office.'  <br/>';
		$c_desc .= '<b>  Customer Logistics Requirements:</b>  '.$c_l.'  <br/>';
		$c_desc .= '<b>  Tender Platform:</b>  '.$tender_platform.'  <br/>';
		$c_desc .= '<b>  Preferred Logistics Partner:</b>  '.$preffered_l.'  <br/>';
		$c_desc .= '<b>  Existing Cooperation  Contact History with GLK:</b> '.$existing_cooperation.'  <br/>';
		$c_desc .= '<b>  Future Business Development Potential with GLK:</b>  '.$future_business.'  <br/>';
		$c_desc .= '<b>  Last Updated by/on:</b>  '.$last_updated.'  <br/>';
		$c_desc .= '<b>  Comments:</b>  '.$comments.'  <br/><br/>';


		// Vendor Registration Details			
			$vr = $adb->query_result($r_consol,$i,"cf_729");
		$vn = $adb->query_result($r_consol,$i,"cf_731");
		$vrn = $adb->query_result($r_consol,$i,"cf_733");
					
		if (($vr) || ($vn) || ($vrn)){
			$vr_d .= '<br/><b>'.$account_name.'</b> <br/>';
		}		
		if ($vr){
			if ($vr == 0) $vr_s = 'Vendor Registration: No'; else $vr_s = 'Vendor Registration: Yes';
			$vr_d .= $vr_s.'<br/>';
		}
		if ($vn){
			$vr_d .= 'Vendor Number: '.$vn.'<br/>';
		}
		if ($vrn){
			$vr_d .= 'Vendor registration validity: '.$vrn.'<br/>';
		}
		...
		//cf_3187 MP country
		//cf_3189 MP City
		$country = $adb->query_result($r_consol,$i,"bill_country");
		$city = $adb->query_result($r_consol,$i,"bill_city");	
		if ($i == ($numRows - 1)) $slah = ''; else $slah = '/';
		if (!empty($country)){
			$s_country .= $adb->query_result($r_consol,$i,"bill_country").$slah;
		}

		if (!empty($city)){
			$s_city .= $adb->query_result($r_consol,$i,"bill_city").$slah;
		}


		// Synchronizing invite people
			$s_invitePeople = "SELECT * FROM `vtiger_invitees` WHERE `activityid` = ?";
			$r_invitePeople = $adb->pquery($s_invitePeople, array($account_id));
			$n_invitee = $adb->num_rows($r_invitePeople);
			
			for($j=0; $j < $n_invitee;$j++){
			$inviteeid = $adb->query_result($r_invitePeople,$j,"inviteeid");
			
				$s_invitePeople2 = "SELECT * FROM `vtiger_invitees` WHERE `activityid` = ? AND inviteeid = ?";
				$r_inviteMP = $adb->pquery($s_invitePeople2, array($masterprofile_id, $inviteeid));
				$n_inviteeMP = $adb->num_rows($r_inviteMP);
				if ($n_inviteeMP == 0){
					
					$s_insertInvite = "INSERT INTO `vtiger_invitees` (`activityid`, `inviteeid`) 
											VALUES ($masterprofile_id, $inviteeid);";
			...
			// Adding in CC "Assigned to" people					
				...
				$r_inviteMPAssignedTo = $adb->pquery($s_invitePeopleAssignedTo, array($masterprofile_id, $smownerid));
				$n_inviteeMPAssignedTo = $adb->num_rows($r_inviteMPAssignedTo);
				if ($n_inviteeMPAssignedTo == 0){
					
					$s_insertInvite = "INSERT INTO `vtiger_invitees` (`activityid`, `inviteeid`) 
											VALUES ($masterprofile_id, $smownerid);";
					$adb->pquery($s_insertInvite);					 
				}
			...
			$q2 = "UPDATE `vtiger_masterprofilescf` SET `cf_3205` = '$c_desc',cf_3903 = '$vr_d', cf_3187 = '$s_country', 
		cf_3189 = '$s_city' WHERE `masterprofilesid` = ".$masterprofile_id;
				$adb->pquery($q2);			
...
/* 			echo "<pre>";
			...
			$masterprofile_id = $account->get('cf_3207');
			if ($masterprofile_id > 0){
				$cc .= $this->arrange_invite_users($masterprofile_id);
				
				// $masterProfile = Vtiger_Record_Model::getInstanceById($masterprofile_id, 'MasterProfiles');

...
class Accounts_Save_Action extends Vtiger_Save_Action {
	...
	public function addNewRecipients($record){
		...
		$existingInvitees = array();
		$result = $adb->pquery("SELECT * FROM vtiger_invitees WHERE activityid=?", array($record));
		
		foreach ($result as $key => $column) {
			$existingInvitees[] = $column['inviteeid'];
		}

		foreach($inviteesCSM as $inviteeId){
			if (!in_array($inviteeId, $existingInvitees)) {
	...
	public function process(Vtiger_Request $request) {
		...
		try {
			...
			// $recordModel2 = $this->getRecordModelFromRequest($request);
			// echo "<pre>"; print_r($recordModel2->getMyCustomMod()); exit;
			...
			if ($request->isAjax()) {
				...
				// $file_function = new ReflectionFunction('getCRMRecordLabel');

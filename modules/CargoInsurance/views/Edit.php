...
Class CargoInsurance_Edit_View extends Vtiger_Edit_View {
	...
	function preProcess(Vtiger_Request $request, $display=true) { 
		...
		$Jysangrant_flag  = false;
		$jysangrant = $request->get('jysangrant');
		if(isset($jysangrant) && $jysangrant==true){
			$Jysangrant_flag  = true;
		}
		//$viewer = $this->getViewer($request);
		$viewer->assign('JYSANGRANT_FLAG', $Jysangrant_flag);

		$Amanat_flag  = false;
		$amanat = $request->get('amanat');
		if(isset($amanat) && $amanat==true){
			$Amanat_flag  = true;
		}
		//$viewer = $this->getViewer($request);
		$viewer->assign('AMANAT_FLAG', $Amanat_flag);

		$IGI_flag  = false;
		$igi = $request->get('igi');
		if(isset($igi) && $igi==true){
			$IGI_flag  = true;
		}
		//$viewer = $this->getViewer($request);
		$viewer->assign('IGI_FLAG', $IGI_flag);
	...
	public function process(Vtiger_Request $request) {
		...
		if(!empty($record) && $request->get('isDuplicate') == true) {
            ...
            $GEN_COUNTRY_CODE = $recordModel->get('cf_3605');
            $GEN_STATE_CODE = $recordModel->get('cargoinsurance_origin_state');
            ...
            $GEN_DEST_COUNTRY_CODE = $recordModel->get('cf_3609');
			$GEN_DEST_STATE_CODE = $recordModel->get('cargoinsurance_destination_state');

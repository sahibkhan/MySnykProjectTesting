...
class CargoInsurance_Detail_View extends Vtiger_Detail_View {
	...
	function preProcess(Vtiger_Request $request, $display=true) {
		...
		$query_rel = "select `cf_3599`,`cf_3621`, `cf_6298`, `cf_3629`, `cf_3631`, `cf_8364` from vtiger_cargoinsurancecf where cargoinsuranceid=?";
		$params_rel = array($recordId);
		$result_rel = $adb_r->pquery($query_rel, $params_rel);
		$row_rel = $adb_r->fetch_array($result_rel);
		$wisref = $row_rel['cf_3621'];
		$wisstatus = $row_rel['cf_8364'];
		$insurance_type = $row_rel['cf_6298'];  // cf_6260 in web2test 
		$insuranceAmountFreight = $row_rel['cf_3629'];
		$insuranceAmountValue = $row_rel['cf_3631'];
		$insuredCompanyId = $row_rel['cf_3599'];
		
		//echo "ins compid".$recordId; //exit;
		
		//get insuraed company name
		$query_icomp = "SELECT cf_996 as CompName FROM `vtiger_companycf` WHERE companyid in (select cf_3599 from vtiger_cargoinsurancecf WHERE cargoinsuranceid=?)";
		
		//echo "<br>sql ".$query_icomp; exit;
		
		$params_icomp = array($recordId);
		$result_icomp = $adb_r->pquery($query_icomp, $params_icomp);
		$row_icomp = $adb_r->fetch_array($result_icomp);
		$insuredCompany = $row_icomp["compname"];
		
		//echo "ins comp".$insuraedCompany; exit;
		...
		/*
		// comments by Azhar 
		if ($insuranceAmountFreight == 0 && $insuranceAmountValue == 0) {
			$canBeCancelled = true;
		}
		*/
		$viewer->assign('WIS_REF', $wisref);
		$viewer->assign('INSURANCE_TYPE', $insurance_type);
		$viewer->assign('CAN_BE_CANCELLED', $canBeCancelled);
		$viewer->assign('INSURED_COMPANY', $insuredCompany);
		

		//wis declaration id
		$sql = "SELECT `referal`, `cancel_sent`, `declaration_id` FROM `vtiger_cargoinsurance_certificate` WHERE `insurance_id` = ? ORDER BY `created_at` DESC LIMIT 1";
		
		//echo "<br>Sql ".$sql; echo "recid :".$recordId; exit;
		$params_rel = array($recordId);
		$result_rel = $adb_r->pquery($sql, $params_rel);
		$row_rel = $adb_r->fetch_array($result_rel);
		$viewer->assign('REFERAL', $row_rel['referal']);
		$viewer->assign('DECLARATION_ID', $row_rel['declaration_id']);
		$viewer->assign('CANCEL_SENT', $row_rel['cancel_sent']);
		$viewer->assign('CURRENT_USER_ID', $current_user_id);
		$viewer->assign('WIS_STATUS', $wisstatus);

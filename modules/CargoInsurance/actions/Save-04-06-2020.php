...
class CargoInsurance_Save_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
		try {
			...
			if(!empty($rate_id))
			{
				...
				$commoditytype_FSL_Black = $commoditytype_info_detail->get('cf_6786');
			...
			$specialrange_FSL_Black = $specialrange_info_detail->get('cf_6788');
			...
			if(!empty($cargo_beneficiary) && $assured_company=='85757' && $cargo_beneficiary!='1667' && $NEW_FSL_BLACK_INSURANCE_FLAG) //Only For KZ and Except Air Astana Client
			{
				...
				{							
					...
					//Email Notification to register selected client in FSL Black Insurance
						if($selling_insurance_flag=='No' && $current_user->id!=1217)
						{
							$eol = PHP_EOL;
							$current_user = Users_Record_Model::getCurrentUserModel();
							$to = $current_user->get('email1');
							$cc  ='b.rustam@globalinklogistics.com;s.mehtab@globalinklogistics.com;';
							...
							$body .='<p>Please notify the customer "'.$request->get('cf_3601_display').'" to insurance team to 
										register customer in ERP system under FSL Black</p>'.$eol;
			...
			if(!empty($rate_id)  && !empty($special_range_id))
			{
				...
				$query_cr = "select * from vtiger_commodityratescf where cf_3573=? AND cf_3575=? ";
				$params_cr = array($rate_id, $special_range_id);
				...
				if($FSL_BLACK_INSURANCE)
				{
					...
					if($shipment_cargo_value>100000)//USD 100000
					{
						//$selling_insurance_flag = 'Yes';
				...
				if(!empty($wis_date))
				{
					...
					if($wis_premium < $check_wis_premium)
					{
			...
			if($FSL_BLACK_INSURANCE)
			{
				...
				$sql = $adb_cr->pquery($sql_q, $params_in);
				if ($adb_cr->num_rows($sql)>0) {
					if (trim($adb_cr->fetch_array($sql)['cf_3621']) == '') {
	...
	public function saveUniqueWISID($module,$column,$table_id,$id) {
		...
		$sql = $adb_r->pquery("SELECT `$column` FROM `$module` WHERE $insurance_type_where `$column` LIKE '$arr0%' ORDER BY `$column` 
									DESC LIMIT 1",array());
		$arr = $adb_r->fetch_array($sql)[$column];
		...
		$adb_r->pquery("UPDATE `$module` SET `$column`='".$res."', cf_3623='".date('Y-m-d')."' WHERE `$table_id`='".$id."' LIMIT 1",array());
	}

...
class CargoInsurance_Save_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
		try 
		{
			...
			if(in_array($cargo_beneficiary, $halyk_insurance_account) && $assured_company=='85757' )
			{
			...
			*/  
			...
			if(!empty($cargo_beneficiary) && $assured_company=='85757' && $cargo_beneficiary!='1667' && $NEW_FSL_BLACK_INSURANCE_FLAG) //Only For KZ and Except Air Astana Client
			{
				...
				{							
					...
					//Email Notification to register selected client in FSL Black Insurance
						$insurance_group = array('1217', '767', '290');
						if($selling_insurance_flag=='No' && !in_array($current_user->id,$insurance_group))
						{
							$eol = PHP_EOL;
							$current_user = Users_Record_Model::getCurrentUserModel();
							$to = $current_user->get('email1');
							$cc  ='b.rustam@globalinklogistics.com;s.mehtab@globalinklogistics.com;a.serikbekkyzy@globalinklogistics.com;r.zhumabekova@globalinklogistics.com';
			...
			//echo "Rate Id:".$rate_id."  primum calculator/specail range id ".$special_range_id; exit;
			...
			if(!empty($rate_id)  && !empty($special_range_id))
			{
				...
				//echo "qry ".$query_cr;
				/*
				$query_cr = "select * from vtiger_wispremiumcommoditycf where cf_3573=? AND cf_3575=? AND cf_7194=? AND cf_7604=?  ";
				
				//echo "<br>Sql ".$query_cr; exit;
				$query_cr .= " AND '".$departure_date."' between cf_7606 AND cf_7608 ";
				...
				*/
				...
				if($rate_limit=="0.0") //if no ratelimit then limt is 7.5millon dollar
				{
					$rate_limit="7500000";
				}
	...
	public function saveRecord($request) {
		...
		//echo "Record Id ".$recordModel->getId();
		//exit;

...
class CargoInsurance_Save_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
		try {
			...
			$selling_insurance_flag_fsl = $request->get('cf_4559');	
			...
			*/  
			...
			if(!$NEW_FSL_BLACK_INSURANCE_FLAG)
			{	
				if(!empty($cargo_beneficiary) && $assured_company=='85757' && $cargo_beneficiary!='1667')
				{
					...
					{	
						...
						*/						
						...
						// && $selling_insurance_flag_fsl!='Yes' 
						if(($discounted_glk_rate==0 || empty($discounted_glk_rate)) )
						{
			...
			if(!empty($rate_id)  && !empty($special_range_id))
			{
				...
				if($FSL_BLACK_INSURANCE)
				{
					...
					if($shipment_cargo_value>100000)//USD 100000
					{
						...
						//$cc = '';
						$subject = "shipment who’s value is more than USD 100,000";
						
						$body = "";
						$body .="<p>Dear&nbsp; ".$current_user->get('first_name').",</p>".$eol;
						$body .='<p>Please note, any shipment who’s value is more than USD 100,000, must be insured under regular cover</p>'.$eol;
						$body .="<p>Name for Look Up: ".$name_for_look_up."</p>".$eol;
						$body .="<p>Legal Name: ".$legal_name."</p>".$eol;
						$body .="<p>Customer Status: ".$customer_status."</p>".$eol;
						$body .="<p>Job Ref #: ".$request->get('name')."</p>".$eol;	
			...
			$insurance_group = array('1217', '767', '290');
			if(in_array($current_user->id,$insurance_group))
			{

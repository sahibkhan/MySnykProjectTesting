		...
		try {
			...
			if(!empty($rate_id))
			{
				...
				if($commoditytype_FSL_Black=='No')
				{
					...
					$selling_insurance_flag = 'Yes';
					//22.04.2022 :: in case of selling value saving with zero value that's why updated it to "Yes" 
			...
			*/  
			...
			if(!$NEW_FSL_BLACK_INSURANCE_FLAG && !$halyk_insurance_flag && !$JYSAN_GARANT_INSURANCE)
			{
			...
			if(!empty($rate_id)  && !empty($special_range_id))
			{
				...
				//echo "Conv name ".$convname; exit;

				$query_cr="select c.name, m.* from vtiger_wispremiumcommoditycf m
					LEFT OUTER JOIN vtiger_wispremiumcommodity c on m.wispremiumcommodityid = c.wispremiumcommodityid
					LEFT OUTER JOIN vtiger_crmentity e on m.wispremiumcommodityid = e.crmid
						WHERE c.name= '".$rate_id."'
						AND wis_premium_calculator='".$special_range_id."'					
						AND wis_policy_cover='".$policy_cover_id."'
						AND wis_conveyance_type='".$convname."'
						AND '".$departure_date."' between wis_agreement_date AND wis_agreement_validity
						AND e.deleted=0
				";
				//echo "qry ".$query_cr; exit;
				/*
				...
				*/
				$result_cr = $adb_cr->pquery($query_cr, array());
				...
				if($count_commodityrates_by_type!=0)
				{
					...
					$commodity_rates_id = $row_cr['wispremiumcommodityid'];
					//Mode priority
					//1::Sea, 2::Air, 3::Road, 4::Rail
					$mode_arr = $request->get('cf_3619');
				
					$commodity_rate_list = Vtiger_Record_Model::getInstanceById($commodity_rates_id, 'WISPremiumCommodity');
				
			
					$wis_rate = $row_cr['wis_net_rate']; //conv rate
					$centras_rate = $row_cr['wis_jusan_markup']; // jusan markup
					$agent_rate = $row_cr['wis_agent_commission_rate']; //agent comm rate
					$rate_limit = $row_cr['wis_rate_limit']; // rate limit
					
					$globalink_rate = $row_cr['wis_gross_rate']; // gross rate
					/*
					echo "B4 <br>WIS Rate :".$wis_rate;
					echo "Agent Rate :".$agent_rate;
					echo "Centras Rate :".$centras_rate;
					echo "Glk premium :".$globalink_premium;
					echo "Agent premium :".$agent_premium;
					echo "Centras Primum :".$centras_premium;
					*/
				...
				//Aigul Message
				//In case the total sum to be insured exceeds  100,000,000 KZT or its equivalent in USD, the insurance shouldn’t be available, with the message 
				//– “The value is above the limit. Please contact Insurance team”.
				// start Jysan insurance
				...
				if($JYSAN_GARANT_INSURANCE)
				{
					if($assured_company=='85757')
					{
						...
						$shipment_cargo_value = exchange_rate_convert($fsl_currency_code, $fsl_file_title_currency, $total_insured_sum, $wis_date);
						//to check KZT value directly
						$shipment_cargo_value = $total_insured_sum;	
						if($shipment_cargo_value>100000000)//KZT 100,000,000
						{
							//$selling_insurance_flag = 'Yes';
							$request->set('cf_4559', 'Yes');
							//$request->set('cf_6260', 'WIS Insurance');//For Test
							...
							$JYSAN_GARANT_INSURANCE = FALSE;
							//Any shipment who’s value is more than KZT 100,000,000 must be insured under regular cover
							$eol = PHP_EOL;
							$current_user = Users_Record_Model::getCurrentUserModel();
							$to = $current_user->get('email1');
							$cc  ='b.rustam@globalinklogistics.com;s.mehtab@globalinklogistics.com;a.serikbekkyzy@globalinklogistics.com;a.nursultan@globalinklogistics.com';
							...
							//$cc = '';
							$subject = "shipment who’s value is more than KZT 100,000,000";
							
							$body = "";
							$body .="<p>Dear&nbsp; ".$current_user->get('first_name').",</p>".$eol;
							$body .='<p>Please note, any shipment who’s value is more than KZT 100,000,000 must be insured under regular cover</p>'.$eol;
							$body .="<p>Name for Look Up: ".$name_for_look_up."</p>".$eol;
							$body .="<p>Legal Name: ".$legal_name."</p>".$eol;
							$body .="<p>Customer Status: ".$customer_status."</p>".$eol;
							$body .="<p>Job Ref #: ".$request->get('name')."</p>".$eol;	
							...
							throw new JYSANGRANTException('-The value is above the limit. Please contact Insurance team');
						...
						if($cargo_insurance_currency_id==2){
							...
							//to check KZT value directly
							$shipment_cargo_value = $total_insured_sum;	
							if($shipment_cargo_value>220000)//USD 220,000
							{
								//$selling_insurance_flag = 'Yes';
								$request->set('cf_4559', 'Yes');
								//$request->set('cf_6260', 'WIS Insurance');//For Test
								...
								//$cc = '';
								$subject = "shipment who’s value is more than USD 220,000";
								
								$body = "";
								$body .="<p>Dear&nbsp; ".$current_user->get('first_name').",</p>".$eol;
								$body .='<p>Please note, any shipment who’s value is more than USD 220,000 must be insured under regular cover</p>'.$eol;
								$body .="<p>Name for Look Up: ".$name_for_look_up."</p>".$eol;
								$body .="<p>Legal Name: ".$legal_name."</p>".$eol;
								$body .="<p>Customer Status: ".$customer_status."</p>".$eol;
								$body .="<p>Job Ref #: ".$request->get('name')."</p>".$eol;	
						...
						elseif($cargo_insurance_currency_id==13)
						{
							...
							//to check KZT value directly
							$shipment_cargo_value = $total_insured_sum;	
							if($shipment_cargo_value>205000)//EUR 205,000.00 
							{
								//$selling_insurance_flag = 'Yes';
								$request->set('cf_4559', 'Yes');
								//$request->set('cf_6260', 'WIS Insurance');//For Test
								...
								//$cc = '';
								$subject = "shipment who’s value is more than EUR 205,000.00 ";
								
								$body = "";
								$body .="<p>Dear&nbsp; ".$current_user->get('first_name').",</p>".$eol;
								$body .='<p>Please note, any shipment who’s value is more than EUR 205,000.00  must be insured under regular cover</p>'.$eol;
								$body .="<p>Name for Look Up: ".$name_for_look_up."</p>".$eol;
								$body .="<p>Legal Name: ".$legal_name."</p>".$eol;
								$body .="<p>Customer Status: ".$customer_status."</p>".$eol;
								$body .="<p>Job Ref #: ".$request->get('name')."</p>".$eol;	
				...
				if(!empty($wis_date))
				{
					...
					//echo "Exchangerate ".$_exchrates; exit;
				...
				{
					$_exchrates=$request->get('cf_3673');;
				}
				...
				//exit;
				//check the limits
				$total_insured_sum_in_dollar = floatval($total_insured_sum)/floatval($_exchrates);
				$request->set('cf_3639', $total_insured_sum);
		
				$request->set('cf_3641', $globalink_rate);
				$request->set('cf_3635', $wis_rate);
				//echo "wis date $wis_date Total Ins sum $total_insured_sum $ ".$total_insured_sum_in_dollar. " Rate Limit ".$rate_limit." exch rate $_exchrates"; exit;
				
				if($rate_limit=="0.0" || $rate_limit=="") //if no ratelimit then limt is 7.5millon dollar
				{
					$rate_limit="7500000";
				}
				//echo "Rate Limit ".floatval($rate_limit);
				if(floatval($total_insured_sum_in_dollar)> floatval($rate_limit) && $rate_limit != "0.0")
				{
					$commodity_rate_info_arr['message']="Insured value is greater than Rate Limit (USD $rate_limit) USD: $total_insured_sum_in_dollar ";
				...
				//echo "<br>assured company ".$assured_company;
				...
				$request->set('cf_3673', $_exchrates);
				
				/*
				echo "AFTER<br>Exchangerate Rate :".$_exchrates;
				echo "Agent Rate :".$agent_rate;
				echo "Centras Rate :".$centras_rate;
				echo "Glk premium :".$globalink_premium;
				echo "Agent premium :".$agent_premium;
				...
				*/
				...
				if($cargo_insurance_currency_id==13 && ($assured_company!='85757')) 
				{
					...
					//when the premium is 9 EUR (which is min) in cargo insurance, the expense section used to calculate 10 USD by default 
					//$wis_premium = ($wis_premium==9 ? floor($b_buy_local_currency_net_EUR) : $b_buy_local_currency_net_EUR);
					$wis_premium = ($wis_premium==9 ? 10 : $b_buy_local_currency_net_EUR);
					//$globalink_premium = $s_sell_local_currency_net_EUR;
					$globalink_premium = $globalink_premium;
					//echo "<br>";
					//echo $globalink_premium;
			...
			$cargo_insurance_ref_no = '';
			if(!empty($insurance_id))
			{
				$cargoinsurance_info_detail_ = Vtiger_Record_Model::getInstanceById($insurance_id, 'CargoInsurance');
				$cargo_insurance_ref_no = $cargoinsurance_info_detail_->get('cf_3621');
			}
			...
			if($count_job_jrer_buying==0)
			{
				...
				if($assured_company!='85757')
				{
					...
					$insurance_expenses = array(
									...
									'b_invoice_no'     => $cargo_insurance_ref_no,
									...
									'b_pay_to_id'  => $b_pay_to_id,     //cf_1367
				...
				if($assured_company!='85757')
				{
					...
					$s_customer_currency_code = Vtiger_CurrencyList_UIType::getDisplayValue($cargo_insurance_currency_id);
					...
					$insurance_selling = array(
									...
									's_customer_currency' => $cargo_insurance_currency_id, //cf_1234
				...
				//After adding cargo insurance ref #
				$check_params_after = array('1',$globalink_premium, $globalink_premium, $globalink_premium, $globalink_premium, $variation_expected_and_actual_buying, $insurance_id, $b_invoice_date, $cargo_insurance_ref_no, $row_insurance_jrer['jobexpencereportid'], $insurance_id);
				...
				//After adding cargo insurance ref #
				$check_params_after = array($b_exchange_rate, $wis_premium, $wis_premium, $b_buy_local_currency_gross, $b_buy_local_currency_net, $variation_expected_and_actual_buying, $insurance_id, $b_invoice_date, $cargo_insurance_ref_no, $row_insurance_jrer['jobexpencereportid'], $insurance_id);	
...
class JYSANGRANTException extends Exception {};

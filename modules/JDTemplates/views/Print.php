...
* All Rights Reserved.
*************************************************************************************/
class JDTemplates_Print_View extends Vtiger_Print_View {
...
/* 		ini_set('display_errors', 1);
		...
		$PHPWord = new PHPWord();
		$document = $PHPWord->loadTemplate('include/BO/template.docx');
		$moduleName = $request->getModule();
		$record = $request->get('record');
		$record = 4572296;
/* 		
		...
		if ($appendixInfo->get('cf_5079')){
 		...
 		// feedback user info
		$document->setValue('job', '11');
		$document->setValue('agreement', '222');

 		$document->setValue('day', Date('d'));
		$document->setValue('date', Date('M-Y'));

		$document->setValue('contact', 'Гусейнов Руслан');
		$document->setValue('contacttype', 'Директора');
		$document->setValue('appendixtype', 'Устава');
		$document->setValue('job2', 'Семена');
		...
		header("Content-Disposition: attachment; filename=AXG-".Date('h:i:s').".docx");

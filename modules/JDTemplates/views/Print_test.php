...
/* 		$this->setValue('legalentitiy', $jobdescription_info->getDisplayValue('cf_6792'), ENT_QUOTES, "UTF-8");
	...
	public function word_print(Vtiger_Request $request){
		...
		$PHPWord = new PHPWord();
		$document = $PHPWord->loadTemplate('modules/InvoiceGLK/InvoiceGLK.docx');
		$moduleName = $request->getModule();
		// $record = $request->get('record');
		$record = 4572296;
		...
		if ($appendixInfo->get('cf_5079')){
 		...
 		// feedback user info
		$document->setValue('number', $jobNo);
		$document->setValue('agreementNo', '123');

 		$document->setValue('day', Date('d'));
		$document->setValue('monthYear', Date('m-Y'));
		...
		header("Content-Disposition: attachment; filename=App File".Date('h:i:s').".docx");

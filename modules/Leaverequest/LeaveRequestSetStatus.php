<?php
	...
	*/
	...
	//echo "Parameters ", $status,$from,$rid," uid ".$cusrid;
	//exit;
	...
	if($sendemail=="Y")
	{
		...
		$empemail = "azhar.naseem@globalinklogistics"; // for checking
		$fromEmail = "s.mehtab@globalinklogistics";
		...
		*/
		...
		//For Auto Reply to IT and Reception Team
		if (($from=="Head" || $from=="HR Head" || $from=="HR") && ($status=="a"))
		{
				...
				if (($hdaprv) && ($hraprv)){	
					...
					$message = getmsgbody();
					$msg_auto="Out of the office details:";
					$txt = str_replace("[ApproveMessage]",$msg_auto,$txt);
					...
					$message = str_replace($srch,"",$message);
					$subject_auto = "Out of the office: " . $autoreply_info['requester_name'];
					...
					$mail->IsHTML(true);
					//Set who the message is to be sent from
					$mail->setFrom($empemail, $username);
					//Set an alternative reply-to address
					//$mail->addReplyTo('replyto@example.com', 'First Last');
					//Set who the message is to be sent to
					$mail->addAddress('a.akbar@globalinklogistics.com', 'Askar Akbar');
					$mail->addAddress('frontdesk@globalinklogistics.com', 'Front Desk');
					//$to[] = "a.akbar@globalinklogistics.com";
					//$to[] = 'erp.support@globalinklogistics.com';
					//$to[] = 'reception@globalinklogistics.com';

					//$mail->AddCC('erp.support@globalinklogistics.com', 'GEMS Support');
					$mail->AddCC($empemail, $username);
					//$mail->addBCC("s.aftab@globalinklogistics.com", 'Ruslan');
					//Set the subject line

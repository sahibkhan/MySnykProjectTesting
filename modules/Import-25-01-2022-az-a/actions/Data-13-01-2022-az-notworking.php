	...
	public function createRecords() {
		...
		for ($i = 0; $i < $numberOfRecords; ++$i) {
			...
			if ($createRecordExists) {
				...
				elseif ($moduleName=="WISPremiumCommodity")
				{
					$comdid = $fieldData["name"];
					echo "Field Data <pre>";
					print_r($fieldData);
					//search commodity id in existing data
					$foundrec = false;
					$fcomid="";
					$fcomname="";
					$sql = "select * from vtiger_commoditytype where $comdid='$comdid'";
					$rs = $db->pquery($sql, array());
					if($db->num_rows($rs) == 0){
						$r_comdty = $adb->fetch_array($rs);
						$fcomname = $r_comdty["name"];
						$fcomid = $r_comdty["wis_key"];
						$foundrec=true;
					}
					
					if(!$foundrec)
					{
						$sqlupdate = "insert into vtiger_commoditytype (name,wis_key)
							values ('$fcomname','$fcomid')";
					...
					echo "Test<pre>"; print_r($fieldData);exit;

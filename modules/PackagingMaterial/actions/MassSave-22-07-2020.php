				...
				if(Users_Privileges_Model::isPermitted($moduleName, 'Save', $recordId)) {
					...
					$job_info = Vtiger_Record_Model::getInstanceById($job_id, $sourceModule_job);
						...
						if($qty_issued > 0 && $from_warehouse_location_id != 0)
						{
							...
							$adb->pquery("UPDATE vtiger_packagingmaterialcf set cf_6124='Received', cf_5748='".$issue_date."' 
										  WHERE packagingmaterialid='".$recordId."'", array());
							...
							if($total_rows_>0)
							{
								...
								$adb->pquery("INSERT INTO vtiger_crmentity SET crmid = '".$new_id."', 
											smcreatorid ='".$current_user->getId()."', smownerid ='".$current_user->getId()."', 
											setype = 'ReportofPackingMaterial', createdtime='".$date_var."', modifiedtime='".$date_var."'", array());
							...
							$adb->pquery("UPDATE vtiger_packagingmaterialcf set cf_6124='Received', cf_6142='".$total_final_item_price_."' 
										  WHERE packagingmaterialid='".$recordId."'", array());	
						}
						...
						for($jj=0; $jj< $adb->num_rows($result_packaging); $jj++ ) {
							...
							if($item_code=="SR-1" || $item_code=="SL-1" )
							{
								...
								$rs_last_purchase = $adb->pquery($sql_last_purchase);
								$rowsrs_last_purchase =  $adb->num_rows($rs_last_purchase);
								$last_purchase_price=0;

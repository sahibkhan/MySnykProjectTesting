	...
	public function process(Vtiger_Request $request) {
		...
		foreach($recordModels as $recordId => $recordModel) {
			if(Users_Privileges_Model::isPermitted($moduleName, 'Save', $recordId)) {
				...
				//For Warehouse Quantity Master module:: when warehouse coordiantor issue packing material
				...
				if($qty_issued > 0 && $from_warehouse_location_id != 0)
				{
					...
					mysql_query("UPDATE vtiger_packagingmaterialcf set cf_6124='Received', cf_5748='".$issue_date."' WHERE packagingmaterialid='".$recordId."'");
					$item_code = $recordModel->get('cf_5738');
					$from_warehouse_location_id = $recordModel->get('cf_6118');
					$transaction_quantity = $recordModel->get('cf_5746');
	        		$transation_operator = "-";
					//Only one time reduce value from ware house item master quantity
					if($packaging_material_info->get('cf_6124')=='Requested' && $mode_view=='LISTING' && $qty_issued > 0){
					...
					mysql_query("UPDATE vtiger_packagingmaterialcf set cf_6124='Received', cf_6142='".$total_final_item_price_."' WHERE packagingmaterialid='".$recordId."'");				
					
				}
				    
					
				$qty_returned = $recordModel->get('cf_5750');
				if($packaging_material_info->get('cf_6124')=='Received'  && $mode_view=='RELATED' && $qty_returned > 0)
				{
					...
					//check return item entries
					$sql_return_receipt = "SELECT reportofpackagingmaterialcf.cf_6166 as issue_quantity, 
										  ...
										  AND reportofpackagingmaterialcf.cf_6184 ='Receipt'
										  AND reportofpackagingmaterialcf.cf_6182 = '{$returned_date}'
										  ORDER BY reportofpackagingmaterialcf.cf_6182 DESC limit 1";
					
					$rs_return_receipt = mysql_query($sql_return_receipt);
					$rowsrs_return_receipt = mysql_num_rows($rs_return_receipt);
					//$last_purchase_price=0;

					if($rowsrs_return_receipt == 0)
					{
						//First fetch Closing quantity and unit for new opening values
						$sql_reportofpackaging_material = "SELECT reportofpackagingmaterialcf.cf_6172 as closing_quantity, 
												  reportofpackagingmaterialcf.cf_6174 as closing_unit,
												  reportofpackagingmaterialcf.cf_6176 as closing_value
												  FROM vtiger_reportofpackingmaterialcf as reportofpackagingmaterialcf
						...
						if($total_rows_>0)
						{
							//To Get issue unit of same item code last time
							$sql_issue_unit = "SELECT reportofpackagingmaterialcf.cf_6166 as issue_quantity, 
											  reportofpackagingmaterialcf.cf_6168 as issue_unit
											  FROM vtiger_reportofpackingmaterialcf as reportofpackagingmaterialcf
											  INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = reportofpackagingmaterialcf.reportofpackingmaterialid
											  WHERE vtiger_crmentity.deleted=0 
											  ...
											  ORDER BY reportofpackagingmaterialcf.cf_6182 DESC limit 1";
							$rs_issue_unit = mysql_query($sql_issue_unit);
							$rowsrs_issue_unit = mysql_num_rows($rs_issue_unit);
							$last_issue_unit=0;
	
							if($rowsrs_issue_unit>0)
							{
								$row_issue_unit = mysql_fetch_array($rs_issue_unit);
								$last_issue_unit = $row_issue_unit['issue_unit']; //last purchase price
							}
							
						$row_last_closing_ = mysql_fetch_array($rs_reportofpackaging_);
						$opening_unit_ = $row_last_closing_['closing_unit'];//last purchase price
						$opening_quantity_ = $row_last_closing_['closing_quantity'];//Closing Quantity
						$opening_value_ = $row_last_closing_['closing_value']; //Closing Value
						//Insert ReportofPackingMaterial Issue activity
						$adb = PearDatabase::getInstance();
						$new_id = $adb->getUniqueId('vtiger_crmentity');
						$current_user = Users_Record_Model::getCurrentUserModel();
						...
						$sqlr = "UPDATE vtiger_whitemqtymastercf  SET cf_5646=cf_5646 + {$qty_returned}
								 WHERE cf_5630 = '{$item_code}' AND cf_5638 = '{$from_warehouse_id}' AND cf_5640='{$from_warehouse_location_id}'
								 AND cf_5632 = '{$inhouse}' AND ".$company_field."  ";
						$rsi =mysql_query($sqlr);  
				...
				$status = $recordModel->get('cf_6124');//packaging material Status
				//if(($status=='Received' || $status=='Issued')  && $mode_view=='RELATED')
				//{
					include("include/Exchangerate/exchange_rate_class.php");
					
					//To calcualte total item amount
					
				   
					//To update expense in packing material own service
					...
					for($jj=0; $jj< $db_packaging->num_rows($result_packaging); $jj++ ) {
						...
						if($item_code=="SR-1" || $item_code=="SL-1" )
						{
							...
							$query_custom_packaging = "SELECT * FROM vtiger_custompackingmaterial
									INNER JOIN  vtiger_custompackingmaterialcf.custompackingmaterialid = vtiger_custompackingmaterial.custompackingmaterialid

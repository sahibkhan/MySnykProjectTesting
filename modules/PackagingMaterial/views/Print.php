  ...
  public function print_packaging_material($request) {
    ...
    $notes='';
  	foreach($models_1 as $key => $model){
       ...
       /*
       commented due to wrong formula and wrong entries from user end
        if(in_array($item_code_cf_5738, $item_code_arr))
        {
          ...
          //$per_item_price_1c = $per_item_price / $c_unit_formula;
          //$per_item_price = (($qty_issued>0) ? ($issued_qty_amount/$qty_issued) : 0 );
         ...
         // $item_consumed_amount_1c = $qty_consumed*$per_item_price_1c;
         $item_consumed_amount_1c = $qty_consumed*$per_item_price;
        }
        */

...
class PackagingMaterial_MassActionAjax_View extends Vtiger_MassActionAjax_View {
	...
	function generatePackagingNo (Vtiger_Request $request){
		...
		if(isset($package_no))
		{
			 ...
			 if(!empty($id))
			 {
				 ...
				 if(empty($_SESSION['packaging_ref_no']) && $s_generate_packaging_flag==true)
				{
					 ...
					 $sql =  'SELECT MAX(p_serial_number) as max_ordering from vtiger_packagingmaterial
									 INNER JOIN vtiger_packagingmaterialcf ON vtiger_packagingmaterialcf.packagingmaterialid = vtiger_packagingmaterial.packagingmaterialid
					...
					$db->pquery('update vtiger_packagingmaterialcf set cf_5754=? where packagingmaterialid=?', array($packaging_ref_no, $packaging_material_id));
				...
				$db->pquery('update vtiger_packagingmaterialcf set cf_5754=? where packagingmaterialid=?', array($_SESSION['packaging_ref_no'], $packaging_material_id));
		 ...
		 $this->savePackingMaterialExpense($packaging_material_id, $packaging_material_info, $packing_material_own_expenses, $job_info_detail, $job_id);
	...
	function showMassEditFormRelated (Vtiger_Request $request){
      ...
      $recordModel = Vtiger_Record_Model::getInstanceById($record, $moduleName);
			...
			$packaging_material_status = $recordModel->get('cf_6126'); //Packaging Material Status
	...
	function savePackingMaterialExpense_MainJRER($packaging_material_id, $packaging_material_info, $packing_material_own_expenses=array(), $job_info_detail, $job_id)
	{
	  ...
	  $params_jobexpencereport= array($current_id, $job_packaging_material_id, $packaging_material_info->get('assigned_user_id'), $job_info_detail->get('assigned_user_id'), $source_id, $packing_material_own_expenses['packaging_ref_no']);

...
class PackagingMaterial_MassActionAjax_View extends Vtiger_MassActionAjax_View {
	...
	function packagingMaterialto1c(Vtiger_Request $request)
	{
		...
		if ($request->has('selected_ids')) {
			...
			foreach ($recordIds as $record) {
				...
				$warehouse_id_formula = $PackagingMaterial_info->get('cf_5764');
				...
				if(!in_array($packaging_ref_no, $check_packaging_ref_no) && !empty($job_ref_no))
				{
					...
					*/						  
					...
					if($warehouse_id_formula=='2107543') // KZARA warehouse
					{
					 	$item_code_arr  = array('PB-1414', 'PB-14', 'PB-16', 'SBS-24', 'NBS-25');
					 	$item_code_gems_unit  = array('PB-1414' => 'Roll','PB-14' => 'Roll', 'PB-16' => 'Roll', 'SBS-24' => 'Meter', 'NBS-25' => 'Meter');
					 	$item_code_1c_unit  = array('PB-1414' => array('Meter', '120'),
					...
					for($jj=0; $jj< $db->num_rows($result_packagingmaterialcf); $jj++ ) {
						...
						//$custom_request!='Yes'	 
						 if($Item_code!="SR-1" && $Item_code!="SL-1" 
							...
							$qty_issued = $sub_PackagingMaterial_info->get('cf_5746');
							$qty_returned = $sub_PackagingMaterial_info->get('cf_5750');

							$qty_consumed = $qty_issued - $qty_returned;

							$issued_qty_amount = $sub_PackagingMaterial_info->get('cf_6142');
							...
							$packagingmaterial_arr_to_1c['Element'][] = array( 
																			...
																			'NQuantity' => $qty_consumed,
																			'NPrice' => $item_consumed_amount,
																			'NVAT' => 'wo',
																			'NSum' => $item_consumed_amount
																			);
		  ...
		  //Email Notification to Branch Warehouse Coordiantor
			...
			foreach($models_1 as $key => $model){
					...
					if($pmitem_info->get('cf_5754')==$packaging['packaging_ref_no'])
					{
						...
						//$body .= "Content-Transfer-Encoding: 7bit".$eol.$eol;
						//$body .= "This is a MIME<br> encoded message.".$eol;
						$current_user_location= $current_user->get('location_id');
						if($current_user_location=='85805')
						{
						$body .="<p>Dear Akbolat / ".$wh_supervisor_first_name.",</p>".$eol;
						}else{
						$body .="<p>Dear ".$wh_supervisor_first_name.",</p>".$eol;
						}
						...
						if($current_user_location=='85805')
						{
						$mailer->AddAddress('a.serikbol@globalinklogistics.com');
						$mailer->AddAddress($to);
						}else{
						$mailer->AddAddress($to);	
						}
	...
	function get_job_id_from_PackagingMaterial($recordId=0) {
	  ...
	  public function print_packaging_material($record) {
		...
		$warehouse_id_formula = $packaging_info_detail->get('cf_5764');
		if($warehouse_id_formula=='2107543') // KZARA warehouse
		{
			$item_code_arr  = array('PB-1414', 'PB-14', 'PB-16', 'SBS-24', 'NBS-25');
			$item_code_gems_unit  = array('PB-1414' => 'Roll','PB-14' => 'Roll', 'PB-16' => 'Roll', 'SBS-24' => 'Meter', 'NBS-25' => 'Meter');
		...
		$notes='';
  		foreach($models_1 as $key => $model){
			...
			if($pmitem_info->get('cf_5754')==$packaging_info_detail->get('cf_5754'))
			{
				...
				$item_consumed_amount_1c = $qty_consumed*$per_item_price;
				...
				<td width="331" valign="top"><p>'.$pmitem_info->getDisplayValue('cf_5738').'</td>
				...
				<td width="85" valign="top"><p align="center">'.$item_consumed_amount_1c.'</p></td>
				...
				*/        
				...
				$total_amount_1c +=$item_consumed_amount_1c;
				
				$custom_request = $pmitem_info->get('cf_6290');
				$Special_Item_Code  = $pmitem_info->get('cf_5738'); //item code
				
				$notes .= (!empty($detail) ? '<br>'.$item_code_cf_5738.'::'.$detail : '' );
			}
		  ...
		  $this->setValue('total_amount_1c',$total_amount_1c);

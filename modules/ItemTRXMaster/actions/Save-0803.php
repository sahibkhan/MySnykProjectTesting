...
class ItemTRXMaster_Save_Action extends Vtiger_Save_Action {
	...
	protected function getRecordModelFromRequest(Vtiger_Request $request) {
		...
		if(!empty($recordId)) {
			...
			$document_number = $this->getNewDocNumber($DocType_Id);
	...
	public function getNewDocNumber($DocType_Id)
	{
		$db = PearDatabase::getInstance();
		$sql =  'SELECT * FROM `vtiger_whdoctypecf` WHERE whdoctypeid=? ';
		$rs = $db->pquery($sql, array($DocType_Id));
		$tr = $db->num_rows($rs);
		$row = $db->fetch_array($rs);
		$document_last_no = $row['cf_5537'];
		$doc_type_code = $row['cf_5535'];
		...
		return $doc_type_code.'-'.$document_no;

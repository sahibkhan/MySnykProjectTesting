...
class ItemTRXMaster_Save_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
		...
		try {
			...
			/*cross check procurement ref# for transfer request*/
			if($request->get('cf_5593')=='Yes' && !empty($request->get('cf_5599')) && $request->get('cf_5583')=='1147813')
			{
				...
				where proc_request_no = '".trim($request->get('cf_5599'))."' AND vtiger_crmentity.deleted=0 AND (proc_order_status='Approved' OR proc_order_status='Paid')");
				if($adb->num_rows($get_procurement_id)<1){
					throw new INCORRECT_REFERENCE_NO('-	This reference number is not correct  -');
				}
			}
		...
		} catch (INCORRECT_REFERENCE_NO $e) {
			$requestData = $request->getAll();
			$moduleName = $request->getModule();
			unset($requestData['action']);
			unset($requestData['__vtrftk']);
			...
			$requestData['view'] = 'Edit';
			$requestData['INCORRECT_REFERENCE_NO'] = true;
			$moduleModel = Vtiger_Module_Model::getInstance($moduleName);
	...
	public function saveRecord($request) {
		...
		if(!empty($recordId)) {
			...
			$ref_doc_procurement_no = trim($request->get('cf_5599'));
			...
			$get_warehouse_location = $adb->pquery("select relcrmid as warehouse_location from vtiger_crmentityrel 
			INNER JOIN vtiger_crmentity ON vtiger_crmentityrel.crmid = vtiger_crmentity.crmid
			where module='WarehouseMaster'
			AND relmodule='WHLocation' and vtiger_crmentityrel.crmid='$warehouse_id'
			AND vtiger_crmentity.deleted = 0 limit 1
			");
			
			if($adb->num_rows($get_warehouse_location)>0){
				$warehouse_location_row = $adb->query_result_rowdata($get_warehouse_location, 0);
				$warehouse_location = $warehouse_location_row['warehouse_location'];
			}
			...
			if($inhouse=='Yes' && !empty($packing_material_reference_id) && $document_type=='Received')
			{
				include("include/Exchangerate/exchange_rate_class.php");
				$pm_info_detail = Vtiger_Record_Model::getInstanceById($packing_material_reference_id, 'Procurement'); //procurement
				
				$procurement_id = $packing_material_reference_id;
				...
				$procurementitems_result = $adb->pquery("SELECT * FROM `vtiger_procurementitemscf` 
				...
				if($adb->num_rows($procurementitems_result)>0){
					for($j=0; $j<$adb->num_rows($procurementitems_result); $j++)
					{
						$row = $adb->query_result_rowdata($procurementitems_result, $j);
						$pm_items_id  = $row['procurementitemsid'];			

						$pm_items_type_id = $row['procitem_proctypeitem_id']; //Get ProcurementTypeItems-ID
						$pmtype_info_detail = Vtiger_Record_Model::getInstanceById($pm_items_type_id, 'ProcurementTypeItems');
						$pm_type_code = $pmtype_info_detail->get('proctypeitem_code'); //Get item code from ProcurementTypeItems
						...
						$rs = $adb->pquery("SELECT SUM(vtiger_itemtrxdetailcf.cf_5621) as pm_items_qty FROM vtiger_itemtrxdetailcf 
										   ...
										   INNER JOIN vtiger_crmentityrel ON vtiger_itemtrxdetail.itemtrxdetailid = vtiger_crmentityrel.relcrmid
										   INNER JOIN vtiger_crmentity ON vtiger_crmentityrel.crmid=vtiger_crmentity.crmid AND vtiger_crmentity.deleted=0
										   WHERE vtiger_itemtrxdetail.name='".$pm_items_id."' 
										   AND vtiger_itemtrxdetailcf.cf_5613='".$pm_type_code."' 
										   AND vtiger_itemtrxdetailcf.cf_5609 = 'Received'
						...
						$pm_items_qty = $row['procitem_qty'];
						if($adb->num_rows($rs)>0)
						{
							$pm_items_qty = $row['procitem_qty'] - $row_ItemTRXDetail_old['pm_items_qty'];
						}
						if($pm_items_qty<=0)
						{
							continue; //skip item as quantity exceeding the limit. It means that Quantity of item already added in previous transaction for same PM request
						}
						$pm_items_price_per_unit = $row['procitem_unit_price'];
						//$pm_items_price_per_line = $pmitem_info->get('cf_4573');
						$pm_items_price_per_line = $pm_items_qty * $pm_items_price_per_unit;
						$pm_items_vat_rate = $row['procitem_vat_unit'];
						...
						$pm_items_currency = $row['procitem_currency'];
						...
						$db->pquery("INSERT INTO vtiger_itemtrxdetailcf SET itemtrxdetailid = '".$new_id."',
																			cf_5609 = '".$document_type."', item_post_status = 'Not Posted',
																			cf_5611 = '".$document_number."', cf_5613 ='".$pm_type_code."',
																			cf_5615 = '".$warehouse_id."', cf_5621 ='".$pm_items_qty."', cf_5617 = '".$warehouse_location."',
			...
			elseif($inhouse=='Yes' && !empty($ref_doc_procurement_no) && $document_type=='Transfer')
			{
				...
				if($adb->num_rows($procurementitems_result)>0){
					for($j=0; $j<$adb->num_rows($procurementitems_result); $j++)
					{
						...
						$rs = $adb->pquery("SELECT SUM(vtiger_itemtrxdetailcf.cf_5621) as pm_items_qty FROM vtiger_itemtrxdetailcf 
										   ...
										   WHERE vtiger_itemtrxdetail.name='".$pm_items_id."' 
										   AND vtiger_itemtrxdetailcf.cf_5613='".$pm_type_code."' 
										   AND vtiger_itemtrxdetailcf.cf_5609 = 'Transfer'
										   AND vtiger_crmentityrel.relmodule='ItemTRXDetail'
										   AND vtiger_crmentityrel.module='ItemTRXMaster'
										   ");
...
class INCORRECT_REFERENCE_NO extends Exception {};

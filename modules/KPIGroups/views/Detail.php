...
class KPIGroups_Detail_View extends Vtiger_Detail_View {
	...
	function preProcess(Vtiger_Request $request, $display=true) {
		...
		//get all group users
		$this->getGroupUsers($request,$viewer);

		//get all group criterias
	...
	public function getGroupCriterias($request,$viewer) {
		...
		if ($request->get('record') > 0) {
			...
			//$gcriteria_qry="select vtiger_criteria.name as criteria_name,kpigroup_criteria.new_weightage from kpigroup_criteria inner join vtiger_kpigroups on vtiger_kpigroups.kpigroupsid=kpigroup_criteria.group_id inner join vtiger_criteria on vtiger_criteria.criteria_id=kpigroup_criteria.criteria_id where kpigroup_criteria.group_id = ".$group_id;

			$gcriteria_qry="select vtiger_kpicriterias.name as criteria_name,kpigroup_criteria.new_weightage from kpigroup_criteria inner join vtiger_kpigroups on vtiger_kpigroups.kpigroupsid=kpigroup_criteria.group_id inner join vtiger_kpicriterias on vtiger_kpicriterias.kpicriteriasid=kpigroup_criteria.criteria_id where kpigroup_criteria.group_id = ".$group_id;
			...
			for($j=0; $j<$adb->num_rows($gcriteria_rs); $j++) {
				$gcriterias[$j]['criteria_name'] = $adb->query_result($gcriteria_rs, $j, 'criteria_name');
	...
	public function getGroupUsers($request,$viewer) {
		//$request,$viewer
		global $adb;
		//$adb->setDebug(true);
		if ($request->get('record') > 0) {
			...
			$guser_qry="select vtiger_users.first_name, vtiger_users.last_name from kpigroup_users inner join vtiger_users on kpigroup_users.user_id=vtiger_users.id where kpigroup_users.group_id = ".$group_id;
			//$gcriteria_qry="select * from kpigroup_criteria where kpigroup_criteria.group_id = ".$group_id;
			$guser_rs = $adb->pquery($guser_qry,array());
			
			$gusers = array();
			for($j=0; $j<$adb->num_rows($guser_rs); $j++) {
				$gusers[$j]['user_name'] = $adb->query_result($guser_rs, $j, 'first_name')." ".$adb->query_result($guser_rs, $j, 'last_name');
				//$gusers[$j]['last_name'] = $adb->query_result($guser_rs, $j, 'last_name');
			}

			if (count($gusers) > 0) {
				$viewer->assign('GROUP_USERS', $gusers);
			}

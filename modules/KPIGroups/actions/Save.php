...
class KPIGroups_Save_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
		try {
			...
			$this->saveGroupCriteria($request);

			$this->saveGroupUsers($request);
	...
	// save saveGroupCriteria 
	public function saveGroupCriteria($request) {

		$group_id = $this->savedRecordId;
		global $adb;
		$adb->setDebug(true);

		if ($request->get('record') > 0) {
			$adb->pquery("Delete from kpigroup_criteria where group_id = ".$group_id);
		}

		foreach($request->get('weightage') as $key=>$weightage) {
			if ($key==0) {
				continue;
			}
			// add criterias in kpigroup_criteria
			//$new_weightage = $weightage;
			
			$criteria_id = (int) $_POST['criteria'][$key];
			if ($weightage > 0 and $criteria_id > 0) {
				$query = "INSERT INTO kpigroup_criteria set group_id = $group_id, criteria_id = $criteria_id, new_weightage = '".$weightage."'";
				//echo $query;
				//die();
				$adb->pquery($query);
			}
	...
	// save saveGroupUsers
	public function saveGroupUsers($request) {
		$group_id = $this->savedRecordId;
		global $adb;
		//$adb->setDebug(true);

		if ($request->get('record') > 0) {
			$adb->pquery("Delete from kpigroup_users where group_id = ".$group_id);
		}
		
		foreach($request->get('group_users') as $key=>$user) {
			$user_id = (int) $user;
			if ($user_id > 0) {
				$query = "INSERT INTO kpigroup_users set group_id = $group_id, user_id = $user_id";
				$adb->pquery($query);
			}
		}
	}

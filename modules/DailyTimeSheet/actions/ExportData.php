			...
			if($exportType=='report_with_task_count')
			{
				...
				foreach($entries as $dailytimesheetid_key => $entryval)
				{
					...
					while($row_dt = $db->fetch_array($result_dt))
					{
						...
						{
							case 'Potentials':
								$inquiriessum = $row_dt['task_total'];
							break;
							case 'Quotes':
					...
					$totalactivity = $inquiriessum + $quatitionssum + $jobsum + $iesum + $oesum + $othersum;
					//$quatitionssum = $row_dt['quotes'];
					$worksheet->setCellValueByColumnAndRow(3, $row, $inquiriessum); //total_inquiries	
					$worksheet->setCellValueByColumnAndRow(4, $row, $quatitionssum); //total_quotes					
					$worksheet->setCellValueByColumnAndRow(5, $row, $jobsum); //total_jobs				
					$worksheet->setCellValueByColumnAndRow(6, $row, $iesum); //total_incoming_emails					
					$worksheet->setCellValueByColumnAndRow(7, $row, $oesum); //total_outgoing_emails
					$worksheet->setCellValueByColumnAndRow(8, $row, $othersum); //total_other								
					$worksheet->setCellValueByColumnAndRow(9, $row, $totalactivity); //total_outgoing_emails	
				}


				$worksheet->getStyle("A2:J".$row)->applyFromArray(
					array(
							'borders' => array(
												'allborders' => array(

...
class MOTIW_Detail_View extends Vtiger_Detail_View {
	...
	function preProcess(Vtiger_Request $request, $display=true) {
		...
		$sql = "SELECT vtiger_approvalroutehistorycf.cf_6792, vtiger_approvalroutehistorycf.cf_6788, vtiger_approvalroutehistorycf.approvalroutehistoryid
						...
						WHERE vtiger_crmentity.deleted = ? AND vtiger_approvalroutehistorycf.cf_6784 = ?";
		$result = $adb->pquery($sql, array(0, $recordId));
...
/* 		$sql_1 = "SELECT * FROM vtiger_approvalroutehistorycf WHERE cf_6784='".$recordId."' AND cf_7042=1";
	...
	public function postProcess(Vtiger_Request $request) {
		...
		$adb = PearDatabase::getInstance(); 
		$sql = "SELECT vtiger_approvalroutehistorycf.cf_6792 as status
						...
						INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_approvalroutehistorycf.approvalroutehistoryid 
						WHERE vtiger_approvalroutehistorycf.cf_6784='".$recordId."' AND vtiger_approvalroutehistorycf.cf_6788=789 AND vtiger_crmentity.deleted=0";
		$result = $adb->pquery($sql);
...
/* 		if(empty($recordId)) {
	...
	function showRecentComments(Vtiger_Request $request) {
		...
		if($rollupsettings['rollup_status']) {
			...
			$recentComments = $parentRecordModel->getRollupCommentsForModule(0, 30);
		}else {
...
/* 		if(is_null($this->isAjaxEnabled)){
	...
	public function generateReferenceNo(Vtiger_Request $request){
		...
		// SELECT * FROM `vtiger_locationcf` where cf_1557 > 0
		// SELECT * FROM `vtiger_departmentcf` WHERE cf_1540 IN ('Operational', 'Support')
	...
	public function createServiceAgreement($request){
		...
		$resultServiceAgreement = $adb->pquery($queryServiceAgreement, array($CMSRecordId, 'ExitList', 'ServiceAgreement'));
		...
		if ($nOfRecords == 0){
			...
			$recordModelServiceAgreement->set('assigned_user_id', $serviceAgreementCreatorId);
...
/* 
			// Agreement validity
			if ($request->get('cf_6774')){
				$recordModelServiceAgreement->set("cf_6024", $request->get('cf_6774'));
			}
			 */
			...
			// Globalink Company
			if ($request->get('cf_7906')){
				$recordModelServiceAgreement->set("cf_6026", $request->get('cf_7906'));
			}
			
			
			// Account Type
			if ($request->get('cf_6770')){
				$recordModelServiceAgreement->set("cf_6028", $request->get('cf_6770'));
			}
...
/* 				if ($recordId > 0 && $currentUserModel->get('id') == $LEGAL_HEAD){
					$sqlFilter = "UPDATE `vtiger_motiwcf` SET `cf_7144` = '1' WHERE `motiwid` = ?";
					$resultFilter = $adb->pquery($sqlFilter, array($recordId));
				}
				 */
		...
		// echo "start from: ".$approvalPartieSequence.'<br>';
		...
		for($i=0; $i<$noofrow ; $i++) {
			...
			// echo "updateApproval: $userid: ".$approvalPartieSequence.'<br>';

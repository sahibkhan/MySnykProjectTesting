...
class MOTIW_Save_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
		...
		end($finaldoc); // Устанавливает указатель на последний элемент массива
	...
	public function updateStatus(Vtiger_Request $request, $prevCounterpartyType, $prevTypeofAgreement){
   	...
   	$CMSRecordModel = Vtiger_Record_Model::getInstanceById($request->get('record'), 'MOTIW');
		// $counterpartyType = $CMSRecordModel->get('cf_7346');
...
/* 		echo "Rows: ".$rows.'<br>';
		echo "prev: ".$prevCounterpartyType.' ____ current: ' . $request->get('cf_7346').'<br>';
		echo "prev: ".$prevTypeofAgreement.'  ____ current: ' . $request->get('cf_6366'); exit;  */
		// echo "<pre>"; print_r($request); exit;

		// if Account type changed
   	if ($rows > 0 && ($prevCounterpartyType != $request->get('cf_7346') || $prevTypeofAgreement != $request->get('cf_6366')) ){
			// echo "account type going to change"; exit;
	...
	public function createApprovalRoute(Vtiger_Request $request){
		...
		if ($request->get('cf_6376')){
			$currentUserId = $request->get('cf_6376');
			$CONTRACT_OWNER_ID = $request->get('cf_6376');
		}	
		...
		// Getting User Info	
		// $CONTRACT_OWNER_ID = $currentUserModel->get('id');
	...
	function getAgencyApprovalParties(){
		...
		return array(array("type" => "CSM", "value" => $CSM_AGENT_APPROVER), array("type" => "CSM", "value" => $CSM_HEAD_AGENT_APPROVER), array("type" => "FD", "value" => $FINANCE_VENDOR_APPROVER));
	}

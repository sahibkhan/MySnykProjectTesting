...
class MOTIW_Save_Action extends Vtiger_Save_Action {
	...
	public function updateStatus(Vtiger_Request $request){
		...
		$sql = "SELECT  * FROM vtiger_approvalroutehistorycf INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_approvalroutehistorycf.approvalroutehistoryid WHERE vtiger_approvalroutehistorycf.cf_6784='".$recordId."' AND vtiger_crmentity.deleted=0";
		$result = $adb->pquery($sql);
		$rows = $adb->num_rows($result);
		...
		$sql_g = "SELECT  * FROM vtiger_approvalroutepartiescf INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_approvalroutepartiescf.approvalroutepartiesid WHERE vtiger_crmentity.deleted=0 AND vtiger_approvalroutepartiescf.cf_6834='".$groupid."'"; 
		...
		$noofrow = $adb->num_rows($result_g);
			...
			for($i=0; $i<$noofrow ; $i++) {
				...
				$recordModel1->save();
					...
					//}elseif($currentAddressCountry != 'Kazakhstan' && $currentAddressCountry != '' && $sequence == 4 && $usersaccountantid != ''){
					}elseif($currentAddressCountry != 'Kazakhstan' && $sequence == 4 && $usersaccountantid != ''){

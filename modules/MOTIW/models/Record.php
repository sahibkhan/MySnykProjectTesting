...
class MOTIW_Record_Model extends Vtiger_Record_Model {
...
/* 	public function getImageDetails() {
	...
	public function getImageDetails() {
		...
		if ($recordId) {
				$sql = "SELECT vtiger_attachments.*, vtiger_crmentity.setype, vtiger_motiwcf.cf_7774
						FROM vtiger_attachments

						INNER JOIN vtiger_seattachmentsrel ON vtiger_seattachmentsrel.attachmentsid = vtiger_attachments.attachmentsid
						INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_seattachmentsrel.crmid
						INNER JOIN vtiger_crmentity crm2 ON crm2.crmid = vtiger_seattachmentsrel.attachmentsid
						INNER JOIN vtiger_motiwcf ON vtiger_motiwcf.motiwid = vtiger_crmentity.crmid
						
						WHERE crm2.deleted = 0 AND vtiger_seattachmentsrel.crmid = ?";
			...
			for($i=0; $i<$count; $i++) {
				...
				$imageType = explode("/",$db->query_result($result, $i, 'type'));
				
				$scanDocs = $db->query_result($result, $i, 'cf_7774');
				$scanDocsArr = array();
				if ($scanDocs) $scanDocsArr = explode(',', $scanDocs);
			...
			if(is_array($imageOriginalNamesList)) {
				...
				for($j=0; $j<$countOfImages; $j++) {
					$imageDetails[] = array(
							...
							'type' => $imageTypeList[$j],
							'isScanDocChecked' => (in_array($imageIdsList[$j], $scanDocsArr) ? true : false)

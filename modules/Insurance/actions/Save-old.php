...
class Insurance_Save_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
		...
		$params_insurance_jrer = array($job_id, '85875');
		}		
		...
		if($count_job_jrer_buying==0)
		{
			...
			$adb = PearDatabase::getInstance();
			$query_insurance_expense_after = "UPDATE vtiger_jobexpencereportcf SET cf_1337=?, cf_1343=?, cf_1347=?, cf_1349=?, cf_1353=?, cf_2325=? WHERE jobexpencereportid=?";
			$variation_expected_and_actual_buying = $row_insurance_jrer['cf_1351'] - $globalink_premium;
			$check_params_after = array($globalink_premium, $globalink_premium, $globalink_premium, $globalink_premium, $variation_expected_and_actual_buying, $insurance_id, $row_insurance_jrer['jobexpencereportid']);
			...
			$query_insurance_selling_after = "UPDATE vtiger_jobexpencereportcf SET cf_1355=?, cf_1357=?, cf_1232=?, cf_1238=?, cf_1240=?, cf_2325=?  WHERE jobexpencereportid=?";
			
			$check_params_after_ss = array(date('Y-m-d'), $globalink_premium, $globalink_premium, $globalink_premium, $globalink_premium, $insurance_id, $row_insurance_jrer_s['jobexpencereportid']);
	...
	public function saveInsuranceSelling($recordModel, $request, $insurance_selling=array(), $job_info_detail)
	{
		...
		$adb->pquery("INSERT INTO vtiger_crmentity(crmid, smcreatorid, smownerid,
			...
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
			array($current_id, $current_user->getId(), $current_user->getId(), 'Jobexpencereport', 'NULL', $date_var, $date_var, 1, 0, $insurance_selling['label']));

...
class InvoiceGLK_Print_View extends Vtiger_Print_View {
	...
	function getJobFileCharges($jobFileId){
		...
		$queryCharges = $adb->pquery("
			...
			vtiger_currency_info.currency_code as currencycode,
			...
			vtiger_jobexpencereportcf.cf_1232 as sell_customer_cur_gross 
			...
			Left JOIN vtiger_currency_info ON vtiger_currency_info.id = vtiger_jobexpencereportcf.cf_1234	
		...
		for ($i=0; $i<$nRows; $i++){
			...
			$chargeCost = $adb->query_result($queryCharges, $i, 'sell_customer_cur_gross');
 		...
 		if ($contactId){
			...
			if ( !empty($appendixInfo->get('cf_7920')) ) {
				...
				if ( !empty($agreementInfoNew->get('name')) ) {
					$agreement = $agreementInfoNew->get('name').' от '. $this->dateConvert($agreementInfoNew->get('cf_6020'));
				}
			}
				//if ( isset($agreementInfoNew->get('name') ) ) {
					//$agreement = $agreementInfoNew->get('name').' от '.$agreementInfoNew->get('cf_6020');
				//}
			//}
			...
			*/
			//$paymentType = str_replace("{summ}",number_format($invoiceAmount, 2 , '.' , ' ')." (".$this->num2str($invoiceAmount),$paymentType);

			///$paymentType = str_replace("{summ}","<FORMAT=B>Some text</FORMAT>",$paymentType);
		...
		foreach($paymentTypeConditionArray as $key => $item){
			if (isset($item)){
				...
				if ($sentence == '{summ}'){	
					...
					$amountPart = number_format($invoiceAmount, 2 , '.' , ' ')." (".$this->num2str($invoiceAmount);
...
/* 		// Save File
	 ...
	 function splitTextByCondition($text){
		...
		do {
			...
			if ($text[$i] == '{' && $text[$i + 1] == 's' && $text[$i + 2] == 'u' && $text[$i + 3] == 'm' && $text[$i + 4] == 'mm' && $text[$i + 5] == '}' ){
				$wordsArr[] = $newText;
				$wordsArr[] = "{summ}";
				$newText = "";
				$i = $i + 6;

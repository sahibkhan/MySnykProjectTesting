...
class InvoiceGLK_Save_Action extends Vtiger_Save_Action {
	...
	public function saveRecord($request) {
		...
		$recordModel->save();
		$record = $request->get('record');
		$jobFileId = $request->get('cf_5079');

		$newlyCreatedAppendixId = $recordModel->getId();
		$queryAppendixNo = $adb->pquery("SELECT vtiger_crmentityrel.crmid
																			FROM vtiger_crmentityrel
																			LEFT JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_crmentityrel.relcrmid
																			WHERE vtiger_crmentity.deleted = 0 AND vtiger_crmentityrel.crmid = ? AND vtiger_crmentityrel.module = ? AND vtiger_crmentityrel.relmodule = ? AND vtiger_crmentityrel.relcrmid = ?", 
																			array($jobFileId, 'Job', 'InvoiceGLK', $newlyCreatedAppendixId));
		
		$nRows = $adb->num_rows($queryAppendixNo);
		if ($nRows == 0){
			$queryInsertRelation = $adb->pquery("INSERT INTO vtiger_crmentityrel(crmid, module, relmodule, relcrmid) VALUES (?, ?, ?, ?)", 
																																			array($jobFileId, 'Job', 'InvoiceGLK', $newlyCreatedAppendixId));
		}

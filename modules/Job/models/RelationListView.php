...
class Job_RelationListView_Model extends Vtiger_RelationListView_Model {
	...
	public function getEntries($pagingModel, $JRER_TYPE='0', $QRY_GROUB_BY='0', $type='', $job_assigned_user_id='') {
		...
		if(!empty($job_assigned_user_id))
		{
			$user_query =' AND vtiger_jobexpencereport.owner_id = "'.$job_assigned_user_id.'" 
						   AND vtiger_jobexpencereport.user_id = "'.$job_assigned_user_id.'" ' ;
						 
					
		}

		if(!empty($type) && $type=='LJCR')
		{
			$user_query .=" AND vtiger_jobexpencereportcf.cf_1214 NOT IN('85773', '85774', '85793', '85794') ";
		}
		...
		for($i=0; $i< $db->num_rows($result); $i++ ) {
			...
			if($relation_module_name=='JER')
			{
				...
				where vtiger_vpocf.cf_1377 = ? and vtiger_crmentityrel.crmid=?", array($row['cf_1176'], $row['crmid']));
				$numRows_2 = $db->num_rows($result_2);
				if($numRows_2>0)
				{
				$row_vpo_agent_vendor = $db->fetch_array($result_2);
				$newRow['vpo_view_id'] = $row_vpo_agent_vendor['vpoid'];
				}
				else{
				$newRow['vpo_view_id'] = '';	
				}			
				...
				$result_bo = $db->pquery("SELECT * FROM `vtiger_bo` 
						INNER JOIN vtiger_crmentityrel ON vtiger_crmentityrel.relcrmid = vtiger_bo.boid
						where vtiger_bo.accountid = ? and vtiger_crmentityrel.crmid=?", array($row['cf_1443'], $row['crmid']));
				$numRows_bo = $db->num_rows($result_bo);
				if($numRows_bo>0)
				{
					$row_bo_agent_customer = $db->fetch_array($result_bo);
					$newRow['bo_view_id'] = $row_bo_agent_customer['boid'];
				}
				else{
					$newRow['bo_view_id'] = '';
				}
			...
			if($relation_module_name=='Jobexpencereport')
			{
				...
				if($numRows_jobexp>0)
				{
					...
					$newRow['gl_account'] = $row_jobexp['gl_account'];
					$newRow['ar_gl_account'] = $row_jobexp['ar_gl_account'];	
					$newRow['accept_generate_invoice'] = ($row_jobexp['accept_generate_invoice']==1 ? 'Accept':'Select');
					$newRow['invoice_no'] = $row_jobexp['invoice_no'];
					$newRow['b_send_to_head_of_department_for_approval'] = ($row_jobexp['b_send_to_head_of_department_for_approval']==1?'Yes':'No');
					$newRow['b_head_of_department_approval_status'] = $row_jobexp['b_head_of_department_approval_status'];
					$newRow['b_send_to_payables_and_generate_payment_voucher'] = ($row_jobexp['b_send_to_payables_and_generate_payment_voucher']==1 ? 'Accept':'Select');
					$newRow['b_payables_approval_status'] = $row_jobexp['b_payables_approval_status'];	
					
					$newRow['b_confirmed_send_to_accounting_software'] = ($row_jobexp['b_confirmed_send_to_accounting_software']==1 ? 'Accept':'Select');
					$newRow['fleettrip_id'] = $row_jobexp['fleettrip_id'];
					$newRow['wagontrip_id'] = $row_jobexp['wagontrip_id'];
					$newRow['invoice_tax'] = $row_jobexp['invoice_tax']; // by amir
				}
				else{

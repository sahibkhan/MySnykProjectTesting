 ...
 */
class Job_ListView_Model extends Vtiger_ListView_Model {
	...
	public function getListViewEntries($pagingModel) {
		...
		//echo "<pre>";
		//print_r($listViewEntries);
		//exit;

		/*
		//Mehtab: Commented due to load
		$index = 0;
		foreach($listViewEntries as $recordId => $record) {
			$rawData = $db->query_result_rowdata($listResult, $index++);
		
			if(empty($rawData['cf_5846'])){
				
				$rawData[count($rawData)/2] = 1;
				$rawData['diff'] = 2;
				
			} else{
				
				$date1 = date_create($rawData['cf_5846']);
				$date2 = date_create(date('Y-m-d'));
				$diff = date_diff($date1,$date2);
				$rawData[count($rawData)/2] = str_replace("+","",$diff->format("%R%a"));
				$rawData['diff'] = str_replace("+","",$diff->format("%R%a"));
			}

...
class Job_Approval_Action extends Vtiger_Action_Controller {

    function __construct() {
        parent::__construct();       
        $this->exposeMethod('JobAssignment');
    ...
    function JobAssignment(Vtiger_Request $request) {
     ...
     $Exporttype = $request->get('report_type');
     $userids = explode(",",$request->get('userid'));
     ...
     foreach($userids as $userid)
     {
        ...
        (
            ...
            'department_id' => Vtiger_DepartmentList_UIType::getDisplayValue($db->query_result($result,$j,'department_id')),
     ...
     $MIS_report_header = array('Job Ref No.', 'Loc','Srv','Cost', 'Profit', 'Coordonator');
     //$Exporttype ='vendor';
     ...
     if($Exporttype == 'vendor')
     {
         ...
         foreach ($entries_vendor as $key => $value) {
             ...
             $info['Customer'] = $customer_details->get('accountname');
    ...
    function writeReportToExcelFile_Vendor($fileName, $headers, $entries, $filterlist='') {
        ...
        $air_headers = array('title','Job Ref #','File Title','Location','Department','Customer','Job status','Charge','Pay To','Buy (Vendor Cur Net)','VAT Rate','VAT','Buy (Vendor Cur Gross)','Vendor Curr','Exch Rate','Buy (Local Curr Gross)','Buy (Local Cur NET)','Cost in ($)');

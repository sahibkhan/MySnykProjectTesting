...
$newhtml .= '<tr>';
            ...
            elseif($Exporttype=='data_collection'){
            	// var_dump($entries);
            	// exit;
               ///// excel spread sheet integration ////
				spl_autoload_register(function ($class_name) {
				...
				$myWorkSheetAir = new \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet($spreadsheet, 'Data Collection Template(Air)');
               $firstHtmlString = '<table>
                ...
                $secondHtmlString = '<table>
                    ...
                    <tr>
                          ...
                          <th>Air Code</th>   
                          <th>Flight No</th>
                          <th>Origin Port Code (IATA)</th>    
                          <th>Origin Cite</th>    
                          <th>Via Port Code 1 (IATA)</th>    
                          <th>Via Port Code 2 (IATA)</th>
                          <th>Destination  Port Code (IATA)</th>
					...
					if($entries>0){
						for($ex=0;$ex<count($entries);$ex++){
							// var_dump($entries[$ex]['cf_2387']);
							// exit;
					        $secondHtmlString .= '<tr>';
					        $secondHtmlString .=  
                            '<td>'.$entries[$ex]['cf_4923'].'</td>
                            <td>'.$entries[$ex]['cf_2387'].'</td>
                            <td>'.$entries[$ex]['cf_1200'].'</td>
                            <td>'.$entries[$ex]['cf_8654'].'</td>
                            <td>'.$entries[$ex]['cf_3527'].'</td>
                            <td>customer COde</td>
                            <td>Airline Code</td>
                            <td>'.$entries[$ex]['cf_4933'].'</td>
                            <td>'.$entries[$ex]['job_air_origin_port_code'].'</td>
                            <td>'.$entries[$ex]['job_air_origin_city'].'</td>
                            <td>Via IATA CODE</td>
                            <td>Via IATA CODE 2</td>
                            <td>'.$entries[$ex]['job_air_destination_port_code'].'</td>
                            <td>'.$entries[$ex]['job_air_destination_city'].'</td>
                            <td>'.$entries[$ex]['cf_4945'].'</td>
                            <td>'.$entries[$ex]['name'].'</td>
                            <td>'.$entries[$ex]['cf_1441'].'</td>'
                            ;
                ...
                $thirdHtmlString = '<table>
	                 <thead>
	                      ...
	                      <tr>
	                            ...
	                            <th>Vessel Name (IMO number)</th>
	                            <th>Port of Loading Country</th>    
	                            <th>Port of Loading City</th>    
	                            <th>Port of Loading Port Code (UN/LOCODE)</th>    
	                            <th>Port of Discharge Country</th>
	                            <th>Port of Discharge City</th>
	                            <th>Port of Discharge Port Code (UN/LOCODE)</th>
	                            <th>FCL/LCL</th>
	                            <th>Dry/Reefer</th>
	                            <th>Container Size</th>
	                            <th>Container Gross Weight (kg)</th> 
	                            <th>CBM</th> 
	                            <th>Job file</th> 
	                            <th>Company name</th>          
	                          </tr>
                        ...
                        if($entries>0){
						for($exx=0;$exx<count($entries);$exx++){
							 
					        $thirdHtmlString .= '<tr>';
					        $thirdHtmlString .=  
                            '<td>'.$entries[$exx]['cf_8618'].'</td>
                            <td>'.$entries[$exx]['cf_8618'].'</td>
                            <td>'.$entries[$exx]['cf_1200'].'</td>
                            <td>'.$entries[$exx]['cf_8614'].'</td>
                            <td>'.$entries[$exx]['cf_3527'].'</td>
                            <td>Shipping Line COde</td>
                            <td>'.$entries[$exx]['cf_5417'].'</td>
                            <td>'.$entries[$exx]['job_sea_loading_country'].'</td>
                            <td>'.$entries[$exx]['job_sea_loading_city'].'</td>
                            <td>'.$entries[$exx]['job_sea_loading_port_code'].'</td>
                            <td>'.$entries[$exx]['job_sea_discharge_country'].'</td>
                            <td>'.$entries[$exx]['job_sea_discharge_city'].'</td>
                            <td>'.$entries[$exx]['job_sea_discharge_port_code'].'</td>
                            <td>'.$entries[$exx]['cf_8904'].'</td>
                            <td>'.$entries[$exx]['cf_8902'].'</td>
                            <td>'.$entries[$exx]['cf_8900'].'</td>
                            <td>'.$entries[$exx]['cf_4945'].'</td>
                            <td>CBM</td>
                            <td>'.$entries[$exx]['name'].'</td>
                            <td>'.$entries[$exx]['cf_1441'].'</td>'
                            ;
               ...
               $thirdHtmlString .= ' </table>';
				...
				$spreadsheet->addSheet($myWorkSheetAir, 1);
					$styleArrayHeaderRow = [
					   'font' => [
					               'bold' => true,
					               'font-size' => 11,
                ...
                $myWorkSheetAir->getStyle('A1:O1')->applyFromArray($styleArrayHeaderRow);
                //$myWorkSheetAir->getColumnDimension('A1:Q1')->setWidth(10);
                $myWorkSheetAir->getStyle('A1:O1')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('E87B30');
				$reader->setSheetIndex(1);
				$spreadhseet = $reader->loadFromString($secondHtmlString, $spreadsheet);
				$myWorkSheetSea = new \PhpOffice\PhpSpreadsheet\Worksheet\Worksheet($spreadsheet, 'Data Collection Template(Sea)');
				$spreadsheet->addSheet($myWorkSheetSea, 2);
				$reader->setSheetIndex(2);
				$spreadhseet = $reader->loadFromString($thirdHtmlString, $spreadsheet);
				...
				$writer = \PhpOffice\PhpSpreadsheet\IOFactory::createWriter($spreadsheet, 'Xlsx');
				ob_clean();
				$fileName = 'Global-Data Collect Template.xls';
				header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');

		...
		if($Exporttype=='profit_share')
			...
			elseif($Exporttype=='kpi')
			{
				...
				$newhtml = '<table>
											<thead>
												<tr>
												<th colspan="2" style="background:#CCCCFF !important">Date</th>
												...
												<th colspan="2" style="background:#CCCCFF !important">Exp.Cost</th>
												<th colspan="2" style="background:#CCCCFF !important">Exp.Profit</th>
												<th colspan="2" style="background:#CCCCFF !important">Act.Revenue</th>
												<th colspan="2" style="background:#CCCCFF !important">Act.Cost</th>
												<th colspan="2" style="background:#CCCCFF !important">Act. Profit</th>
...
$newhtml .= '<tr>';
								...
								$row_job_id_info = $db_first->fetch_array($result_job);
								$job_id = $row_job_id_info['jobid'];
								$job_date = date("Y-m-d", strtotime($entries[$ex]['createdtime']));
								$newhtml .= '<td colspan="2">'.$job_date.'</td>';
								$newhtml .= '<td colspan="2">'.$entries[$ex]['cf_1198'].'</td>
								...
								$report_count  = $db_job->num_rows($result);
									if($report_count>0)
									{
										...
										$entries[$key]['actual_expense_cost_usd'] = number_format($job_report_detail['actual_expense_cost_usd'] , 2 ,  "." , "," );
										$entries[$key]['net_profit'] = number_format($job_report_detail['net_profit'] , 2 ,  "." , "," );
								$newhtml .= '	<td colspan="2" align="right">'.$entries[$key]['expected_revenue_usd'].'</td>
															<td colspan="2" align="right">'.$entries[$key]['expected_cost_usd'].'</td>
															<td colspan="2" align="right">'.$entries[$key]['expected_profit_usd'].'</td>
															<td colspan="2" align="right">'.$entries[$key]['actual_selling_cost_usd'].'</td>
															<td colspan="2" align="right">'.$entries[$key]['actual_expense_cost_usd'].'</td>
															<td colspan="2" align="right">'.$entries[$key]['net_profit'].'</td>';
									}else{
									$newhtml .= '	<td></td>
																	<td></td>
																	<td></td>
            ...
            else if($Exporttype=='data_collection')
			{
				// sahib khan
				$this->getReportXLS_XLSX($request, $translatedHeaders, $entries, $moduleName,$Exporttype);
			}
			else if($Exporttype=='scope_data_collection_air')
			{
				// sahib khan
				$this->getReportXLS_XLSX($request, $translatedHeaders, $entries, $moduleName,$Exporttype);
			}
			else if($Exporttype=='scope_data_collection_sea')
			{
				// sahib khan
				$this->getReportXLS_XLSX($request, $translatedHeaders, $entries, $moduleName,$Exporttype);
			}

			 else if ($Exporttype=='kpi') {
				$this->getReportXLS_XLSX($request, $translatedHeaders, $entries, $moduleName, $Exporttype);
			} else if ($Exporttype == 'vendor') {
				$this->getReportXLS_XLSX($request, $translatedHeaders, $entries, $moduleName, $Exporttype);
			}
			else{
	...
	function getReportXLS_XLSX($request, $headers, $entries, $moduleName, $format = 'kerry_format') {
        ...
        if($moduleName=='Job' && $format == 'data_collection'){
        	$fileName = 'Global_Data_Collection_Template'.'.xlsx';
        }
        elseif($moduleName=='Job' && $format == 'scope_data_collection_air'){
        	$fileName = 'Scope_3_Data_Collection_Template_Air'.'.xlsx';
        }
        elseif($moduleName=='Job' && $format == 'scope_data_collection_sea'){
        	$fileName = 'Scope_3_Data_Collection_Template_Sea'.'.xlsx';
        }
        else{
        ...
        elseif($moduleName=='Job' && $format =='data_collection'){
			$this->writeReportToExcelFile_Job_Data_Collection($tempFileName, $headers, $entries, false);
		}
        elseif($moduleName=='Job' && $format =='scope_data_collection_air'){
			$this->writeReportToExcelFile_Job_Scope_Data_Collection_Air($tempFileName, $headers, $entries, false);
		}
		elseif($moduleName=='Job' && $format =='scope_data_collection_sea'){
			$this->writeReportToExcelFile_Job_Scope_Data_Collection_Sea($tempFileName, $headers, $entries, false);
		}
		elseif ($moduleName=='Job' && $format == 'kpi') {
	...
	function writeReportToExcelFile_Job_Data_Collection($fileName, $headers, $entries, $filterlist='') 
	{
      ini_set('display_errors', 1);
		...
		$objPHPExcel->getProperties()->setCreator($full_name)
									 ->setLastModifiedBy($full_name)
									 ->setTitle('Global Data Collection Template')
									 ->setSubject('Global Data Collection Template')
									 ->setDescription('Global Data Collection Template')
									 ->setKeywords('Global Data Collection Template')
									 ->setCategory('Global Data Collection Template');
    
		//echo date('H:i:s') . " Add data\n";

$style_head = new PHPExcel_Style();
$style_head->applyFromArray(
	array(
		   'fill' 	=> array(
											'type'		=> PHPExcel_Style_Fill::FILL_SOLID,
											'color'		=> array('argb' => 'FCFCFC')
										),
		  'borders' => array( 
		  	                    'allborders' =>array(
								'style' => PHPExcel_Style_Border::BORDER_THIN
       ...
       $sharedStyle1 = new PHPExcel_Style();
			...
			$styleCol = new PHPExcel_Style();
			$styleCol->applyFromArray(
				array('fill' 	=> array(
											'type'		=> PHPExcel_Style_Fill::FILL_SOLID,
											'color'		=> array('argb' => 'FFF2CC')
										),
					  'borders' => array( 
					  	                    'allborders' =>array(
											'style' => PHPExcel_Style_Border::BORDER_THIN
			...
			$styleCol_2 = new PHPExcel_Style();
			$styleCol_2->applyFromArray(
				array('fill' 	=> array(
											'type'		=> PHPExcel_Style_Fill::FILL_SOLID,
		...
		$objPHPExcel->setActiveSheetIndex(0);
		$objPHPExcel->getActiveSheet()->setTitle("Data Collection Template(Air)");
		$objPHPExcel->getActiveSheet()->setCellValue('A1', 'NOTE: (1) Please submit the data of all international Air Export shipments with ATD of the current month.  
		(2) Kindly identify the changes in the required data that was submitted in previous months and then submit the revised data again in the current month datasheet.');
		$objPHPExcel->getActiveSheet()->mergeCells("A1:V1");
		$objPHPExcel->getActiveSheet()->setSharedStyle($style_head,"A1");
		$objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->getColor()->setARGB('FF3A00');
		$objPHPExcel->getActiveSheet()->setCellValue('A2', 'for checking uniqueness');
		$objPHPExcel->getActiveSheet()->mergeCells("A2:B2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($stylebody,"A2:H2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($styleCol,"I2:J2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($styleCol_2,"K2:L2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($styleCol,"M2:N2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($stylebody,"O2:Q2");
		$objPHPExcel->getActiveSheet()->setCellValue('C2', 'we are currently collecting Export only');
		$objPHPExcel->getActiveSheet()->setCellValue('D2', 'for Export only');
		$objPHPExcel->getActiveSheet()->setCellValue('E2', 'Customer refers to the shipper of freehand shipment or the consignee of nominated shipment subject to the contract of carriage');
		$objPHPExcel->getActiveSheet()->mergeCells("E2:F2");
		$objPHPExcel->getActiveSheet()->getStyle('E2')->getFont()->getColor()->setARGB('FF3A00');
		$objPHPExcel->getActiveSheet()->setCellValue('G2', 'IATA code format: CX');
		$objPHPExcel->getActiveSheet()->setCellValue('H2', 'IATA code format: CX251');

		$air_headers =   array('MAWB No', 'HAWB No','Import/Export', 'ATD', 'Customer Name', 'Customer Code', 'Airline Code', 'Flight Number','Origin Port Code (IATA)','Origin city','Via Port Code 1 (IATA)', 'Via Port Code 2 (IATA)', 'Destination Port Code (IATA)', 'Destination city', 'Gross Weight (kg)', 'Job file','Company name');
 
		$objPHPExcel->getActiveSheet()->fromArray($air_headers, null, 'A3');
		
		//$objPHPExcel->getActiveSheet()->getColumnDimension('A')->setAutoSize(true);
	
		$objPHPExcel->getActiveSheet()->getColumnDimension('A3')->setWidth(50);
		...
		foreach($entries as $key => $entry)
		{
            $qry_c ="SELECT vtiger_company.name FROM vtiger_company  INNER JOIN vtiger_companycf on vtiger_companycf.companyid =vtiger_company.companyid where  vtiger_companycf.cf_996='".$entry['cf_1186']."'";
		    $sql_c = $adb->pquery($qry_c);
			$row_company_name = $adb->fetch_array($sql_c);
			$company_name = $row_company_name['name'];
			
			//echo $entry['job_air_origin_city'];
            $qry = "SELECT * from `air_sea_codes` WHERE  port_city_code = '".$entry['job_air_origin_city']."' AND  port_country_code = '".$entry['job_air_origin_country']."'";
			$sql = $adb->pquery($qry);
			$rows = $adb->fetch_array($sql);
			$job_air_origin_country = $rows['port_country'];
			$job_air_origin_state = $rows['state_name'];
			$job_air_origin_city = $rows['port_city'];

			 $qry = "SELECT * from `air_sea_codes` WHERE  port_city_code = '".$entry['job_air_destination_city']."' AND  port_country_code = '".$entry['job_air_destination_country']."'";
			$sql = $adb->pquery($qry);
			$rows = $adb->fetch_array($sql);
			$job_air_destination_country = $rows['port_country'];
			$job_air_destination_state = $rows['state_name'];
			$job_air_destination_city = $rows['port_city'];
			$entries_new[] = array( 
							'mawb_no' => $entry['cf_4923'], 
							'hawb_no' => $entry['cf_2387'], 
							'type' => $entry['cf_1200'],
							'ATD' => $entry['cf_8654'], 
							'CUSTOMER NAME' => html_entity_decode($entry['cf_1441']), 
							'customer code' => '',
							'Airline Code' => $entry['cf_4931'], 
							'Flt No' => $entry['cf_4933'],
							'Job Air Origin Port Code' => $entry['job_air_origin_port_code'],
							'Job Air Origin City' => $job_air_origin_city, 
							'Via IATA CODE'=>'',
							'Via IATA CODE 2' => '', 
							'Job Air Dist Port Code' => $entry['job_air_destination_port_code'],
							'Job Air Destination City' => $job_air_destination_city, 
							'gross_weight' => $entry['cf_1084'],
							'name' => $entry['name'],
							'company name' =>  $entry['cf_1186'].'-'.$company_name
						   );
			
		}
		
		$objPHPExcel->getActiveSheet()->setSharedStyle($sharedStyle1, "A3:Q3");
		...
		$objPHPExcel->getActiveSheet()->getStyle('A4')->getAlignment()->setWrapText(true);
         for($i=0; $i<=17;$i++)
         {
            $objPHPExcel->getActiveSheet()->getColumnDimensionByColumn($i)->setWidth('25');
         }
       
		$length_new = sizeof($entries_new)+4;
		
		for($i=1;$i<$length_new ;$i++) {
		    for($j=1; $j<=17; $j++)
			{
			    if($j<=8)
			    {
	                 $objPHPExcel->getActiveSheet()->setSharedStyle($stylebody, "A4:H".$i);
			    }
			    else if($j>8 && $j<=10)
			    {
	                 $objPHPExcel->getActiveSheet()->setSharedStyle($styleCol, "I4:J".$i);
			    }
			    else if($j>10 && $j<=12)
			    {
	                    
	                   $objPHPExcel->getActiveSheet()->setSharedStyle($styleCol_2, "K4:L".$i);
	            }
	            else if($j>12 && $j<=14)
			    {
	                    
	                   $objPHPExcel->getActiveSheet()->setSharedStyle($styleCol, "M4:N".$i);
	            }
	            else if($j>14 && $j<=17)
	            {
			       $objPHPExcel->getActiveSheet()->setSharedStyle($stylebody, "O4:Q".$i);
			    }
            }  
	    }
		//Create Sea Sheet Job Files
		$objPHPExcel->createSheet();
		$objPHPExcel->setActiveSheetIndex(1);
		$objPHPExcel->getActiveSheet()->setTitle("Data Collection Template(Sea)");
		$objPHPExcel->getActiveSheet()->setCellValue('A1', 'NOTE: (1) Please submit the data of all international Air Export shipments with ATD of the current month.  
		(2) Kindly identify the changes in the required data that was submitted in previous months and then submit the revised data again in the current month datasheet.');
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A2', 'for checking uniqueness');
		$objPHPExcel->getActiveSheet()->mergeCells("A2:B2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($stylebody,"A2:F2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($styleCol_2,"G2:H2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($styleCol,"I2:N2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($stylebody,"O2:R2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($styleCol_2,"S2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($stylebody,"T2:U2");
        $objPHPExcel->getActiveSheet()->setCellValue('C2', 'we are currently collecting Export only');
        $objPHPExcel->getActiveSheet()->setCellValue('D2', 'for Export only');
        $objPHPExcel->getActiveSheet()->setCellValue('E2', 'Customer refers to the shipper of freehand shipment or the consignee of nominated shipment subject to the contract of carriage');
		$objPHPExcel->getActiveSheet()->mergeCells("E2:F2");
		$objPHPExcel->getActiveSheet()->getStyle('E2')->getFont()->getColor()->setARGB('FF3A00');
		$objPHPExcel->getActiveSheet()->setCellValue('G2', 'optional');
			$objPHPExcel->getActiveSheet()->mergeCells("G2:H2");

		$sea_headers =   array('MBL No','HBL No','Import/Export','ATD','Customer Name', 'Customer Code','Shipping Line Code','Vessel Name (IMO number)', 'Port of Loading Country','Port of Loading City','Port of Loading Port Code (UN/LOCODE)', 'Port of Discharge Country','Port of Discharge City', 'Port of Discharge Port Code (UN/LOCODE)', 'FCL/LCL', 'Dry/Reefer', 'Container Size', 'Gross Weight (kg)', 'CBM', 'Job file','Company name');
      
		$objPHPExcel->getActiveSheet()->fromArray($sea_headers, null, 'A3');
		$objPHPExcel->getActiveSheet()->freezePane('A4');
		$objPHPExcel->getActiveSheet()->getPageSetup()->setRowsToRepeatAtTopByStartAndEnd(1, 1);
        for($i=0; $i<=21;$i++)
         {
		...
		foreach($entries as $key => $entry)
		{
           ...
           $qry_c ="SELECT vtiger_company.name FROM vtiger_company  INNER JOIN vtiger_companycf on vtiger_companycf.companyid =vtiger_company.companyid where  vtiger_companycf.cf_996='".$entry['cf_1186']."'";
			...
			$company_name = $row_company_name['name'];
			//echo $entry['job_air_origin_city'];
            $qry = "SELECT * from `air_sea_codes` WHERE  port_city_code = '".$entry['job_sea_loading_city']."' AND  port_country_code = '".$entry['job_sea_loading_country']."'";
			$sql = $adb->pquery($qry);
			$rows = $adb->fetch_array($sql);
			$job_sea_loading_country = $rows['port_country'];
			$job_sea_loading_state = $rows['state_name'];
			$job_sea_loading_city = $rows['port_city'];


			 $qry = "SELECT * from `air_sea_codes` WHERE  port_city_code = '".$entry['job_sea_discharge_city']."' AND  port_country_code = '".$entry['job_sea_discharge_country']."'";
			$sql = $adb->pquery($qry);
			$rows = $adb->fetch_array($sql);
			$job_sea_discharge_country = $rows['port_country'];
			$job_sea_discharge_state = $rows['state_name'];
			$job_sea_discharge_city = $rows['port_city'];

			$entries_new_sea[] = array(
							'MBL No' => $entry['cf_8946'], 
							'HBL No' => $entry['cf_8618'],
							'type' => $entry['cf_1200'], 
							'ATD' => $entry['cf_8614'], 
							'customer' => $entry['cf_1441'], 
							'customer code' => '',
							...
							'Vessel Name (IMO number)' => $entry['cf_5417'], 
							'job_sea_loading_country' => $job_sea_loading_country,
							'job_sea_loading_city' => $job_sea_loading_city,
							'job_sea_loading_port_code' => $entry['job_sea_loading_port_code'], 
							'job_sea_discharge_country' => $job_sea_discharge_country,
							'job_sea_discharge_city'=> $job_sea_discharge_city, 
							'job_sea_discharge_port_code' => $entry['job_sea_discharge_port_code'],
							'FCL-LCL' => $entry['cf_8904'],
							'Dry/Reefer' => $entry['cf_8902'],
							'Container Size' => $entry['cf_8900'], 
							'Gross Weight' => $entry['cf_4945'],
							'CBM' => $entry['cf_1086'], 

							'job file' => $entry['name'],
							'company_name' => $entry['cf_1186'].'-'.$company_name
							);
			
		}
		$objPHPExcel->getActiveSheet()->fromArray($entries_new_sea, null, 'A4');
        $objPHPExcel->getActiveSheet()->setSharedStyle($sharedStyle1, "A3:U3");
		$objPHPExcel->getActiveSheet()->fromArray($entries_new_sea, null, 'A4');
		...
		for($i=1;$i<$length_new ;$i++) {
		      ...
		      for($j=1; $j<=21; $j++)
			{
			    if($j<=6)
			    {
	                 $objPHPExcel->getActiveSheet()->setSharedStyle($stylebody, "A4:F".$i);
			    }
			    else if($j>6 && $j<=8)
			    {
	                 $objPHPExcel->getActiveSheet()->setSharedStyle($styleCol_2, "G4:H".$i);
			    }
			    else if($j>8 && $j<=14)
			    { 
	                   $objPHPExcel->getActiveSheet()->setSharedStyle($styleCol, "I4:N".$i);
	            }
	            else if($j>14 && $j<=18)
			    {  
	                   $objPHPExcel->getActiveSheet()->setSharedStyle($stylebody, "O4:R".$i);
	            }
	            else if($j>18 && $j<=19)
	            {
			       $objPHPExcel->getActiveSheet()->setSharedStyle($styleCol_2, "S4:S".$i);
			    }
			    else if($j>19 && $j<=21)
	            {
			       $objPHPExcel->getActiveSheet()->setSharedStyle($stylebody, "T4:U".$i);
			    }
            }  
	    }
	...
	function writeReportToExcelFile_Job_Scope_Data_Collection_Air($fileName, $headers, $entries, $filterlist='') 
	{
      ini_set('display_errors', 1);
		global $currentModule, $current_language;
		global $adb;
...
$style_head->applyFromArray(
	array(
       ...
       $sharedStyle1 = new PHPExcel_Style();
		...
		$objPHPExcel->getActiveSheet()->setCellValue('F1', '注: (1) 请提交所有于当月始发的国际空运出口货物数据。
(2) 如前几个月提交过的数据有任何变动，请将修改后的数据放于表中再次提交。');
		$objPHPExcel->getActiveSheet()->mergeCells("F1:K1");
		$objPHPExcel->getActiveSheet()->setSharedStyle($style_head,"A1");
		$objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->getColor()->setARGB('FF3A00');
		$objPHPExcel->getActiveSheet()->getStyle('K1')->getFont()->getColor()->setARGB('FF3A00');
		$objPHPExcel->getActiveSheet()->setCellValue('A2', 
			'for checking uniqueness 
			用作检查唯一性
			(if direct, HAWB = "direct")
			(如直接装运, 在航空分运单号填写"direct")
			');
		$objPHPExcel->getActiveSheet()->mergeCells("A2:B2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($stylebody,"A2:H2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($styleCol,"I2:J2");
		...
		$objPHPExcel->getActiveSheet()->setCellValue('C2', 
				'we are currently collecting Export only
				现阶段只收集出口'
				);
		$objPHPExcel->getActiveSheet()->setCellValue('D2', 'for Export only 仅用于出口');
		$objPHPExcel->getActiveSheet()->setCellValue('E2', 
			'Customer refers to the shipper of freehand shipment or the consignee of nominated shipment subject to the contract of carriage  
			客户是指受运输合同约束的自揽货的托运人或指定货的收货人
			');
		$objPHPExcel->getActiveSheet()->mergeCells("E2:F2");
		$objPHPExcel->getActiveSheet()->getStyle('E2')->getFont()->getColor()->setARGB('FF3A00');
		...
		$air_headers =   array('Station Code',
			'MAWB No 主航空运单号', 'HAWB No
航空分运单号','Import/Export
入口/出口', 'ATD (dd/MM/yyyy)
始发时间', 'Customer Name
客户名称', 'Customer Code
客户代码', 'Airline Code
航空公司', 'Flight Number
航班号','Origin Port Code (IATA)
始发港 (IATA机场代码)','Via Port Code 1 (IATA)
径由港1 (IATA机场代码)', 'Via Port Code 2 (IATA)
径由港2  (IATA机场代码)', 'Destination Port Code (IATA)
目的港  (IATA机场代码)', 'Gross Weight (kg)
毛重 (公斤)');
		...
		$objPHPExcel->getActiveSheet()->setSharedStyle($sharedStyle1, "A3:N3");
        $objPHPExcel->getActiveSheet()->getStyle('A3')->getAlignment()->setWrapText(true);
		$objPHPExcel->getActiveSheet()->fromArray($entries_new, null, 'A4');
		$objPHPExcel->getActiveSheet()->getStyle('A4')->getAlignment()->setWrapText(true);
         for($i=0; $i<=14;$i++)
	...
	function writeReportToExcelFile_Job_Scope_Data_Collection_Sea($fileName, $headers, $entries, $filterlist='') 
	{
		ini_set('display_errors', 1);
		global $currentModule, $current_language;
		global $adb;
...
$style_head->applyFromArray(
	array(
       ...
       $sharedStyle1 = new PHPExcel_Style();
		...
		$objPHPExcel->setActiveSheetIndex(0);
		$objPHPExcel->getActiveSheet()->setTitle("Data Collection Template(Sea)");
		$objPHPExcel->getActiveSheet()->setCellValue('A1', 'NOTE: (1) Please submit the data of all international Sea Export shipments with ATD of the current month.  
(2) Kindly identify the changes in the required data that was submitted in previous months and then submit the revised data again in the current month datasheet.');
		$objPHPExcel->getActiveSheet()->mergeCells("A1:G1");
		$objPHPExcel->getActiveSheet()->setCellValue('H1', '注: (1) 请提交所有于当月始发的国际海运出口货物数据。
(2) 如前几个月提交过的数据有任何变动，请将修改后的数据放于表中再次提交。');
		$objPHPExcel->getActiveSheet()->mergeCells("H1:W1");
		$objPHPExcel->getActiveSheet()->setSharedStyle($style_head,"A1");
        $objPHPExcel->getActiveSheet()->setSharedStyle($style_head,"H1");
        $objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->getColor()->setARGB('FF3A00');
        $objPHPExcel->getActiveSheet()->getStyle('H1')->getFont()->getColor()->setARGB('FF3A00');
		$objPHPExcel->getActiveSheet()->setCellValue('A2', 'for checking uniqueness 
用作检查唯一性
(if direct, HBL = "direct")
(如直接装运, 在分单号填写"direct")');
		$objPHPExcel->getActiveSheet()->mergeCells("A2:C2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($stylebody,"A2:F2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($styleCol_2,"G2:I2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($styleCol,"J2:O2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($stylebody,"O2:R2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($stylebody,"P2:X2");
        $objPHPExcel->getActiveSheet()->setCellValue('D2', 'we are currently collecting Export only
现阶段只收集出口');
        $objPHPExcel->getActiveSheet()->setCellValue('E2', 'for Export only
仅用于出口');
        $objPHPExcel->getActiveSheet()->setCellValue('F2', 'Customer refers to the shipper of freehand shipment or the consignee of nominated shipment subject to the contract of carriage  
客户是指受运输合同约束的自揽货的托运人或指定货的收货人');
		$objPHPExcel->getActiveSheet()->mergeCells("F2:G2");
		$objPHPExcel->getActiveSheet()->getStyle('E2')->getFont()->getColor()->setARGB('FF3A00');
		$objPHPExcel->getActiveSheet()->setCellValue('H2', 'optional');
			$objPHPExcel->getActiveSheet()->mergeCells("H2:I2");
        $objPHPExcel->getActiveSheet()->setCellValue('J2', 'Either Country Code + City name or UNLOCODE is mandatory
必需填写: 国家代码+城市 或 UN/LOCODE');
			$objPHPExcel->getActiveSheet()->mergeCells("J2:L2");
			  $objPHPExcel->getActiveSheet()->setCellValue('M2', 'Either  Country Code + City name or UNLOCODE is mandatory
必需填写: 国家代码+城市 或 UN/LOCODE');
			$objPHPExcel->getActiveSheet()->mergeCells("M2:O2");
			$objPHPExcel->getActiveSheet()->setCellValue('R2', 'Number of containers of each type
箱型及箱量');
			$objPHPExcel->getActiveSheet()->mergeCells("R2:W2");
			$objPHPExcel->getActiveSheet()->setCellValue('X2', 'Gross wieight of the whole shipment
整批货物的毛重');
		
		$sea_headers =   array('Station Code','MBL No 主单号','HBL No 分单号','Import/Export 入口/出口','ATD 始发时间','Customer Name 客户名称', 'Customer Name 客户名称','Shipping Line Code 船公司','Vessel Name (IMO number) 船名', 'Port of Loading Country Code 始发港 (国家代码)','Port of Loading City 始发港 (城市)','Port of Loading Port Code (UN/LOCODE)
始发港 (UN/LOCODE)', 'Port of Discharge Country Code 目的港 (国家代码)','Port of Discharge City
目的港 (城市)', 'Port of Discharge Port Code (UN/LOCODE) 目的港 (UN/LOCODE)', 'FCL/LCL 整箱/拼箱', 'Dry/Reefer 干燥/冷藏集装箱', '20','40','40HQ','45','53','Other 其他', 'Gross Weight (kg) 毛重 (公斤)');
		...
		foreach($entries as $key => $entry)
		{
		   ...
		   $qry = "SELECT * FROM `vtiger_jobcf` WHERE `cf_1198` = '".$entry['cf_1198']."'";
		    $sql = $adb->pquery($qry);
			$rows = $adb->fetch_array($sql);
			$jobid = $rows['jobid'];
            //containeer type and size fetch from custom table 

		   $qry_type = "SELECT  * FROM  job_container_type_size WHERE deleted_status=0 and jobid ='".$jobid."'";
			$sql_type = $adb->pquery($qry_type);
			$rows_type = $adb->fetch_array($sql_type);
			$container_type = $rows_type['container_type'];
		    $container_size = $rows_type['container_size'];
			$container_qty = $rows_type['container_qty'];
            $container_fcl_lcl = $rows_type['container_fcl_lcl']; 	
			//echo $entry['job_air_origin_city'];
            $qry = "SELECT * from `air_sea_codes` WHERE  port_city_code = '".$entry['job_sea_loading_city']."' AND  port_country_code = '".$entry['job_sea_loading_country']."'";
			$sql = $adb->pquery($qry);
			$rows = $adb->fetch_array($sql);
			...
			$job_sea_discharge_city = $rows['port_city'];
               ...
               switch($container_size){
               	case '20':
               		     $twenty_size  = $container_qty;
               		break;
               		case '40':
               		     $forty_size  = $container_qty;
               		break;
               		case '40HQ':
               		     $fortyHQ_size  = $container_qty;
               		break;
               		case '45':
               		     $fortyFive_size  = $container_qty;
               		break;
               		case '53':
               		     $fiftythree_size  = $container_qty;
               		break;
               		case 'Other':
               		     $other_size  = $container_qty;
               		break;
               }
			...
			$entries_new_sea[] = array(
							...
							'job_sea_discharge_city'=> $job_sea_discharge_city, 
							'job_sea_discharge_port_code' => $entry['job_sea_discharge_port_code'],
							'FCL-LCL' =>  $container_fcl_lcl,
							'Dry/Reefer' => $container_type,
							'20' =>$twenty_size,
							'40' => $forty_size,
							'40HQ' => $fortyHQ_size, 
							'45' => $fortyFive_size , 
		...
		$objPHPExcel->getActiveSheet()->fromArray($entries_new_sea, null, 'A4');
        $objPHPExcel->getActiveSheet()->setSharedStyle($sharedStyle1, "A3:X3");

        $objPHPExcel->getActiveSheet()->getStyle('A3')->getAlignment()->setWrapText(true);
		$objPHPExcel->getActiveSheet()->getStyle('A4')->getAlignment()->setWrapText(true);
		$entries_new_sea = sizeof($entries_new_sea)+4;
		
		for($i=1;$i<$entries_new_sea ;$i++) {
		      ...
		      for($j=1; $j<=24; $j++)
			{
			    ...
			    {
	                 $objPHPExcel->getActiveSheet()->setSharedStyle($styleCol_2, "G4:I".$i);
			    }
			    else if($j>9 && $j<=15)
			    { 
	                   $objPHPExcel->getActiveSheet()->setSharedStyle($styleCol, "J4:O".$i);
	            }
	            else if($j>15 && $j<=24)
			    {      
			       
	                   $objPHPExcel->getActiveSheet()->setSharedStyle($stylebody, "P4:X".$i);
	            }

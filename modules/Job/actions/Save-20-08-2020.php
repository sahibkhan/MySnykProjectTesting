...
class Job_Save_Action extends Vtiger_Save_Action {
	...
	public function saveRecord($request) {
		...
		if(empty($recordId)) {
			...
			$db->pquery('update vtiger_jobcf set cf_2197=?, cf_1198 = ? ,cf_6934 = ?, cf_6936 = ? where jobid=?', array('No Costing', $ref_no,ucwords($customer_status), $sale_person, $recordModel->getId()));

...
$newhtml .= '<tr>';
	...
	function writeReportToExcelFile_Job_Data_Collection($fileName, $headers, $entries, $filterlist='') 
	{
      ini_set('display_errors', 1);
	    ...
	    $role_id = $current_user->get('is_admin');
		$currentdate = date('Y-m');
        if($role_id == 'off'){
            $query = "SELECT vtiger_jobcf.*, vtiger_job.name FROM vtiger_job
             inner join vtiger_jobcf on vtiger_jobcf.jobid = vtiger_job.jobid 
             inner join vtiger_crmentity on vtiger_jobcf.jobid = vtiger_crmentity.crmid
             where vtiger_crmentity.deleted = 0 and vtiger_jobcf.cf_3527 = '".$user_id."' and  DATE_FORMAT(vtiger_crmentity.createdtime,'%Y-%m')  = '". $currentdate."'";
        }else{
          	$query = "SELECT vtiger_jobcf.*, vtiger_job.name FROM vtiger_job
             inner join vtiger_jobcf on vtiger_jobcf.jobid = vtiger_job.jobid 
             inner join vtiger_crmentity on vtiger_jobcf.jobid = vtiger_crmentity.crmid
             where vtiger_crmentity.deleted = 0 and  DATE_FORMAT(vtiger_crmentity.createdtime,'%Y-%m')  = '". $currentdate."'";
            
           
        }
        $rows_entries = $adb->pquery($query, array());  
		$objPHPExcel->getProperties()->setCreator($full_name)
									 ->setLastModifiedBy($full_name)
...
$style_head->applyFromArray(
	array(
       ...
       $sharedStyle1 = new PHPExcel_Style();
		...
		$entries_new = array();
		foreach($rows_entries  as $key => $entry)
		{   
            //customer name accont
            $qry_user = "SELECT `accountname` FROM `vtiger_account` WHERE `accountid`='".$entry['cf_1441']."'";
            
            $sql_u = $adb->pquery($qry_user);
			$row_customer_name = $adb->fetch_array($sql_u);
			$customer_name = $row_customer_name['accountname'];

            $qry_c ="SELECT vtiger_company.name,vtiger_companycf.cf_996 FROM vtiger_company  INNER JOIN vtiger_companycf on vtiger_companycf.companyid =vtiger_company.companyid where  vtiger_companycf.companyid='".$entry['cf_1186']."'";
			...
			$entries_new[] = array( 
							...
							'type' => $entry['cf_1200'],
							'ATD' => $entry['cf_8654'], 
							'CUSTOMER NAME' => html_entity_decode($customer_name), 
							'customer code' => '',
							...
							'name' => $entry['name'],
							'company name' =>  $row_company_name['cf_996'].'-'.$company_name
						   );
        ...
        foreach($rows_entries as $key => $entry)
		{
           ...
           $qry_c ="SELECT vtiger_company.name,vtiger_companycf.cf_996 FROM vtiger_company  INNER JOIN vtiger_companycf on vtiger_companycf.companyid =vtiger_company.companyid where  vtiger_companycf.companyid='".$entry['cf_1186']."'";
			...
			$entries_new_sea[] = array(
							...
							'job file' => $entry['name'],
							'company_name' => $row_company_name['cf_996'].'-'.$company_name
							);

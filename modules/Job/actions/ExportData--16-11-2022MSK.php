...
目的港 (城市)', 'Port of Discharge Port Code (UN/LOCODE) 目的港 (UN/LOCODE)', 'FCL/LCL 整箱/拼箱', 'Dry/Reefer 干燥/冷藏集装箱', '20','40','40HQ','45','53','Other 其他', 'Gross Weight (kg) 毛重 (公斤)');
		...
		foreach($entries as $key => $entry)
		{
			...
			$job_sea_discharge_city = $rows['port_city'];
				...
				if($entry['cf_8900']=='20'){
				   $twenty_size  = count($entry['cf_8900']);
				}
				if($entry['cf_8900']=='40'){
				    $forty_size  = count($entry['cf_8900']);
				}
				if($entry['cf_8900']=='40HQ'){
				    $fortyHQ_size = count($entry['cf_8900']); 
				}
				
				if($entry['cf_8900']=='45'){
				   $fortyFive_size  = count($entry['cf_8900']); 
				}
				if($entry['cf_8900']=='53'){
				   $fiftythree_size = count($entry['cf_8900']); 
				}
				if($entry['cf_8900']=='Other'){
				   $other_size = count($entry['cf_8900']);
				}
			$entries_new_sea[] = array(
				            'Station_code' => '', 
							'MBL No' => $entry['cf_8946'], 

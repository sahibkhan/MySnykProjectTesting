...
$style_head->applyFromArray(
	array(
       ...
       $sharedStyle1 = new PHPExcel_Style();
		...
		$objPHPExcel->getActiveSheet()->setSharedStyle($styleCol,"I2:L2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($styleCol_2,"M2:N2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($styleCol,"O2:R2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($stylebody,"S2:U2");
		...
		$air_headers =   array('HAWB NO', 'MAWB NO','TYPE', 'ATD', 'CUSTOMER NAME', 'CUSTOMER CODE', 'AIR CODE', 'FLIGHT NO','ORIGIN PORT CODE (IATA)','AIR ORIGIN COUNTRY','AIR ORIGIN STATE','AIR ORIGIN CITY',
							   'VIA PORT CODE 1 (IATA)', 'VIA PORT CODE 2 (IATA)', 'DESTINATION PORT CODE (IATA)', 'DESTINATION PORT COUNTRY','DESTINATION PORT STATE','DESTINATION PORT CITY', 'GROSS WEIGHT (kg)', 'JOB FILE','COMPANAY NAME');
		...
		foreach($entries as $key => $entry)
		{
			...
			$entries_new[] = array( 
									...
									'Job Air Origin Country' => $job_air_origin_country,
									'Job Air Origin State' => $job_air_origin_state,
									'Job Air Origin City' => $job_air_origin_city, 
									'Via IATA CODE'=>'Via IATA CODE',
									'Via IATA CODE 2' => 'Via IATA CODE 2', 
									'Job Air Dist Port Code' => $entry['job_air_destination_port_code'],
									'Job Air Destination Country' => $job_air_destination_country,
									'Job Air Destination State' => $job_air_destination_state,
									'Job Air Destination City' => $job_air_destination_city, 
									'gross_weight' => $entry['cf_1084'],
									'name' => $entry['name'],
									'Compan Name' => $entry['cf_1441']
								   );
		...
		for($i=1;$i<$length_new ;$i++) {
		    for($j=1; $j<=$i; $j++)
			{
			    ...
			    {
	                 $objPHPExcel->getActiveSheet()->setSharedStyle($styleCol, "I4:L".$i);
			    }
			    else if($j>12 && $j<=14)
			    {
	                    
	                   $objPHPExcel->getActiveSheet()->setSharedStyle($styleCol_2, "M4:N".$i);
	            }
	            else if($j>14 && $j<=18)
			    {
	                    
	                   $objPHPExcel->getActiveSheet()->setSharedStyle($styleCol, "O4:R".$i);
	            }
	            else if($j>18 && $j<=21)
	            {
			       $objPHPExcel->getActiveSheet()->setSharedStyle($stylebody, "S4:U".$i);
			    }
            }  
	    }
		//Create Sea Sheet Job Files
		...
		$objPHPExcel->getActiveSheet()->setSharedStyle($styleCol_2,"F2:G2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($styleCol,"H2:P2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($stylebody,"Q2:T2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($styleCol_2,"U2");
		$objPHPExcel->getActiveSheet()->setSharedStyle($stylebody,"V2:W2");
        $objPHPExcel->getActiveSheet()->setCellValue('C2', 'we are currently collecting Export only');
        $objPHPExcel->getActiveSheet()->setCellValue('D2', 'for Export only');
        $objPHPExcel->getActiveSheet()->setCellValue('E2', 'Customer refers to the shipper of freehand shipment or the consignee of nominated shipment subject to the contract of carriage');
		$objPHPExcel->getActiveSheet()->mergeCells("E2:F2");
		...
		$sea_headers =   array('MBL NO', 'HBL NO', 'TYPE', 'ATD', 'CUSTOMER NAME', 'CUSTOMER CODE','SHIPPING LINE CODE', 'VESSEL NAME (IMO NUMBER)', 'PORT OF LOADING COUNTRY', 'PORT OF LOADING STATE', 'PORT OF LOADING CITY',
							   'PORT OF LOADING PORT CODE (UN/LOCODE)', 'PORT OF DISCHARGE COUNTRY','PORT OF DISCHARGE STATE', 'PORT OF DISCHARGE CITY', 'PORT OF DISCHARGE PORT CODE(UN/LOCODE)', 'FCL/LCL', 'DRY/REEFER', 'CONTAINER SIZE', 'GROSS WEIGHT (KG)', 'CBM', 'JOB FILE',
							   'COMPANAY NAME');
		...
		foreach($entries as $key => $entry)
		{
			...
			$entries_new_sea[] = array(
							...
							'job_sea_discharge_state'=> $job_sea_discharge_state, 
							...
							'job file' => $entry['name'],
							'company Name' => $entry['cf_1441']
							);
			
		}
		$objPHPExcel->getActiveSheet()->fromArray($entries_new_sea, null, 'A4');
        $objPHPExcel->getActiveSheet()->setSharedStyle($sharedStyle1, "A3:W3");
		$objPHPExcel->getActiveSheet()->fromArray($entries_new_sea, null, 'A4');
		...
		for($i=1;$i<$length_new ;$i++) {
		      ...
		      for($j=1; $j<=$i; $j++)
			{
			    ...
			    { 
	                   $objPHPExcel->getActiveSheet()->setSharedStyle($styleCol, "I4:P".$i);
	            }
	            else if($j>16 && $j<=20)
			    {  
	                   $objPHPExcel->getActiveSheet()->setSharedStyle($stylebody, "Q4:T".$i);
	            }
	            else if($j>20 && $j<=21)
	            {
			       $objPHPExcel->getActiveSheet()->setSharedStyle($styleCol_2, "U4:U".$i);
			    }
			    else if($j>21 && $j<=23)
	            {
			       $objPHPExcel->getActiveSheet()->setSharedStyle($stylebody, "V4:W".$i);
			    }
            }  
	    }

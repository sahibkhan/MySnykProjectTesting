...
class Job_RelatedList_View extends Vtiger_RelatedList_View {
	...
	function process(Vtiger_Request $request) {
		...
		if($relatedModuleName =='Jobexpencereport')
		{
			...
			$jer_count_sql =  "SELECT count(*) as total_jer FROM `vtiger_crmentity` 
							 INNER JOIN vtiger_crmentityrel as crmentityrel ON vtiger_crmentity.crmid= crmentityrel.relcrmid 
							 WHERE vtiger_crmentity.deleted=0 AND crmentityrel.module='Job' AND crmentityrel.relmodule='JER' AND crmentityrel.crmid=?";
			// parentId = Job Id	
			$job_id = $parentId;		 
			$params = array($parentId);
	...
	{				
				...
				foreach($models as $key => $model){
					...
					$action .='<a href="index.php?module=Jobexpencereport&relatedModule=Documents&view=Detail&record='.$model->getId().'&mode=showRelatedList&tab_label=Documents"><i title="Upload Supporting Document" class="icon-th-list alignMiddle"></i></a>&nbsp;';					
					...
					if(!in_array($expense_record->get('cf_1212'), $check_buying_invoice_no))
					{
						...
						{
							$action_inv ='&nbsp;<a href="PV_export_inv_pdf.php?record='.$model->getId().'&jobid='.$job_id.'" target="_blank"><i title="Print Payment Voucher" class="icon-print alignMiddle"></i></a>&nbsp;';
						}
					}
		...
		if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && $_REQUEST['grid']==2) 
		{
				...
				foreach($models as $key => $model){
					...
					if(!in_array($model->get('preview_instruction_no'), $check_preview_instruction_no) && !empty($pre_instruction_no) && ($s_preview_invoice_instruction=='1') && empty($instruction_no))
					{
						...
						$action .='<a href="invoice_preview_pdf.php?record='.$model->getId().'&jobid='.$job_id.'&bill_to='.$model->get('cf_1445').'&invoice_instruction_no='.$model->get('preview_instruction_no').'"><i title="Preview Invoice Instruction" class="icon-print alignMiddle"></i></a>&nbsp;';
					}
					...
					if(!in_array($model->get('invoice_instruction_no'), $check_invoice_instruction_no) && !empty($instruction_no) && ($s_generate_invoice_instruction=='1'))
					{
						...
						$action .='<a href="invoice_export_pdf.php?record='.$model->getId().'&jobid='.$job_id.'&bill_to='.$selling_record->get('cf_1445').'&invoice_instruction_no='.$model->get('invoice_instruction_no').'"><i title="Generate Invoice Instruction" class="icon-print alignMiddle"></i></a>&nbsp;';
					}
	...
	function process_selling_invoices(Vtiger_Request $request)
	{
		...
		{				
				...
				$kazbek_branch = array('85805','85829','85806','85815', '85822', '85834', '85812', '85813', '85946','85814','85831','85808', '85809', '85824', '85820','85819','85832');

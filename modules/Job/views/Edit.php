...
Class Job_Edit_View extends Vtiger_Edit_View {
	...
	public function process(Vtiger_Request $request) {
		...
		if(!empty($record) && $request->get('isDuplicate') == true) {
			...
			$recordModel = $this->record?$this->record:Vtiger_Record_Model::getInstanceById($record, $moduleName);
			$viewer->assign('MODE', '');
			//select multiple container size
			$sql = "SELECT  JOBCON.id,JOBCON.jobid,JOBCON.container_type,JOBCON.container_size,JOBCON.container_qty,JOBCON.container_fcl_lcl FROM job_container_type_size as JOBCON
			 INNER JOIN vtiger_job as vtiger_job ON vtiger_job.jobid = JOBCON.jobid where 	JOBCON.deleted_status='0' AND vtiger_job.jobid=$record";

			$result = $db->pquery($sql);
			$totalrecords = $db->num_rows($result);
			for($i=0; $i<$db->num_rows($result); $i++) {
		    $container_details[$i]['id'] = $db->query_result($result, $i, 'id');
			$container_details[$i]['container_type'] = $db->query_result($result, $i, 'container_type');
			$container_details[$i]['container_size'] = $db->query_result($result, $i, 'container_size');
			$container_details[$i]['container_qty'] = $db->query_result($result, $i, 'container_qty');
			$container_details[$i]['container_fcl_lcl'] = $db->query_result($result, $i, 'container_fcl_lcl');
			$container_details[$i]['jobid'] = $db->query_result($result, $i, 'jobid');
			}
			$viewer->assign('container_details',$container_details);
		    $viewer->assign('totalrecords',$totalrecords);
			//origin country,city,General block
			$GEN_COUNTRY_CODE = $recordModel->get('cf_1504');
			$GEN_CITY_CODE = $recordModel->get('cf_1508');
		    $GEN_STATE_CODE = $recordModel->get('job_origin_states');
			$viewer->assign('GEN_COUNTRY_CODE',$GEN_COUNTRY_CODE);
			$viewer->assign('GEN_CITY_CODE',$GEN_CITY_CODE);
			$viewer->assign('GEN_STATE_CODE',$GEN_STATE_CODE);
			//Destination country,city,General block
			$GEN_DEST_COUNTRY_CODE = $recordModel->get('cf_1506');
			$GEN_DEST_CITY_CODE = $recordModel->get('cf_1510');
			$GEN_DEST_STATE_CODE = $recordModel->get('job_destination_states');
			$viewer->assign('GEN_DEST_COUNTRY_CODE',$GEN_DEST_COUNTRY_CODE);
			$viewer->assign('GEN_DEST_CITY_CODE',$GEN_DEST_CITY_CODE);
			$viewer->assign('GEN_DEST_STATE_CODE',$GEN_DEST_STATE_CODE);	

		  //origin country,city,airportcode block
			$COUNTRY_CODE = $recordModel->get('job_air_origin_country');
			$CITY_CODE = $recordModel->get('job_air_origin_city');
			$AIR_AIRPORTCODE = $recordModel->get('job_air_origin_port_code');
			$ORIGINAIRPORTCODE = $recordModel->get('job_air_origin_airport_name');
            $AIR_ORGIGIN_STATE = $recordModel->get('job_air_origin_state');            
			$viewer->assign('COUNTRY_CODE',$COUNTRY_CODE);
			$viewer->assign('AIR_ORGIGIN_STATE',$AIR_ORGIGIN_STATE);
			$viewer->assign('CITY_CODE',$CITY_CODE);
			$viewer->assign('AIR_AIRPORTCODE',$AIR_AIRPORTCODE);
			$viewer->assign('ORIGINAIRPORTCODE',$ORIGINAIRPORTCODE);
			//Destination country,city,airportcode block
			$DESTCOUNTRY_CODE = $recordModel->get('job_air_destination_country');
			$DESTCITY_CODE = $recordModel->get('job_air_destination_city');
			$AIR_DESTAIRPORTCODE = $recordModel->get('job_air_destination_port_code');
			$DISAIRPORTCODE = $recordModel->get('job_air_destination_airport_name');
			$AIR_DEST_STATE = $recordModel->get('job_air_destination_state');
			$viewer->assign('DESTCOUNTRY_CODE',$DESTCOUNTRY_CODE);
			$viewer->assign('AIR_DEST_STATE',$AIR_DEST_STATE);
			$viewer->assign('DESTCITY_CODE',$DESTCITY_CODE);
			$viewer->assign('AIR_DESTAIRPORTCODE',$AIR_DESTAIRPORTCODE);
            $viewer->assign('DISAIRPORTCODE',$DISAIRPORTCODE);
           //Sea block country,city,portcode 
			$LOADINGCOUNTRY_CODE = $recordModel->get('job_sea_loading_country');
			$LOADINGCITY_CODE = $recordModel->get('job_sea_loading_city');
			$LOADING_PORTCODE = $recordModel->get('job_sea_loading_port_code');
			$LOADING_STATE_CODE = $recordModel->get('job_sea_loading_state');
			$viewer->assign('LOADINGCOUNTRY_CODE',$LOADINGCOUNTRY_CODE);
			$viewer->assign('LOADING_STATE_CODE',$LOADING_STATE_CODE);
			$viewer->assign('LOADINGCITY_CODE',$LOADINGCITY_CODE);
			$viewer->assign('LOADING_PORTCODE',$LOADING_PORTCODE);
            //Discharge country,city,portcode block
			$DIS_COUNTRY_CODE = $recordModel->get('job_sea_discharge_country');
			$DIS_CITY_CODE = $recordModel->get('job_sea_discharge_city');
			$DIS_PORTCODE = $recordModel->get('job_sea_discharge_port_code');
			$DIS_STATE_CODE = $recordModel->get('job_sea_discharge_state');
			$viewer->assign('DIS_COUNTRY_CODE',$DIS_COUNTRY_CODE);
			$viewer->assign('DIS_STATE_CODE',$DIS_STATE_CODE);
			$viewer->assign('DIS_CITY_CODE',$DIS_CITY_CODE);
			$viewer->assign('DIS_PORTCODE',$DIS_PORTCODE);
			//fcl or lcl code Get
		     $FCL_CODE = $recordModel->get('cf_8620');
		     $viewer->assign('FCL_CODE',$FCL_CODE);
			...
			if(empty($record))
			{
				...
				elseif($request->get('returnmodule') == 'Quotes' && $request->get('returnrecord')) {
					...
					$quote_info = Vtiger_Record_Model::getInstanceById($parentRecordId, $parentModuleName);
						...
						//origin country,city,General block
					$GEN_COUNTRY_CODE = $quote_info->get('cf_1613');
					$GEN_STATE_CODE = $quote_info->get('quotes_origin_state');
					$viewer->assign('GEN_COUNTRY_CODE',$GEN_COUNTRY_CODE);
					$viewer->assign('GEN_STATE_CODE',$GEN_STATE_CODE);
					//Destination country,city,General block
					$GEN_DEST_COUNTRY_CODE = $quote_info->get('cf_1617');
					$GEN_DEST_STATE_CODE = $quote_info->get('quotes_destination_state');
					$viewer->assign('GEN_DEST_COUNTRY_CODE',$GEN_DEST_COUNTRY_CODE);
					$viewer->assign('GEN_DEST_STATE_CODE',$GEN_DEST_STATE_CODE);	
                    
                    //origin country,city,airportcode block
					$COUNTRY_CODE = $quote_info->get('cf_1613');
					$CITY_CODE = $quote_info->get('cf_1615');
					$AIR_ORGIGIN_STATE = $quote_info->get('quotes_origin_state');            
					$viewer->assign('COUNTRY_CODE',$COUNTRY_CODE);
					$viewer->assign('AIR_ORGIGIN_STATE',$AIR_ORGIGIN_STATE);
					$viewer->assign('CITY_CODE',$CITY_CODE);
					
					//Destination country,city,airportcode block
					$DESTCOUNTRY_CODE = $quote_info->get('cf_1617');
					$DESTCITY_CODE = $quote_info->get('cf_1619');
					$AIR_DEST_STATE = $quote_info->get('quotes_destination_state');
					$viewer->assign('DESTCOUNTRY_CODE',$DESTCOUNTRY_CODE);
					$viewer->assign('AIR_DEST_STATE',$AIR_DEST_STATE);
					$viewer->assign('DESTCITY_CODE',$DESTCITY_CODE);
					...
					//Sea block country,city,portcode 
					$LOADINGCOUNTRY_CODE = $quote_info->get('cf_1613');
					$LOADINGCITY_CODE = $quote_info->get('cf_1615');
					$LOADING_STATE_CODE = $quote_info->get('quotes_origin_state');
					$viewer->assign('LOADINGCOUNTRY_CODE',$LOADINGCOUNTRY_CODE);
					$viewer->assign('LOADING_STATE_CODE',$LOADING_STATE_CODE);
					$viewer->assign('LOADINGCITY_CODE',$LOADINGCITY_CODE);
					//Discharge country,city,portcode block
					$DIS_COUNTRY_CODE = $quote_info->get('cf_1617');
					$DIS_CITY_CODE = $quote_info->get('cf_1619');
					$DIS_STATE_CODE = $quote_info->get('quotes_destination_state');
					$viewer->assign('DIS_COUNTRY_CODE',$DIS_COUNTRY_CODE);
					$viewer->assign('DIS_STATE_CODE',$DIS_STATE_CODE);
					$viewer->assign('DIS_CITY_CODE',$DIS_CITY_CODE);
					$viewer->assign('DIS_PORTCODE',$DIS_PORTCODE);
					...
					$request->set('job_origin_states', $quote_info->get('quotes_origin_state')); //origin_state
					$request->set('cf_1508', $quote_info->get('cf_1615')); //origin_city
					$request->set('cf_1506', $quote_info->get('cf_1617')); //destination_country
					$request->set('job_destination_states', $quote_info->get('quotes_destination_state')); //Destination_state
                    ...
                    //$modes = implode("|##|",$quote_info->get('cf_1709'));
                    
                  
				    $mode =  explode(" |##| ",$quote_info->get('cf_1709'));
				    //print_r($mode);
		            if(in_array('Air',$mode))
		            {
		        
						//Air Origin Section
						$request->set('job_air_origin_country', $quote_info->get('cf_1613'));
						$request->set('job_air_origin_state', $quote_info->get('quotes_origin_state'));
						$request->set('job_air_origin_city',$quote_info->get('cf_1615'));
						//Air Destination Section
						$request->set('job_air_destination_country',$quote_info->get('cf_1617'));
						$request->set('job_air_destination_state',$quote_info->get('quotes_destination_state'));
						$request->set('job_air_destination_city', $quote_info->get('cf_1619'));

					}
					if(in_array('Ocean',$mode))
		            {
		            	
						$request->set('job_sea_loading_country', $quote_info->get('cf_1613'));
						$request->set('job_sea_loading_state', $quote_info->get('quotes_origin_state'));
						$request->set('job_sea_loading_city',$quote_info->get('cf_1615'));
						//Air Destination Section
						$request->set('job_sea_discharge_country',$quote_info->get('cf_1617'));
						$request->set('job_sea_discharge_state',$quote_info->get('quotes_destination_state'));
						$request->set('job_sea_discharge_city', $quote_info->get('cf_1619'));

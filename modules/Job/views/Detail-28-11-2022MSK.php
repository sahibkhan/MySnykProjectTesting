	...
	function preProcess(Vtiger_Request $request, $display=true) {
        ...
        //select multiple container size
			$sql = "SELECT  JOBCON.id,JOBCON.jobid,JOBCON.container_type,JOBCON.container_size,JOBCON.container_qty,JOBCON.container_fcl_lcl FROM job_container_type_size as JOBCON
			 INNER JOIN vtiger_job as vtiger_job ON vtiger_job.jobid = JOBCON.jobid where 	JOBCON.deleted_status='0' AND vtiger_job.jobid=$recordId";

			$result = $adb->pquery($sql);
			
			for($i=0; $i<$adb->num_rows($result); $i++) {
		    $container_details[$i]['id'] = $adb->query_result($result, $i, 'id');
			$container_details[$i]['container_type'] = $adb->query_result($result, $i, 'container_type');
			$container_details[$i]['container_size'] = $adb->query_result($result, $i, 'container_size');
			$container_details[$i]['container_qty'] = $adb->query_result($result, $i, 'container_qty');
			$container_details[$i]['container_fcl_lcl'] = $adb->query_result($result, $i, 'container_fcl_lcl');
			$container_details[$i]['jobid'] = $adb->query_result($result, $i, 'jobid');
			}

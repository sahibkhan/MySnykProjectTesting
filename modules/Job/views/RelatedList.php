...
class Job_RelatedList_View extends Vtiger_RelatedList_View
{
    ...
    public function requiresPermission(Vtiger_Request $request)
    {
        ...
        $permissions[] = array(
            'module_parameter' => 'relatedModule',
            'action' => 'DetailView'
        );
    ...
    function process(Vtiger_Request $request)
    {
        ...
        foreach ($searchParams as $fieldListGroup)
        {
            foreach ($fieldListGroup as $fieldSearchInfo)
            {
                ...
                $whereCondition[$fieldSearchInfo[0]] = array(
                    $tableName . '.' . $column,
                    $fieldSearchInfo[1],
                    $fieldSearchInfo[2],
                    $fieldSearchInfo[3]
        ...
        if ($relatedModuleName == 'EDocs')
        {
            ...
            $job_type = $job_info_detail->get('cf_1200');
            $company = $job_info_detail->get('cf_1186');
            $department = $job_info_detail->get('cf_1190');
            $type = $job_info_detail->get('cf_1200');

            $job_status = $job_info_detail->get('cf_2197');
            $current_user = Users_Record_Model::getCurrentUserModel();

            $role_id = $current_user->get('roleid');
            $userid = $current_user->getId();
            $job_ownerid = $job_info_detail->get('assigned_user_id');

            $arr = $query = "SELECT vtiger_edocscf.document_name as document_name
							 FROM `vtiger_edocscf`
							WHERE FIND_IN_SET('$job_id',jobid)
							ORDER BY document_name DESC";
            $existing_arr = $adb->pquery($arr, array());
            $arr_count = $adb->num_rows($existing_arr);

            $arr2 = array();
            for ($m = 0;$m < $arr_count;$m++)
            {
                $arr2[$m]['document_name'] = $adb->query_result($existing_arr, $m, 'document_name');
            }


            if ($role_id == 'H218' || $role_id == 'H74' || $role_id == 'H67')
            {
                $query = "SELECT DISTINCT ercds.edocsid,ercds.user_id,ercds.document_name,vtiger_echecklist.name AS doc_name,ecf.jobid,vtiger_echecklistcf.cf_8382 AS required
            ...
            for ($j = 0;$j < $adb->num_rows($doc);$j++)
            {
                ...
                $docs[$j]['user_id'] = $adb->query_result($doc, $j, 'user_id');
                $docs[$j]['document_type'] = $adb->query_result($doc, $j, 'doc_name');
                $docs[$j]['edocsid'] = $adb->query_result($doc, $j, 'edocsid');
                $docs[$j]['required'] = $adb->query_result($doc, $j, 'required');
            ...
            {
                $doclinks[$k]['edocsrecordsid'] = $adb->query_result($link, $k, 'edocsrecordsid');
                ...
                $doclinks[$k]['user_id'] = $adb->query_result($link, $k, 'user_id');
            ...
            $viewer->assign('JOB_STATUS', $job_status);
            $viewer->assign('FILE_USER_TYPE', $echecklist_usertype);
            $viewer->assign('USERID', $current_user->getId());
        }

        if ($relatedModuleName == 'EDocuments')
        {
            ...
            if ($role_id == 'H218' || $role_id == 'H74' || $role_id == 'H67')
            {
                $query = "SELECT DISTINCT ercds.edocumentsid,ercds.document_name,vtiger_echecklist.name AS doc_name,ecf.jobid,vtiger_echecklistcf.cf_8382 AS required
        ...
        if ($relatedModuleName == 'JER' || $relatedModuleName == 'Jobexpencereport')
        {
            ...
            else
            {
             ...
             //sahib code for expense of the fuel fetch
           $jer_fuel_sql = "SELECT * FROM `vtiger_jercf` as jercf 
                              INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = jercf.jerid
                             INNER JOIN vtiger_crmentityrel as crmentityrel ON vtiger_crmentity.crmid= crmentityrel.relcrmid 
                             where vtiger_crmentity.deleted=0 AND crmentityrel.crmid=? AND jercf.cf_1451=85896  AND crmentityrel.module='Job'  AND crmentityrel.relmodule='JER' ";
            $params = array(
                $parentId
            );
            $result_fuel = $adb->pquery($jer_fuel_sql, $params);
            $row_job_costing_fuel = $adb->fetch_array($result_fuel);
          
             //$row_job_costing_fuel['cf_1156']=$file_title_currency;
        ...
        $viewer->assign('JOBSTATUS', $job_status);
            ...
            if (!empty($profit_share))
            {
                foreach ($profit_share as $key => $p_share)
                {
                    ...
                    if (!in_array($dept_branch, $profit_share_check))
                    {
                        ...
                        {
                            $params_buy_local = array(
                                $parentId,
                                $p_share['cf_1477_location_id'],
                                $p_share['cf_1479_department_id']
                            );
                        ...
                        */
                        ...
                        $profit_share_data[] = array(
                            'brach_department' => $brach_department_name,
        ...
        else if ($relatedModuleName == 'EDocuments')
        {
            ...
            //get the EChecklist based on owner and asignee(company.department,jobtype,usertype)
            $job_office_id = $job_info_detail->get('cf_1188');
            $current_user_office_id = $current_user->get('location_id');
            $echecklist_usertype = 'Job Owner';

            //check if job assigned or not
            //it may be job owner and asignee
            $types = array();
            $types_count = 0;
            ...
            if ($job_info_detail->get('assigned_user_id') == $current_user->getId())
            {
                ...
                $assign_count = $adb->num_rows($result_sub);
            }
            else
            {
            ...
            if ($assign_count > 0)
            {
                if ($job_info_detail->get('assigned_user_id') != $current_user->getId())
                {
                    ...
                    {
                        $echecklist_usertype = 'Job Assignee';
                        $department = $current_user->get('department_id');
                    }
                    else
                    {
                        ...
                        $company = (empty($file_title_info['file_company']) ? $company_id : $file_title_info['file_company']);
                        $department = (empty($file_title_info['file_department']) ? $department_id : $file_title_info['file_department']);
                    }
                }

                //echo "here at print ";
                $query = "SELECT vtiger_echecklist.name as name,vtiger_echecklistcf.cf_8382,vtiger_echecklistcf.echecklistid,vtiger_echecklistcf.types as types,vtiger_echecklistcf.departments as departments,vtiger_echecklistcf.companies as companies FROM `vtiger_echecklistcf`
								INNER JOIN vtiger_echecklist ON 
								vtiger_echecklistcf.echecklistid=vtiger_echecklist.echecklistid
                                INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid=vtiger_echecklist.echecklistid 
								WHERE FIND_IN_SET('$type',vtiger_echecklistcf.types)
                                AND vtiger_crmentity.deleted = 0
								AND FIND_IN_SET('$company',vtiger_echecklistcf.companies) 
								AND FIND_IN_SET('$department',vtiger_echecklistcf.departments) 
								AND FIND_IN_SET('$echecklist_usertype',vtiger_echecklistcf.echecklist_usertype)
								ORDER BY cf_8382 DESC";
                $type = $adb->pquery($query, array());
                $type_count = $adb->num_rows($type);

                $types = array();
                for ($j = 0;$j < $adb->num_rows($type);$j++)
                {
                    $types[$j]['echecklistid'] = $adb->query_result($type, $j, 'echecklistid');
                    $types[$j]['name'] = $adb->query_result($type, $j, 'name');
                    $types[$j]['required_doc'] = $adb->query_result($type, $j, 'cf_8382');
                }
            }
            ...
            $viewer->assign('TYPES_COUNT', $type_count);

            $viewer->assign('JOB_TYPE', $job_type);

            $file_type = @$_SESSION['FILE_TYPE'];
            $required_docs = @$_SESSION['DOCS_REQUIRED'];
           // $required_docs_names = @$_SESSION['DOCS_REQUIRED_NAMES'];

            if ($file_type == 'False')
            {
                $viewer->assign('TYPE', $file_type);
            }
            if ($required_docs == 'False')
            {
                $viewer->assign('DOCS_REQUIRED', $required_docs);
                //$viewer->assign('DOCS_REQUIRED_NAMES', $required_docs_names);
            }
    
            return $viewer->view('RelatedListEDocuments.tpl', $moduleName, 'true');
        }
        
        else if ($relatedModuleName == 'EDocs')
        {
            ...
            if ($assign_count > 0)
            {
                if ($role_id == 'H218' || $role_id == 'H74' || $role_id == 'H67')
                {
                    $query = "SELECT DISTINCT ercds.edocsid,ercds.document_name,vtiger_echecklist.name AS doc_name,ecf.jobid,vtiger_echecklistcf.cf_8382 AS required
                     FROM vtiger_edocsrecords AS ercds
                     INNER JOIN vtiger_edocscf AS ecf ON ecf.jobid = ercds.job_id
                     LEFT JOIN vtiger_echecklist ON vtiger_echecklist.echecklistid = ercds.document_name
                     LEFT JOIN vtiger_echecklistcf ON vtiger_echecklistcf.echecklistid = ercds.document_name 
                ...
                {
                    $docs[$j]['document_name'] = $adb->query_result($doc, $j, 'document_name');
                    $temp_docs[] = $adb->query_result($doc, $j, 'document_name');
                }
            ...
            { 
                if (!(in_array($value['echecklistid'], $temp_docs)))
                {
                    $diff[$key]['echecklist'] = $value['echecklistid'];
            ...
            $coordinator_name = $job_user_info->get('first_name').$job_user_info->get('last_name');
            ...
            $viewer->assign('REMAINING_TYPES', $diff);
            $viewer->assign('REMAINING_TYPES_COUNT', count($diff));

            $viewer->assign('JOB_TYPE', $job_type);

            $viewer->assign('ASSIGNEES', $assignees);
            $viewer->assign('ASSIGNEE_COUNT', $assignee_count);

            $viewer->assign('ASSIGNEE_EDOCS_FETCH', $assignee_edocs_fetch);

            $viewer->assign('ASSIGNEE_EDOCS', $assignee_ercd);
            $viewer->assign('ASSIGNEE_EDOCS_COUNT', $assignee_edocs_count);

            $viewer->assign('COORDINATOR', $coordinator_name);
            $viewer->assign('USERTYPE', $echecklist_usertype);
            ...
            $required_docs = @$_SESSION['DOCS_REQUIRED'];
            $required_docs_names = @$_SESSION['DOCS_REQUIRED_NAMES'];
            $requiredlist =   explode(",",$required_docs_names);
            ...
            {
                $viewer->assign('DOCS_REQUIRED', $required_docs);
                $viewer->assign('DOCS_REQUIRED_NAMES', $requiredlist);
            }


            return $viewer->view('RelatedListEDocs.tpl', $moduleName, 'true');
        }

        else if ($relatedModuleName == 'ProjectCargoCheckList')
        {
            $file_type = @$_SESSION['FILE_TYPE'];
            if ($file_type == 'False')
            {
            ...
            return $viewer->view('RelatedListProjectCargoCheckList.tpl', $moduleName, 'true');
        }
        ...
        elseif ($relatedModuleName == 'Jobexpencereport')
        {
            ...
            if ($count_parent_role > 3)
            {
                if ($current_user->get('roleid') == 'H185')
                {
                    ...
                    $selling_field_invoice[] = array(
                        'name' => 'jobexpencereportid',
                        'key' => true,
                        'index' => 'id',
                        "align" => "center",
                        'hidden' => true,
                    ...
                    $selling_header_invoice[] = 'Tax Invoice No';
                    $selling_field_invoice[] = array(
                        'name' => 'tax_invoice_no',
                        "align" => "center",
                        'width' => '80',
                    ...
                    foreach ($header as $header_info)
                    {
                        ...
                        $selling_field_invoice[] = array(
                            'name' => $RELATED_HEADERNAME,
                            'index' => $RELATED_HEADERNAME,
                            'width' => (in_array($header_info->get('column') , $width_sell_in_arr) ? $width_sell_val[$header_info->get('column') ] : '100') ,
                            'frozen' => $frozen
                        );
                    ...
                    $selling_field_invoice[] = array(
                        ...
                        "edittype" => "select",
                        "editoptions" => array(
                            "value" => "0:Select;accept:Accept;reject:Reject"
                        )
                    );

                    $selling_header_invoice[] = 'Local Invoice #';
                    $selling_field_invoice[] = array(
                        'name' => 'local_invoice_no',
                        "align" => "center",
                        'width' => '120',
                        'sortable' => 'false',
                        'editable' => true,
                    ...
                    $selling_header_invoice[] = 'Tax Invoice #';
                    $selling_field_invoice[] = array(
                        'name' => 'local_tax_invoice_no',
                        "align" => "center",
                        'width' => '120',
                        'sortable' => 'false',
                        'editable' => true,
                    ...
                    $selling_header_invoice[] = 'Choose For Sales Tax Invoice';

                    /*
                    $selling_field_invoice[] = array('name' => 'cf_1248_111', "align" => "center", "editable" => true,
                    "edittype" => 'checkbox', "formatter" => 'checkbox', "formatoptions" =>  array( "disabled" => false ), "editoptions" => array("value" => 'Yes:No'));
                    */
                    $selling_field_invoice[] = array(
                        'name' => 'check_invoice_tax',
                        "align" => "center",
                        'width' => '80'
                    );
                ...
                else if ($current_user->get('roleid') == 'H184')
                {
                    ...
                    foreach ($header as $header_info)
                    {
                        ...
                        $buying_field_payables[] = array(
                            'name' => $RELATED_HEADERNAME,
                            'index' => $RELATED_HEADERNAME,
                            'width' => (in_array($header_info->get('column') , $width_exp_in_arr) ? $width_exp_val[$header_info->get('column') ] : '100') ,
                            'frozen' => $frozen
                        );
                    ...
                    $buying_field_payables[] = array(
                        'name' => 'payables_action',
                        'index' => 'payables_action',
                        "width" => "200",
                        "editable" => true,
                        "edittype" => "select",
                        "editoptions" => array(
                    ...
                    $buying_field_payables[] = array(
                        'name' => 'cf_1975',
                        'index' => 'payables_approval_status',
                        "align" => "center",
                        'width' => '80'
                    );
                ...
                else
                {
                    ...
                    $buying_field[] = array(
                        'name' => 'send_to_head_of_department_for_approval',
                        "align" => "center",
                        "editable" => true,
                        "edittype" => 'checkbox',
                        "formatter" => 'checkbox',
                    ...
                    $buying_field[] = array(
                        'name' => 'head_of_department_approval_status',
                        'index' => 'head_of_department_approval_status',
                        "align" => "center",
                        'width' => '80'
                    );
                    $buying_field[] = array(
                        'name' => 'send_to_payables_and_generate_payment_voucher',
    ...
    function process_expense_payable(Vtiger_Request $request)
    {
        ...
        if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && $_REQUEST['grid'] == 5)
        {
            ...
            foreach ($models as $key => $model)
            {
                ...
                $makpal_currency = array(
                    1
                );
                $makpal_account_type = array(
                    'Bank R',
                    'Cash R'
                );
                $makpal_company = array(
                    '85757',
                    '85772',
                    '85756'
                );
                //$yuliya_om_currency = array(2 , 13, 14, 28);
                $yuliya_om_currency_not = array(
                    1
                );
    ...
    function process_selling(Vtiger_Request $request)
    {
        ...
        if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && $_REQUEST['grid'] == 2)
        {
            ...
            foreach ($models as $key => $model)
            {
                ...
                $col_data['cf_1359'] = str_replace(array(
                    "\r",
                    "\n"
                ) , '', $selling_record->get('cf_1359'));
                $col_data['cf_1361'] = str_replace(array(
                    "\r",
                    "\n"
                ) , '', $selling_record->get('cf_1361'));
                $col_data['cf_1363'] = str_replace(array(
                    "\r",
                    "\n"
                ) , '', $selling_record->get('cf_1363'));
                $col_data['cf_1365'] = str_replace(array(
                    "\r",
                    "\n"
    ...
    function process_selling_invoices(Vtiger_Request $request)
    {
        ...
        if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && $_REQUEST['grid'] == 3)
        {
            ...
            //ARA
            $sarieva_branch = array(
                '85813'
            ); //685 useri
            $sarieva_company = array(
                '85757'
            );
            ...
            foreach ($models as $key => $model)
            {
                ...
                $action_excel = '';
                $invoice_tax = $model->get('invoice_tax');

                if (!in_array($invoice_tax, $invoice_tax_arr) && !empty($invoice_tax))
                {
                    //echo "in action excel";
                    $action_excel = '';
                    $invoice_tax_arr[] = $invoice_tax;
                    //$action .='<a href="invoice_export_pdf.php?record='.$model->getId().'&jobid='.$job_id.'&bill_to='.$selling_record->get('cf_1445').'&invoice_instruction_no='.$model->get('invoice_instruction_no').'"><i title="Generate Invoice Instruction" class="fa fa-print alignMiddle"></i></a>&nbsp;';
                    $action_excel .= '&nbsp;<a href="index.php?module=Jobexpencereport&view=Print&record=' . $model->getId() . '&jobid=' . $job_id . '&invoice_tax=' . $model->get('invoice_tax') . '&expense=GITE" target="_blank" ><i title="Generate Invoice Tax Excel" class="fa fa-table" aria-hidden="true"></i></a>&nbsp;';

                    $action_excel .= '&nbsp;<a href="index.php?module=Jobexpencereport&view=Print&record=' . $model->getId() . '&jobid=' . $job_id . '&invoice_tax=' . $model->get('invoice_tax') . '&expense=GITP" target="_blank" ><i title="Generate Invoice Tax PDF" class="fa fa-sticky-note" aria-hidden="true"></i></a>&nbsp;';

                }
                $col_data['myname'] = $action . $action_excel;
                ...
                $col_data['tax_invoice_no'] = $model->get('invoice_tax'); // by amir
                //$col_data['tax_invoice_no'] = $model->get('cf_invoice_tax');
                if ($selling_record->get('cf_1250') == 'Approved' and strlen($model->get('invoice_tax')) < 1)
                {
                    $col_data['check_invoice_tax'] = "<input type='checkbox' name='check_invoice_tax' data-check_tax='" . urlencode($model->get('invoice_no')) . "' class='check_invoice_tax' value='Yes'>";
                }
                else
                {
                    $col_data['check_invoice_tax'] = "";
                }
                $col_data['coordinator'] = '';

...
class ApprovalRouteHistory_Save_Action extends Vtiger_Save_Action {
	...
	$newApprovalPartyId = $request->get('cf_6788');

	$currentUserModel = Users_Record_Model::getCurrentUserModel();
	$current_user_id = $currentUserModel->get('id');
	$routeapprove = $request->get('routeapprove');
			
	$sql_a = "SELECT  * FROM vtiger_approvalroutehistorycf 
											INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_approvalroutehistorycf.approvalroutehistoryid 
											WHERE vtiger_crmentity.deleted=0 AND vtiger_approvalroutehistorycf.cf_6784 ='".$motiwid."' 
											AND vtiger_approvalroutehistorycf.cf_6788 = '".$current_user_id."' AND cf_6792 = 'inline'"; 
	$result_a = $adb->pquery($sql_a);
	
	$tSequence=$adb->query_result($result_a,0,'cf_6790');	
	$typeofAgreement = $adb->query_result($result_a,0,'cf_6786');	
	$c_approvalroutehistoryid=$adb->query_result($result_a,0,'approvalroutehistoryid');
		
	// If Approve checkbox is disable

	if(empty($routeapprove)){
	
				$request->set('cf_6790', $tSequence);
				$request->set('cf_6792', 'inline');
				if($current_user_id == $LEGAL_HEAD){ $request->set('cf_7118',1); }


				$sql = "SELECT  *
								FROM vtiger_approvalroutehistorycf
								INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_approvalroutehistorycf.approvalroutehistoryid 
								WHERE vtiger_crmentity.deleted=0 AND vtiger_approvalroutehistorycf.cf_6784 ='".$motiwid."' AND cf_6790 >= '".$tSequence."'";
			...
			if(!empty($routeapprove)){
				...
				$sql = "UPDATE vtiger_approvalroutehistorycf SET cf_6792 = 'approved' 
								WHERE vtiger_approvalroutehistorycf.cf_6784 ='".$motiwid."'
								AND vtiger_approvalroutehistorycf.cf_6788 = '".$current_user_id."'";
				$result = $adb->pquery($sql);
				...
				$noofrow = $adb->num_rows($result);
					...
					// if Legal Head is adding approval route, then at end add legal employee
					if($current_user_id == $LEGAL_HEAD){
						...
						$recordModel1->set('assigned_user_id', $current_user_id); // $curid= Assigned User id
						$recordModel1->set('mode', 'create');
						$recordModel1->set("name",'');
						$recordModel1->set('cf_6784', $motiwid);
						$recordModel1->set("cf_6786", $typeofAgreement);
						$recordModel1->set("cf_6788", $newApprovalPartyId);
						$recordModel1->set("cf_6790", $i + 2);
						$recordModel1->set("cf_6792",'pending');		
						$recordModel1->set("cf_6794", $currentDateTime);
						$recordModel1->save();
						// $request->set('record', $sourcerecordid); // NEED TO CHECK ? */
				}
				

				
					// Adding 1 if Timur approved. This is required for Legal head filtering 
					if ($motiwid > 0 && $currentUserModel->get('id') == $LEGAL_HEAD){
						$sqlFilter = "UPDATE `vtiger_motiwcf` SET `cf_7144` = '1' WHERE `motiwid` = ?";
						$resultFilter = $adb->pquery($sqlFilter, array($motiwid));
					}
			...
			if ($motiwid > 0){
				...
				for($w=0; $w<$noofrow ; $w++) {
					$partieNames .= $adb->query_result($row_parties, $w, 'last_name').' '. $adb->query_result($row_parties, $w, 'first_name') .', ';
				}
			
				$sqlOfficer = "UPDATE `vtiger_motiwcf` SET `cf_7158` = '$partieNames' WHERE `motiwid` = $motiwid LIMIT 1";
				$resultOfficer = $adb->pquery($sqlOfficer);
			}


			// Riderect to MOTIW once approval parties saved
			if ($motiwid > 0) {
				$loadUrl = "index.php?module=MOTIW&view=Detail&record=$motiwid&app=MARKETING";
			}

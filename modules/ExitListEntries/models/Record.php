 ...
 */
class ExitListEntries_Record_Model extends Vtiger_Record_Model {
	...
	public function getUserName() {
		global $adb;
			/*
				...
				Fetch request user
			*/
			$queryExitList = $adb->pquery("SELECT vtiger_userlist.name
				FROM `vtiger_crmentityrel`
				LEFT JOIN vtiger_exitlist ON vtiger_exitlist.exitlistid = vtiger_crmentityrel.crmid
				LEFT JOIN vtiger_userlist ON vtiger_userlist.userlistid = vtiger_exitlist.name
				WHERE vtiger_crmentityrel.`relcrmid` = ? AND vtiger_crmentityrel.relmodule = ? AND vtiger_crmentityrel.module = ?",array($this->getId(), 'ExitListEntries', 'ExitList'));
			$resignationId = $adb->query_result($queryExitList, 0, 'name');

...
class EChecklist_Save_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
		try {
			...
			$companies_arr = $request->get('echeck_for_companies');
			$departments_arr = $request->get('echeck_for_departments');
			$types_arr = $request->get('cf_8384');
			$userstype_arr = $request->get('user_type');

			$echeck_for_companies_arr = $request->get('echeck_for_companies');

			$echeck_for_company_arr  = implode(' |##| ', $echeck_for_companies_arr);
			$request->set('echeck_for_companies', $echeck_for_company_arr);
			

			$echeck_for_departments_arr = $request->get('echeck_for_departments');
			$echeck_for_department_arr  = implode(' |##| ', $echeck_for_departments_arr);
			$request->set('echeck_for_departments', $echeck_for_department_arr);
			...
			$companies  = implode(',' , $companies_arr);			
			$departments  = implode(',', $departments_arr);
			$types  = implode(',', $types_arr);

			$usertypes  = implode(',', $userstype_arr);
			

			$query = 'UPDATE vtiger_echecklistcf set companies=? ,departments=? ,types=?,echecklist_usertype=? WHERE echecklistid=?';
            $qparams = array($companies, $departments, $types,$usertypes,$lastid);

		...
		for ($i = 0; $i < $numberOfRecords; ++$i) {
			...
			if ($createRecordExists) {
				...
				else if ($moduleName=="WISPremiumCommodity")
				{
					...
					$pkgid = $fieldData["cf_8302"];
					
					if($pkgid=="")  //if no pkgid , its mean its containerized
					{
						$fieldData["cf_8302"]="457405"; //this may change on production
					}
					else
					{
						$sql = "select * from vtiger_vtiger_specialrange where wis_key='$pkgid'";
						//echo "<br> SQL ".$sql;
						$rs = $db->pquery($sql, array());
						if($db->num_rows($rs) > 0){
							$r_rec = $adb->fetch_array($rs);
							$fpkgid= $r_comdty["specialrangeid"];
							$fieldData["cf_8302"]=$fpkgid;
						}

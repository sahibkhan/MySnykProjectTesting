		...
		try{
			...
			elseif($request->get('cf_5609')=='Opening Balance')
			{
				...
				if($adb->num_rows($duplicate_query)>0){
					throw new DuplicateException('- Same	item already exists. Duplicate Item with code ('.$request->get('cf_5613').') is not allowed.');
				}
				if($qty<=0)
				{
					throw new DuplicateException('- Quantity is missing. Please add valid quantity in "TRX QTY" field.');
				}
				if($unitprice<=0)
				{
					throw new DuplicateException('- Unit Price is missing. Please add valid price in "Price Per Unit" field.');
				}
				include("include/Exchangerate/exchange_rate_class.php");
				//cf_6034 VAT rate
				//cf_6040 currency

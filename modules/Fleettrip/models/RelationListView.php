...
class Fleettrip_RelationListView_Model extends Vtiger_RelationListView_Model {
	...
	public function getEntries($pagingModel) {
		...
		if($relationModuleName=='Jobexpencereport' && $main_module=='Fleettrip')
		{
			$parentRecordModule = $this->getParentRecordModel();
			$parent_record_id =  $parentRecordModule->getId();
			$fleettrip_id 			  = $parent_record_id;
			$sourceModule 		= 'Fleettrip';	
			$fleettrip_info = Vtiger_Record_Model::getInstanceById($fleettrip_id, $sourceModule);
			...
			//$user_query =' AND vtiger_jobexpencereport.owner_id = "'.$fleettrip_info->get('assigned_user_id').'" ' ;
			$expense_type = @$_REQUEST['expense'];
			...
			if($count_parent_role>3)
			{
				...
				else if($expense_type=='Fleettrip' && $view_type=='Print')
				{
					$user_query =' AND vtiger_jobexpencereport.owner_id = "'.$fleettrip_info->get('assigned_user_id').'" ' ;
				}
				else{
					
					if($current_user->get('roleid')=='H190')
					{
						$user_query =' AND vtiger_jobexpencereport.owner_id = "'.$fleettrip_info->get('assigned_user_id').'" ' ;
					}
					else if($fleettrip_info->get('assigned_user_id')!=$current_user->getId())
					{
		...
		if($relationModuleName=='Jobexpencereport' && $main_module=='Fleettrip')
		{
				...
				//GROUP BY vtiger_jobexpencereportcfcf.cf_1479			
		}else{

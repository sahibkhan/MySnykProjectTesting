...
class Procurement_Detail_View extends Vtiger_Detail_View {
	...
	function preProcess(Vtiger_Request $request, $display=true) {
		...
		$cancel_status_checker = 'No';
		if($id=='1663' && $procurement_data->get('proc_order_status')=='Approved')
		{
			$cancel_status_checker = 'Yes';
		}
		...
		//echo $recordModel->get('proc_agreement_no');exit;
		
		$recordStrucure = Vtiger_RecordStructure_Model::getInstanceFromRecordModel($recordModel, Vtiger_RecordStructure_Model::RECORD_STRUCTURE_MODE_DETAIL);
		$summaryInfo = array();
		//echo $recordModel->set->proc_agreement_no('Arif');
    ...
    //$recordModel->set();
		...
		$query = "SELECT vtiger_serviceagreement.name as agreement_no,vtiger_serviceagreementcf.cf_6020 as expiry_date FROM vtiger_serviceagreement 
				  ...
				  WHERE vtiger_serviceagreementcf.cf_6094='".$recordModel->get('proc_supplier')."'  
				
				  AND  vtiger_serviceagreement.name = '".trim($recordModel->get('proc_agreement_no'))."'
				 ";
		$query_rate =  $adb->pquery($query);
		
		if($adb->num_rows($query_rate)>0){ 
			
			$agreement_expiry = $adb->query_result($query_rate, 0, 'expiry_date');
			
			if($agreement_expiry < date('Y-m-d')) 
			{
				$viewer->assign('Agreement_Expired', ' (Agreement Expired)');
		...
		$viewer->assign('unit_price_total', '-');
		...
		$viewer->assign('cancel_status_checker', $cancel_status_checker);

...
class Procurement_Print_View extends Vtiger_Print_View {
	...
	public function print_procurement_request($request){
		...
		$proc_proctypeID =  $procurement_data->get('proc_proctype');
			...
			$approval_exists = 0;
			$creatorUserId = $procurement_data->get('assigned_user_id');
			
			$if_approval_exists = $adb->pquery("SELECT procapproval_person FROM `vtiger_send_approval_list_by_recordid` 
where procapproval_person = $creatorUserId AND deleted=0 limit 1");
$approval_exists = $adb->query_result($if_approval_exists, 0, 'procapproval_person');
			
			/*Ends creator exists check*/
			if($gm_email=='s.khan@globalinklogistics.com' && $approval_exists==0) //add approval entry as Siddique Khan is the GM of request creator
			{
	...
	and vtiger_send_approval.procurement_id='".$recordId."' 
	where proc_id = '".$recordId."' AND procapproval_person='$gm_userid'  AND deleted=0 order by procapproval_sequence  ASC");
	...
	left join vtiger_send_approval on vtiger_send_approval_list_by_recordid.procapproval_person = vtiger_send_approval.who_approve_id 
	and vtiger_send_approval.procurement_id='".$recordId."' 
	where proc_id = '".$recordId."'  AND deleted=0 order by procapproval_sequence  ASC");
		...
		$mpdf->SetHTMLHeader('<table border="0" cellspacing="0" cellpadding="3" width="100%">
    <tbody>
        <tr>
			<td width="50%">
				<table border="0" cellspacing="0" cellpadding="0" width="100%" style="border:0;">
					<tbody>
						<tr>
							<td><strong>Procurement Request Number:</strong></td>
							<td>'.$procurement_data->get('proc_request_no').'</td>
						</tr>
						<tr>
							<td>Request Date:</td>
			...
			<td style="vertical-align:top;">
				<table border="0" cellspacing="0" cellpadding="0" width="100%" style="border:0;">
					<tbody>
						<tr>
							<td align="right"><img width="200" src="include/logo_doc.jpg"></td>
						</tr>
					</tbody>
				</table>
			</td>
...
<hr>');
		...
		$pdf_name = "pdf_docs/".$procurement_data->get('proc_request_no')." - PROCUREMENT".time().".pdf";

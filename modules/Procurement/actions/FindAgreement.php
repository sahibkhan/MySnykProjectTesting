...
class Procurement_FindAgreement_Action extends Vtiger_Save_Action {
	...
	if($customer_id){
			...
			$accounts_info_detail = Vtiger_Record_Model::getInstanceById($customer_id, 'Accounts'); //Account Info
			//echo "<pre>";print_r($accounts_info_detail);exit;
			 ...
			 $query = "SELECT vtiger_serviceagreement.name as agreement_no,vtiger_serviceagreementcf.cf_6020 as expiry_date FROM vtiger_serviceagreement 
													...
													AND  vtiger_crmentity.deleted = 0 AND  vtiger_serviceagreementcf.cf_6068 = 'Administrative'
			...
			<!-- <select name="cf_1445" data-rule-required="true" aria-required="true" class="select2 agreementdropdown" style="width:75%"> -->
				<select required name="proc_agreement_no" class="select2 agreementdropdown" style="width:75%">
			...
			elseif($accounts_info_detail->get('payment_without_agreement')==1)
			{ ?>
				<select name="proc_agreement_no" class="select2 agreementdropdown" style="width:75%">
				<option value="">Select Option</option>
				</select>
				<span class="agreement_error" style="display:block;color:#060;">You can proceed without Agreement for selected supplier</span>
			<?php }
			else{    

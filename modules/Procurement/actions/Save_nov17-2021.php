...
$itemids = $request->get('procurementitemsid');
...
for($a=0;$a<$total_subitems;$a++)
{
	if(!isset($itemids[$a]))
	{
		...
		if(!empty($quantity[$a])){
			...
			"procitem_line_price"=> $localprice[$a],
			"procitem_vat_unit"=> $VAT[$a],
			"procitem_vat_amount"=> $PriceVAT[$a],
			"procitem_proctypeitem_id"=> $expense_type[$a],
			"procitem_currency"=> $local_currency[$a],
			"procitem_total_usd"=> $Total_USD[$a],
			"procitem_gross_finalamount"=> $Total_local_amount[$a],
			"procitem_gross_amount"=> $gross[$a],
			"procitem_gross_local"=> $gross_local[$a],

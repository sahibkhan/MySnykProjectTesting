...
class Procurement_Sendapprovals_Action extends Vtiger_Action_Controller {
	...
	public function process(Vtiger_Request $request) {
		...
		if($page!='recancel')
		{
			$duplicate_check = $adb->pquery("SELECT * FROM `vtiger_send_approval` where procurement_id = '$recordID' and who_approve_id='$user_id' and approval_status='Approved'");
...
$approval_exists = 0;
$if_approval_exists = $adb->pquery("SELECT procapproval_person FROM `vtiger_procurementapprovalcf` inner join  `vtiger_crmentity` on vtiger_crmentity.`crmid` = `vtiger_procurementapprovalcf`.procurementapprovalid where procapproval_person = $currentUserId AND vtiger_crmentity.deleted=0 limit 1");
$approval_exists = $adb->query_result($if_approval_exists, 0, 'procapproval_person'); 
/*Ends creator exists check*/
if($location_status != 'ALA'  && $approval_exists==0){
	$approval_array_id = 1;
	if($gm_id!='412373')
	{
...
$adb->pquery("INSERT INTO `vtiger_send_approval`(`id`, `procurement_id`, `who_approve_id` , `approval_status`, `creator_id`,`creator_date`,`gm_email`,`gm_name`,`next_notification`) VALUES (0, $recordID, $id, 'Pending', $user_id, '$date','$gm_email','$gm_name', '".date('Y-m-d', strtotime($today_date. ' + 3 days'))."')");
$adb->pquery("UPDATE `vtiger_procurementcf` SET `proc_order_status`='Not Approved', proc_current_approval= '$id' WHERE `procurementid` = $recordID"); //set initial status of procurement
...
$adb->pquery("INSERT INTO vtiger_send_approval_list_by_recordid set 
		name = 'GM',
		procapproval_proctype = '".$ProcurementTypeID."',
		procapproval_person = '".$id."',
		procapproval_usd_limit = '0',
		procapproval_sequence = '0',
		proc_id = '".$request->get('recordID')."'
		");
	}
	else
	{
		$date = date('Y-m-d h:i A');
$adb->pquery("INSERT INTO `vtiger_send_approval`(`id`, `procurement_id`, `who_approve_id` , `approval_status`, `creator_id`,`creator_date`,`gm_email`,`gm_name`,`next_notification`) VALUES (0, $recordID, $user_id, 'Pending', $user_id, '$date','','$currentUser_name', '".date('Y-m-d', strtotime($today_date. ' + 3 days'))."')");
$adb->pquery("UPDATE `vtiger_procurementcf` SET `proc_order_status`='Not Approved', proc_current_approval= '$user_id' WHERE `procurementid` = $recordID"); //set initial status of procurement
$approval_information .= '<tr>
				<td>1</td>
...
$adb->pquery("INSERT INTO vtiger_send_approval_list_by_recordid set 
	...
	$receiver_email = $adb->query_result($users_result, 0, 'email1');
	$date = date('Y-m-d h:i A');
	$adb->pquery("INSERT INTO `vtiger_send_approval`(`id`, `procurement_id`, `who_approve_id` , `approval_status`, `creator_id`,`creator_date`,`next_notification`) VALUES (0, $recordID, $who_approve_id, 'Pending', $user_id, '$date', '".date('Y-m-d', strtotime($today_date. ' + 3 days'))."')");
	$adb->pquery("UPDATE `vtiger_procurementcf` SET `proc_order_status`='Not Approved', proc_current_approval= '$who_approve_id' WHERE `procurementid` = $recordID"); //set initial status of procurement
}
...
/*add all approval list for this procurement-request from the ProcurementApproval to the custom table*/
$procurementapproval_list = $adb->pquery("SELECT * FROM `vtiger_procurementapprovalcf` 
inner join vtiger_procurementapproval on vtiger_procurementapprovalcf.procurementapprovalid = vtiger_procurementapproval.procurementapprovalid
inner join  `vtiger_crmentity` on vtiger_crmentity.`crmid` = `vtiger_procurementapproval`.procurementapprovalid 
where procapproval_proctype = $ProcurementTypeID AND vtiger_crmentity.deleted=0 order by procapproval_sequence");
if($adb->num_rows($procurementapproval_list)>0)
{
	for($apl=0; $apl<$adb->num_rows($procurementapproval_list); $apl++) 
	{
		$row_list = $adb->query_result_rowdata($procurementapproval_list, $apl);
		$row_list['procapproval_usd_limit'] = $row_list['procapproval_usd_limit']==''?0:$row_list['procapproval_usd_limit'];
		$adb->pquery("INSERT INTO vtiger_send_approval_list_by_recordid set 
		name = '".$row_list['name']."',
		procapproval_proctype = '".$row_list['procapproval_proctype']."',
		procapproval_usd_limit = '".$row_list['procapproval_usd_limit']."',
		procapproval_person = '".$row_list['procapproval_person']."',
		procapproval_sequence = '".$row_list['procapproval_sequence']."',
		proc_id = '".$request->get('recordID')."'
		");
	}
}
/*end adding approval list*/

	$body_line = 'A new procurement request has been created. Please check details below';
	$order_status = 'Not Approved';
	if($gm_email=='s.khan@globalinklogistics.com' && $approval_exists==0)
	{
	...
	$body .= "<html>
			...
			<table border=\"0\" cellspacing=\"0\" cellpadding=\"3\" width=\"100%\">
				<tr><td style=\"text-align:left;\"><img src=\"{$site_URLinclude}logo_doc.jpg\" width=\"200\"></td></tr>
			</table>
			<hr><p>&nbsp;</p>
	...
	$to = $receiver_email;
	$subject = 'Test Server '.$reference_no.'-'.'Procurement';
	...
	//$mailer->addcc($currentUser_email); 
...
if($page == 'nextapproval'){/////
	...
	$query = "SELECT * FROM vtiger_send_approval_list_by_recordid 
	where deleted=0 AND proc_id = $recordID"; //get all approval authorities related to requested item
		...
		$query .= " AND procapproval_person !=".$currentUserId; //Also exclude current approval person id as it has already been added in next approval list
...
$procurement_approval_result = $adb->pquery("SELECT * FROM `vtiger_send_approval_list_by_recordid` where proc_id = $recordID and procapproval_person = $currentUserId  order by idno  DESC limit 1");
$sequence = $adb->query_result($procurement_approval_result, 0, 'procapproval_sequence'); //current approval sequence
$procurement_max_seq = $adb->pquery("SELECT max(procapproval_sequence) as max_sequence FROM `vtiger_send_approval_list_by_recordid`
 where proc_id = $recordID AND deleted=0 ");
...
$adb->pquery("INSERT INTO `vtiger_send_approval`(`id`, `procurement_id`, `who_approve_id` , `approval_status`, `creator_id`,`next_notification`) VALUES (0, $recordID, $who_approve_id, 'Pending', $user_id, '".date('Y-m-d', strtotime($today_date. ' + 3 days'))."')");
}
...
if($usdamount<$approval_usd_amount_limit && $approval_usd_amount_limit!=''){ //if amount is less than limit then make procurement status approved
	...
	$final_approval = true;
	$adb->pquery("INSERT INTO `vtiger_send_approval`(`id`, `procurement_id`, `who_approve_id` , `approval_status`, `creator_id`, `date_time_of_approval`) VALUES (0, $recordID, $who_approve_id, 'Not Required', $user_id,'$date')");
}
...
if(!$final_approval){
	$adb->pquery("UPDATE `vtiger_procurementcf` SET proc_current_approval= '$who_approve_id' WHERE `procurementid` = $recordID");
}
else
{
	$adb->pquery("UPDATE `vtiger_procurementcf` SET proc_current_approval= '' WHERE `procurementid` = $recordID");
}
$office = $procurement_data->get('proc_location');
$sql_location = $adb->pquery("SELECT cf_1559 FROM `vtiger_locationcf` WHERE `locationid` = '$office'");
$location_name = $adb->fetch_array($sql_location);
...
/*Check if creator exists in approval list*/
$approval_exists = 0;
$if_approval_exists = $adb->pquery("SELECT procapproval_person FROM `vtiger_send_approval_list_by_recordid` 
where procapproval_person = $assigned_user_id AND deleted=0 limit 1");
$approval_exists = $adb->query_result($if_approval_exists, 0, 'procapproval_person');
/*Ends creator exists check*/


	//below make approval authorities list dynamically
	//get all approval authorities from vtiger_send_approval_list_by_recordid for requested item, left join with vtiger_send_approval & get approval status			
	$procurementauthorities_result = $adb->pquery("SELECT * FROM `vtiger_send_approval_list_by_recordid` 
	left join vtiger_send_approval on vtiger_send_approval_list_by_recordid.procapproval_person = vtiger_send_approval.who_approve_id 
	and vtiger_send_approval.procurement_id='".$request->get('recordID')."' 
	where proc_id = '".$request->get('recordID')."'  AND deleted=0 order by procapproval_sequence  ASC");
	if($adb->num_rows($procurementauthorities_result)>0){
		//echo "<pre>"; print_r($procurementauthorities_result); echo "</pre>";
		for($j=0; $j<$adb->num_rows($procurementauthorities_result); $j++) { //loop to build array for approval details  
			//echo "user_id: ".$adb->query_result($procurementauthorities_result, $j, 'procapproval_person')." sequence: ".$adb->query_result($procurementauthorities_result, $j, 'procapproval_sequence')." usd_limit: ".$adb->query_result($procurementauthorities_result, $j, 'procapproval_usd_limit')." <br>";
	...
	$body .= "<html>
			...
			<body> 
			<table border=\"0\" cellspacing=\"0\" cellpadding=\"3\" width=\"100%\">
<tr><td style=\"text-align:left;\"><img src=\"{$site_URL}include/logo_doc.jpg\" width=\"200\"></td></tr>
			</table>
			<hr><p>&nbsp;</p>
			<p>Dear $receiver_name,</p>
			<p>".$approval_notice."</p>
...
$adb->pquery("UPDATE `vtiger_procurementcf` SET `proc_order_status`='Cancelled',proc_current_approval= '' WHERE `procurementid` = $recordID");
$adb->pquery("UPDATE `vtiger_send_approval` SET approval_status='Cancelled', date_time_of_approval = '$date',reject_reason='".addslashes($reject_reason)."' WHERE `procurement_id` = $recordID and who_approve_id=$user_id");
...
/*Ends creator exists check*/
	...
	if($adb->num_rows($procurementauthorities_result)>0){
		...
		for($j=0; $j<$adb->num_rows($procurementauthorities_result); $j++) { //loop to build array for approval details  
			...
			$approval_information .= '<tr>
											...
											<td>'.$approval_user_name.'</td>
											<td>'.$row['name'].'</td>
											<td>'.$row['approval_status'].(trim($row['reject_reason'])!=""?("<br><br> <u>Reason:</u> ".$row['reject_reason']):"").'</td>
											<td>'.$row['date_time_of_approval'].'</td>
										</tr>';
	...
	$subject = 'Test Server '.$reference_no.'-'.'Procurement has been cancelled';
...
$adb->pquery("UPDATE `vtiger_procurementcf` SET `proc_order_status`='Cancelled',proc_current_approval= '' WHERE `procurementid` = $recordID");
$adb->pquery("UPDATE `vtiger_send_approval` SET approval_status='Cancelled', date_time_of_approval = '$date',reject_reason='".addslashes($reject_reason)."' WHERE `procurement_id` = $recordID and who_approve_id=$user_id");
/*get current approval id from send_approval table , reset old approval status*/
$sendapprovalid_result = $adb->pquery("SELECT * FROM `vtiger_send_approval` where procurement_id = $recordID and who_approve_id=$user_id");
$user_approval_id = $adb->query_result($sendapprovalid_result, 0, 'id');
$adb->pquery("UPDATE `vtiger_send_approval` SET approval_status='', date_time_of_approval = '' WHERE `procurement_id` = $recordID and id>$user_approval_id");
/*ends reset old approval status*/
$approval_information = '<tr>
				<td colspan="5"><h3 style="background-color:#f47824;color:white; margin-top: 0; margin-bottom: 0; font-weight: 500; vertical-align: center; font-size: 20px; padding: 3px;" align="left">Approval Information</h3></td>
			</tr>
			<tr>
	...
	$sql_userprofile = $adb->pquery("SELECT `userlistid`, `cf_3355`, `cf_3385`, `cf_3421`
									FROM `vtiger_userlistcf`
									INNER JOIN vtiger_crmentity ON vtiger_userlistcf.userlistid = vtiger_crmentity.crmid
									WHERE vtiger_crmentity.deleted=0 AND `cf_3355` = '$creator_email'");
	$userprofile = $adb->fetch_array($sql_userprofile);
	$gm_id = $userprofile['cf_3385'];
	$sql_gm_email = $adb->pquery("SELECT uf.cf_3355 as email, u.name as name FROM `vtiger_userlistcf` as uf inner join `vtiger_userlist` as u on uf.userlistid = u.userlistid  WHERE uf.`userlistid` = '$gm_id'");
	$gm = $adb->fetch_array($sql_gm_email);
...
where procapproval_person = $assigned_user_id AND deleted=0 limit 1");
		...
		<table border="1" cellspacing="0" cellpadding="3" width="100%">
			<tbody>
				<tr>
					...
					<td style="background-color:#bbb; font-weight:bold">Procurement Type</td>
					<td style="background-color:#bbb; font-weight:bold">Creator Name</td>
					<td style="background-color:#bbb; font-weight:bold">Title</td>
					<td style="background-color:#bbb; font-weight:bold">Location</td>
					<td style="background-color:#bbb; font-weight:bold">DPRT</td>
					'.$mode_content1.'
					<td style="background-color:#bbb; font-weight:bold">Creation Date</td>
				...
				<tr>
					<td><a href="'.$link.'">'.$reference_no.'</a></td>
					<td>'.$procurement_type.'</td>
					<td>'.$creater_name.'</td>
					<td>'.$procurement_title.'</td>
					<td>'.$procurement_data->getDisplayValue('proc_location').'</td>
					<td>'.$procurement_data->getDisplayValue('proc_department').'</td>
					'.$mode_content2.'
					<td>'.$procurement_data->get('createdtime').'</td>
	...
	$mailer->Subject = $subject;
	$mailer->Body = $body;
	//$mailer->AddAddress($creator_email); $mailer->addcc($currentUser_email);
	$mailer->AddAddress('m.arif@globalinklogistics.com'); //$mailer->AddAddress('s.mehtab@globalinklogistics.com'); 
	//$mailer->addcc('m.arif@globalinklogistics.com'); $mailer->addcc('s.mehtab@globalinklogistics.com'); 
	$mailer->Send(true);
} /// recancel if close////

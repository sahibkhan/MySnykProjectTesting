...
class Procurement_Save_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
		...
		$db=$adb;
		try {
		//echo "Initial Request : <pre>"; print_r($request); echo "</pre>";
		
		/*Add duplicate removal filter for PM items for Own-Stock*/
		$ProcurementType_data = Vtiger_Record_Model::getInstanceById($request->get('proc_proctype'), 'ProcurementTypes');
		$ProcurementType_code = $ProcurementType_data->get('proctype_shortcode');
		if($request->get('proc_purchase_type_pm')=='Own-Stock' && $ProcurementType_code=='PM')
		{
			$items_array = $request->get('childvales'); //basic items array
			$duplicate_keys = array_diff_assoc($items_array, array_unique( $items_array )); //trace duplicate keys
			$filtered_arr = Array_unique($items_array); //remove duplicate entries
			$rearranged_items_array = array_values($filtered_arr); // re-arrange array keys
			$duplicate_count = count($duplicate_keys);
			if($duplicate_count > 0) 
			{
				$description_array = $request->get('description');
				$qty1_array = $request->get('qty1');
				$psc1_array = $request->get('psc1');
				$localprice_array = $request->get('localprice');
				$VAT_array = $request->get('VAT');
				$PriceVAT_array = $request->get('PriceVAT');
				$gross_array = $request->get('gross');
				$currency_array = $request->get('currency');
				$gross_local_array = $request->get('gross_local');
				$Total_local_currency_array = $request->get('Total_local_currency');
				$Total_USD_array = $request->get('Total_USD');
				$current_qty_array = $request->get('current_qty');
				$last_qty_array = $request->get('last_qty');
				$last_purchase_price_array = $request->get('last_purchase_price');
				$avg_consumption_array = $request->get('avg_consumption');
				
				foreach($duplicate_keys as $duplicate_key => $val)
				{
					unset($description_array[$duplicate_key]);
					unset($qty1_array[$duplicate_key]);
					unset($psc1_array[$duplicate_key]);
					unset($localprice_array[$duplicate_key]);
					unset($VAT_array[$duplicate_key]);
					unset($PriceVAT_array[$duplicate_key]);
					unset($gross_array[$duplicate_key]);
					unset($currency_array[$duplicate_key]);
					unset($gross_local_array[$duplicate_key]);
					unset($Total_local_currency_array[$duplicate_key]);
					unset($Total_USD_array[$duplicate_key]);
					unset($current_qty_array[$duplicate_key]);
					unset($last_qty_array[$duplicate_key]);
					unset($last_purchase_price_array[$duplicate_key]);
					unset($avg_consumption_array[$duplicate_key]);
				}

				$request->set('childvales',$rearranged_items_array);
				$request->set('description',array_values($description_array));
				$request->set('qty1',array_values($qty1_array));
				$request->set('psc1',array_values($psc1_array));
				$request->set('localprice',array_values($localprice_array));
				$request->set('VAT',array_values($VAT_array));
				$request->set('PriceVAT',array_values($PriceVAT_array));
				$request->set('gross',array_values($gross_array));
				$request->set('currency',array_values($currency_array));
				$request->set('gross_local',array_values($gross_local_array));
				$request->set('Total_local_currency',array_values($Total_local_currency_array));
				$request->set('Total_USD',array_values($Total_USD_array));
				$request->set('current_qty',array_values($current_qty_array));
				$request->set('last_qty',array_values($last_qty_array));
				$request->set('last_purchase_price',array_values($last_purchase_price_array));
				$request->set('avg_consumption',array_values($avg_consumption_array));
			}
		}
		/*Duplicate filter ends*/
		//echo "<br>Filtered Request : <pre>"; print_r($request); echo "</pre>";exit;
...
if($account_id!='')
{
	$accounts_info_detail = Vtiger_Record_Model::getInstanceById($account_id, 'Accounts'); //Account Info
	$request->set('proc_bin_number',$accounts_info_detail->get('cf_2397')); //BIN Number 
	$request->set('proc_company_status',$accounts_info_detail->get('cf_2403')); //Company Status
	$request->set('proc_supplier_location',$accounts_info_detail->get('bill_city'));//Supplier Location 
}
$cleanedAgreementNo = trim($request->get('proc_agreement_no'));

if($account_id=='' || ($request->get('proc_agreement_no')=='' && $accounts_info_detail->get('payment_without_agreement')==0))
{
	throw new IRRELEVANT_AGREEMENT_NO('-	Agreement number is missing OR not relevant to the selected supplier  -');
}
...
$updated_total_usd = '';
	for($i=0;$i<count($Total_local_amounts)-1;$i++){
		...
		$request->set('proc_total_amount',$updated_total_usd);
			...
			if(count($quantity)-1<1 || count($quantity)-1==0 || $request->get('childvales')[0]=='Select Option'){ //$subitems_array[$a]!='Select Option'
...
for($a=0;$a<$total_subitems;$a++) 
{
	...
	if(!empty($quantity[$a]) && !empty($psc1[$a]) && $subitems_array[$a]!='Select Option') 
	{

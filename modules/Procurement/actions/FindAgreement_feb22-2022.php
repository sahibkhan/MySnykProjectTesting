...
class Procurement_FindAgreement_Action extends Vtiger_Save_Action {
	...
	if($customer_id){
			...
			if($record_id!=''){
			 ...
			 $query = "SELECT vtiger_serviceagreement.name as agreement_no FROM vtiger_serviceagreement 
			...
			for($i=0;$i<$adb->num_rows($query_rate);$i++){   /// for loop start from here ////
				...
				$serviceagreementname = $adb->query_result($query_rate, $i, 'agreement_no');
			...
			?>
				...
				<option value="<?php echo $serviceagreementname;?>" <?php if($agreementNo){if($serviceagreementname==$agreementNo){?> selected <?php }}?>><?php echo $serviceagreementname;?></option>

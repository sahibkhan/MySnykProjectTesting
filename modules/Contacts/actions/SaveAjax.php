...
class Contacts_SaveAjax_Action extends Vtiger_SaveAjax_Action {
  ...
  public function process(Vtiger_Request $request) {
    ...
    try {
      ...
      $moduleName = $request->getModule(); // Contact
      $recordId = $recordModel->getId(); // Contact ID
      $accountId = $request->get('account_id');

      // echo "<pre>"; print_r($request); exit;
      
      // $dataJson = json_encode($request);
      // $adb->pquery("UPDATE `a_test` SET `query` = '$accountId' WHERE `userlistid` = 993812");

      if ($moduleName == 'Contacts' && !empty($recordId) && !empty($accountId)){
        $queryContacts = "SELECT crmid 
                          FROM `vtiger_crmentityrel`
                          WHERE crmid = ? AND module = ? AND relcrmid = ? AND relmodule = ?";
        $resultContacts = $adb->pquery($queryContacts, array($accountId, 'Accounts', $recordId, 'Contacts'));
        $nOfRecords = $adb->num_rows($resultContacts);

        if ($nOfRecords == 0){
          $adb->pquery("INSERT INTO vtiger_crmentityrel (crmid, module, relcrmid, relmodule) VALUES ($accountId, 'Accounts', $recordId, 'Contacts')");
        }
      }

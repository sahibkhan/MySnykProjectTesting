...
class Project_Save_Action extends Vtiger_Save_Action {
	...
	public function saveRecord($request) {
		...
		$startdate = $recordModel->get('startdate');
		$deadline = $recordModel->get('targetenddate');
		$actualenddate = $recordModel->get('actualenddate');
		$revisedDeadlineReasion = $recordModel->get('cf_7858');
...
//  echo "<pre>"; print_r($recordModel);	exit;
  	...
  	$creatorUserModel = Vtiger_Record_Model::getInstanceById($recordModel->get('assigned_user_id'), 'Users');
		...
		$departments = $request->get('departments');
 		...
 		//  echo "<pre>"; print_r($taskDates);	exit;
		...
		$isRecordNew = false;
		 ...
		 for ($i=1; $i<=$nOfTasks; $i++){
			...
			if (!empty($taskName) && (int)$recordId == 0){
				//  echo "<pre>"; print_r("insert $recordId");
				...
				$projectTasksArr[] = array("taskId" => $relatedRecordId1, "taskName" => $taskName,
				...
				$projectTasksArr[] = array("taskId" => $recordId, "taskName" => $taskName, 
																	 "taskAssignedTo" => $taskAssignedTo,
																	 "taskAssignedEmail" => $taskAssignedEmail,
																	 "taskStatus" => $taskStatus,
																	 "taskComment" => $taskComment, "taskDate" => $taskDate);

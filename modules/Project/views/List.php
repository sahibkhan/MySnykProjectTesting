...
class Project_List_View extends Vtiger_List_View {
	...
	function preProcess(Vtiger_Request $request, $display=true) {		
		...
		$CURRENT_USER_OFFICE = $currentUser->get('location_id');
		$CURRENT_USER_DEPARTMENT = $currentUser->get('department_id');

		$CURRENT_USER_ID = $currentUser->get('id');
		$CURRENT_USER_EMAIL = strtolower(trim($currentUser->get('email1')));
		

		// CEO, CFO, IT Manager, Senior Dev.
		$ALLOWED_USERS_BY_ID = array(13, 277, 60, 374, 9, 1457);

		$ALLOWED_USERS_FOR_DEVTEAM = array(374, 9, 1635, 13, 60, 277);

		$isReportAccessable = false;		
		if (in_array($CURRENT_USER_ID, $ALLOWED_USERS_BY_ID)){
			$isReportAccessable = true;
		}		
		$queryUser = $adb->pquery("SELECT cf_3385
																FROM vtiger_userlistcf
																WHERE cf_3355 = ?", array($CURRENT_USER_EMAIL));
		$employeeHeadID = $adb->query_result($queryUser, 0, 'cf_3385');
		
		if ($employeeHeadID == 412373) $isReportAccessable = true;
		$viewer->assign('IS_REPORT_ACCESSABLE', $isReportAccessable);

		
		// GET Location List
		if (in_array($CURRENT_USER_ID, $ALLOWED_USERS_BY_ID)){
			$queryLocations = "SELECT * FROM vtiger_location";
		} else {
			$queryLocations = "SELECT * FROM vtiger_location WHERE locationid = ".$CURRENT_USER_OFFICE;
		}


		$SHOW_DEVTEAM_FIELD = false;
		if (in_array($CURRENT_USER_ID, $ALLOWED_USERS_FOR_DEVTEAM)){
			$SHOW_DEVTEAM_FIELD = true;
		}


		$locationResult = $adb->pquery($queryLocations);
		$numRows = $adb->num_rows($locationResult);
		$locationList = array();

		for ($i = 0; $i < $numRows; $i++){
			$locationId = $adb->query_result($locationResult, $i, 'locationid');
			$locationName = $adb->query_result($locationResult, $i, 'name');
			$locationList[] = array('id' => $locationId, 'name' => $locationName);
		}

		// GET Department List

		// ALA and Allowed users
		if ($CURRENT_USER_OFFICE == 85805 && (in_array($CURRENT_USER_ID, $ALLOWED_USERS_BY_ID))) {

			$queryDepartment = "SELECT * FROM vtiger_departmentcf ";

		} else if ($CURRENT_USER_OFFICE == 85805 && (!in_array($CURRENT_USER_ID, $ALLOWED_USERS_BY_ID))) {
			// For ALA GM's
			$queryDepartment = "SELECT * FROM vtiger_departmentcf WHERE departmentid = ".$CURRENT_USER_DEPARTMENT;

		} else if ($CURRENT_USER_OFFICE <> 85805){
			// For Branch BM's
			$queryDepartment = "SELECT * FROM vtiger_departmentcf ";
		}
		
		$departmentResult = $adb->pquery($queryDepartment);
		$numRows = $adb->num_rows($departmentResult);
		$departmentList = array();

		for ($i = 0; $i < $numRows; $i++){
			$departmentId = $adb->query_result($departmentResult, $i, 'departmentid');
			$departmentName = $adb->query_result($departmentResult, $i, 'cf_1542');
			$departmentList[] = array('id' => $departmentId, 'name' => $departmentName);
		}



		$viewer->assign('CURRENT_USER_OFFICE', $CURRENT_USER_OFFICE);
		// $viewer->assign('CURRENT_USER_DEPARTMENT', $CURRENT_USER_DEPARTMENT);

		$viewer->assign('GLK_DEPARTMENT', $departmentList);
		$viewer->assign('GLK_OFFICE', $locationList);
		$viewer->assign('SHOW_DEVTEAM_FIELD', $SHOW_DEVTEAM_FIELD);

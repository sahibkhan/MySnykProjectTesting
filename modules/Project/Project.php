...
class Project extends Vtiger_CRMEntity {
 */
...
class Project extends CRMEntity {
	...
	function save_module($module) {
		$recordId = $this->id;
		$this->insertIntoAttachment($this->id,'Project');

		//Handling for invitees
		$selected_users_string = $_REQUEST['selectedusers'];
		$this->insertIntoInviteeTable($module,$selected_users_string);


/* 		$recordProject = Vtiger_Record_Model::getInstanceById($recordId, 'Project');
		$recordCreatorId = $recordProject->get('assigned_user_id');
		$projectName = $recordProject->get('projectname');
	...
	function getInviteeUsers($crmID){
		global $adb;
		$queryUsers = $adb->pquery("SELECT vtiger_users.email1 
																FROM `vtiger_invitees` 
																INNER JOIN vtiger_users ON vtiger_users.id = vtiger_invitees.inviteeid 
																WHERE vtiger_invitees.activityid = ?", array($crmID));
	...
	function getInvolvedUserEmail($id){
		global $adb;
		$queryUsers = $adb->pquery("SELECT email1 
																FROM `vtiger_users` 
																WHERE id = ?", array($id));
		$email = trim(strtolower($adb->query_result($queryUsers, 0, 'email1')));
		return $email;
	}


/* 	function isFileUploaded($crmid){
		global $adb;
		$year = Date('Y');
		$month = Date('m');
		$day = Date('d');
		$queryAttachment = $adb->pquery("SELECT crmid
																			FROM `vtiger_crmentity` 
																			WHERE setype = ? AND (YEAR(createdtime) = ? AND MONTH(createdtime) = ? AND DAY(createdtime) = ?)
																			AND `crmid` = ?", array('Project Attachment', $year, $month, $day, $crmid));
		$n = $adb->num_rows($queryAttachment);
...
//isRecordNew
	...
	// Email update on project assignment 
	public function taskEmailHandler($details){
		...
		$projectCreatorEmail = $details['projectCreatorEmail']; //
		$taskName = $details['taskName']; 
		$taskStatus = $details['taskStatus'];
		$taskDeadline = $details['taskDeadline'];

		$revisedDeadline = $details['revisedDeadline'];
		$revisedDeadlineReason = $details['revisedDeadlineReason'];
		$taskComments = $details['taskComments'];
		$cc = $details['involvedUserEmails'];

 
		$from = $fromEmail;
		$to = $projectCreatorEmail;

		$message_status .= "<tr><td colspan=2> Dear $projectCreator, </td></tr>";
		$message_status .= "<tr><td colspan=2> Please be informed that task <b>$taskName</b> has been updated </td></tr>";
		$message_status .= "<tr><td> Status </td> <td> $taskStatus </td> </tr>";
		$message_status .= "<tr><td> Deadline </td> <td> $taskDeadline </td> </tr>";
		if ($revisedDeadline){
			$message_status .= "<tr><td> Revised Deadline </td> <td> $revisedDeadline </td> </tr>";
			$message_status .= "<tr><td> Reason </td> <td> $revisedDeadlineReason </td> </tr>";
		}
		$message_status .= "<tr><td> Task Comments </td> <td> $taskComments </td> </tr>";
		// $message_status .= "<tr><td> CC </td> <td> $involvedUserEmails </td> </tr>";
		...
		$link .= "https://gems.globalink.net/index.php?module=Project&view=Detail&record=$recordId";
		...
		$r = send_mail('ProjectMilestone', $to, $fromName, $from, $subject, $body, $cc,'','','','',true);
...
/** Function to insert values in vtiger_invitees table for the specified module,tablename ,invitees_array
    ...
    //function save_related_module($module, $crmid, $with_module, $with_crmid) {    }
		...
		function calculateProjectMilestone($pid) {

			global $adb;
			$cf = 7838;
			$getListMS = $adb->pquery('SELECT * FROM vtiger_projectmilestone pm INNER JOIN vtiger_crmentity crm ON crm.crmid = pm.projectmilestoneid WHERE crm.deleted = 0 AND pm.projectid = ?', [$pid]);
			$getMilestoneCount = $adb->num_rows($getListMS);
			$progressCount = 0;
	
			if($getMilestoneCount > 0) {
	
				for($i=0; $i<$getMilestoneCount; $i++) {
					$getCrmId = $adb->query_result($getListMS, $i, 'projectmilestoneid');
					$getProgressCount = $adb->pquery('SELECT COUNT(*) FROM vtiger_projectmilestonecf WHERE projectmilestoneid = ? AND cf_'.$cf.' = ?', [$getCrmId, 'Completed']);
	
					$checkDeleted = $adb->pquery('SELECT deleted FROM vtiger_crmentity WHERE crmid = ?', [$getCrmId]);
					if($getProgressCount->fields[0] > 0 && $checkDeleted->fields[0] == 0) {
						$progressCount++;
					}
				}
	
				$percentage = ceil(100 * $progressCount / $getMilestoneCount);
				$roundedPercentage = 5 * round($percentage / 5).'%';
	
				$updateProgress = $adb->pquery('UPDATE vtiger_project SET progress = ? WHERE projectid = ?', [$roundedPercentage, $pid]);
    ...
    function delete_related_module($module, $crmid, $with_module, $with_crmid) {
        ...
        foreach($with_crmid as $relcrmid) {
            ...
            $child->save($destinationModule,$relcrmid);
				...
				// $this->calculateProjectMilestone($crmid);

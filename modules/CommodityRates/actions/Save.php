...
class CommodityRates_Save_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
		try {
			$comodity_type_id = $request->get('cf_3573');
			$special_range_id = $request->get('cf_3575');
			$insurance_type = $request->get('cf_7146');
			$recordId = $request->get('record');

			//For New WIS Rates
			$policy_cover_id = $request->get('cf_7848'); // Policy cover
			$agreement_date = $request->get('cf_7850'); // agreement date
			$agreement_date = date('Y-m-d', strtotime($agreement_date));
			$agreement_validity = $request->get('cf_7852'); // agreement validity
			$agreement_validity  = date('Y-m-d', strtotime($agreement_validity));
			
			$db = PearDatabase::getInstance();
			//16.10.2020::Mehtab:: Duplicate check with insurance type
			//$query = "SELECT 1 FROM vtiger_commodityratescf WHERE cf_3573=? AND cf_3575=? ";
			$query = "SELECT 1 FROM vtiger_commodityratescf WHERE cf_3573=? AND cf_3575=? AND cf_7146=? AND cf_7848=? AND cf_7850=? AND cf_7852=?";
			$params = array($comodity_type_id, $special_range_id, $insurance_type, $policy_cover_id, $agreement_date, $agreement_validity);

			
			if ($recordId) {
				$query .= " AND commodityratesid !=?";
				array_push($params, $recordId);
			}
				
			$result = $db->pquery($query, $params);
			if ($db->num_rows($result)) {
				throw new DuplicateException('JS_DUPLIACATE_ENTRIES_FOUND_FOR_THE_VALUE');
			}

...
class CompanyAssets_Save_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
		try {
			$employee_id = $request->get('cf_6202');

			$recordModel = $this->saveRecord($request);
			if ($request->get('returntab_label')){
				$loadUrl = 'index.php?'.$request->getReturnURL();
			} else if($request->get('relationOperation') || !empty($employee_id) ) {
				//$parentModuleName = $request->get('sourceModule');
				$parentModuleName = 'UserList';
				//$parentRecordId = $request->get('sourceRecord');
				$parentRecordId = $employee_id;
	...
	public function saveRecord($request) {
		...
		$recordModel->save();
		if($request->get('relationOperation')  || !empty($employee_id)) {
			//$parentModuleName = $request->get('sourceModule');
			$parentModuleName = 'UserList';

 ...
 */   
	...
	public function get_inquiry_history($record,$history_field){
		...
		if ($history_field != ''){
				...
				$history = $this->arrange_muptiple_users1($details['postvalue'],1);
				} 
				else
				// Route: cf_717
				...
				$history = $this->arrange_route_block_v2($details);
		...
		if ($history_field == ''){
				...
				$array_history = array();
/* 				echo "SELECT * FROM `vtiger_modtracker_detail` where `id`=$max_id and (`fieldname` not in ($exclude_field))";
				exit;
				 */
				$i = 0; 		

...
class Fleet_RelatedList_View extends Vtiger_RelatedList_View {
	function process(Vtiger_Request $request) {
		...
		$viewer->assign('FLEET_RECORD_ID', $parentId);
		...
		if($relatedModuleName =='Jobexpencereport')
		{
			...
			if($current_user->get('is_admin')!='on')
			{
				...
				if($_REQUEST['module']=='Fleet' && $count_parent_role==0)
				{
 ...
 left join vtiger_jobexpencereportcf as vtiger_jobexpencereportcf on vtiger_jobexpencereportcf.jobexpencereportid=vtiger_jobexpencereport.jobexpencereportid 
			...
			//For Bakhytgul access, right now her role is GM. Later we can change.
				if($current_user->get('roleid')=='H3' || $count_parent_role <= 3 || $current_user->get('roleid')=='H2')
				{
					$jrer_sql_expense .=' AND vtiger_jobexpencereport.owner_id = "'.$job_info_detail->get('assigned_user_id').'" ' ;
				}
				else if($job_info_detail->get('assigned_user_id')!=$current_user->getId())
				{
					$jrer_sql_expense .=' AND vtiger_jobexpencereport.owner_id = "'.$current_user->getId().'" AND vtiger_jobexpencereport.user_id = "'.$current_user->getId().'" ' ;
				}
				else
				{
					$jrer_sql_expense .=' AND vtiger_jobexpencereport.owner_id = "'.$current_user->getId().'" ' ;
				}
 ...
 left join vtiger_jobexpencereportcf as vtiger_jobexpencereportcf on vtiger_jobexpencereportcf.jobexpencereportid=vtiger_jobexpencereport.jobexpencereportid 
			...
			//{
				if($current_user->get('roleid')=='H3' || $count_parent_role <= 3 || $current_user->get('roleid')=='H2')
				{
					$jrer_selling_sum_sql .=' AND vtiger_jobexpencereport.owner_id = "'.$job_info_detail->get('assigned_user_id').'" ' ;
				}
				else if($job_info_detail->get('assigned_user_id')!=$current_user->getId())
				{
				 ...
				 foreach($profit_share as $key => $p_share)
				 {
					 ...
					 if(!in_array($dept_branch, $profit_share_check))
					 {
						...
						if($current_user->get('roleid')=='H3'  || $count_parent_role <= 3 || $current_user->get('roleid')=='H2')
						{
							...
							$params_buy_local = array($parentId, $p_share['cf_1477_location_id'], $p_share['cf_1479_department_id'], $current_user->getId());//cf_2001=truck id
						}

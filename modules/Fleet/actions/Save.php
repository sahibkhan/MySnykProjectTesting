...
class Fleet_Save_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
		...
		*/
		...
		if($request->get('isDuplicate') == true)
		{
			...
			$rs_q = mysql_query("SELECT * FROM `vtiger_crmentityrel` where relcrmid='".$recordId."' AND relmodule='Fleet' AND module='Job' ");
			$row_j = mysql_fetch_array($rs_q);
			$job_id = $row_j['crmid'];
			$request->set('sourceRecord', $job_id);
		...
		if(empty($recordId) || $request->get('isDuplicate') == true) 
		{
			...
			*/
			...
			//Fleet Expense in Selling for every truck
			$fleet_selling = array(
								  's_job_charges_id' => '85900', 
								  's_expected_sell_local_currency_net' => 0, 
								  's_bill_to_id' => '',
								  'label'		=> 'Selling Fleet Expense',
								  'parentmodule' => 'Fleet',
								  'truck_id'	 => $truck_id,
								  );
			$this->saveFleetSelling($recordModel, $request, $fleet_selling=array());
			//
			...
			*/
			...
			if($job_info_detail->get('assigned_user_id')!=$current_user->getId())
			{
				...
				$job_id = $job_id;		 
				$params_fleet_jrer = array($job_id, $truck_id, $fleet_id);
			...
			$adb_2 = PearDatabase::getInstance();
			...
			$check_params_rel = array($recordId, 85896); //(fleet_id, b_job_charges_id=fuel)
			$result_fuel_expense_before = $adb_2->pquery($query_fuel_expense_before, $check_params_rel);
			$row_fuel_expense_before = $adb_2->fetch_array($result_fuel_expense_before);
			
			$adb_3 = PearDatabase::getInstance();
			...
			$result_fuel_expense_after = $adb_3->pquery($query_fuel_expense_after, $check_params_after);
	...
	public function saveFleetSelling($recordModel, $request, $fleet_selling=array())
	{
		$adb = PearDatabase::getInstance();
		$current_user = Users_Record_Model::getCurrentUserModel();
		...
		$adb->pquery("INSERT INTO vtiger_crmentity(crmid, smcreatorid, smownerid,
			...
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
			array($current_id, $current_user->getId(), $current_user->getId(), 'Jobexpencereport', 'NULL', $date_var, $date_var, 1, 0, $fleet_selling['label']));
		...
		$jobexpencereportcf_insert_query = "INSERT INTO vtiger_jobexpencereportcf(jobexpencereportid, cf_1479, cf_1477, cf_1455, cf_1242, cf_1457, cf_2195) VALUES(?, ?, ?, ?, ?, ?, ?)";
		$params_jobexpencereportcf = array($current_id, $current_user->get('department_id'), $current_user->get('location_id'), $fleet_selling['s_job_charges_id'],  
										   $fleet_selling['s_expected_sell_local_currency_net'], 'Selling', $fleet_selling['truck_id']);
	...
	public function saveFleetExpense($recordModel, $request, $fleet_expence=array())
	{
		...
		//INSERT data in jobexpencereport module from Fleet
		$adb_e = PearDatabase::getInstance();
		$jobexpencereport_insert_query = "INSERT INTO vtiger_jobexpencereport(jobexpencereportid, name, user_id, owner_id, job_id, fleet_trip_id) VALUES(?,?,?,?,?,?)";
		$params_jobexpencereport= array($current_id, $recordModel->getId(), $current_user->getId(), $current_user->getId(), $source_id, $job_fleet_id);			
		$adb_e->pquery($jobexpencereport_insert_query, $params_jobexpencereport);			
		$jobexpencereportid = $adb_e->getLastInsertID();
	...
	public function saveFleetExpense_MainJRER($recordModel, $request, $fleet_expence=array(), $job_info_detail)
	{
		...
		$adb_e = PearDatabase::getInstance();
		$jobexpencereport_insert_query = "INSERT INTO vtiger_jobexpencereport(jobexpencereportid, name, user_id, owner_id, job_id, fleet_trip_id) VALUES(?,?,?,?,?, ?)";
		$params_jobexpencereport= array($current_id, $recordModel->getId(), $current_user->getId(), $job_info_detail->get('assigned_user_id'), $source_id, $job_fleet_id);			
		$adb_e->pquery($jobexpencereport_insert_query, $params_jobexpencereport);			
		$jobexpencereportid = $adb_e->getLastInsertID();
		...
		//cf_1351 = Expected Buy (Local Currency NET)
		$adb_ecf = PearDatabase::getInstance();
		$jobexpencereportcf_insert_query = "INSERT INTO vtiger_jobexpencereportcf(jobexpencereportid, cf_1479, cf_1477, cf_1453, cf_1214, cf_1351, cf_1457, cf_2193, cf_2195, cf_1337, cf_1343, cf_1347, cf_1349, cf_1353) 
											VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		$params_jobexpencereportcf = array($current_id, $current_user->get('department_id'), $current_user->get('location_id'), $fleet_expence['b_job_charges_id'], $fleet_expence['b_type_id'], 
										   $fleet_expence['b_expected_buy_local_currency_net'], 'Expence', 'Yes', $fleet_expence['truck_id'], 0, 0, 0, 0, 0);
		$adb_ecf->pquery($jobexpencereportcf_insert_query, $params_jobexpencereportcf);
		$jobexpencereportcfid = $adb_ecf->getLastInsertID();

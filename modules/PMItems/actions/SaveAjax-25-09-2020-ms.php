...
class PMItems_SaveAjax_Action extends Vtiger_SaveAjax_Action {
	...
	public function process(Vtiger_Request $request) {
		...
		$cf_6054 = $request->get('cf_6054'); //Current QTY
		$cf_6056 = $request->get('cf_6056'); //AVG Consumption
		$cf_6058 = $request->get('cf_6058'); // Last Purchase
		$cf_6060 = $request->get('cf_6060'); // Last Quantity Received
		...
		if(is_array($cf_4279))
		{
			...
			$request->set('cf_6054',  $cf_6054[$key]); // Current QTY
			$request->set('cf_6056',  $cf_6056[$key]); // AVG Consumption
			$request->set('cf_6058',  $cf_6058[$key]); // Last Purchase Price
			$request->set('cf_6060',  $cf_6060[$key]); // Last QTY Received
		...
		$result['_recordId'] = $recordModel->getId();
			...
			$adb->pquery("update vtiger_pmrequisitionscf set cf_4593='Pending' WHERE cf_4593='In Progress' AND pmrequisitionsid='".$pmrequisitions_id."' ",array());
		}
		...
		$approval_by_arr = array('604', '60', '1245');
		$field_name_arr = array('604' => array('approved_by' => 'cf_4589', 'approved_date' =>'cf_4591'),
								...
								'1245' => array('approved_by' => 'cf_4585', 'approved_date' =>'cf_4587')
								);
		//echo "<pre>";
		...
		{			
			...
			if(isset($delete) && $delete==1)
			{
				...
				{
					if($current_user_id==60 || $current_user_id==604)
					{					
					$adb->pquery("update vtiger_pmrequisitionscf set cf_4593='Approved' WHERE cf_4593='Pending' AND pmrequisitionsid='".$pmrequisitions_id."' ",array());
					}
				}
				else{
					if($current_user_id==604)//Rustam Approval
					{
					$adb->pquery("update vtiger_pmrequisitionscf set cf_4593='Approved' WHERE cf_4593='Pending' AND pmrequisitionsid='".$pmrequisitions_id."' ",array());
					}	
				}			
		...
		$rrs_approval_info = Users_Record_Model::getInstanceById(1245, 'Users');
		$rrs_email   	  = $rrs_approval_info->get('email1');
		$rrs_first_name   = $rrs_approval_info->get('first_name');
		$rrs_last_name    = $rrs_approval_info->get('last_name');
		...
		$to = $owner_email.','.$rrs_email.','.$fd_email;
		//$to = $fd_email;
		$cc = 's.mansoor@globalinklogistics.com, erp.support@globalinklogistics.com';
		$location_id = $pmrequisitions_info->get('cf_4273');
		if($location_id=='85805')
		{
			$cc .='warehouse@globalinklogistics.com, z.smelykh@globalinklogistics.com';
		}
		
		$sql = mysqli_query("SELECT `cf`.*,`user`.`location_id` FROM `vtiger_userlistcf` AS `cf` INNER JOIN `vtiger_users` AS `user` ON `cf`.`cf_3355`=`user`.`email1` WHERE `user`.`id`='".$pmrequisitions_info_email->get('assigned_user_id')."' LIMIT 1");
			if (mysqli_num_rows($sql)>0) {
				$arr = mysqli_fetch_array($sql);
				if ((int)$arr['cf_3385']!==412373&&(int)$arr['location_id']!==85805) {
					$sql1 = mysqli_query("SELECT `cf_3355` FROM `vtiger_userlistcf` WHERE `userlistid`='".$arr['cf_3385']."' LIMIT 1");
		...
		if(!empty($rrs_supervsior))
		{
         ...
         send_mail('PMItems', $to, $from, $from, $subject, $body,$cc,'','','','',true);
		//mail("t.malik@globalinklogistics.com",$subject,$body.$to,$headers);

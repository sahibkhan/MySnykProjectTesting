...
class PMItems_SaveAjax_Action extends Vtiger_SaveAjax_Action {
	...
	public function process(Vtiger_Request $request) {
		...
		//user_id :: 60  :: Mr. Khan
		//user_id :: 1206 :: Dina Kabidullina :: Procurement Manager 
		
		$approval_by_arr = array('604', '60', '1206');
		$field_name_arr = array('604' => array('approved_by' => 'cf_4589', 'approved_date' =>'cf_4591'),
								...
								'1206' => array('approved_by' => 'cf_4585', 'approved_date' =>'cf_4587')
								);
		//echo "<pre>";
		...
		{			
			...
			if(isset($delete) && $delete==1)
			{
				...
				{
					if($current_user_id==60 || $current_user_id==604) //Mr.Khan or Rustam Approval
					{					
		...
		$owner_first_name = $pm_owner_info->get('first_name');
		$owner_last_name   = $pm_owner_info->get('last_name');
		$created_by_name = $owner_first_name.' '.$owner_last_name;
		
		
		$rrs_approval_info = Users_Record_Model::getInstanceById(1206, 'Users');
		...
		$fd_first_name   = $fd_approval_info->get('first_name');
		$fd_last_name     = $fd_approval_info->get('last_name');
		
		$ceo_approval_info = Users_Record_Model::getInstanceById(60, 'Users');
		$ceo_email   		 = $ceo_approval_info->get('email1');
		$ceo_first_name   = $ceo_approval_info->get('first_name');
		$ceo_last_name     = $ceo_approval_info->get('last_name');
		...
		$sql = mysql_query("SELECT `cf`.*,`user`.`location_id` FROM `vtiger_userlistcf` AS `cf` INNER JOIN `vtiger_users` AS `user` ON `cf`.`cf_3355`=`user`.`email1` WHERE `user`.`id`='".$pmrequisitions_info_email->get('assigned_user_id')."' LIMIT 1");
			if (mysql_num_rows($sql)>0) {
				$arr = mysql_fetch_array($sql);
				if ((int)$arr['cf_3385']!==412373&&(int)$arr['location_id']!==85805) {
					$sql1 = mysql_query("SELECT `cf_3355` FROM `vtiger_userlistcf` WHERE `userlistid`='".$arr['cf_3385']."' LIMIT 1");

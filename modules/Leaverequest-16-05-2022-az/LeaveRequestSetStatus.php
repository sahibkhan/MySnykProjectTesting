<?php
	...
	*/
	...
	//echo $status,$from,$rid," uid ".$cusrid; exit;
	...
	$record_link = "http://tiger.globalink.net/live-gems/index.php?module=Leaverequest&view=Detail&record=".$rid;
	...
	if($sendemail=="Y")
	{
		...
		//function sendEmailProc($fromEmail,$empemail,"$Heading","$SubHeading",$message,$username,"$ToCopy","ToFrom","N",$rid);
		
		// send auto reply 	
		if (($from=="Head" || $from=="HR Head" || $from=="HR") && ($status=="a"))
		{		
			$subject = "Request for leave from: ".$username." ID: ".$lvunqid;
			...
			$autoreply_info['requester_name'] = $username;
			$autoreply_info['requester_email'] = $usremail;		
			$autoreply_info['start_date'] = $lvfrom;
			$autoreply_info['end_date'] = $lvto;
			$autoreply_info['leave_hours'] = $rlr["cf_5355"];
			$autoreply_info['comments'] =  $rlr['cf_4677'];
			$autoreply_info['mobile_number'] = $rlr['cf_3401'];
			$autoreply_info['leave_type'] = $rlr['cf_3409'];
			
			$autoreply_info['forward_to'] = $rlr['cf_3405'];
			$autoreply_info['autoreply_text'] = $rlr['cf_3407']; 
	 
			//print_r($autoreply_info); //exit;
	 
			//send_autoreply_new($to, $record, $subject, 	$current_user_email, $current_user_name,$autoreply_info);
			...
			$link = "http://tiger.globalink.net/live-gems/index.php?module=Leaverequest&view=Detail&record=$record&app=HR";
			...
			if ($autoreply_info['mobile_number'])
			{
				$txt .= "<tr><td style='border-bottom:1px solid  #abb2b9;'>Mobile number: </td><td style='border-bottom:1px solid  #abb2b9;'><b>".$autoreply_info['mobile_number']." </b></td></tr>";
			} 
			
			if($autoreply_info['forward_to'])
			{
				$txt .= "<tr><td style='border-bottom:1px solid  #abb2b9;'>Forward to: </td><td style='border-bottom:1px solid  #abb2b9;'><b>".$autoreply_info['forward_to']." </b></td></tr>";
			}
			
			
			$txt .= "<tr><td style='border-bottom:1px solid  #abb2b9;'>Autoreply Text: </td><td style='border-bottom:1px solid  #abb2b9;'><b>".$autoreply_info['autoreply_text']." </b></td></tr>";
			...
			$txtBody = "<p>Dear <b>IT Team</b>,<br>Please put forwarding and autoreply text.</p>";
			$txtBody .= $txt;
				//$txt .= "<br><small>".$ToCopy."</small><br>";
			$txtBody .= "<br>";
			...
			$headers .= 'Reply-To: '.$to.'' . "\n";
			// $to = array();
			//$to[] = "d.bashayev@globalinklogistics.com"; // dont add
			//$to[] = "reception@globalinklogistics.com";
			//$to[] = "r.gusseinov@globalinklogistics.com";	
			$to_emails = implode(",", $to);	
			$cc_emails = implode(",", $cc);
			$subject = "Out of the office: " . $autoreply_info['requester_name'];		
		   
			$to="a.naseem@globalinklogistics.com"; //temp
			...
			*/
			...
			if( $retval == true ) {
				$sendmsg.= "<br>Autoreply Message sent to user successfully...";
			}else {
				$sendmsg.= "<br>Autoreply Message could not be sent to user...";
			}
			//echo $sendmsg; exit;

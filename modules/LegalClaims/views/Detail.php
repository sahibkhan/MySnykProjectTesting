...
class LegalClaims_Detail_View extends Vtiger_Detail_View {
    ...
    $this->exposeMethod('getContractReferenceNumbers');
    $this->exposeMethod('getInsuranceClaimList');
	...
	*/
	...
	function showModuleDetailView(Vtiger_Request $request) {
		...
		$instanceLegalClaims = new LegalClaims();
		$FEES_SUB_DATA = $instanceLegalClaims->getLegalClaimsSubModuleData($legalClaimsFeeType, $recordId);

		foreach ($FEES_SUB_DATA as $FEES_SUB_ITEM){
			$TOTAL_FEES_SUM += $FEES_SUB_ITEM['sum'];
		}
		$SETTLEMENT_SUB_DATA = $instanceLegalClaims->getLegalClaimsSubModuleData($legalClaimsSettlementType, $recordId);
		foreach ($SETTLEMENT_SUB_DATA as $SETTLEMENT_SUB_ITEM){
			$TOTAL_SETTLEMENT_SUM += $SETTLEMENT_SUB_ITEM['sum'];
		}
		...
		$viewer->assign('TOTAL_FEES_SUM', $TOTAL_FEES_SUM);
		$viewer->assign('TOTAL_SETTLEMENT_SUM', $TOTAL_SETTLEMENT_SUM);
 ...
 */
	...
	public function getContractReferenceNumbers(Vtiger_Request $request){
		global $adb;
		$jobFileId = $request->get('jobFileId'); 

		$queryJobFile = "SELECT cf_1441
										 FROM vtiger_jobcf
										 WHERE jobid = ?";
	  $resultJobFile = $adb->pquery($queryJobFile, array($jobFileId));
		$accountId = $adb->query_result($resultJobFile, 0, 'cf_1441');

		$contractNumbers = array();
		$queryContract = "SELECT vtiger_motiwcf.motiwid, vtiger_motiw.name
											 FROM vtiger_motiwcf
											 INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_motiwcf.motiwid
											 INNER JOIN vtiger_motiw ON vtiger_motiw.motiwid = vtiger_motiwcf.motiwid
											 WHERE vtiger_crmentity.deleted = 0 AND vtiger_motiwcf.cf_6362 = ? AND vtiger_motiwcf.cf_7046 NOT IN ('Cancelled')
											 ORDER BY vtiger_motiw.name DESC";
	   $result = $adb->pquery($queryContract, array($accountId));
	   $num_rows = $adb->num_rows($result);

	   for($i = 0; $i<$num_rows; $i++) {
			$contractId = $adb->query_result($result, $i, 'motiwid');
			$contractName = $adb->query_result($result, $i, 'name');
			$contractNumbers[] = array("id" => $contractId, "name" => $contractName);
	   }		 

		$details['data'] = $contractNumbers;
		return json_encode($details);
	}

	public function getInsuranceClaimList(Vtiger_Request $request){
		global $adb;
		$jobFileId = $request->get('jobFileId');
		$insuranceClaimList = array();

		$queryInsuranceClaims = "SELECT vtiger_insuranceclaim.insuranceclaimid, vtiger_insuranceclaim.name, vtiger_insuranceclaimcf.cf_5439
														 FROM vtiger_insuranceclaim
														 
														 INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_insuranceclaim.insuranceclaimid
														 INNER JOIN vtiger_insuranceclaimcf ON vtiger_insuranceclaimcf.insuranceclaimid = vtiger_insuranceclaim.insuranceclaimid
														 INNER JOIN vtiger_jobcf ON vtiger_jobcf.cf_1198 = vtiger_insuranceclaim.name
														 
														 WHERE vtiger_crmentity.deleted = 0 AND vtiger_jobcf.jobid = ?
														 ORDER BY vtiger_insuranceclaim.name DESC";
	   $result = $adb->pquery($queryInsuranceClaims, array($jobFileId));

	   $num_rows = $adb->num_rows($result);
	   for($i = 0; $i<$num_rows; $i++) {
			$insuranceClaimId = $adb->query_result($result, $i, 'insuranceclaimid');
			$insuranceClaim = $adb->query_result($result, $i, 'name').' ('.$adb->query_result($result, $i, 'cf_5439').')';

			$insuranceClaimList[] = array("id" => $insuranceClaimId, "name" => $insuranceClaim);
	   }

		$details['data'] = $insuranceClaimList;
		return json_encode($details);
	}
	...
	public function addComment(Vtiger_Request $request){
		...
		return json_encode(array("data" => 123));
	}

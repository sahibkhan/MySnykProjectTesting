...
class	LegalClaims_Record_Model extends Vtiger_Record_Model {
	...
	public function getInvities($id = 0) {
		$adb = PearDatabase::getInstance();
		$recordId = (!empty($this->getId())) ? $this->getId() : $id;

		$sql = "SELECT *
						FROM vtiger_invitees
						WHERE activityid = ? AND activityid > ?";
		$result = $adb->pquery($sql, array($recordId, 0));
		$invitiesId = array();

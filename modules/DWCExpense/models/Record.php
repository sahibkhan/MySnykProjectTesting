 ...
 */
class DWCExpense_Record_Model extends Vtiger_Record_Model {
	...
	*/
	public function getCurUserInfo() {
		...
		$position = '';
		if ($login == 'a.ahmed') $position = 'GM'; else 
		if ($login == 's.mansoor') $position = 'CFO'; else 
		if ($login == 's.khan') $position = 'CEO';	  
	  ...
	  */
	  public function getDWCExpenseStatus($position) {
	  
		// Status OPS cf_3591
		// Status Fleet cf_3571
		$current_dwc_status = '';
		$record = $this->getId();		
		$dwc_info = Vtiger_Record_Model::getInstanceById($record, 'DWCExpense');
		
		
		if ($position == 'GM'){
		  $current_dwc_status = $dwc_info->get('cf_4743'); 
		}	  
		if ($position == 'GFO'){
		  $current_dwc_status = $dwc_info->get('cf_4747'); 
		}
		if ($position == 'CEO'){
		  $current_dwc_status = $dwc_info->get('cf_4751'); 
		}
    
		return $current_dwc_status;	

...
class BO_Save_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
		try {
			...
			$refno = $request->get('cf_2687');		
			if(empty($refno)){
				// get job ref
				$jobref = $request->get('cf_1295');				
				// get ammount of BOs already created
				$sql = "SELECT * FROM vtiger_bocf where cf_1295='".$jobref."'";
				$result = $adb->pquery($sql, array());
				$boammount = $adb->num_rows($result);
				// incrementing bo ammount
				$boammount ++;
				// setting field value
				$request->set('cf_2687', $jobref.'-'.$boammount);
			}

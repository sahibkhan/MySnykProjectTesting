...
* All Rights Reserved.
*************************************************************************************/
class BO_Print_View extends Vtiger_Print_View {
    ...
    //$text = str_ireplace($breaks, "\r\n", $text);
		...
		// Detect devaluation currenct based on globalink company
		$currencyName = "";
		if (!empty($module_data_info->get('cf_1581'))){
			$globalinkAgent = $module_data_info->get('cf_1581');
			$globalinkCompany = Vtiger_Record_Model::getInstanceById($globalinkAgent, 'Company');
			$companyCurrencyId = $globalinkCompany->get('cf_1459');
			$queryAgent= "select * from vtiger_currency_info where id=".$companyCurrencyId;
			$resultAgent = $db->pquery($queryAgent, array());
			$currencyName = $db->query_result($resultAgent,0,'currency_name');
			// echo "<pre>"; print_r($currencyName); exit;
		}
	...
	//	$sign_details = str_replace($breaks, "\r\n", $sign_details);
		...
		$this->setValue('globalink_currency', $currencyName, ENT_QUOTES, "UTF-8");
...
// error_reporting(E_ALL);
		...
		$agent_sign = str_ireplace($breaks, "\r\n", $this->get_agent_sign($record,$module_data_info->get('cf_1581')));
		$customer_sign = str_ireplace($breaks, "\r\n", $this->get_customer_sign($module_data_info->get('account_id')));
		$customer_name = $this->get_display_name($module_data_info->get('account_id'),"account","accountname");
		$forwarding_agent = $this->get_display_name($module_data_info->get('cf_1581'),"company","name");
		$destination = $module_data_info->getDisplayValue('cf_1463').", ".$this->get_display_name($module_data_info->get('cf_1283'),"countries","country_name");
		...
		// Cargo Details
		$cargo_details = $module_data_info->getDisplayValue('cf_1471').", 
		".$module_data_info->getDisplayValue('cf_1309')."pc, 
		".$module_data_info->getDisplayValue('cf_1289')." 
		".$module_data_info->getDisplayValue('cf_1475').", 
		".$module_data_info->getDisplayValue('cf_1549').", 
		".$module_data_info->getDisplayValue('cf_1287')."
		 ".$module_data_info->getDisplayValue('cf_1473');

		 $cargo_details = str_ireplace($breaks, "\r\n", $cargo_details);
		 $cargo_details = html_entity_decode($cargo_details);

		 // Shipper info
		 $shipper_info  = html_entity_decode($module_data_info->getDisplayValue('cf_1275').', '.$module_data_info->getDisplayValue('cf_1465'));

		 // Customer sign 
		 $customer_sign  = html_entity_decode($customer_sign);

		 // Transportation Cost
		 $transportation_cost = $module_data_info->getDisplayValue('cf_1311');
		 $transportation_cost = str_ireplace($breaks, "\r\n", $transportation_cost);
		 $transportation_cost = html_entity_decode($transportation_cost);
		...
		if($request->get('template') == 'eng_1')
		{
			...
			$templatePrefix = 'BO_new_English_01.docx';
			if ($agentID == 85756) $templatePrefix = 'BO_new_English_01_DWC.docx';
			$document = $PHPWord->loadTemplate('include/BO/'.$templatePrefix);

		}
		elseif($request->get('template') == 'rus_1')
		{
			$templatePrefix = 'BO_Rus_new_01.docx';
			if ($agentID == 85756) $templatePrefix = 'BO_Rus_new_01_DWC.docx';
			$document = $PHPWord->loadTemplate('include/BO/'.$templatePrefix);
		}
		elseif($request->get('template') == 'eng_2')
		{
			$document = $PHPWord->loadTemplate('include/BO/BO_new_English_02.docx');
			$agent_sign = ''; //$account_info;
			$dataarrange = 1;
		}
		elseif($request->get('template') == 'rus_2')
		{
			$document = $PHPWord->loadTemplate('include/BO/BO_Rus_new_02.docx');
			$agent_sign = ''; //$account_info;
			$dataarrange = 1;
		}
		else{exit;}
		$document->setValue('email', $email);
		$document->setValue('bo_ref', $module_data_info->getDisplayValue('cf_2687'));
		$document->setValue('agreement_no', $agreementno);
		$document->setValue('date_created', $request_date);
		$document->setValue('forwarding_agent', $forwarding_agent);
		$document->setValue('customer_name', $customer_name);
		$document->setValue('quotation_no', $module_data_info->getDisplayValue('cf_1793'));
		// $document->setValue('shipper_name', $shipper_info); // Talha
		$document->setValue('shipper_name', $shipper_info);
		// $document->setValue('consignee_name',$module_data_info->getDisplayValue('cf_1465')); //$module_data_info->getDisplayValue('cf_1277'));
		$document->setValue('consignee_name', $module_data_info->getDisplayValue('cf_1277'));
		$document->setValue('loading_place', $module_data_info->getDisplayValue('cf_6924'));
		$document->setValue('order_no', $module_data_info->getDisplayValue('cf_6926'));
		$document->setValue('destination',$module_data_info->getDisplayValue('cf_1467')); //$destination);
		$document->setValue('cargo_details', $cargo_details);
		$document->setValue('hazard', $module_data_info->getDisplayValue('cf_6928'));
		$document->setValue('terms_delivery', $module_data_info->getDisplayValue('cf_1297'));
		$document->setValue('mode_transport', $module_data_info->getDisplayValue('cf_1573'));
		$document->setValue('export_declaration', $module_data_info->getDisplayValue('cf_6930'));
		$document->setValue('value_cargo', $module_data_info->getDisplayValue('cf_1469')." ".$module_data_info->getDisplayValue('cf_1727'));
		$document->setValue('cargo_insurance', $module_data_info->getDisplayValue('cf_6932'));
		$document->setValue('transportation_cost',$account_details->getDisplayValue('cf_1855')); //$transportation_cost);
		$document->setValue('special_instruction', $module_data_info->getDisplayValue('cf_1801'));
		$document->setValue('globalink_currency', $currencyName);
		if($dataarrange == 1)
		{
			
		$document->setValue('customer_details', $customer_sign);
		$document->setValue('agent_details', $agent_sign);
		}
		else
		{
		$document->setValue('agent_details', $agent_sign);
		...
		$document->setValue('director_name', $this->get_director_name($record));
		...
		header("Content-Disposition: attachment; filename=BO.docx");

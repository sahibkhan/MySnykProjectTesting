...
class Jobexpencereport_SaveAjax_Action extends Vtiger_SaveAjax_Action {
	...
	public function process(Vtiger_Request $request) {
		...
		try {
			...
			if($type=='Expence')
			{
				...
				if(trim($sourceModule)=='Fleettrip')
				{
	...
	public function buying($request)
	{
		...
		*/
		...
		if($job_info->get('assigned_user_id')!=$current_user->getId())
		{
			...
			//$saveJobExpense = new Vtiger_Save_Action();
			//$request = new Vtiger_Request();
			//$request->set("module","Jobexpencereport");
	...
	{
		$wagontrip_id_from_expense = $request->get('returnrecord');
		...
		$assigned_extra_buying = array(
			...
			if($assigned_extra_buying['b_buy_local_currency_net'] > 0){
				...
				$adb->pquery("UPDATE vtiger_wagontripcf SET cf_5792='In Progress' WHERE wagontripid='".$wagontrip_id_from_expense."'");
			}
	...
	{
		$fleet_id_from_expense = $request->get('returnrecord');

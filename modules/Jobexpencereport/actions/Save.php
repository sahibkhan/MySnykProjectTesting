...
class Jobexpencereport_Save_Action extends Vtiger_Save_Action {
		...
		try {
					...
					if($profit_share_count>0)
					{
						...
						if(isset($fleet_record_id) && !empty($fleet_record_id))
						{
							...
							$loadURL = 'index.php?module=Job&relatedModule='.$request->get('module').'&view=Detail&record='.$request->get('recordJobId').'&mode=showRelatedList&relationId=167&tab_label=Job%20Revenue%20and%20Expence&app=JOBS';
							echo '<script> window.location = "'.$loadURL.'";</script>';
					...
					if($type=='Expence')
					{
						...
						if($parent_crmmodule!='Fleettrip' && $parent_crmmodule!='WagonTrip')
						{
						...
						elseif($parent_crmmodule=='Fleettrip'){
						...
						elseif($parent_crmmodule=='WagonTrip')
						{
							$recordModel =  $this->railway_fleet_buying_edit($request);
							//$job_id = $fleet_expense;
							$wagon_trip_id = $this->get_wagon_trip_id_from_expense($recordId);
							$job_id  = $wagon_trip_id;	
							$parent_module='WagonTrip';	
							$module_label='Railway%20Expense';						
						}
					}
					else{
	...
	public function get_wagon_trip_id_from_expense($recordId=0)
	{
		 $adb = PearDatabase::getInstance();
		 $checkfleet = $adb->pquery("SELECT rel1.crmid as wagon_trip_id FROM `vtiger_crmentityrel` as rel1 where rel1.relcrmid='".$recordId."' and rel1.module='WagonTrip'", array());
		 $crmId = $adb->query_result($checkfleet, 0, 'wagon_trip_id');
		 $wagon_trip_id = $crmId;
		 return $wagon_trip_id;		  
	}
	...
	public function profit_share_fleet($request)
	{
		...
		if(!empty($internal_selling_arr))
		{
			foreach($internal_selling_arr as $key => $internal_selling)
			{
				...
				$jobexpid_fleet = $current_fleet_id;
				...
				$jobexpcfid_fleet = $current_fleet_id;
	...
	public function profit_share($request)
	{
		...
		if(!empty($internal_selling_arr))
		{
			foreach($internal_selling_arr as $key => $internal_selling)
			{
				...
				$department_id = $branch_department_arr[1];
				//$job_id = $request->get('record');
				$job_id = $request->get('recordJobId');
	...
	public function selling_edit($request)
	{
		...
		if($recordId!=0)
		{
			...
			$query_costing = 'SELECT * FROM vtiger_jer
								   ...
								   //AND vtiger_jercf.cf_1451=?
								   //AND vtiger_jercf.cf_1433=? AND vtiger_jercf.cf_1435=?
			...
			//$params_costing_1 = array(@$job_info->get('record_id'), $job_costing_id, $s_job_charges_id); //Internal auditor requested its showing zero on update
	...
	public function buying_edit($request)
	{
		...
		*/
		...
		{
			 $db_costing_2 = PearDatabase::getInstance();	
			 $sql_jrer_pre = $db_costing_2->pquery("select * from vtiger_jobexpencereport where jobexpencereportid='".$job_jrer_buying['jrer_buying_id']."'");
		 	 $row_jrer_pre = $db_costing_2->fetch_array($sql_jrer_pre);
	...
	public function railway_fleet_buying_edit($request)
	{
		$recordId = $request->get('record');
		...
		if($recordId!=0)
		{
			...
			//jobid as record_id						   
			//$params_buying = array(@$job_info->get('record_id'), $current_user->getId(), $recordId);
			$params_buying = array(@$wagontrip_id_from_expense, $wagon_trip_info->get('assigned_user_id'), $recordId);	
			$result_buying = $db_buying->pquery($jrer_buying,$params_buying);
			$wagontrip_jrer_buying = $db_buying->fetch_array($result_buying);
			...
			if($db_buying->num_rows($result_buying)==0)
			{
				...
				$jrer_buying_2 = 'SELECT * FROM vtiger_jobexpencereport
							      ...
							      and vtiger_jobexpencereportcf.cf_1457="Expence" ';
				//$params_buying_2 = array(@$job_info->get('record_id'), $recordId);
				$params_buying_2 = array(@$wagontrip_id_from_expense, $recordId);
				$result_buying_2 = $db_buying_2->pquery($jrer_buying_2,$params_buying_2);
				$wagontrip_jrer_buying = $db_buying_2->fetch_array($result_buying_2);				
			}
			$expense_wagon_id = $wagontrip_jrer_buying['cf_5830'];		
		...
		$assigned_extra_buying = array(
			...
			if($wagon_trip_info->get('assigned_user_id')!=$current_user->getId())
			{
				...
				$request->set('user_id', $assigned_extra_buying['user_id']);
				$request->set('owner_id', $assigned_extra_buying['owner_id']);
				$request->set('record', $wagontrip_jrer_buying['jobexpencereportid']);
				$recordModel = $this->saveRecord($request);
			}
			
			if($assigned_extra_buying['b_buy_local_currency_net'] > 0){
				$db_up_2 = PearDatabase::getInstance();
				$db_up_2->pquery("UPDATE vtiger_wagontripcf SET cf_5792='In Progress' WHERE wagontripid='".$wagontrip_id_from_expense."'");
			}
	...
	public function round_trip_buying_edit($request)
	{
		...
		if($recordId!=0)
		{
			...
			if($db_buying->num_rows($result_buying)==0)
			{
				...
				$params_buying_2 = array(@$fleet_id_from_expense, $recordId);
				$result_buying_2 = $db_buying_2->pquery($jrer_buying_2,$params_buying_2);
				$fleetrip_jrer_buying = $db_buying_2->fetch_array($result_buying_2);		
			}

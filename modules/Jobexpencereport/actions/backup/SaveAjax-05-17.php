	...
	public function buying($request)
	{
		...
		*/
		...
		if($job_info->get('assigned_user_id')!=$current_user->getId())
		{
			...
			$query_expence_up = 'UPDATE vtiger_jobexpencereport set job_id=?, company_id=?, user_id=?, owner_id=? where jobexpencereportid=?';
			$params_expence_up = array($assigned_extra_buying['job_id'], $assigned_extra_buying['company_id'], $assigned_extra_buying['user_id'], $assigned_extra_buying['owner_id'], $jrer_buying_id_assigned);
		...
		if($job_info->get('assigned_user_id')!=$current_user->getId())
		{
			...
			if($db_count->num_rows($result_count)==0)
			{
				...
				$query_expence_up_1 = 'UPDATE vtiger_jobexpencereport set job_id=?, company_id=?, user_id=?, owner_id=? where jobexpencereportid=?';
				$params_expence_up_1 = array($extra_buying['job_id'], $extra_buying['company_id'], $extra_buying['user_id'], $extra_buying['owner_id'], $jrer_buying_id_jobexpencereportid);

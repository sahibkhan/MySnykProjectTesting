...
class Jobexpencereport_Save_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
		...
		if($type=='Expence')
		{
			$fleet_expense  = $this->get_job_id_from_fleet($recordId);
		...
		if(!empty($internal_selling_arr))
		{
			foreach($internal_selling_arr as $key => $internal_selling)
			{
				...
				$adb_rel->pquery($crmentityrel_insert_query, $params_crmentityrel);
					...
					$adb_ecf = PearDatabase::getInstance();
					$jobexpcf_update_query = "update vtiger_jobexpcf set cf_1257=?, cf_1259=?, cf_1263=? WHERE jobexpid=?";
					$params_jobexpcf_up = array($location_id, $department_id, $internal_selling_value, $jrer_internal_arr_data['jobexpid']);
	...
	public function fleet_buying_edit($request)
	{
		...
		$extra_buying = array(
			...
			if($job_info->get('assigned_user_id')!=$current_user->getId())
				...
				/*$params_minus = array($before_update_jrer_buying_fleet['cf_1337'], $before_update_jrer_buying_fleet['cf_1343'], $before_update_jrer_buying_fleet['cf_1347'], 
									  $before_update_jrer_buying_fleet['cf_1349'], $before_update_jrer_buying_fleet['cf_1353']);*/									  
				...
				/*$params_plus = array($extra_buying['cf_1337'], $extra_buying['cf_1343'], $extra_buying['cf_1347'], 
									  $extra_buying['cf_1349'], $extra_buying['cf_1353']);*/

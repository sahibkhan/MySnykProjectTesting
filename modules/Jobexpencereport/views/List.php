...
class Jobexpencereport_List_View extends Vtiger_List_View {
	...
	function process (Vtiger_Request $request) {
		...
		$payable_status = $get_first_record->rawData['cf_1975'];
		...
		{		
				...
				foreach($models as $key => $model){
					...
					$makpal_company = array('85757', '85772');
					...
					{											
						if($company_account_type!='Cash N')
						{
							continue;
						}
					}
					elseif($current_user->getId()==422)
					{
						if($company_account_type!='Bank N')
						{
							continue;
						}
					}
					elseif($current_user->getId()==447)
					{						
						if($company_account_type=='Bank N')
						{
							continue;
					...
					//elseif($current_user->getId()==463)
					elseif($current_user->getId()==420)
					{
						//if(!in_array($currency_id, $makpal_currency) || ($job_info_detail['cf_1186']!='85757'))
						if(!in_array($currency_id, $makpal_currency) || !in_array($job_info_detail['cf_1186'], $makpal_company))
						{
							continue;
						}
						//85757=kz
					...
					elseif($current_user->getId()==414)
					{
						if(in_array($currency_id, $yuliya_om_currency_not) || ($job_info_detail['cf_1186']!='85757'))
						{
							continue;
						}
					}
					
					/*elseif($current_user->getId()==95)
					{
					...
					*/
					...
					$chartofaccount  = Vtiger_Record_Model::getInstanceById($expense_record->get('cf_1453'), 'ChartofAccount');
					$col_data['cf_1453'] = $chartofaccount->get('name');
					
					/*$customer_id = $expense_record->get('cf_1367');
					if(!empty($customer_id))
					{						
					$customer_name  = Vtiger_Record_Model::getInstanceById($expense_record->get('cf_1367'), 'Accounts');
					$col_data['cf_1367'] = $customer_name->get('accountname');
					}*/
					
					$customer_id = $expense_record->get('cf_1367');
					if(!empty($customer_id))
					{
					$rs = mysql_query("select * from vtiger_crmentity where crmid='".$customer_id."'");
					$rs_row = mysql_fetch_array($rs);
					if($rs_row['deleted']==1)
					{
						$col_data['cf_1367'] =$expense_record->get('cf_1367');
					}
					else{
						$accounts  = Vtiger_Record_Model::getInstanceById($expense_record->get('cf_1367'), 'Accounts');
						$col_data['cf_1367'] = $accounts->get('accountname');
					}
					}
					else{
					$col_data['cf_1367'] = '';	
					...
					{						
					$CompanyAccountType  = Vtiger_Record_Model::getInstanceById($expense_record->get('cf_1214'), 'CompanyAccountType');
					$col_data['cf_1214'] = $CompanyAccountType->get('name');
					}
					...
					{
						if($payable_status=='Approved')
						{
							$col_data['send_to_1c'] = '';
						}
						else if($payable_status=='Posted')
						{
		...
		$buying_field_payables[] = array('name' => 'myname','width'=>'50', 'frozen' =>true,  "align" => "center");
		...
		{			
			if($payable_status=='Approved')
			{
				$expence_header_payables[] = 'Send to 1C';
				$buying_field_payables[] = array('name' => 'send_to_1c', "align" => "center", "editable" => true, 
		...
		{				
			...
			$roselle_branch = array('85805','85806','85807','85808','85809','85810','85811','85812','85813','85814','85815','85816','85817','85818','85819',
									...
									'85835','85946','85947','85948');
			$roselle_company = array('85772','85756','85768','85761', '85771', '85763', '85762');						
			...
			if(!empty($models))
			{
				foreach($models as $key => $model)
				{
					...
					if($ref_no!='')
					{
						if($ref_no != $job_info_detail['cf_1198'])
						{
							continue;
						}
					}
					...
					if($current_user->getId()==389)
					{
						if(!in_array($job_info_detail['cf_1186'], $roselle_company))
						{
							continue;
						}
					}
					elseif($current_user->getId()==436)
					{
						if(!in_array($job_info_detail['cf_1186'], $kazbek_company))
						{
							continue;
						}
					}
					...
					$cf_1445 = $selling_record->get('cf_1445');
					if(!empty($cf_1445))
					{
					$rs = mysql_query("select * from vtiger_crmentity where crmid='".$cf_1445."'");
					$rs_row = mysql_fetch_array($rs);
					if($rs_row['deleted']==1)
					{
						//$col_data['cf_1445'] =$selling_record->get('cf_1445');
					}
					else{
					...
					$cf_1477 = $selling_record->get('cf_1477');
					if(!empty($cf_1477))
					{
					...
					$cf_1479 = $selling_record->get('cf_1479');
					if(!empty($cf_1479))
					{
					...
					$cf_1455 = $selling_record->get('cf_1455');
					if(!empty($cf_1455))
					{
					$chartofaccount  = Vtiger_Record_Model::getInstanceById($selling_record->get('cf_1455'), 'ChartofAccount');
		...
		if($count_parent_role>3)
		{
			...
			{				
				...
				$viewer->assign('PAYABLE_STATUS', $payable_status);

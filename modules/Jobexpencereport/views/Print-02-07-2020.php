...
class Jobexpencereport_Print_View extends Vtiger_Print_View {
	...
	public function print_job_invoice($request){
		...
		$this->setValue('commodity',htmlentities($cf_1518, ENT_QUOTES, "UTF-8"));	

		$this->setValue('consignee',htmlentities($job_expense_info_detail->get('cf_1361'), ENT_QUOTES, "UTF-8"));	
		$this->setValue('consigneeaddress',htmlentities($job_expense_info_detail->get('cf_1363'), ENT_QUOTES, "UTF-8"));	
		$this->setValue('bill_to_address',htmlentities($job_expense_info_detail->get('cf_1359'), ENT_QUOTES, "UTF-8"));	
		...
		$creator_name = $selling_from ;
			...
			if(!empty($bill_to))
			{
				...
				$c_account_type = @$billtoinfo->get('c_account_type');

				$BankDetails = @$billtoinfo->get('cf_1833').'<br>'.@$billtoinfo->get('cf_1835').'<br>'.@$billtoinfo->get('cf_1837').'<br>'.@$billtoinfo->get('cf_1841').'<br>'.@$billtoinfo->get('cf_1845').'<br>'.@$billtoinfo->get('cf_1849').'<br>BIC:'.@$billtoinfo->get('cf_2397');
			...
			$this->setValue('bill_to',htmlentities($bill_to, ENT_QUOTES, "UTF-8"));
			...
			$this->setValue('selling_additional_remark',htmlentities($selling_additional_remark, ENT_QUOTES, "UTF-8"));		
	...
	public function print_job_pv($request)
	{
		...
		$eng_location = array('85832', '85947', '85820', '85808', '85948');
			...
			if(!empty($pay_to_id))
			{
				...
				//$DepId = $r['cf_1479'];
				$DepId = $job_expense_info_detail->get('cf_1479');
				$c_account_type = $paytoinfo->get('account_type');
				$agreement_where = " AND vtiger_serviceagreementcf.cf_6068='Freight Forwarding' ";
					if($DepId=='85837')
					{
						$agreement_where = " AND vtiger_serviceagreementcf.cf_6068='Customs Brokerage' ";
			...
			$this->setValue('pv_Client', htmlentities($client_accountname, ENT_QUOTES, "UTF-8"));

...
class Jobexpencereport_Print_View extends Vtiger_Print_View {
 ...
 //echo $agreement_checker;exit;
   /// //// check the agreement status (previous or current) code end here////
	...
	public function invoice_tax_excel($request){
		...
		*/
		...
		//$spreadsheet->getActiveSheet()->getStyle('H9:K9')->getAlignment()->setWrapText(true);
		
		$spreadsheet->getActiveSheet()->mergeCells('H9:K9');
		//$spreadsheet->getActiveSheet()->mergeCells('H9:K10');
		...
		if ( $adb->num_rows($jobexpencereport_query) ) {
			...
			*/
			...
			if (@$billtoinfo->get('bill_street') ) {
				...
				//$company_address = 'Office 27- A, Gold Tower, Jumeirah Lakes Office 27- A, Gold Tower, Jumeirah Lakes Jumeirah Lakes Office 27- A, Gold Tower, Jumeirah Lakes Gold Tower, Jumeirah Lakes Office 27- A, Gold Tower, Jumeirah Lakes Jumeirah Lakes Office 27- A, Gold Tower, Jumeirah Lakes';
				//$company_address = 'Office 27- A, Gold Tower, Jumeirah Lakes Office 27- A, Gold Tower, Jumeirah Lakes Jumeirah Lakes Office 27- A';
				//$company_address = 'Office 27- A, Gold Tower, Jumeirah Lakes Gold Tower, Jumeirah Lakes';
				//$co_lines = wordwrap($company_address, 40, "<br>");
				$co_lines = explode("<br>\n", wordwrap($company_address, 40, "<br>\n"));
				$set_co_lines = round((count($co_lines)) / 2);

				if (count($co_lines) > 1) {
					$g_rows_per_page = $g_rows_per_page - $set_co_lines;
					$margin_top = $margin_top + (20*$set_co_lines);
				}
			...
			$detail1 = "Ship to: ".$job_info_detail->get('cf_1074');
			$detail2 = "AWB/MAWB: ".$job_info_detail->get('cf_1096')."/".$job_info_detail->get('cf_2387');
			$detail3 = $job_info_detail->get('cf_1429').' units;'.$job_info_detail->get('cf_1084').' '.$job_info_detail->get('cf_1520').' /  '.$job_info_detail->get('cf_1086').' '.$job_info_detail->get('cf_1522');
			$detail4 = "Commodity: ".$job_info_detail->get('cf_1518');

			//$this->setValue('details',htmlentities($details, ENT_QUOTES, "UTF-8"));
			//$lines = explode("\n", wordwrap($detail1, 12, "\n"));
			$detail1Arr = explode("<br>\n", wordwrap($detail1, $wordlimit, "<br>\n"));

			foreach ($detail1Arr as $key => $value) {

			$loop_htmlArr[] = '<tr>
									<td style="border-left: 1px solid #000000; border-right: 1px solid #000000;  white-space: nowrap;" height="19" align="left" valign=bottom><font face="Arial" color="#000000">'.$value.'</font></td>
			...
			$detail2Arr = explode("\n", wordwrap($detail2, $wordlimit, "\n"));

			foreach ($detail2Arr as $key => $value) {
			...
			$detail3Arr = explode("\n", wordwrap($detail3, $wordlimit, "\n"));

			foreach ($detail3Arr as $key => $value) {
			$loop_htmlArr[] = '<tr>
									<td style="border-left: 1px solid #000000; border-right: 1px solid #000000;  white-space: nowrap;" height="19" align="left" valign=bottom><font face="Arial" color="#000000">'.$value.'</font></td>
									<td style="border-right: 1px solid #000000;" align="center" valign=bottom></td>
									<td style="border-right: 1px solid #000000;" align="center" valign=bottom></td>
			...
			$detail4Arr = explode("\n", wordwrap($detail4, $wordlimit, "\n"));

			foreach ($detail4Arr as $key => $value) {
			$loop_htmlArr[] = '<tr>
									<td style="border-left: 1px solid #000000; border-right: 1px solid #000000; white-space: nowrap;" height="19" align="left" valign=bottom><font face="Arial" color="#000000">'.$value.'</font></td>
									<td style="border-right: 1px solid #000000;" align="center" valign=bottom></td>
									<td style="border-right: 1px solid #000000;" align="center" valign=bottom></td>
			...
			while ($jobexpencereportTax = $adb->fetch_array($jobexpencereportTax_query)) {
				...
				$description = wordwrap($jobexpencereportTax['cf_1365'],$wordlimit,"<br>\n"); // description
				...
				//$taxable_amount_aed = $this->numberFormat($taxable_amount * $ex_rate , 2 ,  "." , "" );
				$taxable_amount_aed = number_format($taxable_amount * $ex_rate, 2, '.', '');
				//$taxable_amount_aed = $this->numberFormat($taxable_amount * 3.6725 , 2 ,  "." , "" ); // for DW file type for now , convert usd to aed
				...
				$vat_amount_aed = number_format($vat_amount * $ex_rate , 2 ,  "." , "");

				//$gross_amount = $this->numberFormat($jobexpencereportTax['cf_1232'] , 2 ,  "." , "," );
				$gross_amount = number_format($jobexpencereportTax['cf_1232'] , 2 ,  "." , "," );

				$gross_amount_aed = str_replace(",","",$taxable_amount_aed) + str_replace(",","",$vat_amount_aed);

				//$gross_amount_aed = $this->numberFormat($gross_amount_aed, 2 ,  "." , "" );


				//$total_amount = $total_amount + str_replace(",","",$taxable_amount_aed);
				$total_amount = number_format($total_amount + $taxable_amount_aed, 2 ,  "." , "");
			...
			//if ($loop_ind == $total_rec) {
				...
				$trsArr = $this->gettrs(count($loop_htmlArr),$g_rows_per_page);
			...
			$this->setValue('margin_top', $margin_top);
    ...
    public function get_invoice_tax_body ($request) {
		...
		<table cellspacing="0" border="0" style="font-size: 11px;">
			...
			<tr>
				...
				<td align="left" valign=bottom><font face="Arial" color="#000000">Ship to: '.$job_info_detail->get('cf_1074').'</font></td>
			...
			<tr>
				<td width="5%" height="12" style="border-left: 1px solid #000000" align="left" valign=bottom><font face="Arial" color="#000000"><br></font></td>
				<td align="left" valign=bottom><font face="Arial" color="#000000">AWB/MAWB: '.$job_info_detail->get('cf_1096').'/'.$job_info_detail->get('cf_2387').'</font></td>
				<td align="left" valign=bottom></td>
				<td align="left" valign=bottom></td>
				<td align="left" valign=bottom></td>
				<td align="left" valign=bottom></td>
			...
			<tr>
				<td width="5%" height="12" style="border-left: 1px solid #000000" align="left" valign=bottom><font face="Arial" color="#000000"><br></font></td>
				<td align="left" valign=bottom><font face="Arial" color="#000000">'.$job_info_detail->get('cf_1429').' units;'.$job_info_detail->get('cf_1084').' '.$job_info_detail->get('cf_1520').' /  '.$job_info_detail->get('cf_1086').' '.$job_info_detail->get('cf_1522').'</font></td>
				<td align="left" valign=bottom></td>
				<td align="left" valign=bottom></td>
				<td align="left" valign=bottom></td>
				<td align="left" valign=bottom></td>
			...
			<tr>
				<td width="5%" height="12" style="border-left: 1px solid #000000" align="left" valign=bottom><font face="Arial" color="#000000"><br></font></td>
				<td align="left" valign=bottom><font face="Arial" color="#000000">Commodity: '.$job_info_detail->get('cf_1518').'</font></td>
				<td align="left" valign=bottom></td>
				<td align="left" valign=bottom></td>
				<td align="left" valign=bottom></td>
				<td align="left" valign=bottom></td>
			...
			//$description .= "<br>Ship to: ".$job_info_detail->get('cf_1074');
			//$description .= "<br>".$job_info_detail->get('cf_1429')." units;".$job_info_detail->get('cf_1084').' '.$job_info_detail->get('cf_1520')." /  ".$job_info_detail->get('cf_1086')." ".$job_info_detail->get('cf_1522');
			...
			$taxable_amount = $jobexpencereportTax['cf_1357'];
			//var_dump($taxable_amount);
			//$taxable_amount_aed = $this->numberFormat($taxable_amount * $ex_rate , 2 ,  "." , "" );
			$taxable_amount_aed = number_format($taxable_amount * $ex_rate , 2 ,  "." , "" );
			//$taxable_amount_aed = "123.00";
			//var_dump($taxable_amount_aed);
			//echo "<br>".$taxable_amount_aed;
			//$taxable_amount_aed = $this->numberFormat($taxable_amount_aed , 2 ,  "." , "" );
			//$taxable_amount_aed = $this->numberFormat($taxable_amount * 3.6725 , 2 ,  "." , "" ); // for DW file type for now , convert usd to aed
			$vat_persent = $jobexpencereportTax['cf_1228'];
	...
	*/
	...
	function gettrs($total_rec,$g_rows_per_page) {
		$trs = "";
		$trsArr = array();
		$rows_per_page = $g_rows_per_page;
		////$rows_per_page = 14;

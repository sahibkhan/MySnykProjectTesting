	...
	public function process(Vtiger_Request $request) {
		...
		{	
			...
			{	
				...
				$cost_exchange_rate   = exchange_rate_currency(date('Y-m-d'), $pay_to_currency_code);						
			...
			$customer_currency = $cf_1164[$key];
			$request->set('cf_1164', $customer_currency);
			if($file_title_currency=='KZT')
			{
				$customer_currency_code = Vtiger_CurrencyList_UIType::getDisplayValue($customer_currency);
				$revenue_exchange_rate   = exchange_rate_currency(date('Y-m-d'), $customer_currency_code);				
			}
			elseif($file_title_currency=='USD')
			{
			...
			*/	
			   ...
			   $source_id = $request->get('sourceRecord');
				...
				$jobexpencereport_insert_query = "INSERT INTO vtiger_jobexpencereport(jobexpencereportid, name, jerid, job_costing_type) VALUES(?,?,?,?)";
				$params_jobexpencereport= array($current_id, $source_id, $job_costing_id, 'Expense');			
				...
				$jobexpencereport_insert_query = "INSERT INTO vtiger_jobexpencereport(jobexpencereportid, name, jerid, job_costing_type) VALUES(?,?,?,?)";			
				$params_jobexpencereport= array($current_id, $source_id, $job_costing_id, 'Selling');

...
class Leads extends CRMEntity
{
	...
	public function handleEmailNotification($record)
	{
		...
		$lead = Vtiger_Record_Model::getInstanceById($record, 'Leads');
		...
		if ($lead->get('cf_4047')) {
			$accountName = Vtiger_Record_Model::getInstanceById($lead->get('cf_4047'), 'Accounts')->get('accountname');
		...
		if ($event_status == 1)  $message_status = "Please note Sales Lead $accountName on $subject was assigned to you";
		if ($event_status > 1)   $message_status = "Please note Sales Lead from $subject has been updated";
	...
	public function handleLeadDetails($record, $assigned_users, $all_field_details, $event_status)
	{
		$table = '';
		...
		if ($n > 0) {
			...
			for ($i = 1; $i <= $n; $i++) {
				...
				if ($label == 'Modifiedby') {
					...
					if ($event_status > 1) $label = "Updated by";
				...
				if ($label == 'Assigned_user_id') {
					$label = 'Local Sales Manager';
					$user_model = Users_Record_Model::getInstanceById($value, 'Users');
					$value = $user_model->get('first_name') . ' ' . $user_model->get('last_name');
				}
				if ($label == 'Record_module') $label = 'Module';
				if ($label == 'Lead_no') $label = 'Lead number';
				if ($label == 'Createdtime') $label = 'Created time';
				if ($label == 'Salutationtype') $label = 'Salutation';
				if ($label == 'Pobox') $label = 'PO Box';
				if ($label == 'Lane') $label = 'Street';
				if ($label == 'Cf_831') $label = 'Deadline';
				if ($label == 'Cf_833') $label = 'Lead name';
				...
				if ($label == 'Cf_4047') {
					...
					$value = Accounts_Record_Model::getInstanceById($value, 'Accounts')->get('accountname');
				}

				if ($label == 'Cf_8388') $label = 'Reason for status';

				if ($label == 'Description') {
					require_once('include/finediff.php');
					//$value = str_replace("\n","<br>",$value);
					//$value = hightlight_updated_description($prevalue,$value);
					$value = $this->highlight_updated_description($prevalue, $value);
				}

				if ($label != 'Updates are provided to ') $table .= "<tr> <td> $label </td> <td> $value </td></tr>";
			}
		}
		return $table;
	}
	...
	public function prepareLeadEmailNotification($creator_email, $creator_name, $who_updated_email, $who_updated_name, $module, $record, $subject, $message_status, $sum, $cc, $event_status)
	{

		$link = "https://gems.globalink.net/index.php?module=Leads&view=Detail&record=" . $record;
		// $link = $record;
		//$to = $who_updated_email; <----- UNCOMMENT AFTER TESTING -|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-|-
		$to = $creator_email;
		$cc = '';
		$cc .= $creator_email;
		...
		$body = '';
		$body .= "<html><head> <style> #calendar_notification tr td{ margin:3px; } .edited {font-weight: bold; color:green;}</style> </head>
		...
		$body .= "<tr><td colspan=2> $message_status </td></tr>
				 $sum
				<tr><td colspan=2>Full description may be checked per below link.</td></tr>	
		...
		$r = send_mail('Leads', $to, $from, $from, $subject, $body, $cc, '', '', '', '', true);
	}
 ...
 */
	...
	public function get_lead_history($record, $history_field)
	{
		global $adb;
		...
		if ($history_field != '') {
			...
			if ($history_field == 'cf_851') {
				...
				$history = $this->arrange_muptiple_users_lead($details['postvalue'], 1);
			}
		} else  
			if ($history_field == '') {
	...
	function arrange_muptiple_users_lead($users, $format)
	{
	...
	function transferRelatedRecords($module, $transferEntityIds, $entityId)
	{
		...
		$rel_table_arr = array(
			"Activities" => "vtiger_seactivityrel", "Documents" => "vtiger_senotesrel", "Attachments" => "vtiger_seattachmentsrel",
			"Products" => "vtiger_seproductsrel", "Campaigns" => "vtiger_campaignleadrel", "Emails" => "vtiger_seactivityrel"
		);

		$tbl_field_arr = array(
			"vtiger_seactivityrel" => "activityid", "vtiger_senotesrel" => "notesid", "vtiger_seattachmentsrel" => "attachmentsid",
		...
		$entity_tbl_field_arr = array(
			"vtiger_seactivityrel" => "crmid", "vtiger_senotesrel" => "crmid", "vtiger_seattachmentsrel" => "crmid",
	...
	function highlight_updated_description($from_text, $to_text)
	{

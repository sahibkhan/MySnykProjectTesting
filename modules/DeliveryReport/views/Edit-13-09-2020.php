		...
		if(!empty($record) && $request->get('isDuplicate') == true) {
			...
			{				
				if($request->get('returnmodule') == 'Job' && $request->get('returnrecord')) 
				{
					...
					{
						$request->set('cf_3613', date('d-m-Y', strtotime($job_info->get('cf_1591')))); //Expected date of arrival
					}
					$expected_date_of_delivery = $job_info->get('cf_1583');
					if(!empty($expected_date_of_delivery))
					{
						$request->set('cf_3615', date('d-m-Y', strtotime($job_info->get('cf_1583')))); //Expected date of delivery
					}

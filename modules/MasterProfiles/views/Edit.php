...
Class MasterProfiles_Edit_View extends Vtiger_Edit_View {
	...
	function preProcess(Vtiger_Request $request, $display=true) {
		...
		// echo "<pre>"; print_r($recordModel->getMPRecipientUsers()); exit;
		$viewer->assign('MASTERPROFILE_RECIPIENT_USERS', $recordModel->getMPRecipientUsers());
...
/* 		$INVITIES_SELECTED = $recordModel->getInvities();

		$INVITE_USERS='';
		foreach($accessibleUsers as $USER_ID => $USER_NAME){
			$INVITE_USERS .='<option value="'.$USER_ID.'"';
			 if( in_array($USER_ID,$INVITIES_SELECTED)){
			  $INVITE_USERS .='selected';
			 }
			$INVITE_USERS .='>';
			$INVITE_USERS .=$this->getUserNameById($USER_ID);
			$INVITE_USERS .='</option>';
		}
		$viewer->assign('INVITE_USERS', $INVITE_USERS); */
/* 		print_r($recordModel->getMPRecipientUsers());
		exit;
	...
	public function getUserNameById($id){
		global $adb;
		$resUsers = $adb->pquery("SELECT first_name, last_name FROM `vtiger_users` WHERE `id` = ?", array($id));
		$name = $adb->query_result($resUsers, 0, "first_name") .' '. $adb->query_result($resUsers, 0, "last_name");
		return $name;
	}

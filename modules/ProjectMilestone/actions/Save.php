...
class ProjectMilestone_Save_Action extends Vtiger_Save_Action {
	...
	public function saveRecord($request) {
		...
		// Project Task
		$taskRecordModel = Vtiger_Record_Model::getInstanceById($request->get('record'), 'ProjectMilestone');

		// Project
		$projectRecordModel = Vtiger_Record_Model::getInstanceById($request->get('projectid'), 'Project');
		$projectName = $projectRecordModel->get('projectname');


		$projectCreatorModel = Vtiger_Record_Model::getInstanceById($projectRecordModel->get('assigned_user_id'), 'Users');
		$projectCreator = $projectCreatorModel->get('first_name').' '.$projectCreatorModel->get('last_name');
		$projectCreatorEmail = trim($projectCreatorModel->get('email1'));

		// Collect Involved Users
		$projectid = $taskRecordModel->get('projectid');

		$queryInvolvedUsers = $adb->pquery("SELECT vtiger_users.email1				

																								FROM vtiger_crmentityrel											 
																								INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_crmentityrel.relcrmid
																								INNER JOIN vtiger_users ON vtiger_users.id = vtiger_crmentity.smownerid
																								WHERE vtiger_crmentityrel.crmid = ? AND vtiger_crmentityrel.module = ? AND vtiger_crmentityrel.relmodule = ?
																								AND vtiger_crmentity.deleted = 0", array($projectid, 'Project', 'ProjectMilestone'));
		$nRows = $adb->num_rows($queryInvolvedUsers);
		for ($i=0;$i<$nRows;$i++){
			$userEmail .= strtolower(trim($adb->query_result($queryInvolvedUsers, $i, 'email1'))).',';
		...
		$details['taskStatus'] = $taskRecordModel->get('cf_7838');
		$details['taskDeadline'] = $taskRecordModel->get('projectmilestonedate');
		$details['revisedDeadline'] = $taskRecordModel->get('cf_7860');
		$details['revisedDeadlineReason'] = $taskRecordModel->get('cf_7862');
		$details['taskComments'] = $taskRecordModel->get('description');
		$details['taskName'] = $taskRecordModel->get('projectmilestonename');
		$details['involvedUserEmails'] = $userEmail;

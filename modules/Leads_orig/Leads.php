...
class Leads extends CRMEntity {
	...
	*/
	...
	//	$record = $this->id; 
		$this->handleEmailNotification($this->id);
/* 		$adb->pquery("UPDATE `a_test` SET `user_name` = '$record' WHERE `a_test`.`record_id` = 990679"); */
...
/* 		$cc .= get_user_details($creator_id,1).',';	 */
		...
		//$adb->pquery("UPDATE `a_test` SET `user_name` = '$event_status' WHERE `a_test`.`record_id` = 990679");

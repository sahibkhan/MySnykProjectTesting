...
Class NewEmail_Edit_View extends Vtiger_Edit_View {
	...
	public function process(Vtiger_Request $request) {
		...
		if(!empty($record) && $request->get('isDuplicate') == true) {
			...
			$viewer->assign('MODE', '');
			$request->set('name','-');
			$request->set('cf_4065', $current_user->department_id);
			$request->set('cf_4067', $current_user->location_id);
    ...
    {	
      if($request->get('returnmodule') == 'UserList' && $request->get('returnrecord')) 
      {
        ...
        $userInfo = Vtiger_Record_Model::getInstanceById($sourceRecord, $sourceModule);
...
/* 				$request->set('cf_4065', $current_user->department_id);
				...
				// User name
				$userName = explode(' ', $userInfo->get('name'));
				$request->set('cf_4053', $userName[0]);
				$request->set('cf_4055', $userName[1]);
				$request->set('cf_4059', $userInfo->get('name'));

        $request->set('cf_4067', $userInfo->get('cf_3421')); // Location
        $request->set('cf_4065', $userInfo->get('cf_3349')); // Department
				$request->set('cf_4061', $userInfo->get('cf_3355')); // Email

				$request->set('cf_4063', $userInfo->get('cf_3341')); // Position
 
        // $request->set('assigned_user_id', $job_info->get('assigned_user_id')); // Assigned to */

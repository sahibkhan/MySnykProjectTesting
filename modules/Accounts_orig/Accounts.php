...
class Accounts extends CRMEntity {
	...
	*/
	function save_module($module) {
		$rec_pdf = $this->id;
		$_SESSION['customer_current_record'] = $this->id;
		global $adb;
		$recordId = $this->id;
		/*
		$accountname = $this->column_fields['accountname'];
		$sql =  "SELECT COUNT(*) as total FROM a_account_buffer_status WHERE accountid = $recordId LIMIT 1";
		$result = $adb->pquery($sql);
		$total = $adb->query_result($result, $i, "total");
		if ($total == 0) {
			$sql_create_new_client = "INSERT INTO a_account_buffer_status (accountid, accountname, axg_status, pro_status, rtd_status, rrs_status, ctd_status, flt_status) VALUES ($recordId, '$accountname', 'No Business', 'No Business', 'No Business', 'No Business', 'No Business', 'No Business')";
			$adb->pquery($sql_create_new_client);
		}
		*/

		// Get consolidated descriptions
		//$this->syncRegionalAccountsWithMasterProfile();	  

		//Handling for invitees
		$selected_users_string = $_REQUEST['selectedusers'];
		 $this->insertIntoInviteeTableAccount($module,$selected_users_string);
	}
	...
	/** Function to insert values in vtiger_invitees table for the specified module,tablename ,invitees_array
 	 ...
 	 */
	  function insertIntoInviteeTableAccount($module,$invitees_array){
		global $log,$adb;
		$log->debug("Entering insertIntoInviteeTableAccount(".$module.",".$invitees_array.") method ...");
		if($this->mode == 'edit'){
			 ...
			 $numRows = $this->db->num_rows($dresult);
				...
				for($i=0; $i < $numRows;$i++){
					...
					if (!in_array($cuser_id,$ivnitees_array)){
						// Exept Dennis Ruiter account, remove all					
		...
		$log->debug("Exiting insertIntoInviteeTableAccount method ...");
	}
	...
	*/
	...
	/**
	* Function to get Account related Tenders
	* @param  integer   $id      - accountid
	* returns related Tanders record in array format
	*/
	function get_tenders($id, $cur_tab_id, $rel_tab_id, $actions=false) {
		global $log, $singlepane_view,$currentModule,$current_user;
		$log->debug("Entering get_tenders(".$id.") method ...");
		$this_module = $currentModule;
		...
		$query = "SELECT case when (vtiger_users.user_name not like '') then $userNameSql else vtiger_groups.groupname end as user_name, vtiger_users.id,
				vtiger_tender.name, vtiger_tender.tenderid AS crmid,
				vtiger_crmentity.smownerid, vtiger_crmentity.modifiedtime
				FROM vtiger_tender
				INNER JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_tender.tenderid
				LEFT JOIN vtiger_tendercf ON vtiger_tender.tenderid = vtiger_tendercf.tenderid
				LEFT JOIN vtiger_users ON vtiger_users.id=vtiger_crmentity.smownerid
				LEFT JOIN vtiger_groups ON vtiger_groups.groupid = vtiger_crmentity.smownerid
				WHERE  vtiger_crmentity.deleted = 0 and (vtiger_tender.accountid = $id)";
		...
		$log->debug("Exiting get_tenders method ...");
		return $return_value;
	}
	...
	*/
		...
		foreach($with_crmids as $with_crmid) {
			...
			elseif($with_module == 'Campaigns') {
				...
				parent::save_related_module($module, $crmid, $with_module, $with_crmid);
				  ...
				  // Master profile synchronizing
				parent::add_masterprofile_crmentityrel('MasterProfiles',$crmid,$with_module,$with_module);
			}
		}
	}
	...
	function syncRegionalAccountsWithMasterProfile() {
		global $adb;
		$accountid = $this->id;
		
		$accountResult = $adb->pquery("SELECT * FROM `vtiger_accountscf` WHERE `accountid` = ?", array($accountid));
		$masterProfileId = $adb->query_result($accountResult, 0, 'cf_3207');	
		if ($masterProfileId){
			$queryAccountFields = "SELECT vtiger_crmentity.description,vtiger_account.accountname,vtiger_accountscf.cf_729,
																		vtiger_accountscf.cf_731,vtiger_accountscf.cf_733,vtiger_accountbillads.bill_country,
																		vtiger_accountbillads.bill_city,vtiger_account.accountid,vtiger_crmentity.smownerid,
																		vtiger_accountscf.cf_6572, vtiger_accountscf.cf_6574, vtiger_accountscf.cf_6576, 
																		vtiger_accountscf.cf_6578, vtiger_accountscf.cf_6580, vtiger_accountscf.cf_6582, 
																		vtiger_accountscf.cf_6584, vtiger_accountscf.cf_6586, vtiger_accountscf.cf_6588
														 ...
														 INNER JOIN vtiger_account ON vtiger_account.accountid = vtiger_crmentity.crmid
													 	 LEFT JOIN vtiger_accountbillads ON vtiger_accountbillads.accountaddressid = vtiger_crmentity.crmid

														 WHERE vtiger_crmentity.deleted = 0 and vtiger_accountscf.cf_3207 = ?";
				$resAccountFields = $adb->pquery($queryAccountFields, array($masterProfileId));
			  $noOfAccounts = $adb->num_rows($resAccountFields);

				$accountsDescriptions = '';
				$vendorRegistrationDetails = '';
				$masterProfileCountry = '';
				$masterProfileCity = '';

				for($i=0; $i < $noOfAccounts;$i++){

					$description = $adb->query_result($resAccountFields,$i,"description");
					$account_name = $adb->query_result($resAccountFields,$i,"accountname");	
					$bill_country = $adb->query_result($resAccountFields,$i,"bill_country");	
					$bill_city = $adb->query_result($resAccountFields,$i,"bill_city");	
					$accountId = $adb->query_result($resAccountFields,$i,"accountid");	
					$smownerid = $adb->query_result($resAccountFields,$i,"smownerid");

					$c_b = $adb->query_result($resAccountFields,$i,"cf_6572");
					$head_office = $adb->query_result($resAccountFields,$i,"cf_6574");
					$c_l = $adb->query_result($resAccountFields,$i,"cf_6576");
					$tender_platform = $adb->query_result($resAccountFields,$i,"cf_6578");
					$preffered_l = $adb->query_result($resAccountFields,$i,"cf_6580");
					$existing_cooperation = $adb->query_result($resAccountFields,$i,"cf_6582");
					$future_business = $adb->query_result($resAccountFields,$i,"cf_6584");
					$last_updated = $adb->query_result($resAccountFields,$i,"cf_6586");
					$comments = $adb->query_result($resAccountFields,$i,"cf_6588");


					$accountsDescriptions .= '<br/><b>'.$account_name.'</b> / '.$bill_city.' <br/>';
					$accountsDescriptions .= $description.'<br/><br/>';


					$accountsDescriptions .= '<br/><b>'.$account_name.'</b> / '.$bill_city.' / Customer Profile Questionnaire <br/><br/>';
					$accountsDescriptions .= '<b>Customer Business:</b>  '.$c_b.'  <br/>';
					$accountsDescriptions .= '<b>Head Office / Decision Maker for CIS/CA  :</b>  '.$head_office.'  <br/>';
					$accountsDescriptions .= '<b>  Customer Logistics Requirements:</b>  '.$c_l.'  <br/>';
					$accountsDescriptions .= '<b>  Tender Platform:</b>  '.$tender_platform.'  <br/>';
					$accountsDescriptions .= '<b>  Preferred Logistics Partner:</b>  '.$preffered_l.'  <br/>';
					$accountsDescriptions .= '<b>  Existing Cooperation  Contact History with GLK:</b> '.$existing_cooperation.'  <br/>';
					$accountsDescriptions .= '<b>  Future Business Development Potential with GLK:</b>  '.$future_business.'  <br/>';
					$accountsDescriptions .= '<b>  Last Updated by/on:</b>  '.$last_updated.'  <br/>';
					$accountsDescriptions .= '<b>  Comments:</b>  '.$comments.'  <br/><br/>';


					// Vendor Registration Details			
					$vendorRegistration	 = $adb->query_result($resAccountFields, $i, "cf_729");
					$vendorNumber = $adb->query_result($resAccountFields, $i, "cf_731");
					$vendorRegistrationValidity = $adb->query_result($resAccountFields, $i, "cf_733");
							
					if (($vendorRegistration) || ($vendorNumber) || ($vendorRegistrationValidity)){
						$vendorRegistrationDetails .= '<br/><b>'.$account_name.'</b> <br/>';
					}
					
					$vendorRegistrationDetails .= $vendorRegistration ? "Vendor Registration: Yes" : "Vendor Registration: No";
					$vendorRegistrationDetails .= $vendorNumber ? "Vendor Number: ".$vendorNumber.'<br/>' : "";
					$vendorRegistrationDetails .= $vendorRegistrationValidity ? 'Vendor registration validity: '.$vendorRegistrationValidity.'<br/>' : "";

					// Country and City consolidating
					$country = $adb->query_result($resAccountFields,$i,"bill_country");
					$city = $adb->query_result($resAccountFields,$i,"bill_city");	

					if ($i == ($numRows - 1)) $slah = ''; else $slah = '/';

					if (!empty($country)){
						$masterProfileCountry .= $adb->query_result($resAccountFields,$i,"bill_country").$slah;
					}

					if (!empty($city)){
						$masterProfileCity .= $adb->query_result($resAccountFields,$i,"bill_city").$slah;
					}

					$this->addInviteeUserToMasterProfile($accountId, $masterProfileId);

					// Adding in CC "Assigned to" people
					$this->addAccountCreatorToInviteeList($masterProfileId, $smownerid);

				}

				$queryUpdateMasterProfile = "UPDATE `vtiger_masterprofilescf` SET 
																						cf_3205 = '$accountsDescriptions',
																						cf_3903 = '$vendorRegistrationDetails',
																						cf_3187 = '$masterProfileCountry',
																						cf_3189 = '$masterProfileCity' 
																						WHERE `masterprofilesid` = ?";
				$adb->pquery($queryUpdateMasterProfile, array($masterProfileId));

		} // end of master profile existing
		
	}


	public function addAccountCreatorToInviteeList($accountId, $masterProfileId){
		global $adb;
					
		$s_invitePeopleAssignedTo = "SELECT * FROM `vtiger_invitees` WHERE `activityid` = ? AND inviteeid = ?";
		$r_inviteMPAssignedTo = $adb->pquery($s_invitePeopleAssignedTo, array($masterProfileId, $smownerid));
		$n_inviteeMPAssignedTo = $adb->num_rows($r_inviteMPAssignedTo);
		if ($n_inviteeMPAssignedTo == 0){
			
			$s_insertInvite = "INSERT INTO `vtiger_invitees` (`activityid`, `inviteeid`) 
									VALUES ($masterProfileId, $smownerid);";
			$adb->pquery($s_insertInvite);					 
		}
	...
	public function addInviteeUserToMasterProfile($accountId, $masterProfileId){
		global $adb;
		$resultInvitePeople = $adb->pquery("SELECT * FROM `vtiger_invitees` WHERE `activityid` = ?", array($accountId));
		$noOfinvitee = $adb->num_rows($resultInvitePeople);

		for($j=0; $j < $noOfinvitee; $j++){
			$inviteeId = $adb->query_result($resultInvitePeople, $j, "inviteeid");
 
				$resInviteeMasterProfile = $adb->pquery("SELECT activityid
																		FROM `vtiger_invitees`
																		WHERE `activityid` = ? AND inviteeid = ?", array($masterProfileId, $inviteeId));
				$noOfInvitees = $adb->num_rows($resInviteeMasterProfile);
				if ($noOfInvitees == 0){
					$adb->pquery("INSERT INTO `vtiger_invitees` (`activityid`, `inviteeid`) VALUES ($masterProfileId, $inviteeId);");					 
				}
		}
	}
		 ...
		 /*
				This is function is handling messages for all accounts
		 */
 ...
 */
			...
			// Including additional API for data diffirence		
			...
			$account = Vtiger_Record_Model::getInstanceById($record, 'Accounts');
			$users = Users_Record_Model::getInstanceById($account->get('assigned_user_id'), 'Users');
			$creator_id = $account->get('assigned_user_id');
			...
			//$who_updated_id = get_crmentity_details($record,'modifiedby')
/* 			echo "<pre>";
			print_r($modifiedby);
			exit; */
			

			$updatedByInfo = Users_Record_Model::getInstanceById($account->get('modifiedby'), 'Users');
			$who_updated_email = $updatedByInfo->get('email1');
			$who_updated_name = $users->get('first_name') . ' ' . $users->get('last_name');
			$subject = $account->get('accountname');	  
			$isconverted = $account->get('isconvertedfromlead');
			
											// #2   Gathering Message Body Details
		
						// Gathering history details
			$all_field_details = $this->get_customer_history($record,'');			
			$cc .=  $users->get('email1').',';

											// #3   Gathering people to put in CC	
						
						// Gathering Invited People
			$cc .= $this->arrange_invite_users($record);
			
											// Adding Pasha in copy for Alevtina's update 
			if ($creator_id == 5) $cc .= 'p.karim@globalinklogistics.com,';	
			
									// Subject changing to master profile's
									
			$masterProfileId = $account->get('cf_3207');
			if ($masterProfileId > 0){
				$cc .= $this->arrange_invite_users($masterProfileId);
				
				// $masterProfile = Vtiger_Record_Model::getInstanceById($masterProfileId, 'MasterProfiles');
				// $masterProfileId = $masterProfile->get('record_id');
			}

						// Gathering Event Status (Just created or updated);					
			$event_status = $this->detect_message_status($record);
			if ($event_status == 1) {
				
				if ($isconverted == "1") {
					$message_status = "Please be advised , that lead was converted to account with profile for $subject";
				}
				else {
					
											// #4 Add in copy Ayakoz, Alevtina, Zhanna or Sohail	  
				$acc_type = $account->get('accounttype');
				if ($acc_type == 'Customer') $cc .= 'a.pominova@globalinklogistics.com,k.tursyngazy@globalinklogistics.com,a.alekpayeva@globalinklogistics.com,'; else 
				if ($acc_type == 'Agent') $cc .= 'z.kaynarbaeva@globalinklogistics.com,k.tursyngazy@globalinklogistics.com,'; else
				if ($acc_type == 'Vendor') $cc .= 'k.tursyngazy@globalinklogistics.com, m.makhmutov@globalinklogistics.com,';
				
				$message_status = "Please note that an account was created for $subject.";
				}
			}
			else {
			if ($event_status > 1) 
				$message_status = "Please note that an account was updated for $subject"; 	  
			}
			$arranged_details = $this->arrange_customerfetch_details($record,$assigned_users,$all_field_details,$event_status);	
				
			$creator_email =  $users->get('email1');
			$this->send_customer_notification($creator_email,$creator_name,$who_updated_email,$who_updated_name,$module,$record,$subject,$message_status,$arranged_details,$cc);
			 
	}



	// SHIFT TO GENERAL CLASS

	// Get invitee users
	public function arrange_invite_users($record){
		global $adb;	
		$cc = '';

		$sqlInvitees = "  SELECT vtiger_invitees.inviteeid, vtiger_users.email1
		FROM `vtiger_invitees`
		LEFT JOIN vtiger_users ON vtiger_users.id = vtiger_invitees.inviteeid
		WHERE vtiger_users.status = 'Active' AND vtiger_invitees.`activityid` = ?
		";
		$resultInvitees = $adb->pquery($sqlInvitees, array($record));
		While ($row = $adb->fetch_array($resultInvitees)){
...
public function detect_message_status($record){
	global $adb;
	$sqlModtracker = "SELECT * FROM `vtiger_modtracker_basic` where `crmid` = ?";
	$resultModtracker = $adb->pquery($sqlModtracker, array($record));  
	return $adb->num_rows($resultModtracker);
}



public function send_customer_notification($creator_email,$creator_name,$who_updated_email,$who_updated_name,$module,$record,$subject,$message_status,$sum,$cc){
	global $adb;	


 
	// $link .= arrange_correct_link($record,$module,'notification');
	//$link .= "http://75.101.161.219/index.php?module=Accounts&view=Detail&record=$record&app=SALES";
	global $site_URL;
	$link = $site_URL;
	$link .= "/index.php?module=Accounts&view=Detail&record=$record&app=SALES";
	$to = $creator_email;	
	...
	$body .= "<tr><td colspan=2> $message_status </td></tr>
			 $sum
			<tr><td colspan=2>Link: <a href='$link'> Link to customer </a></td></tr>";
	...
	parent::controlEmailSending($to,$cc,$who_updated_email,$subject,$body,$headers,$link,$record);
 
}


public function arrange_customerfetch_details($record,$assigned_users,$all_field_details,$event_status){
	$table = '';
	global $adb;
/* 	ini_set('display_errors', 1);
	...
	if ($n > 0){
		if ($event_status == 1) $lbl = 'Customer'; else $lbl = 'Updated';
		...
		$prevalue = $all_field_details[$i][3];
			...
			if ($label == 'Description'){
				 ...
				 $label = 'Regional profile description:';
				 $value = $this->hightlight_updated_description_3($prevalue,$value);
			}
		...
		if ($label == 'Modifiedby'){
			...
			$value = $r_user['first_name'].' '.$r_user['last_name']; 
				...
				//	$login = get_user_details($value,3);
					//$users = Users_Record_Model::getInstanceById($value, 'Users');
					//$login = $users->get('username');
					//$value = arrange_user_format($login, 1);
		}
		
		if ($label == 'Accountname') $label = 'Customer name';  
		if ($label == 'Cf_725') $label = 'Relationship status';  
		if ($label == 'Cf_721') $label = 'Location';  
		if ($label == 'Cf_727') $label = 'Global account'; 
		...
		if ($label == 'Cf_3207'){
			//if ($value) $s_t = 'yes'; else $s_t = 'no';
				if ($value){
					$label = 'Master Profile';
					$m_p = $this->get_masterprofile_details_acc($value);
					$value = $m_p['name'];
			} else {
			 $label = 'Master Profile';
			 $value = '-';
		...
		// 	IF account type has been changed, then reset approval fields
	//	if ($label == 'Accounttype') updateAccountApprovalData($record);
 
		
		if ($label == 'Cf_731')  $label = 'Vendor number';	  
		if ($label == 'Cf_733') $label = 'Vendor registration validity';
		if ($label == 'Bill_street')$label = 'Address';	  
		if ($label == 'Bill_city')  $label = 'City';	  
		if ($label == 'Bill_state') $label = 'State';	  
		if ($label == 'Bill_code')$label = 'Postal code';	  
		if ($label == 'Bill_country') $label = 'Country';  
		if ($label == 'Bill_pobox') $label = 'PO box';	  
		if ($label == 'Email1') $label = 'E-mail';	  
		if ($label == 'Cf_1829') $label = 'Network';
		if ($label == 'Cf_1831') $label = 'Black list';
		if ($label == 'Cf_1833') $label = 'Bank Name';
		if ($label == 'Cf_1835') $label = 'Account No.';
		if ($label == 'Cf_1837') $label = 'Bank Address';
		if ($label == 'Cf_1839') $label = 'Credit Limit Required ';
		if ($label == 'Cf_1841') $label = 'Bank Country';
		if ($label == 'Cf_1843') $label = 'Terms and Conditions ';
		if ($label == 'Cf_1845') $label = 'Bank City';
		if ($label == 'Cf_1847') $label = 'Credit Terms (days)';
		if ($label == 'Cf_1849') $label = 'Bank State';
		if ($label == 'Cf_1851') $label = 'Credit limit';
		if ($label == 'Cf_1853') $label = 'Agency agreement';
		if ($label == 'Cf_1855') $label = 'Payment terms';
		if ($label == 'Cf_1857') $label = 'Agency Number';
		if ($label == 'Cf_1859') $label = 'Agency agreement date';
		if ($label == 'Cf_1861') $label = 'Agency agreement validity'; 
		if ($label == 'Cf_2395') $label = 'Legal Name'; 
		if ($label == 'Cf_2397') $label = 'BIC';
		if ($label == 'Cf_2399') $label = 'PPN';
		if ($label == 'Cf_2403') $label = 'Company Status';
		if ($label == 'Cf_2405') $label = 'KBE';
		if ($label == 'Cf_2407') $label = '1C Code';
		if ($label == 'Cf_2429') $label = 'SWIFT Code';  
		if ($label == 'Cf_4913') $label = 'Type of KLN services required';  
		if ($label == 'Cf_6250') $label = 'Network Name';  
		
		if ($label == 'Cf_5155') $label = 'FLT Services';    
		if ($label == 'Cf_4785') $label = 'AXG Services';    
		if ($label == 'Cf_4787') $label = 'PRO Services';
		
		if ($label == 'Cf_4789') $label = 'RTD Services';	    
		if ($label == 'Cf_4791') $label = 'RTD Services';	    
		if ($label == 'Cf_4793') $label = 'CTD Services';  	  
		
		if ($label == 'Cf_5840') $label = 'Transport modes';  
		if ($label == 'Cf_5836') $label = 'Import directions';
		if ($label == 'Cf_5838') $label = 'Export directions';

		if ($label == 'Cf_6918') $label = 'Import directions';
		if ($label == 'Cf_6920') $label = 'Export directions';
		if ($label == 'Cf_6012') $label = 'Approved On';
		 
		if ($label == 'Cf_6814'){
			//	$userProfile = get_userlist_details($value);
				//$value = $userProfile['name'];
				$label = 'Regional Sales';
		}	

		if ($label == 'Cf_6572') $label = 'Customer Business';	  
		if ($label == 'Cf_6574') $label = 'Head Office / Decision Maker for CIS/CA';	  
		if ($label == 'Cf_6576') $label = 'Customer Logistics Requirements';	  
		if ($label == 'Cf_6578') $label = 'Customer RFQ System';	  
		if ($label == 'Cf_6580') $label = 'Preferred Logistics Partner ';	  
		if ($label == 'Cf_6582') $label = 'Existing Cooperation / Contact History with GLK';	  
		if ($label == 'Cf_6584') $label = 'Future Business Development Potential with GLK ';	  
		if ($label == 'Cf_6586') $label = 'Last Updated by/on ';	  
		if ($label == 'Cf_6588') $label = 'Comments';	  
		...
		// Consolidated descriptions  
		 ...
		 // $masterProfileId = get_accountcf_details($record,'cf_3207');	
 		 $accInfo = Vtiger_Record_Model::getInstanceById($record, 'Accounts');
		 $masterProfileId = $accInfo->get('cf_3207');
		 
 					// Gathering history details
		  // $mp_description_details = $this->get_customer_history($record, 'cf_3205');
		  // $all_field_account = $this->get_customer_history($record, '');
			$mp_details = $this->get_masterprofile_details_acc($masterProfileId);
		
		if ($masterProfileId > 0){
				$value = $mp_details['cf_3205'];	
...
/* 		$n = count($assigned_users);
	if ($n > 0){
		$table .= "<tr><td colspan='2'> <b> Updates are provided to </b> </td> </tr>";
		for($i = 1; $i <= $n; $i++){
...
function hightlight_updated_description_3($from_text,$to_text){
...
public function get_customer_history($record,$history_field){
		 global $adb;
		// Excluding already mentioned fields
		$exclude_field = "'cf_861','account_no','record_id','createdtime','assigned_user_id','record_module','cf_1703'";
		

		$sqlModtracker = "SELECT max(`id`) as `maxid` FROM `vtiger_modtracker_basic` where `crmid` = ?";
		$resultModtracker = $adb->pquery($sqlModtracker, array($record));
		$max_id =  $adb->query_result($resultModtracker, 0, 'maxid');
	...
	// Description field in Master profile
		if ($history_field == 'cf_3205'){
	 			$query_details = $adb->pquery("SELECT * FROM `vtiger_modtracker_detail` where `id` = $maxid and `fieldname`='cf_3205'");
				$details = $adb->fetch_array($query_details);
				$nMp = $adb->num_rows($query_details);
				$array_history = array();

				if ($nMp > 0){
					$array_history[1][1] = $details['fieldname'];
					$array_history[1][2] = $details['postvalue'];
					$array_history[1][3] = $details['prevalue'];
	...
	if ($history_field == ''){
		...
		$array_history = array();
		$sqlModtracker = "SELECT * FROM `vtiger_modtracker_detail` WHERE `id`= $max_id AND (`fieldname` NOT IN ($exclude_field))";
		$resultModtracker = $adb->pquery($sqlModtracker);
		While ($row = $adb->fetch_array($resultModtracker)){
			$i ++;
			$array_history[$i][1] = $row['fieldname'];
			$array_history[$i][2] = $row['postvalue']; 	
			$array_history[$i][3] = $row['prevalue'];	
		}
		$history = $array_history;
...
public function get_crmentity_details($crmid){
		global $adb;
		$sqlCrmEntity = "SELECT * FROM `vtiger_crmentity` where `crmid`=?";
		$resultCrmEntity = $adb->pquery($sqlCrmEntity, array($crmid));
		$row = $adb->fetch_array($resultCrmEntity);
		return $row;
}



// Get Master profile details

public function get_masterprofile_details_acc($crmid){
	global $adb;
	$sqlCrmEntity = "	SELECT * FROM `vtiger_masterprofiles` mp
	INNER JOIN  `vtiger_masterprofilescf` mp_cf ON mp.masterprofilesid = mp_cf.masterprofilesid	
	where mp.masterprofilesid = ? ";
	$resultCrmEntity = $adb->pquery($sqlCrmEntity, array($crmid));
	$row = $adb->fetch_array($resultCrmEntity);
	return $row;
}

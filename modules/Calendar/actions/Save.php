...
class Calendar_Save_Action extends Vtiger_Save_Action
{
	...
	public function handleEmailNotification($record)
	{
		...
		// Adding owner of VR/CR in invite list
		$currentUserModel = Users_Record_Model::getCurrentUserModel();
		$currentUserId = $currentUserModel->id;

		$queryCalendar = $adb->pquery("SELECT smcreatorid 
														FROM  vtiger_crmentity
														WHERE setype = ? AND crmid = ? AND smcreatorid = ? AND smownerid <> ?", array('Calendar', $record, $currentUserId, $currentUserId));
		$countOfCalendarRecords = $adb->num_rows($queryCalendar);
		if ($countOfCalendarRecords > 0) {
			$queryInvitees = $adb->pquery("SELECT smcreatorid
																		 FROM vtiger_invitees
																		 WHERE activityid = ? AND inviteeid = ?", array($record, $currentUserId));
			$countOfInvetees = $adb->num_rows($queryInvitees);
			if ($countOfInvetees == 0) {
				$adb->pquery("INSERT INTO vtiger_invitees(`activityid`, `inviteeid`, `status`) VALUES ($record, $currentUserId, 'sent')");
			}
		}

		// #1    Gathering Basic details for email			
		$calendar_info = Vtiger_Record_Model::getInstanceById($record, 'Calendar');
		if ($calendar_info->get('parent_id')) $related_account_info = Vtiger_Record_Model::getInstanceById($calendar_info->get('parent_id'));
		...
		if (($activity_fld_value == 'Meeting') || ($activity_fld_value == 'Call')) {
			...
			// Message title 
			if ($eventstatus != 'Held') {
				$subject = ucfirst($event_type) . ' ' . $subject;
				if ($related_account_info) {
					$subject = ucfirst($event_type) .' for ' . $related_account_info->get('accountname') . '_' . date('d-m-y', strtotime($calendar_info->get('createdtime')));
				}
		...
		} else if ($activity_fld_value == 'Task') {
	...
	function get_calendar_invited_RFQ($record, $mode)
	{
		...
		for ($i = 0; $i < $numRows; $i++) {
			...
			$n++;
			$res_inner =  $adb->pquery("SELECT * FROM `vtiger_users` where status = 'Active' AND `id`=" . $inviteeid);

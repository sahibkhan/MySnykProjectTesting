...
Class Calendar_Edit_View extends Vtiger_Edit_View {
	...
	function Events($request, $moduleName) {
		...
		$record = $request->get('record');
		 ...
		 if(!empty($record) && $request->get('isDuplicate') == true) {
			...
			if ($sourceModule == 'Accounts' && !empty($sourceRecord)){
				$customerProfileQuestionnaireDetails = $this->getCustomerProfileQuestionnaireDetails($sourceRecord);

				$request->set('cf_6566', $customerProfileQuestionnaireDetails['customerBusiness']);
				$request->set('cf_6568', $customerProfileQuestionnaireDetails['headOffice']);
				$request->set('cf_6570', $customerProfileQuestionnaireDetails['customerLogisticsRequirement']);

				$request->set('cf_6590', $customerProfileQuestionnaireDetails['customerRfqSystem']);
				$request->set('cf_6592', $customerProfileQuestionnaireDetails['preferredLogisticsPartner']);
				$request->set('cf_6594', $customerProfileQuestionnaireDetails['existingCooperation']);

				$request->set('cf_6596', $customerProfileQuestionnaireDetails['futureBusiness']);
				$request->set('cf_6598', $customerProfileQuestionnaireDetails['lastUpdated']);
				$request->set('cf_6600', $customerProfileQuestionnaireDetails['comments']);
			}
	...
	public function getCustomerProfileQuestionnaireDetails($recordId){
		global $adb;
		$data = array();

    $queryAccount = $adb->pquery("SELECT vtiger_accountscf.cf_6572, vtiger_accountscf.cf_6574, vtiger_accountscf.cf_6576, vtiger_accountscf.cf_6578, vtiger_accountscf.cf_6580,
																				 vtiger_accountscf.cf_6582, vtiger_accountscf.cf_6584, vtiger_accountscf.cf_6586, vtiger_accountscf.cf_6588
																	FROM `vtiger_crmentity`
																	INNER JOIN vtiger_accountscf ON vtiger_accountscf.accountid = vtiger_crmentity.crmid
																	WHERE vtiger_crmentity.deleted = 0 AND vtiger_crmentity.`crmid` = ?", array($recordId));

    $customerBusiness = trim($adb->query_result($queryAccount, 0, 'cf_6572'));
    $headOffice = trim($adb->query_result($queryAccount, 0, 'cf_6574'));
    $customerLogisticsRequirement = trim($adb->query_result($queryAccount, 0, 'cf_6576'));

		$customerRfqSystem = trim($adb->query_result($queryAccount, 0, 'cf_6578'));
		$preferredLogisticsPartner = trim($adb->query_result($queryAccount, 0, 'cf_6580'));
		$existingCooperation = trim($adb->query_result($queryAccount, 0, 'cf_6582'));

		$futureBusiness = trim($adb->query_result($queryAccount, 0, 'cf_6584'));
		$lastUpdated = trim($adb->query_result($queryAccount, 0, 'cf_6586'));
		$comments = trim($adb->query_result($queryAccount, 0, 'cf_6588'));
		
		$data['customerBusiness'] = $customerBusiness;	
		$data['headOffice'] = $headOffice;	
		$data['customerLogisticsRequirement'] = $customerLogisticsRequirement;

		$data['customerRfqSystem'] = $customerRfqSystem;
		$data['preferredLogisticsPartner'] = $preferredLogisticsPartner;
		$data['existingCooperation'] = $existingCooperation;
		
		$data['futureBusiness'] = $futureBusiness;

<?php
  ...
  //$file_title_currency = 'KZT';		
  	...
  	$b_buy_vendor_currency_net = $_REQUEST['b_buy_vendor_currency_net']; 
	...
	//for VAT Amount
	$vat_included = $_REQUEST['vat_included'];
	if($vat_included=='VAT Amount')
	{
		$b_vat = $_REQUEST['b_vat_amount'];
	}
	else{
	...
	if(!empty($b_buy_vendor_currency_net))
	{
		...
		$buy_vendor_currency_net['b_vat']  = number_format($b_vat, 2, '.', '');
		$buy_vendor_currency_net['b_buy_vendor_currency_gross']  = number_format($b_buy_vendor_currency_gross, 2, '.', '');			
		$buy_vendor_currency_net['b_exchange_rate']	 = number_format($b_exchange_rate, 2, '.', '');
		$buy_vendor_currency_net['b_buy_local_currency_gross']  = number_format($b_buy_local_currency_gross, 2, '.', '');
		$buy_vendor_currency_net['b_buy_local_currency_net']  = number_format($b_buy_local_currency_net, 2, '.', '');			
		$buy_vendor_currency_net['b_variation_expected_and_actual_buying']  = (empty($b_expected_buy_local_currency_net)?0:number_format($b_variation_expected_and_actual_buying, 2, '.', ''));
		$buy_vendor_currency_net['b_expected_buy_local_currency_net'] = (empty($b_expected_buy_local_currency_net)?0:number_format($b_expected_buy_local_currency_net,2,'.',''));
		
		
		
	}
   echo json_encode($buy_vendor_currency_net);
?>
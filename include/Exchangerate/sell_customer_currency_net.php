<?php
	...
	global $adb;
    $sell_customer_currency_net =array();
	...
	$s_selling_customer_currency_net = $_REQUEST['s_selling_customer_currency_net']; 
	...
	//for VAT Amount
	$s_vat_included = $_REQUEST['s_vat_included'];
	if($s_vat_included=='VAT Amount')
	{
		$s_vat	  = $_REQUEST['s_vat_amount'];
	}
	else{
	...
	if(!empty($s_selling_customer_currency_net))
	{
			...
			$sell_customer_currency_net['s_vat']  = number_format($s_vat, 2, '.', '');
			$sell_customer_currency_net['s_selll_customer_currency_gross']  = number_format($s_sell_customer_currency_gross, 2, '.', '');			
			$sell_customer_currency_net['s_exchange_rate']	 = number_format($s_exchange_rate, 2, '.', '');
			$sell_customer_currency_net['s_sell_local_currency_gross']  = number_format($s_sell_local_currency_gross, 2, '.', '');
			$sell_customer_currency_net['s_sell_local_currency_net']  = number_format($s_sell_local_currency_net, 2, '.', '');			
			$sell_customer_currency_net['s_variation_expected_and_actual_selling']  = number_format($s_variation_expected_and_actual_selling, 2, '.', '');
			echo json_encode($sell_customer_currency_net);
	}
	 echo FALSE;

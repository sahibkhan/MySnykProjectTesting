...
$oneCLog = "1C cron Job Started".'<br>';
...
*/
		...
		if ( $db->num_rows($query_job) ) {
			...
			$oneCLog .= "Total jobs: ".$db->num_rows($query_job)."<br>";
			//$recordIds  = $request->get('selected_ids');

			while ($job_rs = $db->fetch_array($query_job)) {
				$oneCLog .= "Job ID: ".$job_rs['jobid']."<br>";
...
//die();
				...
				*/
				...
				$oneCLog .= "Array to submit<br>";
				$oneCLog .= print_r($jobRecord,true)."<br>";
					...
					*/	
					...
					$con1C = array( 'login' => 'AdmWS', //AdmWS Live
						...
						if (!function_exists('is_soap_fault')) {
							...
							$oneCLog .= "not found module php-soap.<br>";
							return false;
						}		
						...
						try {
							...
							var_dump($e);
							$oneCLog = print_r($e, true)."<br>";
							echo '<br>error at connecting to 1C<br>';
							$oneCLog .= "error at connecting to 1C<br>";
							return false;
						}
						if (is_soap_fault($Client1C)){
							echo '<br>inner server error at connecting to 1C<br>';
							$oneCLog .= "inner server error at connecting to 1C<br>";
							return false;
						}
						...
						if (is_object($idc)) {
							try {
							  ...
							  $oneCLog .= "Before CreateJobFile<br>";

							  $ret1c = $idc->CreateJobFile($par);

							  $oneCLog .= "CreateJobFile return<br>";

							  $oneCLog .= print_r($ret1c,true)."<br>";
							  ...
							  if ($ret1c->return == $ref) {
								  ...
								  $oneCLog .= "Db updated after CreateJobFile<br>";
								...
								//print_r($e);
								//echo 'Failed_';
								$oneCLog .= "LBL_JOBFILE_FAILED: ".vtranslate('LBL_JOBFILE_FAILED', $moduleName)."__".print_r($e,true)."<br>";
							...
							$oneCLog .= "LBL_CONNECTION_TO_1C: ".vtranslate('LBL_CONNECTION_TO_1C', $moduleName)."__".print_r($idc,true)."<br>";
		...
		$oneCLog .= "No Record Found<br>";
	}

	//$db->disconnect();
	//mysqli_close($db->database->_connectionID);

	$oneCLog .= "1C cron Job Ended";
...
/*
	...
	//$emailData = GET_EMAIL_TEMPLATECONTENT_BY_TEMPLATE_ID();
	$subject = "1C cron job status";
	$contents = $oneCLog;
	$to_emailid = "a.sarwar@globalinklogistics.com";

	send_mail('Contacts', $to_emailid, $HELPDESK_SUPPORT_NAME, $HELPDESK_SUPPORT_EMAIL_ID, $subject, $contents,'','','','','',true);
*/

	include_once '/var/www/html/live-gems/vtlib/Vtiger/Mailer.php';
	//include_once '../vtlib/Vtiger/Mailer.php';
	global $HELPDESK_SUPPORT_EMAIL_ID;
	$mailer = new Vtiger_Mailer();
	$From = "s.mehtab@globalinklogistics.com";
	$mailer->IsHTML(true);
	$mailer->ConfigSenderInfo($From);
	$mailer->Subject ="Testing For Approval";
	$mailer->Body = "Testing ";
	$mailer->AddAddress("a.sarwar@globalinklogistics.com");
	
	//echo $status = $mailer->Send(true);
	echo $status = $mailer->send(true); 	
	exit;

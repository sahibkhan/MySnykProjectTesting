 ...
 $query_jobcf = "SELECT vtiger_jobcf.jobid, cf_1186 as file_title, cf_1188 as location, cf_1190 as department, cf_1198 as ref_no, smcreatorid, smownerid 
  ...
  while($job_info_detail = mysql_fetch_array($rs_job))
  {
	 ...
	 if($count_last_modified>0)
	 {
		...
		$deviation_cost = ($entries['actual_expense_cost_usd'] - $entries['expected_cost_usd'])/$entries['actual_expense_cost_usd'];
		$deviation_revenue = ($entries['actual_selling_cost_usd'] - $entries['expected_revenue_usd'])/$entries['actual_selling_cost_usd'];

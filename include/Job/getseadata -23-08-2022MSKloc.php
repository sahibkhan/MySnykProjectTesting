...
//Fetch value for fill read only input on the base of  Airport Name  select Qry
   if(!empty($_POST["port_unlocode"])) {
	   $sql = "select * FROM sea_codes WHERE port_city_code='".$_POST["portcitycode"]."' && port_country_code='".$_POST["portcountrycode"]."' &&  port_unlocode like '%".$_POST["port_unlocode"]."%'";
	    $res = $db->getData($sql);
...
//Fetch value for fill read only input on the base of  dis unloccode  select Qry
   if(!empty($_POST["dis_port_unlocode"])) {
	    $sql = "select * FROM sea_codes WHERE port_city_code='".$_POST["disport_city_code"]."' && port_country_code='".$_POST["disportcountrycode"]."' && port_unlocode like '%".$_POST["dis_port_unlocode"]."%'";
	    $res = $db->getData($sql);
	    echo json_encode($res);  	 

    ...
    foreach($master_job_file_arr_final as $key => $value)
    {
			...
			$db_fuel_expenses = PearDatabase::getInstance();
			 $query_fuel_expenses = 'SELECT 	vtiger_jobexpencereportcf.jobexpencereportid,vtiger_jobexpencereportcf.cf_1367 ,sum(vtiger_jobexpencereportcf.cf_1337) as b_buy_vendor_currency_net, 
							...
							AND vtiger_jobexpencereportcf.cf_1457="Expence" AND vtiger_jobexpencereportcf.cf_1453 =85896 AND 
							 vtiger_jobexpencereportcf.cf_1345 <> ""
				...
				// // echo $percentage_per_job;
				// // echo "<br>";
			 //   echo $jer_fuel_expenses['currencyTitle'];
      ...
      //    exit;
		...
		//$jobCostingJRfid = 121141;
		//$jobCosting_info_detail = Vtiger_Record_Model::getInstanceById(121141, $sourceModule_JER);
		...
		$params_vtiger_crm_entity= array($current_id, $job_info->get('assigned_user_id'), $job_info->get('assigned_user_id'), 'JobCosting', 'NULL', '', '', 1, 0,'Fuel Expense');					
		...
		$jobcosting_jer_insert_query = "INSERT INTO vtiger_jer(jerid, name, accountid) VALUES(?,?,?)";
		$params_jobcosting_jer= array($current_id, $job_info->get('cf_1198'), $job_info->get('assigned_user_id'));					
		...
		$jobcosting_jercf_insert_query = "INSERT INTO vtiger_jercf(jerid, cf_1024, cf_1026, cf_1028, cf_1154, cf_1156, cf_1158, cf_1160, cf_1162, cf_1164, cf_1166, cf_1168, cf_1170, cf_1176, cf_1433, cf_1435,cf_1443,cf_1451,cf_6350,cf_6352,cf_6354,cf_6356) 
												VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?,?)";
		$params_jobcosting_jercf = array($current_id,0 , 0, 0,$fuelExpJob_vendor_currency ,$jer_fuel_expenses['currencytitle'],$jer_fuel_expenses['exchangeRate'], $fuelExpJob_local_local_currency,0 ,0, 0, 0, 0, Null, $jer_fuel_expenses['location'], $jer_fuel_expenses['department'],0 ,$jer_fuel_expenses['fuel_type'], $jer_fuel_expenses['b_vat_rate'],$fuelExpJob_local_currency_cross, 0, 0);

   ...
   if(!empty($_POST["air_port_code_iata"]))
    {
        ...
        $sql = "select DISTINCT * From air_sea_codes WHERE port_country_code='".$_POST["country"]."' && port_city_code='".$_POST["GenralCitySelect_CODE"]."'";
	    $res = $db->getData($sql);
	     echo json_encode($res); 
     ...
     if(isset($_POST['id']))
			   ...
			   echo  $query_delete =  $db->getData("DELETE FROM `job_container_type_size`  WHERE `id`  = $deletedID");
	}

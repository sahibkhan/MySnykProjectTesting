...
//General Origin States select on the base General Origin Country only for USA and Canada 
   if(!empty($_POST["country_states"])) {
	   ...
	   $sql = "select DISTINCT state_code,state_name FROM air_sea_codes WHERE port_country_code='".$_POST["country_states"]."' limit 20";
	   $res = $db->getData($sql);
...
//select General Origin Cities on the base of General Origin Country instead of USA and Canada 
   ...
   if(!empty($_POST["country_id"])) {
	    ...
	    $sql = "select DISTINCT port_city_code,port_city From air_sea_codes WHERE port_country_code='".$_POST["country_id"]."' LIMIT 20";
	    $res = $db->getData($sql);
	    $json_data = json_encode($res);
	    //var_dump($json_data);?>
	    <option   value="">Select An Option</option>
...
//Select General Origin City on the base of General Origin States of USA and Canada
   if(!empty($_POST["origin_state_code"])) {
	  $sql = "select DISTINCT port_city_code,port_city From air_sea_codes WHERE state_code ='".$_POST["origin_state_code"]."'  && port_country_code='".$_POST["port_country_code"]."' limit 20";
	    $res = $db->getData($sql);?>
	  <option value="">Select an Option</option>
	      <?php
     ...
     if(!empty($_POST["air_origin_state_code_general"])) {
	   $sql = "select DISTINCT state_code,state_name From air_sea_codes WHERE  port_country_code='".$_POST["port_country_code"]."'  limit 20";
	    $res = $db->getData($sql);?>
	      <?php
...
//Air Origin AND Destination Country Select on the base of General Origin AND Destination Country 
   if(!empty($_POST["General_Code"]) ) {
   	 ...
   	 $sql = "select DISTINCT port_city_code,port_city From air_sea_codes WHERE  port_country_code='".$_POST["Origin_Country_Code"]."' LIMIT 20";
	    $res = $db->getData($sql);
	    $json_data = json_encode($res);
	    //var_dump($json_data);?>
	     <option value="">Select an Option</option>
...
//Destination
   ...
   if(!empty($_POST["Dest_General_Code"])) {
	    ...
	    foreach ($res as $countries) {?>
   	  ...
   	  $sql = "select DISTINCT port_city_code,port_city From air_sea_codes WHERE  port_country_code='".$_POST["Dest_Destination_Code"]."'";
	    $res = $db->getData($sql);
	    $json_data = json_encode($res);
	    //var_dump($json_data);?>
   ...
   if(!empty($_POST["GenralCitySelect"])) {
   	if($_POST["job_origin_states"]!=''){
   		$sql = "select DISTINCT port_city_code,port_city From air_sea_codes WHERE  port_country_code='".$_POST["port_country_code"]."' &&  state_code ='".$_POST["job_origin_states"]."'   LIMIT 20";
   	}else{
	   $sql = "select DISTINCT port_city_code,port_city From air_sea_codes WHERE  port_country_code='".$_POST["port_country_code"]."' LIMIT 20";
     	}
	    $res = $db->getData($sql);
	    $json_data = json_encode($res);
	    //var_dump($json_data);?>
...
//Origin Airport Name  select Qry
   if(!empty($_POST["AirportName_CODE"])) {
	   ...
	   foreach($res as $airportName) {?>
				<option   value="<?php echo $airportName['airport_code'];?>"  <?php if($_POST["AirportName_CODE"]==$airportName["city_code"]) {?> SELECTED <?php }?> ><?php echo  $airportName["airport_name"]; ?></option>
		   <?php
	        } 
    }else {

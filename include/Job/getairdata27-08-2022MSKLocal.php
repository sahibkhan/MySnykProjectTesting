...
//General Origin States select on the base General Origin Country only for USA and Canada 
   if(!empty($_POST["country_states"])) {
	   ...
	   $sql = "select DISTINCT state_code,state_name FROM air_sea_codes WHERE port_country_code='".$_POST["country_states"]."' limit 50";
	   $res = $db->getData($sql);
...
//select General Origin Cities on the base of General Origin Country instead of USA and Canada 
   ...
   if(!empty($_POST["country_id"])) {
	    ...
	    $sql = "select DISTINCT port_city_code,port_city From air_sea_codes WHERE port_country_code='".$_POST["country_id"]."'";
	    $res = $db->getData($sql);
	    $json_data = json_encode($res);
	    //var_dump($json_data);?>
	    <option   value="">Select An Option</option>
...
//Select General Origin City on the base of General Origin States of USA and Canada
   if(!empty($_POST["origin_state_code"])) {
	  $sql = "select DISTINCT port_city_code,port_city From air_sea_codes WHERE state_code ='".$_POST["origin_state_code"]."'  && port_country_code='".$_POST["port_country_code"]."'";
	    $res = $db->getData($sql);?>
	  <option value="">Select an Option</option>
	      <?php
...
//Air Origin AND Destination Country Select on the base of General Origin AND Destination Country 
   if(!empty($_POST["General_Code"]) ) {
   	 ...
   	 $sql = "select DISTINCT port_city_code,port_city From air_sea_codes WHERE  port_country_code='".$_POST["Origin_Country_Code"]."'";
	    $res = $db->getData($sql);
	    $json_data = json_encode($res);
	    //var_dump($json_data);?>
	     <option value="">Select an Option</option>
...
//Destination
   ...
   if(!empty($_POST["GenralCitySelect"])) {
	   ...
	   $sql = "select DISTINCT port_city_code,port_city From air_sea_codes WHERE  port_country_code='".$_POST["port_country_code"]."'";
	    $res = $db->getData($sql);
	    $json_data = json_encode($res);
	    //var_dump($json_data);?>

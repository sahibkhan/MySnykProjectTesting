...
//Destination
   ...
   if(!empty($_POST["GenralCitySelect"])) {
   	if($_POST["job_origin_states"]!=''){
   		$sql = "select DISTINCT port_city_code,port_city From air_sea_codes WHERE  port_country_code='".$_POST["port_country_code"]."' &&  state_code ='".$_POST["job_origin_states"]."'  ";
   	}else{
    ...
    if(!empty($_POST["city_id_air_code"])) {
	  ...
	  // echo  json_encode($res_portcode);?>
	   ...
	   foreach ($res_portcode as $portCode) {?>
				<option value="<?php echo trim($portCode['airport_code']);?>" <?php if($_POST["city_id_air_code"]==$portCode["city_code"]) {?> SELECTED <?php }?>><?php echo trim($portCode["airport_code"]); ?></option>
		   <?php
	   } 

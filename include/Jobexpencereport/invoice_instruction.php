...
foreach($selling_arr as $key => $selling)
{
	...
	$jrer_selling_lock_count = $adb->pquery('select COUNT(*) as total_invoice from vtiger_jobexpencereportcf 
										...
										AND vtiger_jobexpencereport.job_id="'.$job_id.'" and vtiger_jobexpencereportcf.jobexpencereportid="'.$selling.'"');

	$data=$adb->fetch_array($jrer_selling_lock_count);	
	if($data['total_invoice']==0)
	{
		if(empty($_SESSION['invoice_instruction_no']) && $s_generate_invoice_instruction_flag==true )
		{	
		$s_generate_invoice_instruction_flag = false;				
		$db_obj = $adb->pquery('select MAX(invoice_instruction_no) as max_invoice_instruction_no from vtiger_jobexpencereport
						   INNER JOIN 	vtiger_jobexpencereportcf on vtiger_jobexpencereportcf.jobexpencereportid = vtiger_jobexpencereport.jobexpencereportid
	 					   where vtiger_jobexpencereport.job_id="'.$job_id.'" and vtiger_jobexpencereportcf.cf_1457="Selling" ');

			if ($adb->num_rows($db_obj) == 0 or !$db_obj)
			{
				$invoice_instruction_ordering = 0;
			}
			else
			{
				$order_row = $adb->fetch_array($db_obj);
				//$order_row = $db_obj->row();		
				if ( ! is_numeric($order_row['max_invoice_instruction_no']))
				{
					$invoice_instruction_ordering = 0;
				}
				else
				{
					$invoice_instruction_ordering = $order_row['max_invoice_instruction_no'];
				}
				$invoice_instruction_no = $invoice_instruction_ordering + 1;
				$_SESSION['invoice_instruction_no'] = $invoice_instruction_no;	
			}
		
		}
			
		$adb->pquery("update vtiger_jobexpencereport set invoice_instruction_no='".$invoice_instruction_no."' where jobexpencereportid='".$selling."' ");
		$adb->pquery("update vtiger_jobexpencereportcf set cf_1248='1', cf_1250='Pending' where jobexpencereportid='".$selling."' ");

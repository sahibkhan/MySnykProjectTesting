...
foreach($expense_arr as $key => $expense)
{
	if(!empty($expense))
	{
		...
		if($data['total_send']==0)
		{
			$adb->pquery("update vtiger_jobexpencereport set b_send_to_head_of_department_for_approval='1', b_head_of_department_approval_status='Pending', head_role='".$head_role."' where jobexpencereportid='".$expense."' ");
			//Submitted to head of department
			$adb->pquery("update vtiger_jobexpencereportcf set  cf_1973='Submitted' where  jobexpencereportid='".$expense."' ");

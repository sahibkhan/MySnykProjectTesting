...
$web1C = 'http://89.218.38.221/glws/ws/createinvoiseT?wsdl';
//$web1C = 'http://89.218.38.220/glws/ws/CreateInvoiseT?wsdl';
		...
		foreach($invoice_submitted as $key_invoice => $invoice)
		{
			...
			if($adb->num_rows($jrer_selling)>0)
			{
				...
				if ($InvNum > 0) {
					...
					$adb->pquery("update vtiger_jobexpencereport set invoice_no='".$InvNum."'															

	...
	foreach($expense_arr as $key => $expense)
	{
		if(!empty($expense))
		{
           ...
           $jrer_expense_lock_count = $adb->pquery('select vtiger_jobexpencereport.b_send_to_head_of_department_for_approval, vtiger_jobexpencereport.jrer_buying_id from vtiger_jobexpencereportcf 
		    ...
		    if($data['b_send_to_head_of_department_for_approval']==0)
		    {
                $sub_jrer_buying_id =$data['jrer_buying_id'];

                $adb->pquery("update vtiger_jobexpencereport
                            set b_send_to_head_of_department_for_approval='1', 
                                b_head_of_department_approval_status='Approved', 
                                b_payables_approval_status='Approved', 
                                b_confirmed_send_to_accounting_software='1',
                                b_send_to_payables_and_generate_payment_voucher='1'
                            where jobexpencereportid='".$expense."' ");
                //cf_1975 For payables
                //cf_1973 for Head
                $adb->pquery("update vtiger_jobexpencereportcf set  cf_1975='Approved', cf_1973='Approved' where  jobexpencereportid='".$expense."' ");
                ...
                if(!empty($sub_jrer_buying_id))
                {
                    ...
                    //cf_1975 For payables
                    //cf_1973 for Head
                    $adb->pquery("update vtiger_jobexpencereportcf set  cf_1975='Approved', cf_1973='Approved' where  jobexpencereportid='".$sub_jrer_buying_id."' ");
                }

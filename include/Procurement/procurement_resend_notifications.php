...
$get_procurement_list = $adb->pquery("SELECT * FROM `vtiger_procurement` 
inner join vtiger_procurementcf ON vtiger_procurement.procurementid = vtiger_procurementcf.procurementid
inner join vtiger_send_approval on vtiger_send_approval.procurement_id = vtiger_procurementcf.procurementid 
inner join vtiger_crmentity on vtiger_procurementcf.procurementid = vtiger_crmentity.crmid 
WHERE vtiger_crmentity.deleted=0 
group by vtiger_procurement.procurementid");

		/*add all approval list for this procurement-request from the ProcurementApproval to the custom table*/
	for($pl=0; $pl<$adb->num_rows($get_procurement_list); $pl++) 
	{
		$procurementDetail = $adb->query_result_rowdata($get_procurement_list, $pl);
		$get_duplicate_approvals = $adb->pquery("SELECT * FROM `vtiger_send_approval_list_by_recordid` where proc_id = '".$procurementDetail['procurementid']."' and name = 'GM' limit 1 ");
		
		if($adb->num_rows($get_duplicate_approvals)>0) {echo "$pl ";continue;}
		
		$office = $procurementDetail['proc_location'];
		...
		if($location_status != 'ALA'){
			
			$procurement_data = Vtiger_Record_Model::getInstanceById($procurementDetail['procurementid'], "Procurement");
			...
			$adb->pquery("INSERT INTO vtiger_send_approval_list_by_recordid set 
				name = 'GM',
				procapproval_proctype = '".$procurement_data->get('proc_proctype')."',
				procapproval_person = '".$id."',
				procapproval_usd_limit = '0',
				procapproval_sequence = '0',
				proc_id = '".$procurementDetail['procurementid']."'
				");
			}
		}
		...
		where procapproval_proctype = '".$procurementDetail['proc_proctype']."' AND vtiger_crmentity.deleted=0 order by procapproval_sequence");
...
$re_adjust_reminder_notification = date('Y-m-d', strtotime($today_date. ' + 1 days'));
...
{
	for($iteration=0; $iteration<$adb->num_rows($pending_approvals); $iteration++) 
	{
		
		$pending_approval = $adb->query_result_rowdata($pending_approvals, $iteration);
...
//end items information
		...
		elseif($location_status != 'ALA' && $approval_exists==0){
			...
			if($mailer->Send(true))
			{
				...
				echo "update vtiger_send_approval set next_notification = '".$next_reminder_notification."' where vtiger_send_approval.id='".$pending_approval['id']."'<br>";
			}	
			else
			{	
				$adb->pquery("update vtiger_send_approval set next_notification = '".$re_adjust_reminder_notification."' where vtiger_send_approval.id='".$pending_approval['id']."'");			
			}
